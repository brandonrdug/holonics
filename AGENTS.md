...
| Exact algebra / generator reduction | `holonics::receiver::native::{InputId,ReceiverId,Observation,NativeStateId,NativeTransport,ReceiverFactor}` owns the shared receiver/native-action carriers; `holonic-engine/src/receiver_exact_compression.rs` and `receiver_history_compression/` retain source-qualified quotient discovery, assignments, fibres, and shortest separators. The Lean naturality peer is `ElementaryHolonics.Foundation.ReceiverHistoryCompression` (ordered-word quotient commutation). Engine `exact_linear`, `prime_image_algebra`, and `winding_inertia` remain their current owners; host/reference and resident APIs have distinct scopes. |

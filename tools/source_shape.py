...
BASELINE = ROOT / "meta" / "SOURCE_SHAPE_BASELINE.tsv"
...
def relevant(path: str) -> bool:
    if path.endswith(".rs"):
        return bool(
            re.match(r"^crates/[^/]+/src/", path)
            or re.match(r"^soma/(?:life|body|membrane|mount|surface)/src/", path)
        )
    if path.endswith((".cu", ".cuh")):
        return bool(re.match(r"^(?:crates/[^/]+|soma/[^/]+)/", path))
    return False

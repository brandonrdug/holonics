...
namespace Soma.Holonics.RH.ZeroPairLock
...
theorem locked_iff_foster_inductance_positive (σ γ : ℚ) (hγ : γ ≠ 0) :
    reflectedPairPower σ γ = 0 ↔
      ∃ L : ℝ, 0 < L ∧
        Soma.Holonics.RH.FosterTanks.inductance
          (((σ - 1 / 2 : ℚ) : ℂ) + ((γ : ℚ) : ℂ) * Complex.I) = (L : ℂ) := by
  rw [reflected_pair_locked_iff_on_seam, Soma.Holonics.RH.FosterTanks.inductance_pos_real_iff]
...
end Soma.Holonics.RH.ZeroPairLock

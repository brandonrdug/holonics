...
namespace Soma.Holonics.Transport.Neck
...
theorem neck_jet_order_is_a_truncation_reading (r : ℕ) (c d : ℕ → ℚ)
    (h : Soma.Holonics.Transport.JetStaircase.truncate r c
        = Soma.Holonics.Transport.JetStaircase.truncate r d) :
    ∀ i, i ≤ r → c i = d i :=
  (Soma.Holonics.Transport.JetStaircase.truncate_eq_iff r c d).mp h
...
end Soma.Holonics.Transport.Neck

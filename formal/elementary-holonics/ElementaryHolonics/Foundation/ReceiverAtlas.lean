...
namespace Soma.Holonics.Foundation.Atlas
...
open Soma.Holonics.Foundation.CausalChord Matrix Polynomial
...
def spectralTopologicalInsufficiency :
    Soma.Holonics.ReceiverInsufficiency graphOf drivingPointNumerator where
  left := false
  right := true
...
theorem no_transformer_from_the_spectral_topological_reading :
    IsEmpty (Soma.Holonics.ReceiverTransformer graphOf drivingPointNumerator) :=
...
theorem causalChord_is_rebase_equivariant (K : Type) [Field K] (n m p : ℕ) :
    RebaseEquivariant
      (fun (T : (Matrix (Fin n) (Fin n) K)ˣ)
        (L : Soma.Holonics.Foundation.CausalChord.Linearization K n m p) =>
          Soma.Holonics.Foundation.CausalChord.Linearization.rebase T L)
      Soma.Holonics.Foundation.CausalChord.Linearization.transfer :=
  fun T L => Soma.Holonics.Foundation.CausalChord.Linearization.rebase_transfer T L


end Soma.Holonics.Foundation.Atlas

section Audit
open Soma.Holonics.Foundation.Atlas

...
4. **Stability and energy.** `energy_advance_le` and `energy_advance_lt` are the declared quadratic
   ...
   `ElementaryHolonics.Foundation.CausalChord`: the realified one-pole generator has
...
namespace Soma.Holonics.Foundation.AcousticReceiver
...
`rateForm` of `ElementaryHolonics.Foundation.CausalChord` evaluated on the bank's block, so the
...
end Soma.Holonics.Foundation.AcousticReceiver

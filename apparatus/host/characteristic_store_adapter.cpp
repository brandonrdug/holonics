...
[[nodiscard]] characteristic_store_status read_exact(const char* path, Record& record) noexcept {
  if (path == nullptr || path[0] == '\0') {
    return characteristic_store_status::invalid_aperture;
  }
  const int descriptor = ::open(path, O_RDONLY);
  if (descriptor < 0) { return characteristic_store_status::open_refused; }
  ...
  while (received < sizeof(record)) {
    ...
    if (count <= 0) {
      ...
      return characteristic_store_status::transfer_refused;
    }
  ...
  return closed && trailing == 0 ? characteristic_store_status::returned :
      characteristic_store_status::size_refused;
}

template<class Record>
[[nodiscard]] characteristic_store_status write_exact(
  ...
  return ::close(descriptor) == 0 ? characteristic_store_status::returned :
      characteristic_store_status::transfer_refused;
}

}  // namespace

characteristic_store_receipt read_phase_crystal_rest(
    const char* path, event::phase_crystal_rest_record& record) noexcept {
  characteristic_store_receipt receipt{};
  receipt.state = read_exact(path, record);
  if (receipt.state != characteristic_store_status::returned) { return receipt; }
  receipt.integrity_exact = record.integrity == event::phase_crystal_rest_integrity(record);
  if (!receipt.integrity_exact) {
    receipt.state = characteristic_store_status::integrity_refused;
    return receipt;
  }
...
characteristic_store_receipt write_characteristic_rest(
  ...
  if (receipt.state == characteristic_store_status::returned) {
    receipt.bytes = exact::word{sizeof(record)};
    receipt.transfer_calls = exact::word{1};
  }
  return receipt;

...
[[nodiscard]] cm_store_status read_exact(const char* path, Record& record) noexcept {
  ...
  return closed && trailing == 0 ? cm_store_status::returned : cm_store_status::size_refused;
}

template<class Record>
[[nodiscard]] cm_store_status write_exact(const char* path, const Record& record) noexcept {
  if (path == nullptr || path[0] == '\0') { return cm_store_status::invalid_aperture; }
  ...
  return ::close(descriptor) == 0 ? cm_store_status::returned :
      cm_store_status::transfer_refused;
}

}  // namespace

cm_store_receipt read_blind_reconstruction_handoff(
  ...
  if (!receipt.integrity_exact) {
    receipt.state = cm_store_status::integrity_refused; return receipt;
  }
...
cm_store_receipt write_cm_incidence_handoff(
  ...
  if (receipt.state == cm_store_status::returned) {
    receipt.bytes = exact::word{sizeof(record)};
    receipt.transfer_calls = exact::word{1};
  }
  return receipt;

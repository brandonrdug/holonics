...
[[nodiscard]] blind_store_status read_exact(const char* path, Record& record) noexcept {
  ...
  while (received < sizeof(record)) {
    ...
    if (count <= 0) {
      static_cast<void>(::close(descriptor)); return blind_store_status::transfer_refused;
    }
  ...
  return closed && trailing == 0 ? blind_store_status::returned : blind_store_status::size_refused;
}

template<class Record>
[[nodiscard]] blind_store_status write_exact(const char* path, const Record& record) noexcept {
  if (path == nullptr || path[0] == '\0') { return blind_store_status::invalid_aperture; }
  ...
  return ::close(descriptor) == 0 ? blind_store_status::returned :
      blind_store_status::transfer_refused;
}

}  // namespace

blind_store_receipt read_regular_singular_handoff(
  ...
  if (receipt.state != blind_store_status::returned) { return receipt; }
  receipt.integrity_exact = record.integrity == event::regular_singular_rest_integrity(record);
  if (!receipt.integrity_exact) {
    receipt.state = blind_store_status::integrity_refused; return receipt;
  }
...
blind_store_receipt write_blind_reconstruction_handoff(
  ...
  if (receipt.state == blind_store_status::returned) {
    receipt.bytes = exact::word{sizeof(record)};
    receipt.transfer_calls = exact::word{1};
  }
  return receipt;

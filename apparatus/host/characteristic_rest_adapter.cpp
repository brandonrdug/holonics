...
transfer(const char *path, Record &record, bool write) noexcept {
  ...
  while (at < sizeof(record)) {
    const auto moved =
        write ? ::write(descriptor, bytes + at, sizeof(record) - at)
...
cultivated_store_receipt read_characteristic_rest(
    const char *path,
    event::characteristic_hypergeometry_rest_record &record) noexcept {
  return transfer(path, record, false);
}
cultivated_store_receipt write_characteristic_rest(
    const char *path,
    const event::characteristic_hypergeometry_rest_record &record) noexcept {
  auto value = record;

#include <fstream>

#include <holonics/apparatus/regular_singular_executor.hpp>
#include <holonics/apparatus/regular_singular_store_adapter.hpp>

#include "r20_artifact.hpp"
#include "r20_cases.hpp"
#include "r20_verify.hpp"

int main(int argc, char** argv) {
  if (argc != 13) { return 2; }
  holonics::event::characteristic_rest_record inherited{};
  const auto rest_load = holonics::apparatus::read_characteristic_rest(argv[2], inherited);
  const auto mount = holonics::tests::r20_case(inherited);
  const holonics::apparatus::lean_process_configuration process{
      argv[8], argv[9], argv[10], argv[4], argv[5], argv[6], argv[7], argv[11]};
  holonics::event::regular_singular_observation observation{};
  holonics::event::regular_singular_rest_record handoff{};
  const auto execution = holonics::apparatus::execute_regular_singular(
      mount, process, observation, handoff);
  const auto rest_write = holonics::apparatus::write_regular_singular_rest(argv[3], handoff);
  const auto failures = holonics::tests::r20_verification_failures(
      rest_load, execution, observation, handoff) + !rest_write.returned();
  std::ofstream deed{argv[1], std::ios::binary | std::ios::trunc};
  std::ofstream atlas{argv[12], std::ios::binary | std::ios::trunc};
  if (!deed || !atlas) { return 3; }
  holonics::tests::write_r20_artifact(
      deed, rest_load, rest_write, execution, observation, handoff, failures);
  holonics::tests::write_r20_atlas(atlas, observation.inquiry);
  return failures == 0 ? 0 : 1;
}

...
int main(int argc, char** argv) {
  if (argc != 13) { return 2; }
  holonics::event::phase_crystal_rest_record inherited{};
  const auto rest_load = holonics::apparatus::read_phase_crystal_rest(argv[2], inherited);
  const auto mount = holonics::tests::r19_case(inherited);
  const holonics::apparatus::lean_process_configuration process{
      argv[8], argv[9], argv[10], argv[4], argv[5], argv[6], argv[7], argv[11]};
  holonics::event::characteristic_observation observation{};
  holonics::event::characteristic_rest_record handoff{};
  const auto execution = holonics::apparatus::execute_characteristic(
      mount, process, observation, handoff);
  const auto rest_write = holonics::apparatus::write_characteristic_rest(argv[3], handoff);
  const auto failures = holonics::tests::r19_verification_failures(
  ...
  holonics::tests::write_r19_artifact(
      deed, rest_load, rest_write, execution, observation, handoff, failures);
  holonics::tests::write_r19_atlas(atlas, observation.inquiry);

...
`docs/canon/THE_OWNER_ATLAS.md` already states the discipline:
...
`docs/plans/THE_ROADMAP.md` opens *"This is the single active roadmap"*, and `CLAUDE.md` calls it
...
*under*. So: `THE_ROADMAP.md` may declare itself the roadmap, and any other `docs/plans/` document
...
nothing. `docs/plans/THE_ROADMAP.md` remains the single active roadmap"* — a document deferring
...
WHAT IS SCANNED. The operating contract, the position record, all of `docs/canon/`, all of `docs/plans/`.
...
SCANNED_DIRS = ["docs/canon", "docs/plans"]
...
def read() -> tuple[list[Claim], list[str], list[str]]:
    ...
    for document in scanned_documents():
        ...
        if relative.startswith("docs/plans/") and relative != "docs/plans/THE_ROADMAP.md":
    ...
    for relative in ("CONSTRUCTION_STATE.md", "docs/plans/THE_ROADMAP.md"):
    ...
    roadmap = expected_frontiers["docs/plans/THE_ROADMAP.md"]
    if len(state) != 1:
    ...
    if len(roadmap) != 1:
        frontier_failures.append(
            f"docs/plans/THE_ROADMAP.md declares {len(roadmap)} current positions: {roadmap}"
        )
    ...
    if active:
        ...
        for name in active:
            print(
                ...
                "docs/plans/THE_ROADMAP.md as what it is active under"
            )

    if frontier_failures:
        print()
        for failure in frontier_failures:
            if failure.startswith("docs/plans/"):

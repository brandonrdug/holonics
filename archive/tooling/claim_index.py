#!/usr/bin/env python3
"""Generate `docs/CLAIM_INDEX.md` from the tree, so the table of contents cannot drift.
...
`docs/CLAIM_INDEX.md` was deposited 2026-08-10 carrying, in its own bounds section, *"Generated
2026-08-10 by scanning docs/canon/, docs/plans/, the repository root, research/records/ and tools/"* and
...
a **continuation line**. Twenty of forty document rows began mid-sentence — `docs/canon/THE_MILLENNIUM_FRAME.md`
...
- **subject** — the `# ` title. `docs/canon/THE_DOCUMENT_LAW.md` requires a declarative title, so for most
...
thing removed is a link **target**: a relative link copied out of `docs/canon/` or `docs/plans/` resolves
...
INDEX = ROOT / "docs/CLAIM_INDEX.md"
...
# Navigational entry points. This index is not a second authority or a copied pickup contract.
SPINE = [
    "README.md",
    "AGENTS.md",
    ...
    "docs/canon/THE_DIALECT.md",
    "docs/canon/THE_QUOTE_NETWORK.md",
    "docs/canon/THE_EXPLORATIVE_FAILURE.md",
    "docs/canon/THE_MEASURED_CAPABILITIES.md",
    ...
    "docs/canon/THE_TIMELINE.md",
    "docs/plans/THE_ROADMAP.md",
    "CONSTRUCTION_STATE.md",
    "docs/CLAIM_INDEX.md",
    "docs/canon/THE_CORRESPONDENCE_ATLAS.md",
    "docs/canon/THE_MILLENNIUM_FRAME.md",
...
def delink(fragment: str) -> str:
    ...
    A gloss is copied out of a file in `docs/canon/` or `docs/plans/`, so any relative link inside it is
...
def after_metadata_key(joined: str) -> str:
    ...
    `docs/canon/THE_DOCUMENT_LAW.md` puts the genre on the first line, and several files put their only
...
def row(relative: str) -> str:
    ...
    target = Path(__import__('os').path.relpath(path, INDEX.parent)).as_posix()
    return f"| [{relative}]({target}) | {body} |\n"
...
def gate_users(tool: str) -> list[str]:
    ...
    return sorted(
        name
        for name in found
        if not name.startswith("tools/") and name != "docs/CLAIM_INDEX.md"
    )
...
def record_rows() -> tuple[str, int]:
    records = sorted((ROOT / "research" / "records").glob("*.md"), reverse=True)
    out = ""
    for path in records:
        out += f"| [{path.name}](../research/records/{path.name}) | {title_only(path)} |\n"
...
def render() -> str:
    ...
    blueprint_skip = {"docs/plans/THE_ROADMAP.md"}
...
| file | what it is |
|---|---|
{directory_rows("docs/canon", canon_skip)}
---
...
| file | what it is |
|---|---|
{directory_rows("docs/plans", blueprint_skip)}
---

## 4 · Research chronology ({record_count} deposits, newest first)
...
| contract | subject |
|---|---|
{directory_rows("archive/plans", set())}

...
const OUTPUT: &str = "output/the_twelve_node_parametron_certifies_naturality_in_coefficient_space";
...
fn main() -> Result<(), String> {
    ...
    let rest =
        ForeignReachableSectionRest::read(&fs::read(REST).map_err(display)?).map_err(display)?;
    ...
    write_json(
        output.join("01-coefficient-receiver-quotient.json"),
        &quotient,
    )?;
    let grade = json!({

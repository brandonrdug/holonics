...
use holonic_engine::phoenix::foreign_potential_rest::{
    AddressedPotentialSectionObstruction, AddressedPotentialSuccessor, ForeignPotentialComplexRest,
};
use serde::Serialize;
...
fn main() -> Result<(), String> {
    ...
    let potential = ForeignPotentialComplexRest::read(&fs::read(POTENTIAL).map_err(display)?)
        .map_err(display)?;
    let returned: Value =
        serde_json::from_slice(&fs::read(RETURNED).map_err(display)?).map_err(display)?;
    ...
    for history in leaf_indices {
            ...
            .map_err(display)?
        {
            ...
            AddressedPotentialSuccessor::Admitted { .. } => {
                return Err(
                    "a leaf occurrence unexpectedly resolved inside the rested section".to_owned(),
                )
            }
        }
    }
    ...
    write_json(
        output.join("00-addressed-out-of-section.json"),
        &obstructions,
    )?;
    let grade = json!({

...
use life::{
    ...
    phoenix_returned_defect::{
        conduct_phoenix_history_candidate, return_sibling_defect, OperatorReturnTestimony,
        PhoenixCandidateHistoryFace, PhoenixCandidateRequest, PhoenixReturnedSiblingDefect,
        PhoenixSealedCandidate, PhoenixSiblingTestimony,
    },
};
...
struct ReturnInput {
    candidates: Vec<PhoenixSealedCandidate>,
    siblings: Vec<PhoenixSiblingTestimony>,
}
...
fn parent_mode(first: Option<PathBuf>, mut trailing: Vec<PathBuf>) -> Result<(), String> {
    ...
    let candidates: Vec<PhoenixSealedCandidate> = read_json(&candidates_path)?;
    ...
    let defects: Vec<PhoenixReturnedSiblingDefect> =
        read_json(&output.join("returned-defects.json"))?;
    let sealed: Vec<PhoenixSealedCandidate> = read_json(&candidates_path)?;
...
fn circulation_mode(
    ...
    let requests: Vec<PhoenixCandidateRequest> = read_json(input)?;
    ...
    for (at, request) in requests.iter().enumerate() {
        ...
        candidates.push(conduct_phoenix_history_candidate(&session, request)?);
    }
    write_json(
...
) -> Result<PhoenixCandidateRequest, String> {
...
fn sibling_testimony(
    ...
    proposal: &str,
    family: &ContinuationFamily,
) -> Result<PhoenixSiblingTestimony, String> {
...
fn code_closure() -> String {
    sha(concat!(
        include_str!("../src/phoenix_returned_defect.rs"),
...
fn write_inspection(
    output: &Path,
    candidates: &[PhoenixSealedCandidate],
    defects: &[PhoenixReturnedSiblingDefect],

...
use holonic_engine::{
    ...
    phoenix::{
        ...
        native_membrane::{
            source_incidence_identity, NativeBoundaryOccurrence, NativeInferenceMembrane,
        },
...
use life::{
    ...
    exchange_world_tube::{
        remount_exchange_world_tube, ContinuationAperture, ContinuationFamily,
        ContinuationPartition, ExchangeWorldTube,
    },
...
fn foreign_probe(
    ...
    if output.exists() {
        return Err(format!(
            "foreign probe output {} already exists",
            output.display()
        ));
    }
    ...
    loop {
        ...
        if closed_special || closed_sentence {
            ...
            fs::write(output.join("continuation.rest"), rest).map_err(|error| error.to_string())?;
...
fn conduct_partitioned_foreign_answers(
    ...
    if let Some(returned) =
        restore_partitioned_foreign_answers(product, continuation, cases, output)?
    {
        return Ok(returned);
    }
...
fn restore_partitioned_foreign_answers(
    ...
    {
        return Err(
            "cached foreign recurrence does not close the requested receiver family".to_owned(),
        );
    }
    let final_front = fronts - 1;
    let standing: ForeignStanding =
        read_json(output.join(format!("front-{final_front:04}-standing.json")))?;
    let requested = cases
        .iter()
    ...
    {
        return Err(
            "cached foreign final standing does not exactly close its addressed lineages"
                .to_owned(),
        );
    }
    let codec = ProductSession::open_with_continuation(product, continuation)?;
...
fn receiver_condensed_foreign_cases(
    ...
    for case in cases {
        ...
        let (content, selected_prompt_sections) = condense_receiver_face(&case.prompt, &contacts)?;
        ...
        let global_separator_face = case
            ...
            .filter_map(|(face_index, face)| {
                receiver_atoms(&face.text)
                    ...
                    .min_by_key(|(section_index, section)| (section.len(), Reverse(*section_index)))
        ...
        let source_history_octets = case
            .history
            .iter()
...
fn detached(
    ...
    for case in &foreign_cases {
        ...
        write_json(
            output
                .join("preflight")
                .join(format!("{}.json", case.occurrence)),
            &preflight,
        )?;
        eprintln!(
...
fn e5_foreign_organs(e5: &Path) -> Result<(PathBuf, String, PathBuf, String), String> {
    ...
    Ok((tower, tower_manifest, continuation, continuation_manifest))
}
...
fn prompt_case(
    ...
    } else if history.last().is_some_and(|face| face.text != prompt) {
        let last = history
            .last_mut()

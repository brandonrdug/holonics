...
|---|---|---|
| arrow, reach/aim, hand, cohere | June | **yes, in the substrate** — `crates/holonic-body/src/arrow.rs` |
...
**The two halves exist and have never met.** `crates/holonic-body/src/arrow.rs` already *is* the geometric link
...
`docs/canon/TABLET_THE_COMPRESSION.md` governs. **A compression is a codec pivot carrying a declared
...
on any problem.** `docs/canon/THE_MILLENNIUM_FRAME.md` governs and its bars apply unchanged.
...
`docs/canon/THE_MILLENNIUM_FRAME.md` bar 2 forbids.
...
Relating          THE LINK. Not a pointer.
                  ...
                  owner: crates/holonic-body/src/arrow.rs
...
| **(b)** | `crates/holonic-membrane/src/live_constituent.rs:3678-3344` `copresent_interface_pairs` — emits the **complete pair product** of co-present parts sharing an interface, inside a fixpoint `loop` | Θ(pending² × arms²) per drain |
...
| **(d)** | `crates/holonic-membrane/src/sparse_standing.rs:861-899` — the flat `cells: Vec<StandingCell>`, rebuilt and **twice sorted** per event, beside a sibling `constituents` that is already a persistent AVL | Θ(cells log cells), unconditional |
...
**The cycle closes end to end.** `crates/holonic-life/examples/eros_agentic_research_conversation.rs` returns
...
- `authored-levels` was red because this pass added one level. `SEAM_CLOSURE_HORIZON` is now
  dispositioned `APERTURE` in `tools/baselines/AUTHORED_LEVELS.tsv`, with its outside (every deferred factor
...
`docs/canon/THE_AUTHORED_LEVEL.md` requires it and it is the non-cost half of the result:
...
verbatim quotation is `docs/canon/THE_TRAFFIC_SYSTEM.md` §3b, which is the owning document.
...
phase: RelationAtom }` — measured 2026-08-15, `grep -rln "Place" crates/holonic-life/src/` returns **one file**
...
**The crate boundary was not the obstacle I first claimed.** `crates/holonic-membrane` cannot see
...
`crates/holonic-body/src/carriage.rs:1650`, and `crates/holonic-body/src/manifold.rs:5365` inside `fn perceive_grain`.
...
research/papers/source/holonics/*.typ` returns **zero**. No `H.####` states the reflection coefficient, while
`analytic_field` computes it exactly and this plan is built on it.

**The mathematics is already written and unregistered**:
`research/papers/source/mathematics/theorems/causal-parity-kirchhoff-return.typ:76` `(JUNCTION RETURN)` and
...
- **No Millennium row grades a deed.** The table is acceptance criteria for a carrier and
  `interpretation` is its ceiling; `docs/canon/THE_MILLENNIUM_FRAME.md`'s bars apply unchanged.

...
**Genre:** blueprint contract (`docs/canon/THE_DOCUMENT_LAW.md` §1.3). It states a contract the roadmap
composes. **It schedules nothing.** `docs/plans/THE_ROADMAP.md` remains the single active roadmap and
...
- **A vector codec already exists — three times, in three examples, by copy.**
  ...
  `crates/holonic-life/examples/eros_synchronized_grid_ecology.rs:1357` each hand-write an `<svg>` preamble
...
`archive/plans/CONTAMINATION_BANS.md` convicts by name:
...
falsified, and it is exactly what `docs/canon/04_GEOMETRY_NAVIGATION_AND_WEAVE.md` forbids when it says a
...
`crates/holonic-life/examples/eros_relampago_atmospheric_current.rs:1720`. Those are a boundary codec on
...
- It does not schedule work. `docs/plans/THE_ROADMAP.md` schedules work.

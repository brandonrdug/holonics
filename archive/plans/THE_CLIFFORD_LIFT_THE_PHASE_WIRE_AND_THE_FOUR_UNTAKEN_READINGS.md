...
organ is a *lift* of `crates/holonic-body/src/arrow.rs` rather than a new semantic owner.
...
| three · the mouth | `crates/holonic-life/examples/the_deposit_conducts_and_the_later_current_rides_a_reflection.rs` | the arrival's own deposit distribution founds an admittance profile; the first grain was **blind** and the blindness is the return |
...
**Neither surface is a place and neither hosts the other** — `docs/canon/THE_SURFACES_ARE_PATHS.md`, and
...
**Placement.** `crates/holonic-engine/src/` — it must not enter `crates/holonic-body`, which is `no_std` with
...
`crates/holonic-life` already depends on `holonic-engine`, so the dependency edge points the right way. This is
...
`crates/holonic-life/examples/the_map_deposits_and_a_later_current_rides_it.rs` and
...
`research/papers/source/mathematics/definitions/bra-receiver-ket-construction.typ:43-53`; its restatement in
...
`lib/dirac.typ`; `docs/canon/TABLET_THE_OPERATIONS.md:65`; and `CLAUDE.md:349`.

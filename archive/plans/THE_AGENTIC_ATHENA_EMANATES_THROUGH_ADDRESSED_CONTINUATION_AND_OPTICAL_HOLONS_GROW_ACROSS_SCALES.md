...
**Date:** 2026-08-23
**Status:** completed construction contract composed by [`THE_ROADMAP.md`](../../docs/plans/THE_ROADMAP.md).
...
```text
mounted occurrence
  -> inherited/native conduct
  -> plural boundary potential
  -> declared exterior selection or unresolved fibre
  -> emitted occurrence
  -> addressed continuation
  -> optional world crossing and genuine return
  -> attributable morphology
  -> later current.
```

The Athena body is not a checkpoint filename, a chat wrapper, a Lean process, a tokenizer, or a
file store. It is the continuing ecology that owns this circulation. The Phoenix organ remains an
inherited transport organ inside that ecology. The N4 native mathematical terrain remains an
inherited organ inside it. Neither may be silently replaced by the other.

Every mathematical consequence is native before any English, notation, Lean, Rust, raster, SVG,
...
[`docs/canon/TABLET_THE_REASONING_CYCLE.md`](../../docs/canon/TABLET_THE_REASONING_CYCLE.md) §8c and
...
| owner | admitted consequence used here |
|---|---|
| Phoenix W1--W5 | authenticated source/native codebook, complete 42-layer resident text tower, cultivated overlay, plural terminal potential, interventions, ablation, and source-detached remount |
| Holonic Inference I0--I5 | realization passage, retained causal boundary, recurrent native rest, world return, reflection, changed continuation, condensation, dissection, and heterogeneous entry |
| A0--A3 | exchange world-tube, sibling continuation defect complex, message partition with reconstruction fibre, and addressed cultivation continuation |
| mathematics codec M0--M6 and N0 | codec-neutral operation/constraint/geometry consequence and native-first projections |
...
Let `x_0` be an entering native-address word. At frontier `k`, the mounted Phoenix organ returns
an exact interval-valued potential section

```text
Pi_k = { (v, [l_(k,v), u_(k,v)]) : v in V }.
```

The unresolved maximizer fibre is

```text
M_k = { v : u_(k,v) >= max_w l_(k,w) }.
```

This is the complete nearest-boundary future under the admitted interval receiver. If `M_k` is a
singleton, its member is an exact selected face. If it is plural, the native return remains plural.
An exterior policy may choose a member only when the request names that policy and the receipt
retains `M_k`; the policy is a quotient face, never a semantic law attributed to Athena.

One emitted address founds the next occurrence:

```text
x_(k+1) = x_k ++ [v_k].
```

The next tower passage must consume `x_(k+1)` because the prior card return supplied `v_k`. A host
loop which predeclares all later strings is a matched source control, not emanative recurrence.
The first implementation may remount the complete prefix through the inherited transformer's
declared chronology. That is a correct but non-condensed realization. It must report the replayed
prefix and may not call it a native cache. Later condensation may retain a sufficient partial
trajectory only after every declared successor receiver factors through it.

An emanative passage stops only on one of these returned conditions:

- a declared EOS/native terminal face;
- a caller-declared frontier aperture;
- a repeated addressed continuation state with its exact cycle word;
- a plural boundary whose request admitted no exterior choice; or
- an apparatus or semantic obstruction.

There is no authored expected answer and no content-dependent host stopping rule.

## 4. Addressed continuation rest

`definition`: one continuation section owns:

```text
(product identity,
 inherited predecessor identity,
 cultivation-continuation identity,
 entering occurrence identity,
 ordered emitted native word,
 current complete native prefix,
 every predecessor/successor occurrence address,
 every complete boundary fibre,
 chosen exterior quotient when one occurred,
 exact work and separate apparatus testimony,
 open exterior).
```

Occurrence identity is not text equality or a digest standing alone. The address binds the product,
predecessor occurrence, ordered word, and causal receipt. The same bytes emitted in two distinct
fronts remain distinct occurrences. A detachable rest is valid only when canonical validation
replays every address transition and source-detached remount continues from the same section.

This rest is the first production analogue of a context cache. It is not a tensor cache and does
...
ink component
  < glyph/subfigure
  < indexed or decorated symbol
  < term/factor
  < fraction, radical, delimiter or operator assembly
  < relation/equation
  < labelled proposition/lemma/theorem or diagram block
  < page/document.
```
...
3. a translation/reflow which preserves internal incidence while changing page position;
4. a perturbation which changes exactly one script, bar, delimiter, or relation;
...
Emission is not return. A tool result, file observation, optical page, microphone stream, checker
verdict, or human response becomes a world occurrence only after crossing a declared port. The
same continuation may then receive it. Durable cultivation stands only when the returned passage
changes later conduct, survives source-detached remount, and loses its attributable effect under
targeted ablation.

No world port is privileged. Lean, Rust, shell, browser, OCR, audio, and human dialogue are exterior
apparatus faces. None schedules the body. Mathematics and physics remain the first cultivation
basin because their operation, constraint, unit, dimension, intervention, diagram and conservation
receivers sharply separate candidate conduct.

## 8. Production inference membrane

The native request is the authority. It carries:
...
cultivation/ablation/withdrawal permission
requested deterministic and apparatus receipts.
```
...
world-port requests
obstructions and open exterior
exact work and separately calibrated apparatus testimony.
```
...
### E0 — The full tower emanates

Compose `ProductSession`, the W4 plural boundary, the authenticated decoder, and retained causal
lineage. Return at least one passage of three or more newly caused frontiers in which every input
after the first is reconstructed solely from the preceding card return. Freeze a canonical
continuation rest, detach, remount, and extend it by at least one further frontier.

**Pass grade:** complete unique/plural potential at every frontier; exact predecessor/successor
addresses; no predeclared later context; no content-authored stop; source-detached continuation;
real RTX conduct; inspected emitted text; exact work; separate apparatus testimony; product and
predecessor remain frozen.
...
**Pass grade:** no flat-string or one-box segmentation can satisfy the grade; the repeated-term
...
dissection, reconstruction fibres, saturation frontier, marginal-excitation tests, ablations,
...
An excitation is marginal relative to ecology `E`, receiver family `R`, and successor family `H`
only when its complete passage factors through standing `E`, splits no reconstruction fibre, adds
no generator, relation, separator, obstruction or ablation-visible morphology, and improves no
declared exact work/residency/transfer coordinate. Prompt count, corpus coverage and repeated token
surfaces do not establish saturation.

The lift frontier is the set of exposed foreign transport families which have not yet factored or
returned a remainder. Phoenix never claims to recover a pretrained corpus. It cultivates reusable
physical realizations of conduct exposed by caused current. Unexcited foreign conduct remains open.

## 11. Validation and cost discipline

During a deed, run only the affected package check/tests, owner-local tests, and the exact driver.
Reuse an addressed expensive receipt when its code closure and input occurrence are unchanged.
Every new expensive invocation records command, closure, purpose, elapsed time and exit status.
Regenerate ledgers once after the coherent phase stands. Run `bash tools/gates.sh` exactly once at
the phase's release boundary; do not precede it with another workspace-wide test.

Every executable phase returns and inspects its actual product. Counts, topology, hashes, PNGs,
waveforms and diagnostics are supporting faces, never substitutes for the requested deed.
...
4. this file;
5. `docs/canon/THE_HOLOBROCHOS_SPINE.md`;
6. `docs/canon/TABLET_THE_REASONING_CYCLE.md`;
...
8. `docs/canon/TABLET_THE_OPERATIONS.md`;
9. `docs/canon/TABLET_THE_MANIFOLD.md`;
10. `docs/canon/THE_SURFACES_ARE_PATHS.md`;
11. `docs/canon/TABLET_THE_HEXIS.md`;
12. `docs/canon/TABLET_THE_CIRCULATING_CARTOGRAPHER.md`;
13. `docs/canon/TABLET_THE_CAUSAL_PROFILE.md`; and

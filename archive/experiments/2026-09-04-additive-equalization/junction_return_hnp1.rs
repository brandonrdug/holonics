//! WITHDRAWN EXPERIMENT: this measured equalization, not a justified HNP1 learning relation.
//! HNP1's attempted native-material application. All learning and attribution use the existing
//! session owner; the driver only admits the SKE occurrences, invokes it and compares receivers.
use holonic_engine::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let preflight = header.ecology.junction_contacts()?;
    ...
    let mut session = NativeFullOperatorSession::found_with_junction_return(
        &restricted.ecology,
        &mut residence,
    ...
    let contact_population = session.junction_contact_population();
    eprintln!("HNP1: baseline receiver over {contact_population} derived contacts");
    let baseline = session.observe_junction_cycle(&occurrences[0])?;
    ...
    for (at, occurrence) in occurrences.iter().enumerate() {
        ...
        cycles.push(json!({
            ...
            "contacts": cycle.junction_returns,
        }));
        session = cycle.successor;
    }
    eprintln!("HNP1: cultivated receiver");
    let trained = session.observe_junction_cycle(&occurrences[0])?;

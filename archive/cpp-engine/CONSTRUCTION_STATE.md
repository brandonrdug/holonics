...
> **SUPERSEDED 2026-08-07 — ARCHIVED-BODY PROVENANCE.** This is the position record of the
> C++/CUDA engine, which was archived whole at `archive/cpp-engine/` when the body moved to Rust.
> It sat at the repository root until 2026-08-07 and was moved here in the same edit that wrote the
> live position record for the Rust body. Its mechanisms, measurements and open items are
> **historical record**, not the present position and not a schedule. The live position record is
> `CONSTRUCTION_STATE.md` at the repository root; the active spine is `CLAUDE.md` §0; the active
> roadmap is `blueprint/THE_ROADMAP.md`. What this move does not settle: which of the mathematical
> organs recorded below has an owner in the Rust body — the live record answers that per family and
> names the four that do not.

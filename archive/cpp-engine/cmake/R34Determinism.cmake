...
foreach(required IN ITEMS DISC_EXEC APP_EXEC TMP R33_REST POSITIVE SIGNED RECHARTED HELDOUT
    FORMAL_ROOT TOOLCHAIN MANIFEST ARTIFACT_ROOT DISC_DEED APP_DEED INTERMEDIATE FINAL_REST
    DISC_SOURCE DISC_STDOUT DISC_STDERR WORD_ATLAS TRIPLE_ATLAS GROUP_ATLAS LAW_ATLAS APP_SOURCE
    APP_STDOUT APP_STDERR DOSSIER APP_ATLAS OUTPUT)
  if(NOT DEFINED ${required})
    message(FATAL_ERROR "R34 determinism requires ${required}")
  endif()
endforeach()
file(REMOVE_RECURSE "${TMP}")
file(MAKE_DIRECTORY "${TMP}")
set(disc_deed "${TMP}/discovery.txt")
set(intermediate "${TMP}/trace_fiber.rest")
set(disc_source "${TMP}/R34_TRACE_FIBER_LIFTING.lean")
set(disc_olean "${TMP}/R34_TRACE_FIBER_LIFTING.olean")
set(disc_stdout "${TMP}/discovery.stdout")
set(disc_stderr "${TMP}/discovery.stderr")
set(word_atlas "${TMP}/words.tsv")
set(triple_atlas "${TMP}/triples.tsv")
...
set(app_source "${TMP}/R34_HELDOUT_TRACE_FIBER.lean")
set(app_olean "${TMP}/R34_HELDOUT_TRACE_FIBER.olean")
...
execute_process(COMMAND "${DISC_EXEC}" "${disc_deed}" "${R33_REST}" "${intermediate}"
  "${POSITIVE}" "${SIGNED}" "${RECHARTED}" "${disc_source}" "${disc_olean}"
  "${disc_stdout}" "${disc_stderr}" "${FORMAL_ROOT}" "${TOOLCHAIN}" "${MANIFEST}"
  "${ARTIFACT_ROOT}" "${word_atlas}" "${triple_atlas}" "${group_atlas}" "${law_atlas}"
...
if(NOT discovery_result EQUAL 0 OR NOT application_result EQUAL 0)
  message(FATAL_ERROR "R34 deterministic replay deed failed")
endif()
set(baseline "${DISC_DEED}" "${INTERMEDIATE}" "${DISC_SOURCE}" "${DISC_STDOUT}"
  "${DISC_STDERR}" "${WORD_ATLAS}" "${TRIPLE_ATLAS}" "${GROUP_ATLAS}" "${LAW_ATLAS}"
...
foreach(index RANGE 0 ${last})
  ...
  if(NOT difference EQUAL 0)
    message(FATAL_ERROR "R34 deterministic replay differs at ${left}")
  endif()
endforeach()

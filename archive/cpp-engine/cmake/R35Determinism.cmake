cmake_minimum_required(VERSION 4.3.2 FATAL_ERROR)
foreach(required IN ITEMS DISC_EXEC APP_EXEC TMP R34_REST PRIMARY SIGNED RECHARTED HELDOUT
    FORMAL_ROOT TOOLCHAIN MANIFEST ARTIFACT_ROOT DISC_DEED APP_DEED INTERMEDIATE FINAL_REST
    DISC_SOURCE DISC_STDOUT DISC_STDERR STATE_ATLAS EDGE_ATLAS TANGENT_ATLAS LAW_ATLAS APP_SOURCE
    APP_STDOUT APP_STDERR DOSSIER APP_ATLAS OUTPUT)
  if(NOT DEFINED ${required})
    message(FATAL_ERROR "R35 determinism requires ${required}")
  endif()
endforeach()
file(REMOVE_RECURSE "${TMP}")
file(MAKE_DIRECTORY "${TMP}")
set(disc_deed "${TMP}/discovery.txt")
set(intermediate "${TMP}/trace_rebase.rest")
set(disc_source "${TMP}/R35_TRACE_CHARACTER_REBASES.lean")
set(disc_olean "${TMP}/R35_TRACE_CHARACTER_REBASES.olean")
set(disc_stdout "${TMP}/discovery.stdout")
set(disc_stderr "${TMP}/discovery.stderr")
set(state_atlas "${TMP}/states.tsv")
set(edge_atlas "${TMP}/edges.tsv")
set(tangent_atlas "${TMP}/tangents.tsv")
set(law_atlas "${TMP}/laws.tsv")
set(app_deed "${TMP}/application.txt")
set(final_rest "${TMP}/application.rest")
set(app_source "${TMP}/R35_HELDOUT_TRACE_REBASE.lean")
set(app_olean "${TMP}/R35_HELDOUT_TRACE_REBASE.olean")
set(app_stdout "${TMP}/application.stdout")
set(app_stderr "${TMP}/application.stderr")
set(dossier "${TMP}/dossier.md")
set(app_atlas "${TMP}/application.tsv")
execute_process(COMMAND "${DISC_EXEC}" "${disc_deed}" "${R34_REST}" "${intermediate}"
  "${PRIMARY}" "${SIGNED}" "${RECHARTED}" "${disc_source}" "${disc_olean}"
  "${disc_stdout}" "${disc_stderr}" "${FORMAL_ROOT}" "${TOOLCHAIN}" "${MANIFEST}"
  "${ARTIFACT_ROOT}" "${state_atlas}" "${edge_atlas}" "${tangent_atlas}" "${law_atlas}"
  RESULT_VARIABLE discovery_result)
execute_process(COMMAND "${APP_EXEC}" "${app_deed}" "${intermediate}" "${final_rest}"
  "${HELDOUT}" "${app_source}" "${app_olean}" "${app_stdout}" "${app_stderr}"
  "${FORMAL_ROOT}" "${TOOLCHAIN}" "${MANIFEST}" "${ARTIFACT_ROOT}" "${dossier}"
  "${app_atlas}" RESULT_VARIABLE application_result)
if(NOT discovery_result EQUAL 0 OR NOT application_result EQUAL 0)
  message(FATAL_ERROR "R35 deterministic replay deed failed")
endif()
set(baseline "${DISC_DEED}" "${INTERMEDIATE}" "${DISC_SOURCE}" "${DISC_STDOUT}"
  "${DISC_STDERR}" "${STATE_ATLAS}" "${EDGE_ATLAS}" "${TANGENT_ATLAS}" "${LAW_ATLAS}"
  "${APP_DEED}" "${FINAL_REST}" "${APP_SOURCE}" "${APP_STDOUT}" "${APP_STDERR}"
  "${DOSSIER}" "${APP_ATLAS}")
set(replay "${disc_deed}" "${intermediate}" "${disc_source}" "${disc_stdout}"
  "${disc_stderr}" "${state_atlas}" "${edge_atlas}" "${tangent_atlas}" "${law_atlas}"
  "${app_deed}" "${final_rest}" "${app_source}" "${app_stdout}" "${app_stderr}"
  "${dossier}" "${app_atlas}")
list(LENGTH baseline count)
math(EXPR last "${count}-1")
foreach(index RANGE 0 ${last})
  list(GET baseline ${index} left)
  list(GET replay ${index} right)
  if(index EQUAL 3 OR index EQUAL 4)
    file(READ "${left}" left_message)
    file(READ "${right}" right_message)
    string(REPLACE "${DISC_SOURCE}" "<R35_TRACE_CHARACTER_REBASES.lean>"
      left_message "${left_message}")
    string(REPLACE "${disc_source}" "<R35_TRACE_CHARACTER_REBASES.lean>"
      right_message "${right_message}")
    if(NOT left_message STREQUAL right_message)
      message(FATAL_ERROR "R35 path-relative checker replay differs at ${left}")
    endif()
  elseif(index EQUAL 12 OR index EQUAL 13)
    file(READ "${left}" left_message)
    file(READ "${right}" right_message)
    string(REPLACE "${APP_SOURCE}" "<R35_HELDOUT_TRACE_REBASE.lean>"
      left_message "${left_message}")
    string(REPLACE "${app_source}" "<R35_HELDOUT_TRACE_REBASE.lean>"
      right_message "${right_message}")
    if(NOT left_message STREQUAL right_message)
      message(FATAL_ERROR "R35 path-relative checker replay differs at ${left}")
    endif()
  else()
    execute_process(COMMAND "${CMAKE_COMMAND}" -E compare_files "${left}" "${right}"
      RESULT_VARIABLE difference)
    if(NOT difference EQUAL 0)
      message(FATAL_ERROR "R35 deterministic replay differs at ${left}")
    endif()
  endif()
endforeach()
file(WRITE "${OUTPUT}" "truth_status=established-bounded\nevidence=implemented-exact\n"
  "discovery_replay_exit=${discovery_result}\napplication_replay_exit=${application_result}\n"
  "compared_artifacts=${count}\npath_relative_checker_transcripts=4\ndeterministic=1\n")

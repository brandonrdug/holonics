...
struct characteristic_rest_record final {
...
struct characteristic_rest_receipt final {
...
struct characteristic_remount_receipt final {
  body::rest_receipt body{};
  exact::word theory{};
  bool same_body{};
...
[[nodiscard]] HOLONICS_CALLABLE constexpr std::uint64_t characteristic_rest_integrity(
    const characteristic_rest_record& record) noexcept {
  ...
  const std::uint64_t characteristic[5]{record.characteristic.identity.value(),
...
static_assert(std::is_trivially_copyable_v<characteristic_rest_record>);

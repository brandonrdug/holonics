...
[[nodiscard]] cultivated_store_receipt transfer(
    const char *path, event::trace_fiber_rest_record &record,
    bool write) noexcept {
  ...
  out.integrity_exact = out.state == cultivated_store_status::returned &&
                        record.integrity == event::trace_fiber_rest_integrity(record);
...
read_trace_fiber_rest(const char *path,
                      event::trace_fiber_rest_record &record) noexcept {
  return transfer(path, record, false);
}
cultivated_store_receipt write_trace_fiber_rest(
    const char *path, const event::trace_fiber_rest_record &record) noexcept {
  auto value = record;

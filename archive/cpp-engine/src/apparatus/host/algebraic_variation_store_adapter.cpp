...
[[nodiscard]] variation_store_status read_exact(const char* path, Record& record) noexcept {
  ...
  return closed && trailing == 0 ? variation_store_status::returned :
      variation_store_status::size_refused;
}

template<class Record>
[[nodiscard]] variation_store_status write_exact(const char* path,
  ...
  return ::close(descriptor) == 0 ? variation_store_status::returned :
      variation_store_status::transfer_refused;
}

}  // namespace

variation_store_receipt read_toric_cycle_handoff(
    const char* path, event::toric_cycle_rest_record& record) noexcept {
  variation_store_receipt receipt{}; receipt.state = read_exact(path, record);
  ...
  if (!receipt.integrity_exact) { receipt.state = variation_store_status::integrity_refused;
    return receipt; }
  receipt.bytes = exact::word{sizeof(record)}; receipt.transfer_calls = exact::word{1};
  return receipt;
}

variation_store_receipt write_algebraic_variation_handoff(
    const char* path, const event::algebraic_variation_rest_record& record) noexcept {
  variation_store_receipt receipt{};
  ...
  if (receipt.state == variation_store_status::returned) {
    receipt.bytes = exact::word{sizeof(record)}; receipt.transfer_calls = exact::word{1};
  }
  return receipt;
}

# Archived operating contract before HNA consolidation

**Status:** ARCHIVED. This is the exact contract at `08965182`, preserved as chronology.
The current root AGENTS.md governs; instructions below are not a second active contract.

# AGENTS.md — Pure holonics operating contract
...
- Treat `Λ_F ⊆ Q_F × A_F × L_F × P_F × Π_(B,F)` as the typed event schema. The historical tensor
  spelling is shorthand only inside a declared monoidal category and interaction doctrine. Do not
  create a solver module, language module, checker-shaped core, repository ontology, processor
...
- One continuing ecology has one owner. It is never `Clone` material and is never duplicated for
  rollback, branch search, inspection, scheduling, or device placement.
- Branches share immutable standing and own only their local differences. Atomic change uses a
  staged delta or recoverable ownership transfer.
...
- No floating-point operation exists in the production semantic/device-core dependency cone. A
  float may not determine control flow, index, coefficient, topology, identity, or any integer/bit
  later committed in a delta even if the float itself disappears before commit.
- CPU, GPU, accelerator, memory, disk, network, sensor, checker, and display are apparatus charts
  over one causal program. Placement is an exterior realization decision backed by a receipt.
- An admitted device hot path must return exact local/delta testimony and must not replay a full
  host algorithm after every device deed.
- Resource pressure either changes the same body's partition, factorization, placement, residency,
  schedule, or aperture, or remains an explicit obstruction. Raising a magic number and retrying
  the same morphology is forbidden.
- Physical telemetry is calibrated, aperture-bound testimony. Unknown energy stays unknown.

## Construction grade

Before code, name the source owners, port types, event occurrence, predecessor identity, local
constitutive law, receiver question, returned consequence, open alternatives, and grade. A
generated text, proof, image, action, or other requested deed must itself be returned and inspected;
diagnostic topology is not a substitute.

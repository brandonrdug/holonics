...
5. **Per-path identity for reuse.** One `SCH_SCREEN`, many `SCH_SHEET`s; identity = `KIID_PATH`; per-instance reference/unit; connections indexed by `SCH_SHEET_PATH` — instancing without copying via context-indexed attributes.

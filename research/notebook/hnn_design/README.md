...
| `hnn_lattice_growth.rs growth` | The deposited constitution's exact bits after each deposit under the budgeted lattice law (precision `k_m = 2⌊log₂ m⌋ + 1` at the locus's deposit clock since its founding), split into lattice entries, carried remainders and solved charts, with the widest carried remainder, the residuals each deposit releases and their bits, the entries whose lattice coordinate moved, the wall time of each refine, compare and deposit with the projected wall time of an exposure of `n*` cells and of the declared population, and (at the first aeons) the largest move of the admitted logits between `Θ` and `Θ + r`, in grains | Budgeted law under Decision 26's receiving law (receipts of September 25, this tree, rerun after the exogenous normal law and the standing read; the earlier law's receipts are in git history). **`growth chain 128 declared`** (the chain control, declared steps, 128 deposits): the bits rise and level off near 30,000 (19,323 at 16, 27,085 at 64, 30,288 at 128, the largest: entries 7,742, remainders 17,687 on 552 carried entries, solved 4,859); the widest remainder 51 bits; 22 aeon boundaries release no locus and leave the bits unchanged; from deposit 8, 78 deposits each release 433–542 residuals of 29,613–48,068 bits and the other 43 each release 39 or 43 residuals of 2,545–3,098 bits (reported, never retained); per refine 2 rem 63 over 128 ms, per compare 9 rem 14 over 128 ms and per deposit 10 rem 97 over 128 ms; the carried remainders move the admitted logits by `23129/2^19`, `20870987/2^24`, `653668119/2^28`, `138940425/2^27`, `108838355/2^27` and `1653797539/2^30` of a grain at the first six boundaries: more than one grain at four of them (at most `2 rem 116797207 over 2^28` grains, `\|Δf\|` reading `0 + 2/16 + ε` at `L_R = 16`), where the earlier law's stayed under one grain (the word-level certificate is owed, #62). **`growth campaign 40 declared`** (campaign 1's declared field over the pinned cut, `fed5488c…:docs/plans/THE_REBUILD.md`, 171,754 bytes; declared steps, 40 deposits): 225,836 → 1,096,555 → 1,285,576 → 1,333,881 bits at deposits 1, 16, 32, 40 (at 40: entries 473,556, remainders 642,962 on 22,090 carried entries, the widest 45 bits, solved charts 217,363 and still growing as the Grams fill), against `B_Θ = 2^33`; the first deposit, before any cell is ingested, steps only the receiving parametron's harmonic coordinate and its statistic (23 entries: the face reads the uniform logits and returns `Π Rᵀ ∇`); from deposit 4 each deposit releases 20,659–21,542 residuals of 2,247,613–2,442,597 bits (reported, never retained); no aeon boundary within 40 deposits; per refine 12 rem 35 over 40 ms, per compare 22 rem 21 over 40 ms and per deposit 83 rem 39 over 40 ms, so `n* = 6,148` cells (3,074 windows) project to 366,958 rem 30 over 40 ms (5,235 ms for the 40 deposits). Superseded, with no command in this tree (an earlier law, or the live file before the cut was pinned): the exact law on the chain control, 1,126 → 10,883 → 623,415 bits over two deposits (249 s for the second); the lattice with the remainders released at the aeon collapse; campaign 1 on the live file of 194,092 bytes; and both receipts before the lattice word, whose sizes and wall times were recorded only as decimals (git history) |
| `hnn_lattice_growth.rs equality` | The integral chart's equality with the termwise rational arithmetic, on the real cases: at every deposit each update recomputed over `Rat` alone (`ΔH`, `ΔW`, `Δh_x`, `Δx` with `Ratio`'s product; at the receiving map the exogenous law's `ΔB = Σ w χ fᵀ` and `ΔW = Σ w (χ − W f)(X̂ f)ᵀ`, and the harmonic coordinate's `Δh_x` and `Δh_R`, Decision 26), the carry's accounting `x' + r' + e = x + r + Δ` checked on every carried entry (`B`, `h_R` and its statistic included), each normal law's solved chart checked against its certificate (Decision 24: the exact left residual `‖1 − X̂H‖∞` at most the chart's certified `δ`), counted on an unmoved and on a moved Gram, and at the receiving map the exogenous law's receipt: `(W'H' − B') − (WH − B) = −(T − WF)(1 − X̂H')` exactly at the termwise step, the chart term at the carried Gram within the released bound `δ Σ |w| ‖χ − Wf‖∞ ‖f‖₁` (equal to the deposit's), and the reading's `‖W'H' − B'‖∞` and prior weight `tr(X̂)/n` recomputed, with the count of receipts at which the prior weight rose | Every value equal (receipts of September 25, this tree, rerun under Decision 26's receiving law). **`equality chain 8 declared`**: 5,048 carried entries, 26 solved charts certified on an unmoved Gram and 14 on a moved one, 6 exogenous receipts and 8 harmonic steps (191 ms). **`equality chain 32 declared`** (the chain control, 32 deposits at the declared steps): 20,192 carried entries (631 a deposit), 160 solved charts certified, 81 on an unmoved Gram and 79 on a moved one, 30 exogenous receipts (the first two windows reach nothing at the receiving map) and 32 harmonic steps; the prior weight falls from `129143/2^17` at deposit 3 to `825587/2^20` at deposit 32 and never rises (885 ms). **`equality chain 32 normal`** (the factor steps off): 20,192 carried entries, 117 solved charts on an unmoved Gram and 43 on a moved one, 16 exogenous receipts, the prior weight rising at one of them, by `1/2^17` (559 ms). **`equality campaign 8 declared`** (campaign 1 on the pinned cut, 8 deposits at the declared steps): 850,944 carried entries (106,368 a deposit), 11 solved charts certified on an unmoved Gram and 37 on a moved one, 7 exogenous receipts, the prior weight never rising, and 8 harmonic steps (19,091 ms). The earlier laws' receipts are in git history |

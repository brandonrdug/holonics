...
`holonics::hnn::landmark`) on the standing cut. The grain `L_R` and `|A|` are campaign 1's field's,
declared at the cut's population. Each emission (`Digits`: the cell's binary odometer digits, each
predicted at its dyadic cell and mixed over the preceding cells; `Cell`: the whole cell with
256-ary KT masses) chooses its address depth on the development cells only (`choose_depths`: `D`
rises from 1 while the development code length decreases strictly, charged `⌈log₂⌉` of the family
tried). Then `prequential` scores every cell, development and held-out, at the current standing
before its own deposit, and deposits it, for both trees and the online baselines over the same
cells in the same order. It prints each coder's bits a cell at the grain on both populations, each
tree's strict ordering against order-0, order-1 and PPM-2 (disjoint exact enclosures, or undecided
with the overlap), the β chart's rebases and bounds, and the executed dyadic face's cost against
the ideal ℚ face:
...
| `hnn_exposure.rs` | Campaign 1's exposure protocol (design (d)) on the standing real cut (`cut-file`) or the public development control, and its complete readout (design (f)). It prints the bits on the training and held-out targets against uniform, order-0 and order-1 KT and PPM of order 2, with the verdict against order-0; beside the model's, the count face's own code length (Decision 27: the receiving parametron's masses at each window's region read at the grain, no wave, at the model's constitution and window), each baseline against it, and the model against it, the wave's contribution. It prints `Kt` with the published keys against the literal over the cells read. Per key location, it prints each ring's fibre, orbits, fallback, failing loop, candidates, propagation work and re-keying jump. Per aeon, it prints the length, lift points, readings, epochs, collapse, the first law (exchange, deposition, total and change, each checked to telescope) and the face against the literal (`code + gain = literal`, checked). It prints the state and constitution bits per source bit with and without the collapse, the constitution's curve by carrier per commit, the budget stop or deadline, the work counted, the executed word's readout (Decision 24: the declared precisions, the charts' refinements with their starts, steps and largest certificate, the remainders the words and their returns released, and the tick balances' residuals against their certified bounds), and the host's wall time by phase (refine read, release, compare read, holon and covector, `pull_back`, `compose`, `deposited`, re-read and ingest, with the rest of the exposure) | Receipts of September 25, this tree, under Decision 27's receiving law (the count face at the preceding-cell region plus the wave; `\|describe\|` 1,415 bits, which codes the region partition, the receiving law 2 and `α = 1/2`). **`windows 8`** (the smoke: the `n*` cut of 6,148 cells, held out 4,958..6,148, deadline 8 windows): incomplete at its deadline at cell 16. 8 windows, 8 deposits, the path open on 8 of 8, and the peak bits inside a word 32. On the 16 training targets the model takes exact `[2531690487742021914390748721759/2^94, 10126761950968087657592396433175/2^96]` bits, `127 + 13/16 + ε` at `L_R = 16` (a cell: `7 + 15/16 + ε`), the count face alone `128 + 0/16 + ε`, against uniform 128, order-0 KT `124 + 8/16 + ε`, order-1 KT `128 + 0/16 + ε` and PPM `131 + 7/16 + ε`: above order-0 (model − order-0 KT reads `3 + 4/16 + ε`), below the count face (model − counts `−1 + 12/16 + ε`), order-1 and PPM. No held-out target is read, no aeon closes and no key is located. The constitution grows from 388,880 to 763,850 bits over 8 deposits (entries 428,054, remainders 235,921, solved 99,875). The dense moment takes 657,931 bits, and the counted state `⌈log₂N(16)⌉` takes 386: per source bit `657931/128` (5140 rem 11 over 128) and `193/64` (3 rem 1 over 64). `Kt` is exact `[30855758586591522604082710966879/2^94, 123423034346366090416360245413655/2^96]` bits, `1557 + 13/16 + ε` (keys 0, work 15), above the literal of 128 by `1429 + 13/16 + ε`. Host 1,132 ms (141 rem 4 over 8 a window), card 1,177 ms (147 rem 1 over 8); the two readouts are identical line for line (71 lines) outside the wall times and the traffic. **Campaign 1's exposure under Decision 27, resident on the card** (`hnn_exposure -- cut-file .local/cuts/standing-real-cut-campaign-1.bin cells all realization card`, 3,074 windows, 2,479 deposits, complete, no budget stop): held out (1,190 cells) the model takes `7118 + 2/16 + ε` bits (a cell `5 + 15/16 + ε`), the count face alone `5924 + 7/16 + ε` (a cell `4 + 15/16 + ε`), online order-0 KT `5666 + 12/16 + ε`, online order-1 KT `5164 + 13/16 + ε`, PPM `3923 + 4/16 + ε`: the model above order-0 by `1451 + 5/16 + ε` (FAILURE, disjoint exact enclosures) and above the count face by `1193 + 11/16 + ε` (the wave raises the code length), the count face above order-0 by `257 + 10/16 + ε`; the enclosures' exact endpoints go with the receipt to #73. 396,596 ms on the card (129 rem 50 over 3,074 a window), 530,508 ms on the host (172 rem 1780 over 3,074); the two readouts are identical line for line (2,678 lines) outside the wall times and the traffic. The earlier laws' and trees' smokes and receipts are in git history. **The host realization on the standing real cut** (`hnn_exposure -- cut-file .local/cuts/standing-real-cut-campaign-1.bin cells all windows 24`, run from the repository root, 24 windows on 24 workers): 6,729 → 1,352 ms a window. Per window, before → after: refine read 453 → 198, release 21 → 9, compare read 452 → 0 (the compare takes the refine's kept read), holon and covector 16 → 9, `pull_back` 1,420 → 253, `compose` 2,597 → 479, `deposited` 1,238 → 147, re-read 494 → 218, ingest 0 → 0, the rest 33 → 35. The readout outside the wall times is identical, line for line, to the tree before the change (the bits and code lengths, `Kt`, the constitution's curve, the state and the work). **Campaign 1's first-law exposure** (`hnn_exposure -- cut-file .local/cuts/standing-real-cut-campaign-1.bin cells all`, 3,074 windows; 477,915 ms on the host in the resident exposure's receipt below): its receipt is recorded in #73. **The lattice word** (Decision 24): below the table |
| `hnn_diagnose.rs` | Campaign 1's located failure on the standing real cut (review E1): the exposure protocol step for step through the port, with the decoder's, learning's, the keys' and the source's readings between its methods. Under Decision 27 its decoder readings take the count face out of the machine's face and read the wave alone | Not rerun under Decision 27. Receipt of September 25, under the receiving law before Decision 26 (at `13d6bb92`; `hnn_diagnose -- cut-file .local/cuts/standing-real-cut-campaign-1.bin`, 614 s for the exposure; its rerun with the exact print reads the same values). The exposure's bits reproduce exactly (held-out `7 + 9/16 + ε` against order-0 KT `4 + 12/16 + ε`, bits a cell at `L_R = 16`). Held out, bits a cell through the machine's face: the declared prior's reading `R_0 z` alone `10 + 9/16 + ε`, the machine `7 + 9/16 + ε`, the learned part alone `6 + 11/16 + ε`, the least-squares oracle `R = y wᵀ` on the machine's own features `5 + 0/16 + ε` (`4 + 15/16 + ε` in-sample), the static order-0 face `4 + 12/16 + ε`. The prior's reading holds a share in `[51/56, 3713/4077]` of the held-out logits' energy; `Σh = 141 + ε`, `ε ∈ [2871/3571, 3154/3923]`, against `ln det H_T ∈ [62482/407, 621904/4051]`, below it by `[30317/2588, 26768/2285]`; the constant's reach from ring 2's dormant mode lies in `[3255/4057, 1108/1381]`. Keys: the truth in the fibre on 128 of 128 synthetic cribs; on the cut every fibre empty and the best injective partial closure within its shuffles at 15 of 16 ring-locations. Source: online KT given `τ_0 mod 5` `4 + 14/16 + ε` and given the whole configuration `7 + 12/16 + ε`, against order-0's `4 + 12/16 + ε`; the recent cells a median `1/132` of the source ring's current bin. The reading is the [located-failure record](../../records/2026-09-25_CAMPAIGN_ONE_LOCATED_FAILURE.md) |
| `hnn_landmark.rs` | The landmark tree (Decision 28, count-only; `hnn::landmark`) on the standing real cut: each emission's depth sweep on the development cells, then the prequential run of the tree `Digits` (its executed dyadic face and its ideal ℚ face beside it), the tree `Cell` and the online baselines (uniform, order-0 and order-1 KT, PPM of order 2) over the same cells in the same order, every cell scored before its own deposit; each tree's strict ordering against order-0, order-1 and PPM-2; the β chart's rebases and bounds; the executed face's residual against the ideal | Receipt of September 26, this tree (`hnn_landmark -- cut-file .local/cuts/standing-real-cut-campaign-1.bin`; 6,148 cells, held out 4,958..6,148 from the manifest; `n* = 6,148`, `L_R = 16`, `W = 20`, `B = 8`, `M_f = 23`). **The sweeps** (development, 4,958 cells, bits a cell): `Digits` `D = 1` `4 + 0/16 + ε`, `D = 2` `3 + 12/16 + ε`, `D = 3` `3 + 10/16 + ε` (in all `18152 + 7/16 + ε`), `D = 4` `3 + 10/16 + ε` (`18067 + 2/16 + ε`), `D = 5` `3 + 10/16 + ε` (`18098 + 7/16 + ε`, above `D = 4`): `D = 4`, charged `⌈log₂ 5⌉ = 3` bits; `Cell` `D = 1` `4 + 15/16 + ε` (`24539 + 0/16 + ε`), `D = 2` not strictly below it: `D = 1`, charged 1 bit. **Held out** (1,190 cells, bits a cell): tree `Digits` `3 + 1/16 + ε` (its ideal ℚ face `3 + 1/16 + ε`), tree `Cell` `4 + 12/16 + ε`, uniform 8, online order-0 KT `4 + 12/16 + ε`, online order-1 KT `4 + 5/16 + ε`, PPM-2 `3 + 4/16 + ε`, `ε ∈ [4417870558441969212515442601779/(2^97·595), 4417870558441969212515443198871/(2^97·595)]` (the exact PPM enclosure the located-failure record owed). Tree `Digits`, charged its 3 bits, lies **below** order-0 by `−1987 + 15/16 + ε` bits (a cell `−2 + 5/16 + ε`), below order-1 by `−1485 + 14/16 + ε` (a cell `−2 + 12/16 + ε`) and below PPM-2 by `−243 + 7/16 + ε` (a cell `−1 + 12/16 + ε`), each by disjoint exact enclosures: Decision 28's criterion holds. Tree `Cell` lies above all three (above order-0 by `[0 + 15/16 + ε, 1 + 0/16 + ε]` bits in all, its charged bit: its 256-ary KT contexts never earn their split, and its face stays at the root's order-0 KT face). **Development** (bits a cell): tree `Digits` `3 + 10/16 + ε`, tree `Cell` `4 + 15/16 + ε`, order-0 `4 + 15/16 + ε`, order-1 `5 + 1/16 + ε`, PPM-2 `3 + 12/16 + ε`; tree `Digits` below all three (`−6468 + 1/16 + ε`, `−7257 + 12/16 + ε`, `−666 + 10/16 + ε` bits). **The charts**: tree `Digits` 63,320 nodes, 91,781 rebases (6,120 at the most-rebased node; bound `765/16384` bits there, below `1/16`), the executed face's cost over the ideal `0 + 0/16 + ε` bits in all on either population (held out `ε ∈ [2935342533586913480524057/2^96, 183458908349182092606305/2^92]`), the largest certified per-cell residual at most `463939/2^31` bits against the rule's `24596/2794153`; tree `Cell` 108 nodes, 6,050 rebases, all at its root (bound `3025/2^16` bits). The run's development code lengths equal the sweeps' at the chosen depths. Wall: the sweeps 216,970 ms (the two emissions together), the prequential run 56,757 ms. A second run, identical outside its wall times and that residual's print, confirmed it. An earlier attempt read the wide exact faces directly through `log2_enclosure` (about 80 ms a call on a face of about 3,000 bits, against about 3 ms on its 98-bit mantissa) and was stopped for the mantissa reading (`landmark::code_length`) |

...
[project-postulate] The current view is the [continuing coupled field](CONTINUING_FIELD.md).
It evolves the source currents, thermal modes and spatial support before computing the receiver
image. The fixed-emitter/aperture construction below is retained as a local optical control;
it did not fulfill the request for an evolving Holonic field. Use the continuing-field build
recipe for the live visualization.
...
## Rebuilding the earlier optical control

...
**Position:** Actual observed continuation is committed through `1697bed1`. The [fixed-condition audit and projection reuse](research/records/2026-09-11_FIXED_CONDITION_COLLAPSE_IS_EXACT_AND_ITS_PROJECTION_IS_REUSABLE.md) now return the exact cause of the 73-way ties: all 9,254 calibration rows were checked against the saved native relation; its 72-dimensional vertical space fills the zero-mass return hyperplane, and its unit-source equations force h=(1,0). The scoped collapse theorem builds in Lean. Immutable projection reuse passes its dedicated control, all 15 coupled and four session tests. The actual 199-passage field returns byte-identical complete model/predictions/cursor/progress with field time 287,069→148,563 ms and 9,248,000 extra resident bytes. This improves execution while preserving the model's ambiguity. Further exposure in that unchanged fixed chart cannot repair contextual discrimination. Actual situated source/condition and producing-family material bindings, lawful refinement/formation and useful-model evaluation remain. The goal is active; AC0–AC5 is incomplete.

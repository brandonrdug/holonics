...
| First prediction against reference (September 19, #8) | [Boltz-2 on the M5 sequences](research/experiments/m5_predicted_vs_reference/README.md), four predictions entered through the existing intake and read with the two Protenix runs and the design — seven presentations. Three recorded M5 numbers reproduce exactly through an independent composition. On the RING core (41–80) rigidity agrees across all seven and only the contact family and `dim ker Jᵀ` separate them; on the N-terminal arm everything separates. Boltz-2's seed-to-seed contact separation (21 of 10,368) is of the order of its separation from the design (30, 44). Recurring section minima on the core each have one rigid cluster straddling them: packing features, not hinges. The reference is a design; no measured structure has entered. Limits met by execution: analytic width undecided above 24 coordinates in all fourteen chain readings; the exact chain reading costs 11–40 s at 36 coordinates and its resolvent ceiling stops at 32 residues, so the 204-monomer complex is out of reach. |
| Measured structures and the theorem-backed chain (September 19, Wave 9, #47) | Three measured RBX1 structures (2LGV NMR, 3DPL X-ray, 7Z8R cryo-EM) and 32 Boltz-2 runs (8 seeds × target/complex × with/without zinc) entered through the existing intake: 57 presentations. At the within-chain contact receiver **predicted-vs-measured separation (34–51 of 2,926) is of the order of the predictor's seed spread (21–47) and of the experiment's own spread (crystal vs cryo-EM 40; NMR ensemble 44); the design is the outlier (65, 61)**. Zinc halves the seed spread and moves the arm from 9.6 Å to 2.2 Å. In the library, the chain's rank is now determined by the nullity theorem (no resolvent; 0.34 s against 8 s), `SpectralReading` carries structural licences (real, on-axis, non-growth with the conservative core, Pontryagin bound; split and hand stated separately), and the elastic network's analytic face is decided by structure at any extent. The join read 30 chains on measured, designed and predicted windows with zero refusals: measured and Boltz-2 agree on the core's minimal section (cut 19, section 5–6); the design does not (cut 11, section 8). Reached 180 coordinates in 568 s; 324 did not return; the 612-coordinate complex is refused by the contact-assembly ceiling. [Experiment](research/experiments/m5_predicted_vs_reference/README.md). |
| Next action | One worker: the contact assembly — sparse faces, positive semidefiniteness by construction instead of an `O(n³)` elimination, and one assembled form carried through a reading — so the whole target and the complex are reachable without raising a ceiling. Then #9's receiver family with a rigidity class stable under sampling, the rigid-cluster hinge verdict inside `HingeSearch` (#38), and the identity atlas's first instance (#49). |
...
and [worker brief](docs/WORKER_BRIEF.md) set the wave shape. Wave 8 ran it: two parallel workers,
one join worker, one reviewer that re-ran nothing, seven confirmed findings repaired in-wave, and
one integrated closing run (3,231 engine tests, full Lean build) recorded in the receipt log.

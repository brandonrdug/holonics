...
| Next action | One worker at a time, driven by what execution exposed: reduce the chain reading through the neck's `r` channels so width and transfer are decidable at real sizes (#36, with #42's owners in `RH/` and `Millennium/Hodge*`); fold a receiver family into one held-out agreement count (#9); put the rigid-cluster hinge verdict inside `HingeSearch` (#38); rerun with the zinc cofactor and against a measured structure. |

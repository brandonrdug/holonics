...
**Position:** Coupled emission/session integration is committed through `1354085b`. The [actual-continuation return](research/records/2026-09-11_ACTUAL_CONTINUATIONS_JOIN_THE_COMPLETE_COUPLED_FAMILY.md) adds the explicit whole-family `(p,c)→(c,v)` observation passage, rest v4 and public `receive-next-symbol`. All 15 coupled native, four session, three host rest and seven delivery checks pass. The actual sequence-67 prefix `I ` was received and saved at epoch 13. A fresh process generated two newlines and re-entered their pair at epoch 16, with successful complete persistence. Both choices were 73-way projected ties at 1/73; the source continuation was `fo`. Conditional admission now works in this observed-state chart, but the result is not useful. Two observations erase old current dependence; this is not a replacement for unpaired contextual source actuation. Contextual producing-family material return, condition-bearing formation/reuse and useful-model evaluation remain. The goal is active; AC0–AC5 is incomplete.

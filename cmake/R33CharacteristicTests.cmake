...
holonic_found(NAME r33.characteristic_discovery_device_deed
  EXECUTABLE r33_characteristic_discovery_device_deed
  COMMAND
    "${R33_DISC_DEED}" "${R32_FINAL_REST}" "${R33_INTERMEDIATE_REST}" "${R33_POSITIVE}"
    "${R33_SIGNED}" "${R33_RECHARTED}" "${R33_DISC_SOURCE}" "${R33_DISC_OLEAN}"
    "${R33_DISC_STDOUT}" "${R33_DISC_STDERR}" "${R33_FORMAL_ROOT}" "${R33_TOOLCHAIN}"
    "${R33_FORMAL_MANIFEST}" "${PROJECT_BINARY_DIR}/artifacts" "${R33_WORD_ATLAS}"
    "${R33_PAIR_ATLAS}" "${R33_GROUP_ATLAS}" "${R33_LAW_ATLAS}")
holonic_found(NAME r33.characteristic_application_device_deed
  EXECUTABLE r33_characteristic_application_device_deed
  COMMAND
    "${R33_APP_DEED}" "${R33_INTERMEDIATE_REST}" "${R33_FINAL_REST}" "${R33_HELDOUT}"
    "${R33_APP_SOURCE}" "${R33_APP_OLEAN}" "${R33_APP_STDOUT}" "${R33_APP_STDERR}"
    "${R33_FORMAL_ROOT}" "${R33_TOOLCHAIN}" "${R33_FORMAL_MANIFEST}"
    "${PROJECT_BINARY_DIR}/artifacts" "${R33_DOSSIER}" "${R33_APP_ATLAS}")

holonic_found(NAME r33.characteristic_host_conformance
  EXECUTABLE r33_characteristic_host_conformance
  COMMAND
...
holonic_found(NAME r33.source_access_audit
  COMMAND
    "${CMAKE_COMMAND}" -DDISC_EXEC=$<TARGET_FILE:r33_characteristic_discovery_device_deed>
    -DAPP_EXEC=$<TARGET_FILE:r33_characteristic_application_device_deed>
    -DPROBE=$<TARGET_FILE:r33_open_probe> -DDISC_LOG=${R33_DISC_LOG}
    -DAPP_LOG=${R33_APP_LOG} -DOUTPUT=${R33_SOURCE_AUDIT} -DDISC_DEED=${R33_DISC_DEED}
    -DAPP_DEED=${R33_APP_DEED} -DR32_REST=${R32_FINAL_REST}
    ...
    -DHELDOUT=${R33_HELDOUT} -DDISC_SOURCE=${R33_DISC_SOURCE} -DDISC_OLEAN=${R33_DISC_OLEAN}
    -DDISC_STDOUT=${R33_DISC_STDOUT} -DDISC_STDERR=${R33_DISC_STDERR}
    -DWORD_ATLAS=${R33_WORD_ATLAS} -DPAIR_ATLAS=${R33_PAIR_ATLAS}
    -DGROUP_ATLAS=${R33_GROUP_ATLAS} -DLAW_ATLAS=${R33_LAW_ATLAS}
    -DAPP_SOURCE=${R33_APP_SOURCE} -DAPP_OLEAN=${R33_APP_OLEAN}
    -DAPP_STDOUT=${R33_APP_STDOUT} -DAPP_STDERR=${R33_APP_STDERR} -DDOSSIER=${R33_DOSSIER}
    -DAPP_ATLAS=${R33_APP_ATLAS} -DFORMAL_ROOT=${R33_FORMAL_ROOT}
    -DTOOLCHAIN=${R33_TOOLCHAIN} -DMANIFEST=${R33_FORMAL_MANIFEST}
    -DARTIFACT_ROOT=${PROJECT_BINARY_DIR}/artifacts -P
    "${PROJECT_SOURCE_DIR}/cmake/R33SourceAccessAudit.cmake")

holonic_found(NAME r33.determinism
  WITHHOLD TMP
  COMMAND
    "${CMAKE_COMMAND}" -DDISC_EXEC=$<TARGET_FILE:r33_characteristic_discovery_device_deed>
    -DAPP_EXEC=$<TARGET_FILE:r33_characteristic_application_device_deed>
    ...
    -DAPP_STDOUT=${R33_APP_STDOUT} -DAPP_STDERR=${R33_APP_STDERR} -DDOSSIER=${R33_DOSSIER}
    -DAPP_ATLAS=${R33_APP_ATLAS} -DOUTPUT=${R33_DETERMINISM} -P
    "${PROJECT_SOURCE_DIR}/cmake/R33Determinism.cmake")
holonic_found(NAME r33.seal
  COMMAND
    "${CMAKE_COMMAND}" -DOUTPUT=${R33_SEAL} -DSOURCE_AUDIT=${R33_SOURCE_AUDIT}
    -DDISC_DEED=${R33_DISC_DEED} -DAPP_DEED=${R33_APP_DEED}
    -DHOST_CONFORMANCE=${R33_HOST_CONFORMANCE} -DINTERMEDIATE=${R33_INTERMEDIATE_REST}
    -DFINAL_REST=${R33_FINAL_REST} -DDISC_SOURCE=${R33_DISC_SOURCE}
    -DDISC_OLEAN=${R33_DISC_OLEAN} -DWORD_ATLAS=${R33_WORD_ATLAS}
    -DPAIR_ATLAS=${R33_PAIR_ATLAS} -DGROUP_ATLAS=${R33_GROUP_ATLAS}
...
holonic_found(NAME r33.post_seal_literature_comparison
  COMMAND
    "${CMAKE_COMMAND}" -DSEAL=${R33_SEAL} -DSOURCE=${R33_DISC_SOURCE}
    -DLAW_ATLAS=${R33_LAW_ATLAS} -DLITERATURE=${R33_LITERATURE_SOURCE}
    -DOUTPUT=${R33_LITERATURE_COMPARISON} -P
    "${PROJECT_SOURCE_DIR}/cmake/R33LiteratureComparison.cmake")

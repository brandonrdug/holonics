...
| Gemma 4 complete multimodal excitation and lift | SCF2 exterior-chart boundary under `NativeTransportScaffold.lean` | `holonic_intelligence/{foreign_multimodal,scaffold_excitation_receipt,scaffold_lift}.rs`; move-owned `NativeEcologyRest` handoff | complete text tower on CPU BF16; vision/audio towers and native conduct on CUDA | **exact-bounded:** all 2,130 declarations reconcile; 42 language, 16 vision, and 12 audio frontiers returned actual BF16 codewords for text, image, sampled video, and audio; 14 returned occurrences founded a source-neutral scaffold while modality, tensor, source, intervention, and codeword testimony remained cold |
| Common multimodal scaffold cycle | `NativeTransportScaffold.lean::{RepeatedInferenceCirculation,ScaffoldReleasePassage}` | `complete_native_transport_scaffold_cycle_scf6.rs` through the public lift, handoff, circulation, cultivation, and release owners | resident word/current apparatus beside five exterior contact charts | **exact-bounded:** text-to-text, image-to-text, audio-to-text, sampled-video-to-text, and interleaved multimodal-to-text use one routing law; cold label mutation leaves native wire unchanged; generated surfaces are probes and do not select continuation or cultivation |

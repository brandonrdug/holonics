...
| Finite neural ecology local-current step | `Computation/HolonicNeuralEcology.lean` | `native_ecology/mod.rs`; `native_ecology/inference_ecology.rs`; `native_ecology/recurrent.rs`; `native_ecology/continuation.rs` | `cuda_refine/device_recurrence.rs`; `device_ecology_types.rs` | **UAR-open:** full dependent complex continuation is not structural yet |
...
| Eros cultivation and Athena rest | `Computation/ErosAthenaNeuralObjects.lean`; `Computation/HolonicCultivationCharts.lean` | `cultivation_derivation.rs`; `receiver_history_cultivation.rs`; `native_ecology/cultivation_overlay.rs`; `soma/life/src/athena_native/exterior_relational_witness.rs`; `soma/life/src/athena_native/source_neutral_rest/`; `situated_cultivation/` | `athena_integrated_front.rs`; `complex_parametron.rs`; `membrane_moment_*` | **UAR-open:** the direct typed construction owner exists; detached live remount, attributable child, withdrawal, and later conduct remain |

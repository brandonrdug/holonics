...
| `ReceiverHistory` | `Framework.Core → Foundation.CausalRelevance → Millennium.ReceiverHistory`; `Framework.Information` separately imports it in the full Framework | `Foundation.CausalRelevance` uses complete receiver-history quotient; other Framework and computation clients import it directly. |
...
| [`Millennium/ReceiverHistory.lean`](../../../formal/elementary-holonics/ElementaryHolonics/Millennium/ReceiverHistory.lean#L22), 89 lines | Entire declaration family is generic and used by Foundation: `CompleteReceiverHistoryQuotient`; `causalSignature`; `quotientEq_iff_causalSignatureEq`; `quotientNe_returnsSeparatingReceiverHistory`; `quotientEqualityRetainsBothOccurrences`. Candidate owner: `Holonics.Receiver` or a receiver-history submodule under it. It extends the existing `ReceiverHistoryCompression`, so its dependency should point inward to that generic owner. | No distinct Millennium-specific instance theorem found in this module; these are generic receiver/history laws. Keep the actual research applications in their callers. | **Generic cut clear.** Check namespace/path and avoid duplicating the corresponding declarations already in `Foundation.ReceiverHistoryCompression`. The latter is not this module and must not be removed by name similarity. |
...
This is the current source-import set found by exact `import ElementaryHolonics.Millennium.<name>`
search; it is not a claim that every listed importer consumes the generic declaration being moved.
...
| `ReceiverHistory` | `Foundation/CausalRelevance.lean`, `Framework/Information.lean`, `Computation/{HolonicExcitationFoundedQuotient,HolonicNeuralEcology}.lean`, `Millennium/{HolonicGranularBoundaryRadiation,HolonicFiniteCausalAperture}.lean` | Core importers switch to generic owner. Research importers stay in `HolonicsResearch` and should receive any moved declarations from `Holonics`. |

...
| `holonic-structure/src/membrane.rs`: `CausalMembrane` trait (`:7-22`), re-exported by structure `src/lib.rs:56` and previously re-exported by membrane `src/lib.rs:34` | Duplicate membrane re-export, not a membrane-owned implementation | `AgenticLanguageEcology` implements the trait (`holonic-life/src/agentic_language/ecology.rs:1686`); its parent module imports it from `holonic_structure`, and its documentation references that in-scope trait method at `:767`. All live Rust imports/implementations resolve through `holonic_structure`; no caller used `soma_membrane::CausalMembrane`. Removed the unused membrane re-export. |

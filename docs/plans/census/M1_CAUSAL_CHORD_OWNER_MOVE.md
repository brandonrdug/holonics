...
The portable synthetic law and checked-wire tests move beside the main owner. The engine keeps four cross-owner tests as `tests/causal_chord_engine.rs`: agreement with `lattice_gauge::exact_spectrum`,
...
Verification passed on the stacked CUDA-mount branch. Focused gates:

- `cargo test -p holonics --lib receiver::causal_chord::tests`: 43 passed
- `cargo test -p holonic-engine --test causal_chord_engine`: 2 passed, 2 M5 measurements ignored
- `cargo check --locked -p holonics -p holonic-engine --all-targets`: passed

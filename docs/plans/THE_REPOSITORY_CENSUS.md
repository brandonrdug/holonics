...
| R3 Core edges cut (#66) | At the R0 snapshot `Framework.Core` reached five historical Millennium owners transitively. Their generic declarations now live in Foundation; a source-import traversal reaches 26 in-package modules and no Millennium or RH module. The [declaration-level edge audit](census/LEAN_CORE_R3_EDGES.md) preserves each research remainder. The Ratio and MomentStorage direct research ingress paths have been cut in their generic owners; other full-Framework research ingress remains. | Keep the unique research theorems; measure and curate the remaining full Framework closure and rebuild affected research importers. The Core and full package root gates passed for the five cuts. |
| R4 in progress (#67) | Normal-wave rest v5 was emitted only for pending `LegacyCut` sources, the frozen producing-cut representation. V6 is current when transport is `Applied` and pending is empty; v7–10 are coupled forms and v11 carries pending source joints. | Retire v5 and its decoder/remount path; keep v6 only for empty-pending Applied transport, reject any v6 pending IDs, preserve v1–4 and v7–11. Verify current writer selection and the v6/v11 readers before merging. |
| R4 complete (#67) | `receiver_release::Generator` was a compatibility alias for `ZonotopeGenerator`, with no source caller; it also collided by name with the distinct core clocked `Generator`. | Removed the alias and pointed the owner map at `ZonotopeGenerator`; the current zonotope serde name and wire shape remain `Generator`. Receiver-release, standing and workspace gates passed. |

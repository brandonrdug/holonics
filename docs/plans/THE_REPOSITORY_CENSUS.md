...
| R3 Core edges cut (#66) | At the R0 snapshot `Framework.Core` reached five historical Millennium owners transitively. Their generic declarations now live in Foundation; a source-import traversal reaches 26 in-package modules and no Millennium or RH module. The [declaration-level edge audit](census/LEAN_CORE_R3_EDGES.md) preserves each research remainder. The separate `Objects.Ratio → HolonicGaugeCovariance → HolonicConnectionVariation` and `Holon.MomentStorage → HolonicQuadraticMomentCondensation → HolonicGranularBoundaryRadiation` ingress paths remain in the full Framework. | Keep the unique research theorems; cut the remaining Ratio and MomentStorage edges by actual declaration use, then measure the full Framework closure and rebuild affected research importers. The Core and full package root gates passed for the five cuts. |

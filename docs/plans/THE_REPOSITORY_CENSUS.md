...
|---|---|---|
| R1 | `platform_x11.rs` was the only x11rb implementation and `lib.rs` cfg-gated its export. Both type-level callers are retained and recharted: `arithmetic_dimensional_receiver` uses `MemoryPlatform` after its existing headless snapshot path; `desktop_receiver` uses `MemoryPlatform` for one scripted local-star traversal and retains its TSV trace schema. | Remove the X11 source/module/re-export, default feature and x11rb dependency atomically with both example edits. Verify the named examples, `platform`/`display` tests, engine library and workspace all-targets. The source compositions remain; only the X11 input/display adapter retires. |

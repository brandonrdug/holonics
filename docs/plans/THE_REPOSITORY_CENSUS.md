...
| R4 complete (#67) | Normal-wave rest v5 was emitted only for pending `LegacyCut` sources, the frozen producing-cut representation. V6 remains current when transport is `Applied` and pending is empty; v7–10 are coupled forms and v11 carries pending source joints. | Retired v5 and its decoder/remount path; v6 rejects pending IDs and retains Applied transport. Typed refusal tests, current v6/v11 and coupled CUDA roundtrips, and locked workspace targets passed. |

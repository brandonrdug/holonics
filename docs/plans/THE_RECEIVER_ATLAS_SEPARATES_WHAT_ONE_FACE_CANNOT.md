...
[`holonics::receiver::causal_chord`](../../crates/holonics/src/receiver/causal_chord.rs) and
...
`holonics::receiver::causal_chord` founds one: axis roots with multiplicity from `gcd(P,Q)` for
...
`holonics::receiver::causal_chord` returns exactly `diag(-2 gamma_b e_b)` — the rotation part of each block cancels
...
`holonics::receiver::causal_chord::cospectral_graphs_are_separated_by_the_response_atlas` — degree sequences

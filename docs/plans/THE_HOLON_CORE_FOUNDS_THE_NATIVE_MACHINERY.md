...
| 12b coupled continuation and field internals | Original `wip/consolidation-phase-12b` untouched; current return cut on `codex/consolidation-phase-12b` (draft #111) | Stacked on Phase 12a | Current coupled v7/v12 and published v7 rests; read-only coupled v8–v10 and dependent v1–v6 programme readers retired; the unconsumed field-history exterior-placement ladder is retired. Ordinary remount keeps every current historical carrier resident. The focused archive packet's rest/source/material/operative tests and engine lib/all-target checks pass; see the [Phase 12b packet record](census/C_PHASE12B_INTEGRATION.md). | `current_history_source`, contextual lift and material transport remain in scope. The HNA session contract, behavior measurement and `NormalWaveHolon` reception/actuation chart also remain; Phase 14 remains open. |

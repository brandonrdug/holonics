...
| Acoustic receiver: a resonator bank consuming the colour receiver's own co-present mode population | `Foundation/AcousticReceiver.lean::{pole,cayley,gain,cayley_den_ne_zero,normSq_cayley_lt_one,normSq_cayley_eq_one,realBlock,trace_realBlock,det_realBlock,charpoly_realBlock,rateForm_realBlock,Bank,Bank.drive,Bank.drive_add,Bank.advance,Bank.run,Bank.run_causal,Bank.run_add,Bank.pressure,Bank.pressure_add,Bank.timbre,energy,energy_advance_le,energy_advance_lt,colour,colour_does_not_distribute,quantize,quantize_then_superpose_ne_superpose_then_quantize,metamer_sounds_different,unison_looks_different,population_change_moves_both}` | — (the standalone `holonic-engine` Rust mirror was retired in R1; distinct native Athena acoustic charts remain under `holonic-life::native_intelligence::membrane_acoustic` pending R2) | — | [proved-derived; formal-checked; Rust mirror historical] The receiver **R2** of `docs/plans/THE_RECEIVER_ATLAS_SEPARATES_WHAT_ONE_FACE_CANNOT.md`. The continuous resonator's `exp(λt)` is not rational, so the declared law is the exact Cayley map `z=(2+hλ)/(2−hλ)` at a declared rational step, a Gaussian rational in the same carrier the colour receiver's currents use; `‖2+hλ‖²−‖2−hλ‖²=8h·Re λ` makes stability preservation exact, and `Re(2−hλ)=2+hγ≥2` makes the step total. The bank was the exact block-diagonal `Linearization` realization, so each audible component's source mode, excitation, transport path, pole factor, residue, approximation error and residual were the corresponding `ChordComponent`, not a second account. Superposition through the bank and pressure projection last are formal laws; the quadratic colour response and integer readout have concrete counterexamples. Colour and timbre are two maps out of one `ExactReceiverPhasePopulation` and neither refines the other: exchanging which mode carries which quadrature is a colour metamer the bank separates, and current on an uncoupled mode is an acoustic unison colour separates. The checked Lean owner remains current; Rust resource ceilings and the standalone PCM/WAV exporter were properties of the retired mirror. The separate native Athena acoustic charts are not this Cayley resonator-bank implementation. |

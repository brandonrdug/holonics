...
| Landmarks and normalized constraint modes | `LandmarksAndModuli`, `Farey`, `Polarity`, `PiIterationConstraint`, `MachinPhaseConstraint`, distinct Copson/Newman source families | `holonics::geometry::winding_inertia` owns the exact symmetric-circulant passage operator; `holonic-engine::winding_inertia` owns growth/form receiver search and lattice application; exact analytic/recurrence and polynomial owners | Existing period, branch, winding and threshold relations precede the atlas. Follow [the source distinction](MATHEMATICS_AND_NATIVE_CONDUCT.md#constraint-defined-modes-and-named-de-bruijn-boundaries). |
...
The existing `holonics::ratio::surprisal` owner retains
...
| Continuing tower, gluing trichotomy, and materialized face | `Foundation/ContinuingTower.lean::{Tower,CompatibleSection,ObservationFibre,GluingResult,gluingResult_total,MaterializedFace,ComputableTower}` | `crates/holonics/src/restriction/tower.rs::{Tower,CompatibleSection,ObservationFibre,GluingResult,MaterializedFace,ComputableTower,check_restriction_laws,glue_chain,computable_section}` | — | [established-bounded; formal-checked, implemented-exact] Moved to the Holon core's restriction facet in restructure phase 6 and reached directly at `holonics::restriction::tower`. The two owners cite each other by declaration name. Lean quantifies over the whole index; the Rust owner checks `restrict_refl`/`restrict_trans` over a declared chart aperture and returns a `RestrictionReceipt`, and a violation is a typed `TowerRefusal`, never a panic. `unique`/`plural`/`obstructed` all have inhabitants on both sides (`unitTower`/`UnitTower`, `padicTower`/`ResidueTower`, `shiftTower`/`ShiftTower`). The tower joins `Millennium/HolonicDirectedPassage.lean::SuccessorWitnessSystem` through `nonempty_compatibleSection_of_surjectiveAdjacent` and `Millennium/Gluing.lean::GluingPassage` through `gluingPassage`/`obstruction_nonempty_iff`; it founds no parallel family. Carrier item **C2** is now returned in the same pair (see the two rows below) |
| Observation fibre identified with the preimage fibre | `Foundation/ContinuingTower.lean::Tower.observationFibre_eq_preimageFibre`; `Foundation/Holon.lean::PreimageFibre` | one `holonics::restriction::tower::ObservationFibre`; no second Rust type by construction | — | [proved-derived; formal-checked] Carrier item **C3** holds definitionally: the restrict-to-chart receiver is `Tower.chartHolon`, and its preimage fibre is the tower's observation fibre by `rfl`. `Tower.chartReceiverTransformer_iff` is `Foundation/Receiver.lean`'s descent criterion at two charts, with `padicChartInsufficiency` as an explicit `ReceiverInsufficiency` witness |
...
|---|---|---|---|
| Normalized exponential, sigmoid and adjoint geometry | `Computation/HolonicAdjointNormalization.lean` | `holonics/src/ratio/exponentiated.rs` and `holonics/src/ratio/exponentiated/transport.rs`; `holonics/src/ratio/surprisal.rs`; `kernels/exact_resident_section.cu`; `kernels/exact_resident_adjoint.cuh` | Exact symbolic ratios, normalized vector-current JVP/pullback/material step, binary sigmoid derivative; resident exponential/contact/return keep their own numerical scope. |

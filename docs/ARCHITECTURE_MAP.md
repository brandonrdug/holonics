...
The [research routes](../research/records/README.md) connect the dated derivations to these owners.
...
| Winding and carry; pair lock and modular address; trace faces conserved by phase carriage; cell holonomy and the dormant harmonic class | `Geometry/{PhaseCarry,PairResonance}`, `Transport/{GeneratorTraceFaces,CellHolonomy}`, joining `Millennium/{Farey,LocalFactor,TraceSequence}` and `Foundation/HodgeReceiver`; imported by `Framework.{Geometry,Dynamics}` | `relational_geometry::winding::{phase,winding,carry,Odometer,LockAddress,SiteFactor,Machine,triangle_holonomy,regauge}` | [Winding guide](WINDING_CARRY_AND_PLACEMENT.md). Analytic staircases, p-adic limits, coupled-site factors and continuum curvature are outside the checked scope; the zero-pair and primes-as-knots correspondences are graded interpretations. Consumers: #17 machine geometry, #18/#61 economy checks, #62. |
| **Helical pair interaction: pair contact, phase-carried material, reflected return, menu closure** | [`Transport/HelicalPairInteraction`](../formal/elementary-holonics/ElementaryHolonics/Transport/HelicalPairInteraction.lean), joining `Geometry/ScrewGeometry` to `Transport/HolonicInteraction`; imported by `Framework.Dynamics` | `relational_geometry::screw::{ScrewPair,PairQuadranceJet}` with `holonic_interaction::{ContactFace,Medium,HolonicInteraction}` and `holonic_chain`. `holonic_interaction/helical.rs` now constructs the checked pair contact/interaction through its explicit rate port, and `holonic_chain/serial.rs` returns the supported serial joint/contact and target-fibre laws. HNA now consumes both the legacy quadrance-current receiver and `GeneratorMachineSpec` through the fixed body with realified currents, affine score/value maps and pair-factor sparse D; the public ordered source/session now consumes that body; constrained geometry and standing remain #17 | The [unit](HOLON.md#the-helical-pair-interaction-unit) is the HNN site. Returned local pair adapter #28/#48 and Cayley/prismatic serial consumer #27; generator-machine geometry/source/phase binding remains #17, broader formal counterparts #62. |
...
| Situated screw pair, finite phase closure and moment reuse | `Geometry/ScrewGeometry`; `Millennium/HolonicQuadraticMomentCondensation`; `LandmarksAndModuli`/`Farey`/`Polarity` phase owners | `relational_geometry::screw::{SituatedScrew,ScrewPair,PairFiniteMotion,RationalPhase}` → `holonic-engine::receiver_history_compression::HelicalMomentReuse` | Exact proper affine actions only; the supported phase/map relation is validated, lifted winding retains polygon and extra-turn contributions, and ordered words retain their complete sequence. The decoder's separator is scoped to the ambient linear moment fibre; physical rank-one/homogeneous/clock constraints remain explicit. |
...
bilinear/reaction/reflection, transposed scatters and held refinements form one complete paired
...
| Dynamic reflection and retained interior return | `Physics/ReflectedBoundaryMemory.lean` | Linear-map residual/Riccati defect, changing-chart derivative, boundary reduction iff `E_next A=U E` and `E_next B=0`, forcing and moving-encoder rate; passive static-elimination counterexample; imported by `Framework.Physics` |
...
`GradedCausalComplex`, which gives the constraint complex its first library consumers and, through
it, `rebase_invariants`, `sheaf_diffusion` and `lattice_gauge::exact_spectrum`. The aperture's
third class is not resolved by the adapter: an `Open` reading indexes a family of `2^n` complexes
whose two bounding members are constructed, and a member's `OpenResolution` travels with it. The
new Lean owner `Foundation/AperturedGradedComplex.lean` states the graded complex, `∂∘∂ = 0`, the
aperture trichotomy and the admissibility condition; nothing in it uses `sorryAx`.

[established-bounded; implemented-exact; formal-checked; measured] `grain_tower.rs` founds the
grain axis `Component ⊑ Residue ⊑ Atom` that the M5 driver's alpha-carbon intake filter
(`crates/holonic-life/examples/m5/cif.rs::REPRESENTATIVE`) had asserted rather than built. Over the same 70,632
residue pairs at an equal 8 Å aperture in exact rational arithmetic, the complete atom-grain
restriction returns 1,397 `Inside` where the alpha-carbon receiver returns 301, with 0 pairs in the
other direction; the least coarse aperture that carries every fine contact on this data is
`116656202097359145/10^14`, that is 34.1550 Å or 4.2694 times the fine aperture, attained at a
CUL1 residue whose own carbonyl oxygen sits 27.6 Å from its alpha carbon. The inflation is measured
and carries that witness; it is not a chosen safety factor. One open reading exists at each grain
and they are at different pairs, so the open class does not transport. The new Lean owner
`Foundation/GrainRestriction.lean` states the coarse-implies-fine theorem, the counterexample to its
converse, the triangle-inequality inflation law and the `ApertureRelation` trichotomy; nothing in it
uses `sorryAx`.

[established-bounded; implemented-exact; formal-checked; measured] `physical_intake.rs` with its
`mmcif`, `numpy` and `deflate` submodules is item **B2** of the biological ecology: the library path
from an exterior file to a founded face, which previously existed only inside an example driver
(`crates/holonic-life/examples/m5/{cif,npy,input,artifact}.rs`) and a test module
(`grain_tower/tests.rs::read_all_atoms`). It retains **every** `_atom_site` row rather than alpha
carbons, admits `<f2`, `<f4` and `<f8` uncertainty words through the exact decoders at
`exact_value::ieee754::decode_binary{16,32,64}_bits`, and reads a `.npz` container directly — the RFC 1951 and ZIP
decoder in `physical_intake/deflate.rs` is an exterior container codec that decides no mathematics
and verifies each member's CRC-32. The coordinate discipline the library owns is **per token**: a
written coordinate's enclosure is its own last decimal place, and the scaled integer wire is a
separately declared, always-outward projection, so an atom's enclosure never depends on how many
digits an unrelated atom was written with. The four M5 cross families reproduce through this path
exactly — 70,632 pairs, fine-inside 1,397, coarse-inside 301, fine-only 1,096, coarse-only 0, per
family 239 / 307 / 215 / 335 — so the measured table now has two independent readers agreeing rather
than one reader asserted twice. An occurrence cannot be founded without its environment index:
`AddressedUncertainty::found` is the only constructor and takes it by value, and a Boltz-2 `.npz`
that carries only `pae` is refused with every absent environment array named until a caller declares
the index that run was produced under. The new Lean owner `Foundation/ExteriorIntake.lean` states
that decoding an exterior codeword is exact and total on finite words, that a wider format's dyadic
grid carries every value of a narrower one, that an outward projection contains its source and
therefore never flips a decision, and that no occurrence is founded from a presentation with no
environment; nothing in it uses `sorryAx`.

[established-bounded; implemented-exact; formal-checked; measured] `rigidity_receiver.rs` owns the
receiver **R4** of the receiver atlas and the rigidity half of **B7** of the biological ecology: the
constraint Jacobian `J_eta = D F_eta(q)` of a contact complex's one-skeleton over exact rational
coordinates, `ker J` as the infinitesimal motions, `ker J^T` as the self-stress and constraint
reaction space, the trivial motions measured as an exact rank rather than assumed to be six, rigid
clusters as maximal pairwise-implied sets, and contact-removal sensitivity read off the self-stress
support. `exact_linear`'s `rank`, `kernel_basis` and `cokernel_annihilator` are reused rather than
reimplemented, and a presented position whose coordinate box has width is refused by name rather
than collapsed into a representative. The open class stays plural exactly as it does in
`physical_constraint_grading`: the return is a family whose refusing and admitting bounds are both
constructed. On the authenticated M5 release, a 24-residue RBX1 alpha-carbon window at an 8 Å
aperture separates the three presentations sharply — the designed structure returns
rank 62, `dim ker J = 10`, `dim ker J^T = 8` over 70 constraints; the free Protenix prediction
rank 53, 19 and 6 over 59; and the CUL1-bound Protenix prediction rank 50, 22 and 0 over 50, so the
Maxwell count is a strict bound on the first two and exact only on the third. The new Lean owner
`Foundation/RigidityReceiver.lean` states the constraint system, the Jacobian as the differential
through an exact algebraic expansion, both null spaces, both rank–nullity identities, the Maxwell
relation and the theorem that a rigid motion lies in `ker J`; nothing in it uses `sorryAx`.
...
| Addressed causal-natural holon, generated section, tensor receiver, and artifact passage | `Foundation/Holon.lean::{Rebase,PreimageFibre,ofEvolution,mapReceiver}`; `Foundation/CausalNaturalHolon.lean`; `Foundation/HolonTensorLens.lean` | Native section/relation/field realizations are audited in `docs/HOLON.md`; no general automatic compiler | — | **formal-only** for the general correspondence: generated faces commute through encoded evolution; serial deterministic refinement retains one seed population; tensor lenses reuse the common receiver operation; parameter arrows transport preimages; homogeneous slot permutations lift coherently; exact remount retains its stated scope |
...
| Continuing tower, gluing trichotomy, and materialized face | `Foundation/ContinuingTower.lean::{Tower,CompatibleSection,ObservationFibre,GluingResult,gluingResult_total,MaterializedFace,ComputableTower}` | `crates/holonic-core/src/restriction/tower.rs::{Tower,CompatibleSection,ObservationFibre,GluingResult,MaterializedFace,ComputableTower,check_restriction_laws,glue_chain,computable_section}` | — | [established-bounded; formal-checked, implemented-exact] Moved to the Holon core's restriction facet in restructure phase 6 and re-exported at `holonic-engine::continuing_tower`. The two owners cite each other by declaration name. Lean quantifies over the whole index; the Rust owner checks `restrict_refl`/`restrict_trans` over a declared chart aperture and returns a `RestrictionReceipt`, and a violation is a typed `TowerRefusal`, never a panic. `unique`/`plural`/`obstructed` all have inhabitants on both sides (`unitTower`/`UnitTower`, `padicTower`/`ResidueTower`, `shiftTower`/`ShiftTower`). The tower joins `Millennium/HolonicDirectedPassage.lean::SuccessorWitnessSystem` through `nonempty_compatibleSection_of_surjectiveAdjacent` and `Millennium/Gluing.lean::GluingPassage` through `gluingPassage`/`obstruction_nonempty_iff`; it founds no parallel family. Carrier item **C2** is now returned in the same pair (see the two rows below) |
| Observation fibre identified with the preimage fibre | `Foundation/ContinuingTower.lean::Tower.observationFibre_eq_preimageFibre`; `Foundation/Holon.lean::PreimageFibre` | one `continuing_tower.rs::ObservationFibre`; no second Rust type by construction | — | [proved-derived; formal-checked] Carrier item **C3** holds definitionally: the restrict-to-chart receiver is `Tower.chartHolon`, and its preimage fibre is the tower's observation fibre by `rfl`. `Tower.chartReceiverTransformer_iff` is `Foundation/Receiver.lean`'s descent criterion at two charts, with `padicChartInsufficiency` as an explicit `ReceiverInsufficiency` witness |
| Non-invertible chart transition carrying its residual | `Foundation/ContinuingTower.lean::{Transition,Transition.comp,comp_residual,ofEquiv_residual_subsingleton,injective_of_subsingleton_residual,laterReceiverFactors,residual_separates,ofLinearSection,toReceiverHistoryCompression,apply_transportWord}`; `Tower.restrictTransition`; `Tower.section_witness_reopened`; `coarseGrain_comp_residual_reassembles` | `continuing_tower.rs::{Transition,ComposedTransition,NoResidual,CoarseGrain,ExactShift,RationalFloor,TowerRestrictTransition}` | — | [established-bounded; formal-checked, implemented-exact] Carrier item **C4**. Composition composes residuals (the composite residual *is* the pair, by `rfl` in Lean and by the associated type in Rust); an invertible transition has a `PUnit`/`NoResidual` residual and a subsingleton residual forces injectivity; the residual is exactly what a later finer receiver reopens, in both directions (`laterReceiverFactors` for sufficiency, `residual_separates` for necessity). `Foundation/SectionResidual.lean`'s linear owner is exhibited as one instance through `ofLinearSection`, and `Foundation/ReceiverHistoryCompression.lean::generatorExact` supplies the every-ordered-word law rather than a second induction. Rust residuals are `BigUint`/`BigInt`/`Rat`; no float participates. A `Migration` as a functor of index categories with a natural transformation is now returned in the same pair (see the three rows below) |
| Canonical non-Archimedean instance: `ℤ_p` is the tower's compatible-section family | `Foundation/ContinuingTower.lean::{padicSectionEquiv,padicOfSection,toZModPow_padicOfSection,padicSection_injective,padic_toZModPow_surjective,Tower.CompatibleSection.ext}` | `crates/holonic-core/src/restriction/tower.rs::{ResidueTower,computable_section}` (an exact `BigUint` state stands for the `ℤ_p` element a program can hold) | — | [proved-derived; formal-checked, implemented-exact] Carrier item **C2**, both directions. Forward is `padicSection` (`PadicInt.toZModPow` at every level); reverse is `padicOfSection`, built with Mathlib's `PadicInt.ofIntSeq` from the section's own compatibility and **not** a rebuilt inverse limit. `PadicInt.ext_of_toZModPow` closes one round trip and `Tower.CompatibleSection.ext` the other. The Rust side holds only the integers a program can hold, which is the declared aperture and is stated as such |
| Exact fibre splitting of the residue tower | `Foundation/ContinuingTower.lean::{padicDigitGap,padicReopenGap,padicFibreEquiv,padicFibre_card,padicAdjacentFibre_card,padic_sameFace_iff_sub_mem_ker,padicObservationFibre_reaches_exactly_p,padicRestrictTransition,padicRestrictTransition_residual_complete}` | `continuing_tower.rs::{FibreSplitting,SplittingRefusal,ResidueTower::split_fibre,ResidueTower::coset_count_bits}` | — | [proved-derived; formal-checked, implemented-exact] The fibre over a level-`m` face is in *exact* bijection with `ZMod (p ^ k)` at level `m + k`, so `Nat.card` of it is `p ^ k` on the nose and one refinement step splits it into exactly `p` cosets — an equality, not an asymptotic count. `padic_sameFace_iff_sub_mem_ker` identifies the fibre with a coset of `RingHom.ker (PadicInt.toZModPow n) = Ideal.span {p ^ n}` through Mathlib's `PadicInt.ker_toZModPow`. C2's coset index **is** C4's residual: `padicRestrictTransition` is `Tower.restrictTransition` with `padicDigitGap` as the residual, and `padicRestrictTransition_residual_complete` proves the residual is a complete invariant of the fibre. Rust is `BigUint` throughout; a hostile chart gap is measured by `coset_count_bits` and refused as `SplittingRefusal::CarrierAboveCeiling` before any carrier of that width is allocated |
| `Migration`: functor of index categories with a natural transformation on faces | `Foundation/ContinuingTower.lean::{Migration,Migration.comp,comp_assoc,comp_identity,identity_comp,carrySection,carrySection_comp,carryObservationFibre}` | `continuing_tower.rs::{Migration,check_migration_naturality,carry_section,ComposedMigration,IdentityMigration,HalvingMigration}` | — | [established-bounded; formal-checked, implemented-exact] Carrier item **C4**'s missing structure. Naturality is the whole content and is stated and proved; migrations compose, composition is associative and the identity migration is a two-sided unit, all three by `rfl`. `carrySection` is the consuming operation — an object saved under one chart family read under another — and `carrySection_comp` makes a schema *history* replayable step by step or composed first with the same result. The Rust owner checks naturality over a declared chart aperture and returns a `NaturalityReceipt`, or a typed `MigrationRefusal`; `padicHalfMigration`/`HalvingMigration` is the lossy witness whose every face map drops `p ^ j` cosets while the continuing object survives unchanged |
| `Holon.Rebase` exhibited as the invertible `Migration` | `Foundation/ContinuingTower.lean::{DiagramIndex,diagramTower,diagramTowerSectionEquiv,rebaseMigration,rebaseMigrationTransition,rebaseMigration_residual_subsingleton,rebaseMigration_carryObservationFibre_occurrence}`; `Foundation/Holon.lean::{Rebase,Rebase.preimageFibreEquiv}` | `continuing_tower.rs::IdentityMigration` (identity index, `NoResidual` by type) | — | [proved-derived; formal-checked] `Rebase` **fits**, and is not founded again: a holon's own diagram is a height-two tower over `{sourcePort, targetPort, facePort} ⊑ occurrence`, `diagramTowerSectionEquiv` shows its compatible sections are exactly the occurrences, and `rebaseMigration` is the migration with identity index whose four components are `Rebase`'s four equivalences. Its three `_natural` fields are exactly the three nonreflexive instances of `Migration.naturality`; `rebaseMigration_carryObservationFibre_occurrence` and `rebase_preimageFibreEquiv_occurrence` are the two `rfl`s saying `carryObservationFibre` and `preimageFibreEquiv` carry the same occurrence. The one recorded bookkeeping discrepancy: `Tower.Face : Index → Type v` puts a tower's faces in one universe while `Holon.{u,v,w,x}` allows four, so the presentation is stated for a holon whose four types share a universe; no mathematical content is lost |
| Relation between `Migration` and `Transition` | `Foundation/ContinuingTower.lean::{ResidualMigration,ResidualMigration.transition,ofTransitionFamily,transition_ofTransitionFamily,ofTransitionFamily_transition,square_routes_agree,square_either_route_reopens,Transition.tautological,residual_is_not_determined_by_the_face_map,naturality_is_genuine_content}` | `continuing_tower.rs::{ResidualMigration,ResidualMigration::check_reopen,check_migration_square_reopen}` | — | [proved-derived; formal-checked, implemented-exact] The relation is **two-sided and neither containment holds**. A bare `Migration` is strictly less than a family of `Transition`s: `residual_is_not_determined_by_the_face_map` exhibits one face map carrying the exact remainder and the tautological residual, so the residual is deposited data. A bare family of `Transition`s is strictly less than a `Migration`: `naturality_is_genuine_content` exhibits a family that is a lawful transition at every index and still fails the square. `ResidualMigration` is exactly the conjunction — `ofTransitionFamily` and `transition` are mutually inverse by `rfl` in both directions — so a residual migration **is** an index functor together with a family of `Transition`s satisfying naturality, nothing more and nothing less. `square_either_route_reopens` is the lossy-codec statement: one transported face, two different retained residual pairs, each reopening the source exactly |
| A migration can connect charts a tower relates in no way: the non-factoring condition | `Foundation/ContinuingTower.lean::{Migration.StaysAtItsChart,Migration.FollowsRefinement,Migration.ConnectsIncomparableCharts,Migration.FactorsThroughRefinement,not_factorsThroughRefinement_of_connectsIncomparableCharts,TwoCharts,twoCharts_no_common_refinement,twoChartTower,swapMigration,swapMigration_not_factorsThroughRefinement,rebaseMigration_staysAtItsChart,padicHalfMigration_factorsThroughRefinement}` | `continuing_tower.rs::{ChartRoute,IndexRouteReceipt,check_index_routes,FactorisationReceipt,check_factors_through_restriction,TwoChartTower,SwapMigration}` | — | [proved-derived; formal-checked, implemented-exact] A tower's passage is typed `i ≤ j → Face j → Face i`, so it exists only along refinement; between incomparable charts it does not exist, and a common refinement supplies none either because a span `Face i ← Face k → Face j` is not a map. `ConnectsIncomparableCharts` names the migrations that cross that gap and is decided by the **index map alone**; `not_factorsThroughRefinement_of_connectsIncomparableCharts` separates it from `FactorsThroughRefinement`, which is "read the finer chart, restrict back down". The condition is neither vacuous nor universal: `swapMigration` over an index with `twoCharts_no_common_refinement` has it, while `rebaseMigration` (identity index) and `padicHalfMigration` (lossy but refinement-following) do not. Lossiness and non-factoring are independent properties. Where no common refinement exists the migration is the only passage there is |
| Traversability is the retained residual | `Foundation/ContinuingTower.lean::{Migration.ReversePassage,injective_of_reversePassage,reversePassage_of_injective,not_reversePassage_of_not_injective,ResidualMigration.residual_reverse_passage,ResidualMigration.injective_of_subsingleton_residual,ResidualMigration.traversability_is_the_residual,padicHalfMigration_face_not_injective,padicHalfMigration_no_reverse_passage,padicHalfMigration_residual_restores_the_reverse_passage}` | `continuing_tower.rs::{ReversePassageReceipt,check_reverse_passage}` | — | [proved-derived; formal-checked, implemented-exact] At one chart the reverse passage **from the migrated face alone** exists exactly when the component is injective — equivalently exactly when the residual could have been dropped — and when it does not, the retained residual restores it exactly through `reopen_apply`. `padicHalfMigration` is the sharp instance: at every positive chart its component merges C2's `p ^ j` cosets, so nothing carries the migrated face back, and the retained digit block returns the source face with no remainder |
| A tower is the transverse section of a tube; a wormhole is exact | `Transport/ContinuingTube.lean::{ObserverAt,VisibleSquare,visibleSquare_shift,CoveredBy,CoverAdjacent,DeclaredTube,profileFlat_of_chartwise,tube_profile_is_flat_at_every_horizon,cycleTube,flat_near_curved_far,ladderStep,the_fine_observer_sees_at_one_step_what_the_coarse_one_sees_at_three,receiver_defect_is_a_structural_defect,ladderReceiverInsufficiency,descending_chain_is_the_tubes_own,rising_step_needs_the_retained_residual,the_three_kinds_of_cross_rank_passage,ChartwiseMigration,toMigration_staysAtItsChart,ofStationaryMigration,Tube,Tube.square,transport_roundTrip,refinementRoute,transport_is_not_a_wormhole,refinementRoute_is_not_a_wormhole,Wormhole,swapWormhole,lossySwapMigration,lossiness_is_independent_of_being_a_wormhole,SquareVerdict,squareVerdict_total,grainSquareDefect,grainSquare_defect_is_not_a_loss,ChartwiseMigration.cocycleDefect,tube_circuit_has_no_defect,flipCircuit_carries_no_invariant_end,padicTube_end_equiv,padicTube_crossSection_branching,Tube.clockedSpan,clockedSpan_split_rejoin,boundaryScaleMigration,projectionScalePassage_no_reverse_passage}`; `Transport/WorldTube.lean::{ClockedSpan,resegmentation_natural,openGap_has_no_joined_occurrence}` | `crates/holonic-engine/src/continuing_tube.rs::{StationedTower,check_commuting_square,SquareVerdict,check_circuit_holonomy,HolonomyVerdict,wormhole_receipt,WormholeReceipt,ConstantTube,FlipTube,LossySwapMigration,GrainReadingTube,TubeRefusal,Observer,IndexDirection,IndexReading,index_distance,HorizonDeclaration,HorizonReach,horizon_reach,FaceReading,two_axis_width,DefectProfile,defect_profile,SquareReading,CrossRankPassage,classify_cross_rank,RouteMove,Route,RouteBound,RouteSet,RoutePlan,plan_routes,PresentationTube,GrainContactCount,HorizonRefusal}` | — | [established-bounded; formal-checked, implemented-exact] The join of the tube and tower families; it founds no third carrier. A tube's transverse section at each station is a `Tower` and its longitudinal axis is `ClockedSpan`; the two commute, and that square **is** `Migration.naturality` with the identity index map (`ChartwiseMigration` is proved to be exactly a `Migration` that `StaysAtItsChart`). A **wormhole** is `Migration.ConnectsIncomparableCharts` on that object: a tube's own longitudinal transports and refinement routes never are, `swapWormhole` is, traversability is `traversability_is_the_residual` (cited), and lossiness is independent — all four cells inhabited. Neither axis of a functorial tube carries holonomy (`Tower.restrict_roundTrip`, `transport_roundTrip`), so non-orientability is a declared circuit and its owner is `Foundation/ReceiverAtlas.lean::CocycleDefect`. The `p`-adic tube's cross-section branches `p ^ k` ways and its end is `ℤ_[p]`, both cited. `boundaryScaleMigration` identifies `BoundaryScalePassage` as a chartwise migration and `projectionScalePassage_no_reverse_passage` shows it is not a `Transition`. Rust checks the square over a declared aperture and returns `SquareVerdict::Commutes \| ::Defect` with its witness, refusing a declared aperture, face population or work product above its ceiling with checked arithmetic; the defect arm runs on `grain_tower.rs`'s own `GrainFace` and `GrainSelection`, whose M5 census is cited rather than re-measured here. **T5 extends it to the two-axis horizon**: an observer is a station *and* a chart, `index_distance` walks covers inside the declared aperture and returns `NoChain` — the wormhole case — rather than an infinite distance, `horizon_reach` restricts toward the coarse (a determined face) and opens the fibre toward the fine (plural), and `defect_profile` reads curvature as the non-commuting squares and circuit holonomies inside `(h, k)` with each defect's exact receiver discrepancy and its `RelationLadder` rung. A functorial tube's profile is identically zero; two observers of one region are related by `visibleSquare_shift`, the triangle inequality; `classify_cross_rank` separates the tube's own composite, the passage a retained residual buys and a wormhole; `plan_routes` enumerates every route inside a declared bound with its receipt and returns both faces where two routes disagree, ranking nothing. `PresentationTube` composes `GrainReadingTube` over the three M5 RBX1 presentations: the grain square fails and the environment square commutes, and the two routes' exact discrepancies 239/307/550 are the measured 1,096 of 1,397 fine contacts against 301
| Cost of a migration against the refinement route | `Foundation/PresentationCost.lean::{CostedTower,costBoundedByRefinementRoute_of_route,costBoundedByRefinementRoute_is_caller_decided,padicCostedTower,unitCostedTower,costBoundedByRefinementRoute_padic,no_costedTower_with_squaredGapReceipt,swapMigration_route_isEmpty}`; `Foundation/ContinuingTower.lean::Migration.CostBoundedByRefinementRoute`; `Foundation/ReceiverCodeCost.lean::serial_boundary_balance` | `presentation_cost.rs::{RouteComparison,CostedResidueTower,squared_gap_receipt}` | — | [proved-derived; formal-checked] **Discharged over a concrete cost, and the shape shown caller-decided as it was stated.** Carrier item **C5** is owned by `Foundation/PresentationCost.lean` and `presentation_cost.rs`; `ReceiverCodeCost.serial_boundary_balance` owns the one-boundary additivity law and is cited, never rebuilt. Over a caller-supplied ordered cost the inequality records a choice of the caller (`costBoundedByRefinementRoute_is_caller_decided`); over the concrete `CostVector` it is a theorem about a `CostedTower` for every migration that factors through refinement. Costed towers exist and their `route_dominates` is derived, not assumed — `padicCostedTower` carries the receipts `padicTower`'s own restrictions determine and `costBoundedByRefinementRoute_padic` discharges the comparison against `padicHalfMigration` with neither side supplied by a caller; `unitCostedTower` is the minimal instance; `no_costedTower_with_squaredGapReceipt` exhibits a receipt assignment no costed tower can carry, so the law is neither vacuous nor automatic. On the residual axis the law is `codeBits` subadditivity (`codeBits_residual_comp_le`) and **not** `serial_boundary_balance`. For a migration that connects incomparable charts the comparison is undefined rather than unproved, since there is no refinement route to measure |
| Cyclotomic irreducibility over a coprime cyclotomic base, and the `Z_p`-tower layer degree | `Foundation/IwasawaTower.lean::{isPrimitiveRoot_mul_of_coprime,isCyclotomicExtension_mul_of_coprime,theRelativeCyclotomicDegreeIsTheTotient,theCyclotomicPolynomialIsIrreducibleOverTheCoprimeLevel,theCoprimeIrreducibilityIsNotVacuous,thePPowerLayerHasDegreeP,theRamifiedLayerCyclotomicIsReducible,theRelativeOrbitFillsTheCoprimeLevel}`; `Millennium/NormRelation.lean::TheRelativeOrbitFillsTheCoprimeLevel` | formal-only | — | [proved-derived; formal-checked] Carrier item **C8**'s named blocker, discharged. `cyclotomic n` is irreducible over `Q(zeta_m)` for coprime `m, n`, by the tower law over `Q` (`IsCyclotomicExtension.finrank` twice plus `Nat.totient_mul`) rather than a new irreducibility argument, and `NormRelation`'s open `Prop` is proved. Two corrections travel with it: the `Prop` does not actually need the relative irreducibility (the composed level is already cyclotomic over `Q`, and CRT supplies the unit of `(Z/ml)^x`), and the relative irreducibility is **false** in the vertical layers — `theRamifiedLayerCyclotomicIsReducible` — because those are totally ramified of degree `p` |
| `Lambda = Z_p[[T]]`, `omega_n` and its divisibility chain | `Foundation/IwasawaTower.lean::{Lambda,omegaPoly,omega,omegaPoly_monic,omegaPoly_natDegree,omegaPoly_coeff,omegaPoly_isDistinguished,omegaPoly_dvd_succ,omegaPoly_dvd_of_le,omega_dvd_of_le,omegaQuotient_finrank,groupRingPoly,omegaGroupRingEquiv,groupRingQuotient_finrank_eq_omegaQuotient_finrank,gammaSectionEquiv}` | `crates/holonic-engine/src/iwasawa_tower.rs::{IwasawaLevel,omega,omega_divides_omega_succ}` | — | [proved-derived; formal-checked, implemented-exact] `Gamma ~ Z_p` is **cited** from `ContinuingTower.padicSectionEquiv`, not rebuilt. `omega_n = (1+T)^(p^n) - 1` is deposited as a polynomial so that monicity, degree `p^n`, the distinguished shape (`Nat.Prime.dvd_choose_pow`) and the free rank `p^n` of `Z_p[T]/(omega_n)` are theorems; `omegaGroupRingEquiv` proves `Z_p[T]/(omega_n) ~ Z_p[gamma]/(gamma^{p^n} - 1)` as `Z_p`-algebras, so the finite level **is** the group ring of `Gamma/Gamma_n`. The power-series form of that quotient is Weierstrass division and is cited, not proved. The Rust owner computes the same coefficients as exact `BigInt` binomials and verifies the chain by exact polynomial division |
| An Iwasawa module is a `Tower`; the control kernel and cokernel are retained | `Foundation/IwasawaTower.lean::{transitionOfHom,transitionOfHom_residual_subsingleton_iff,quotientTower,quotientTowerSection,quotientTowerSection_injective_iff,QuotientTowerComplete,quotientTowerSection_bijective_of,quotientTowerSection_not_injective_of_top,omegaSubmodule,omegaTower,omegaTowerSection_injective_iff,SelmerTowerData,SelmerTowerData.tower,ControlData,ControlData.kernel,ControlData.cokernel,ControlData.transition,cokernel_subsingleton_iff,control_bijective_iff,control_bijective_iff_residuals,theControlKernelAndCokernelAreBothRetained}`; `Millennium/SelmerCalculus.lean::selmer` | `crates/holonic-engine/src/iwasawa_tower.rs::{IwasawaTower,OmegaRestriction,LambdaPresentation,FiniteSpecialization}` | — | [proved-derived; formal-checked, implemented-exact] Carrier item **C8**'s tower half. The Rust `IwasawaTower` is an *instance* of `continuing_tower::Tower` and `OmegaRestriction` an instance of `continuing_tower::Transition` whose residual is the exact quotient by the coarser `omega`; no second tower vocabulary is founded. `quotientTower` makes any antitone filtration a C1 `Tower`; the section map is injective **exactly** when the filtration is separated, and completeness is a carried hypothesis, never asserted. `SelmerCalculus.selmer` is lifted levelwise at that file's own abstraction — abstract groups with an abstract receiver family, no Galois cohomology constructed — with corestriction as the tower restriction and `cores_res` the degree law. The control map is a C4 `Transition` whose retained residual **is** its kernel, and `theControlKernelAndCokernelAreBothRetained` exhibits both residuals nonzero, so C8's control statement is "the two residuals are carried", not "the control map is an isomorphism" |
| The characteristic face is conditional and blind above codimension one | `Foundation/IwasawaTower.lean::{StructureData,StructureData.characteristicFace,squareStructure,splitStructure,theCharacteristicFaceDoesNotDetermineTheModule,PseudoNull,pseudoNull_annihilated,pseudoNullStructure,thePseudoNullResidueIsInvisibleToTheFace,WeierstrassData,omegaWeierstrassData,weierstrassData_isEmpty_at_zero,TheGrowthLaw,TheMainConjecture,theMainConjectureIsNotSourceIdentity}`; `Foundation/ContinuingTower.lean::theOrderFaceDoesNotDetermineTheModule` | `crates/holonic-engine/src/iwasawa_tower.rs::{CharacteristicFace,LambdaPresentation,SpecializationType,GrowthExponents,measure_growth}` | — | [counterexample; formal-checked, implemented-exact] `StructureData` is a hypothesis carrier, never an `axiom`; both counterexamples are unconditional because they construct the datum. `Lambda/(T^2)` and `Lambda/(T) + Lambda/(T)` share the face `(T^2)` and are not isomorphic (`T` kills the second, not the first); a nonzero finite module annihilated by `p` and `T` has unit characteristic face. `mu` and `lambda` are growth exponents read off `WeierstrassData`; the growth law is an open `Prop` cited to Iwasawa 1959 / Washington Thm 13.13 and **not** proved, and it is never called entropy. `TheMainConjecture` is receiver-exactness and `theMainConjectureIsNotSourceIdentity` proves it is not source identity. Rust measures the same pair at `p = 3`: equal orders `3^2, 3^4, 3^6` with types `[2]/[1,1]`, `[1,3]/[2,2]`, `[2,4]/[3,3]` at levels 0,1,2, and the exact fits `(mu, lambda, nu)` for `T-p`, `p`, `p^2`, `(T-p)(T-p^2)` each verified against a fourth level; the module computes no `p`-adic `L`-function and bears on no main conjecture |
| Apertured graded complex and its boundary law | `Foundation/AperturedGradedComplex.lean::{GradedComplex,BoundarySquaresToZero,d_comp_d,d_d_apply,classify,constraint_boundary_squared,constraint_d_comp_d,founded_face_boundary_stands,adapter_contract}` | `algebraic.rs::GradedCausalComplex`; `physical_constraint_complex.rs`; `physical_constraint_grading.rs` | — | **exact** for arbitrary natural grade with integer coefficients: grade compatibility, a grade-zero cell without boundary, and `∂∘∂ = 0` lifted from cells to chains. The Lean statement is unconditional in the aperture, the interval and the resolution; the Rust `found_cell` re-derives it at every founding. `#print axioms` returns `[propext, Classical.choice, Quot.sound]` with no `sorryAx`. **T5 adds the horizon's second coordinate**: `Horizon` is the pair `(h, k)` with both ceilings checked at its constructor and its wire route, its order is the product order and is *not* total, `ChainDistanceAtMost` is one symmetric, order-dual-invariant distance serving both axes, `twoAxisWidth` is this width over what a horizon reaches and is monotone in each coordinate, at `k = 0` it **is** the width above, and `looking_toward_the_coarse_is_determined_and_toward_the_fine_is_plural` proves the asymmetry the two directions actually have: a face one way, a fibre the other |
| An undecided contact returns a family, never a decision | `Foundation/AperturedGradedComplex.lean::{ExactInterval,ContactClass,classify_eq_openContact_iff,classify_point_ne_openContact,Resolution,admits,decided_admission_is_resolution_free,openContact_is_plural,determinate_of_no_openContact}` | `physical_constraint_complex.rs::{ContactClass,DistanceAperture::classify}`; `physical_constraint_grading.rs::{ConstraintComplexFamily,OpenContactLaw,OpenResolution,OpenContact,enumerate_family}` | — | **exact** for the declared open set. An `Open` reading is neither promoted nor demoted: it indexes a `2^n` family whose refusing and admitting bounds are both constructed, a declared resolution must name every open contact and nothing else, and enumeration refuses above an explicit bound. The two bounds of the worked fixture carry different homology, so the open class is a topological question and not bookkeeping |
| Grain tower and its restriction as a non-invertible transition | `Foundation/GrainRestriction.lean::{Grain,classJoin,classJoin_eq_inside_iff,foldr_classJoin_eq_inside_iff,FineInside,FineAdmissible,selectionReading,selectionTower,selectionTransition,grain_residual_reopens_the_source,fineReading_factors,residual_separates_fineOnly}` | `crates/holonic-engine/src/grain_tower.rs::{Grain,GrainAddress,GrainCell,GrainPair,GrainFace,GrainTower,GrainResidual,GrainSelection,SelectionResidual,check_grain_reopen,GrainCensus,GrainDisagreement}` | — | [established-bounded; formal-checked, implemented-exact, measured] The first physical instance of carrier item **C4**. The tower is `Foundation/ContinuingTower.lean::Tower` at the grain axis and its restriction is `Tower.restrictTransition`; nothing is reproved. `reopen_apply` holds on all four measured M5 occurrences, and `laterReceiverFactors` reconstructs the fine restriction from the coarse face plus the residual. `residual_separates` is instantiated on real data by deleting exactly the 1,096 fine-only contacts: the coarse face is unchanged and the residuals differ. A selection *is* a lawful tower — `restrict_refl` and `restrict_trans` both hold for it — which is precisely why the relation below must be declared |
| The coarse receiver's standing is a declared `ApertureRelation` | `Foundation/GrainRestriction.lean::{ApertureRelation,PseudoDistance,GrainRadius,coarse_distance_le_fine_aperture_add_radii,inflated_carries_every_fine_contact,selection_inside_implies_fine_inside,equal_aperture_is_not_lawful,selection_is_insufficient_for_the_fine_reading,MutuallyInsufficient,independent_is_mutual_insufficiency,nativeFine_refusal_is_a_residual_difference}` | `grain_tower.rs::{ApertureRelation,InflationWitness,IndependenceDeclaration,FineNativeDeclaration,CoarseReading,certified_coarse_aperture_squared,rational_root_upper_bound,GrainRefusal}` | — | **exact** for the three declared arms. There is no `Default` and no inference from the data: `GrainTower::found` demands a relation and it travels with the tower. `InflationWitness::measure` returns the attained maximum with its extremal pair and its forcing fine pair, checks it against the triangle-inequality certificate built from measured grain radii, and refuses a declared aperture that does not carry. Independence must be earned by a measured separation; a native-fine declaration refuses at the first disagreeing pair. Every aperture, radius and ratio is a `relational_geometry::Rat`, and `rational_root_upper_bound` is exact integer arithmetic, never a rounded float |
| A grain residual and an undecided reading are two objects | `Foundation/GrainRestriction.lean::{DeclaredFamily,no_function_realizes_a_plural_family,openAdmissionFamily,openAdmissionFamily_pluralAt,open_admission_is_not_a_residual,grain_residual_and_open_class_are_two_objects,residual_does_not_close_an_open_reading,narrower_interval_decides,decided_reading_is_resolution_free}` | the verdict in `grain_tower.rs`'s header; `physical_constraint_grading.rs::{OpenResolution,ConstraintComplexFamily}` for the open side | — | [proved-derived; formal-checked] Both are content a coarse receiver fails to carry, and they are **not** one object. A residual is a *function of the source* and `reopen_apply` returns the source exactly; the admission of an open contact is not a function of the source at all, because `openContact_is_plural` gives two resolutions that disagree on it. Reopening returns the source including its open readings, so a residual can never close an open class; what closes one is a strictly narrower exact interval — a different source on the **precision** axis of the index — or an exterior declaration, which enlarges the source. The grain residual lives on the **grain** axis, and the tower's `restrict` has no component along the precision axis: that is why the open class does not transport between grains |
| Exterior intake decodes exactly and projects outward | `Foundation/ExteriorIntake.lean::{FloatFormat,Codeword,Codeword.value,Codeword.Finite,decode,decode_total_on_finite,decode_exact_on_finite,decode_refuses_nonfinite,decode_isSome_iff,BoundedDyadic,wider_format_carries_every_value,binary32_strictly_refines_binary16,DecimalToken,DecimalToken.enclosure,enclosure_contains_centre,Contains,projectOutward,projection_contains_source,widening_never_flips_a_decision,decided_under_projection_is_decided_at_source}` | `crates/holonic-engine/src/physical_intake/numpy.rs::{UncertaintyWordFormat,NpyArray,NumpySource,ExactWord}`; `physical_intake/mmcif.rs::{DecimalToken,AtomOccurrence,ResidueOccurrence,ChainOccurrence,StructurePresentation}`; `physical_intake/deflate.rs::{read_zip_members,inflate,crc32_of}`; `exact_value.rs::ieee754::{decode_binary16_bits,decode_binary32_bits,decode_binary64_bits}` | — | [established-bounded; formal-checked, implemented-exact] **exact** for the three admitted interchange wires and for every written decimal token. An IEEE word is an exterior codeword read as an integer and turned into an exact dyadic immediately; a non-finite word is refused by name with its index and its pattern, and the whole array refuses at founding so no consumer holds a non-finite cell. Admitting `<f4` and `<f8` beside `<f2` strictly enlarges the representable set: `wider_format_carries_every_value` scales the significand and lowers the ulp exponent by the same amount, which is an identity on the value. A coordinate's enclosure is its own last decimal place and the resident wire is an always-outward projection, so `projection_contains_source` holds for every declared denominator and `widening_never_flips_a_decision` makes the coarsening safe rather than silent: it can open a decided contact and can never reverse one. `no_float_participates_in_the_intake` checks by source scan that none of the four files mentions `f32`, `f64`, `to_bits` or `from_bits`. `#print axioms` returns `[propext, Classical.choice, Quot.sound]` with no `sorryAx` |
| An occurrence is not founded without its environment index | `Foundation/ExteriorIntake.lean::{TargetEcology,DesignLineage,TokenAddress,EnvironmentIndex,Occurrence,Presentation,foundWith,foundWith_environment,found,found_none_of_environment_absent,found_environment,no_occurrence_without_environment,addressedPairs,addressed_pair_population,token_index_unique,reading_total_on_addressed_pairs,reading_is_directional,intake_contract}` | `crates/holonic-engine/src/physical_intake.rs::{TargetEcology,DesignLineage,TokenAddress,EnvironmentProvenance,EnvironmentIndex,UncertaintyArray,AddressedUncertainty,AddressedOccurrence,ComponentGrain,ContactPresentation,found_constraint_complex,graded_family,scaled_component_wire,enact_grain_family,GrainCensusTotals,IntakeRefusal}` | — | [established-bounded; formal-checked, implemented-exact, measured] Item **B2**. `AddressedUncertainty::found` is the only constructor and takes the index by value; in Lean the array's index type *is* the token population, so an array without one is not a well-typed occurrence. A source that carries only `pae` — which is what Boltz-2 emits — is refused with all ten absent environment arrays named at once, never defaulted, and is admitted once a caller declares the index with a stated ground that travels as `EnvironmentProvenance::DeclaredByCaller`. A blank declared coordinate is presented testimony and is named by `TargetEcology::blank_coordinates` rather than filled in. The addressed pair population is exactly `\|left\| · \|right\|` and a reading is directional: row and column are retained separately and never symmetrized. Measured through this path: the four M5 cross families return 70,632 pairs, fine-inside 1,397, coarse-inside 301, fine-only 1,096, coarse-only 0, per family 239 / 307 / 215 / 335, and the required coarse aperture squared `116656202097359145/10^14` — the same table `grain_tower/tests.rs` measures through its own reader, so the number now has two independent readers |
| A population of faces over one candidate, its complete separator sets and its exact minimal separating receivers | `Foundation/PhysicalOccurrence.lean::{ContactFace,separatesB,separatesB_irrefl,separatesB_of_open,separatorSet,mem_separatorSet_iff,Indistinguishable,indistinguishableB,indistinguishableB_iff,indistinguishableB_refl,separatorSet_eq_nil_iff_indistinguishable,unanimousAt,unanimous,unanimousAt_antitone,unanimous_antitone,Source,preimageFibre,silentIndex,statedIndex,silentIndex_ne_statedIndex,agreement_does_not_prove_realization,fibre_stays_plural,one_separating_contact_refutes_the_merge,Hits,SeparatesAll,hittingSet_separates_all,MinimumHittingSet,minimum_hittingSet_separates_all,minimum_no_smaller,no_hittingSet_of_empty_separator_set,fibre_contract}` | `crates/holonic-engine/src/physical_occurrence/plural_fibre.rs::{DecidedClass,PluralFibre,SeparatingContact,PairwiseSeparator,ContactRole,FibrePartition,RealizationProof,UnanimityReceipt,HittingSetBound,MinimalSeparation,role_table,FibreRefusal}` | — | [established-bounded; formal-checked; implemented-exact; measured] Item **B5** of `docs/plans/THE_BIOLOGICAL_ECOLOGY_INSTANTIATES_THE_CARRIER.md`, in the `physical_occurrence` owner. `PluralFibre` is the join of that owner's vertical and horizontal indices restricted to one object — the population neither `VerticalFamily` (which refuses a repeated environment) nor `HorizontalFamily` (which refuses a second environment) admits. The separator set between two members is complete, not a shortest witness; `RealizationProof` is uninhabited, so agreement is `R`-indistinguishability by type and never identity of sources; the unanimous set is antitone under `adjoin`; and the minimum separating receiver is an exact minimum hitting set computed under a declared `HittingSetBound` whose worst-case node count is a `BigUint` checked before any recursion, with `HittingSetSearchTooWide` rather than an approximation. On the three M5 RBX1 presentations: 38 unanimously formed, 10,288 unanimously excluded, 41 separating and 1 open-carrying of 10,368 contacts; pairwise (agreeing, separating, open-carrying) `(10338,29,1)`, `(10337,30,1)`, `(10344,24,0)`; and 564 distinct two-contact receivers tell all three apart, all of them returned |
| A passage is a typed event carrying receipts, and its reverse exists exactly when the residual is retained | `Foundation/PhysicalOccurrence.lean::{PassageAxis,vertical_ne_horizontal,PassageEvent,PassageEvent.axis,mutation_is_horizontal,the_other_three_are_vertical,Delta,Delta.before,Delta.after,Delta.Composable,Delta.comp,Delta.comp_assoc,Delta.comp_before,Delta.comp_after,Delta.comp_composable,Delta.idOn,Delta.idOn_comp,Delta.comp_idOn,TypedPassage,no_vertical_passage_carries_a_mutation,no_horizontal_passage_carries_an_environment_change,TypedPassage.comp,TypedPassage.comp_assoc,ClassAction,ClassAction.apply,ClassAction.residual,ClassAction.reopen,ClassAction.reopen_apply,ClassAction.Reversible,ClassAction.traversability_is_the_residual,ClassAction.collapsing,ClassAction.collapsing_merges_two_faces,ClassAction.collapsing_not_reversible,passage_contract}` | `crates/holonic-engine/src/physical_occurrence/passage.rs::{PassageAxis,Vertical,Horizontal,PassageEvent,PassageEnd,OccurrenceFace,PairTransition,ConstraintDelta,EnvironmentDelta,PassageResidual,PassageStep,Passage,census_table,PassageRefusal}` | — | [established-bounded; formal-checked; implemented-exact; measured] Item **B6**, in the `physical_occurrence` owner and built on its `EnvironmentPassage`. `(K,Theta)_{t+} = Phi_e((K,Theta)_{t-})` is a first-class event: environment change, binding and protonation are `Passage<Vertical>` and mutation is `Passage<Horizontal>`, two types with no coercion, each constructor checking its own evidence — a binding refuses unless the named partner really enters the oligomeric state, a protonation refuses an undeclared acidity axis, a mutation refuses an environment that also moved. `ConstraintDelta` is the complete per-contact class transition computed from the two complexes; composition is exact and associative and a class that moves away and back collapses into a retention. Serial composition retains the joining occurrence as a whole value and is associative as an equality of passages. The `Transition` impl retains the source face as its residual, `check_reopen` returns `reopen(apply(x),residual(x)) = x`, and `reverse_passage` returns `continuing_tower::ReversePassageReceipt::OnlyWithTheResidual` naming the two merged faces. Measured: M5 free RBX1 to CUL1-bound RBX1 is a binding of CUL1 with added copies 1, breaking 19 alpha-carbon contacts and forming 5 with nothing opening or closing, retaining 40 formed and 10,304 excluded |
| Selection and design equivalence: the cascade's five typed stages, and a merge licensed only by an exhibited isomorphism | `Foundation/DesignSelection.lean::{MergeVerdict,MergeVerdict.licensesMerge,notSeparatedWithinBound_does_not_license_merge,refuted_does_not_license_merge,only_equivalentBy_licenses_merge,oneSeparatingFutureReceiverRefutesTheMerge,presentAgreementDoesNotImplyEnvironmentAgreement,declaredEnvironmentAgreementDoesNotImplyUndeclared,toleranceClosenessAtOneReceiverLicensesNothing,interfaceAgreementWithoutAffinityAgreement,HardConstraint,admitted,violations,violating_design_is_never_admitted,refusal_names_the_constraint,theBestDesignIsStillRefused,Reading,WorstVerdict,worstOver,worst_is_attained,worst_is_an_upper_bound,undeclared_environment_is_absent,unread_at_a_declared_environment_refuses,scalarFirstDiscardsAFrontierDesign,indistinguishabilityIsNotTransitive,aComponentCanContainASeparatedPair,selection_contract}` | `crates/holonic-engine/src/design_selection.rs::{DesignId,Sense,DeclaredReceiver,DeclaredEnvironment,ReceiverReading,UnreadReason,Design,DesignFamily,HardConstraint,ConstraintViolation,AdmissionStage,WorstVerdict,WorstEnvironmentRanking,SeparatingContact,DesignSeparator,StructuralClusters,QualityDiversity,ReceiverWeighting,SituatedDesign,AdmittedTransformation,MergeVerdict,merge_verdict,merge,MergedClass,CascadeReport,SelectionRefusal}`; consumes `relation_ladder::{Situation,PotentialVerdict,Separator}`, `presentation_cost::{frontier_by,AxisComparison}`, `physical_occurrence::{VerticalFamily,HorizontalFamily,Passage,DecidedClass}` and `topological_receiver::compare_values` | — | [established-bounded; formal-checked; implemented-exact; measured] Item **B8** of `docs/plans/THE_BIOLOGICAL_ECOLOGY_INSTANTIATES_THE_CARRIER.md`. A **new** owner, not a `physical_occurrence` submodule: the cascade consumes the receivers that read that owner's complexes, and its population is the rectangle — several objects at several environments — whose rows `VerticalFamily` founds and whose columns `HorizontalFamily` founds, and which `PluralFibre` refuses because it is one object. Design equivalence **is** `RelationLadder.EqualPotential` over a situation whose generators are admitted `Passage`es; `merge_verdict` returns `Refuted \| NotSeparatedWithinBound \| EquivalentBy` and only the third reaches `merge`, whose `MergedClass` has private fields, so a bounded search reaches no merged class by any path. `presentation_cost::pareto_frontier` is re-expressed through the new `frontier_by`, the same rule over a declared three-valued axis comparison, so an undecided reading never licenses a discard. On the mounted release the three RBX1 presentations are one object at three environments — a design family over them is refused by name — and the formed-contact receiver reads 64 / 59 / 45 with 1 / 0 / 0 open over the 10,368 addressed pairs; the horizontal designs are synthetic exact perturbations of those complexes. **Correction to the plan:** the release carries no dissociation constant and no cross-species assay, so its recorded two per cent / six-fold observations are modelled as a synthetic exact-rational instance and not as measurements |
| The cost cascade: one cascade, two readings, and when a cheap stage may discard | `Foundation/DesignSelection.lean::{CascadeStage,Certified,hardConstraintStage,hardConstraintStage_is_certified,cascadeSurvivors,fullSurvivors,all_cheap_and_expensive,certified_cascade_preserves_the_frontier,proxyStage,proxyStage_is_not_certified,uncertified_proxy_loses_a_frontier_design,certifiedStage,certifiedStage_is_certified,expensiveWork,expensiveWork_le_population,the_certified_filter_saves_expensive_work,cost_cascade_contract}` | `crates/holonic-engine/src/design_selection.rs::{CascadeStage,DiscardLaw,Certified,CostedCascade,CascadeRun,CascadeReading,StageRun,Discard,AxisSaving,CostDirection,cost_saving,survivors_agree,CASCADE_STAGE_CEILING}`; consumes `presentation_cost::{CostReceipt,Counted,Axis}` | — | [established-bounded; formal-checked; implemented-exact; measured] Item **B10**'s cascade half, extending the B8 owner rather than founding a second pipeline: the same stages decide and are costed. A stage may discard only under a `HardConstraint` or a `CertifiedBound`, whose `Certified` token has no public constructor and is minted only by `CascadeStage::certified_bound` after checking, at every candidate of a declared probe, that the cheap reading refuses only where the expensive receiver does. An `UncertifiedProxy` **discards nothing**: what it would have removed is recorded in `StageRun::would_have_discarded`, and `run_admitting_every_proxy_discard` exists only to exhibit the loss. `cost_saving` returns the exact per-axis `BigUint` difference with a typed direction, never a float and never clamped to zero |
| Evaluation splits are typed partitions of lineage classes with a leakage law, and a run records its tool, version, mode and clocks | `Foundation/EvaluationDiscipline.lean::{Lineage,Lineage.setoid,Lineage.Class,Lineage.cls,Lineage.cls_eq_of_rel,Lineage.rel_of_cls_eq,Split,Straddles,ofClassPredicate,ofClassPredicate_does_not_straddle,allOneLineage,rawSplit,rawSplit_straddles,FittedReceiver,Leaks,evaluateAt,evaluateAt_refuses_a_fitted_occurrence,evaluateAt_refuses_a_lineage_equivalent_occurrence,evaluateAt_admits_when_nothing_leaks,Calibration,AssayBridge,transport,within_one_assay_needs_no_bridge,across_assays_without_a_bridge_is_refused,across_assays_with_a_bridge_is_admitted,a_bridge_between_other_assays_is_refused,TimedFace,ExternalComparison,compare,the_return_does_not_relabel_the_prediction,the_comparison_is_not_at_the_predictions_time,the_external_return_is_a_new_occurrence,DisagreementClass,classifyContact,an_open_reading_carries_the_contact,the_unanimous_contact_is_unanimous,the_separating_contact_is_separating,correct_on_the_unanimous_contact_and_wrong_on_the_separating_one,UniversalClaim,universalClaim_is_uninhabited,Conclusion,Conclusion.scope,Conclusion.universalClaim,Conclusion.atAnotherSplit,a_conclusion_does_not_travel_to_another_split,a_conclusion_stands_at_its_own_split,evaluation_contract}`, namespace `Soma.Holonics.Foundation.EvaluationDiscipline` | `crates/holonic-engine/src/evaluation_discipline.rs::{EditRadius,LineageEdge,LineageStraddle,LineageClasses,KeyAxis,Discipline,DesignKeys,Split,FittedReceiver,ReceiverAdmission,AssayBridge,Calibration,CalibrationTransport,TimedReading,ExternalComparison,DisagreementClass,DisagreementSubsets,disagreement_subsets,SubsetPerformance,performance_on,UniversalClaim,EvaluationConclusion,read_held_out,ToolVersion,Recorded,python_package_version,git_head,Nanoseconds,ClockName,Stopwatch,ClockReading,RunReceipt,RunRecorder,EvaluationRefusal}`; consumes `design_selection::{DesignFamily,AdmittedTransformation,ReceiverReading}`, `physical_occurrence::{PluralFibre,DecidedClass,ExteriorDeclaration,OccurrenceId}`, `relation_ladder::Rung` and `standing::TimedFace` | — | [established-bounded; formal-checked; implemented-exact; measured] Item **B9** of `docs/plans/THE_BIOLOGICAL_ECOLOGY_INSTANTIATES_THE_CARRIER.md` together with **B10**'s receipt half. A `Split` is constructible only from a partition of *lineage classes* — the quotient of the design population by the relation the horizontal `Passage<Horizontal>` mutations generate within a declared `EditRadius` — so a split cannot separate two close variants of one lineage; `from_raw_designs` refuses a straddle naming the pair and the connecting chain, and `LineageClasses::straddle_of` returns the same leak as data. Leakage through receivers is typed as well: a `FittedReceiver` carries the occurrences and designs it was fitted on and refuses an evaluation at any of them or at a lineage-equivalent design, and a `Calibration` does not cross an assay without an `AssayBridge`. A wet or external return is a **new occurrence**: `ExternalComparison` leaves the earlier prediction's `TimedReading` unchanged and carries its own time index, with the rung computed from the two exact readings. `EvaluationConclusion` has no universal arm, `UniversalClaim` is uninhabited, and `read_held_out` can only refute a supplied generalization or return `NotRefutedWithinThisEvaluation`. `RunReceipt`'s fields are private and its only constructor is `RunRecorder::finish`; `ToolVersion::Read` carries a `Recorded` token with no public constructor, so a version is read or is `Unavailable{why}` and never guessed. The four clocks are exact integer nanoseconds from `std::time::Instant` |
| Rigidity Jacobian, its two null spaces and the rank–nullity identities | `Foundation/RigidityReceiver.lean::{ConstraintSystem,delta,constraintMap,constraintMap_place,bump,sum_bump,rigidityMatrix,rigidityMatrix_mulVec,rigidityLin,infinitesimalMotions,mem_infinitesimalMotions_iff,selfStresses,self_stress_pairing,constraint_expansion,jacobian_is_the_differential,finrank_flat,rank_nullity,self_stress_dimension,maxwell_relation,motion_dimension_eq_count_of_no_self_stress}` | `crates/holonic-engine/src/rigidity_receiver.rs::{ExactConfiguration,DistanceConstraint,RigidityJacobian,rigidity_reading,RigidityReading,MaxwellCount}`; `exact_linear.rs::ExactRatMatrix::{rank,kernel_basis,cokernel_annihilator}` | — | [established-bounded; formal-checked, implemented-exact] The receiver **R4** of `docs/plans/THE_RECEIVER_ATLAS_SEPARATES_WHAT_ONE_FACE_CANNOT.md`. `J` is identified with the differential by the exact expansion `F_c(q + t v) = F_c(q) + t·(J v)_c + t²·\|δv\|²`, which needs no analysis and no complete field, so the Rust owner's rational Jacobian is the same object the Lean owner differentiates. Both identities `rank J + dim ker J = d·n` and `rank J + dim ker J^T = m` are re-derived at every reading and refuse by name on disagreement. `#print axioms` returns `[propext, Classical.choice, Quot.sound]` with no `sorryAx` |
| Trivial motions are measured at the configuration, never assumed | `Foundation/RigidityReceiver.lean::{IsSkew,rigidMotion,delta_rigidMotion,skew_double_sum_eq_zero,skew_quadratic_form_eq_zero,rigid_motion_mem_ker,translation_mem_ker,rotation_mem_ker,rigidMotionSpace,rigidMotionSpace_le_motions,InfinitesimallyRigid,infinitesimallyRigid_iff_eq,rotation_is_a_translation_of_coincident,rigidityMatrix_row_eq_zero_of_coincident}` | `rigidity_receiver.rs::{TrivialMotionReading,ConfigurationDegeneracy,RigidityReading::is_infinitesimally_rigid,RigidityJacobian::coincident_constraints}` | — | [proved-derived; formal-checked, implemented-exact] The rotation generators are linearized about `q`, so the trivial count is a property of the configuration and not of the dimension. The Rust owner measures it as an exact rank and cross-checks it against the closed form `d + d(d−1)/2 − (d−k)(d−k−1)/2` for `k` the affine span dimension, refusing with `TrivialDimensionDisagrees` if they part. In three dimensions that is six for a spanning **or coplanar** configuration, five for a collinear one and three for a coincident one; the coincident case keeps nonzero rotation generators that are merely dependent, so a vanishing test would be the wrong receiver. Every generator is applied to `J` and required to vanish, which is `rigid_motion_mem_ker` executed rather than asserted. The claim is **infinitesimal** rigidity at the given configuration and never generic or finite rigidity |
| Rigid clusters, contact-removal sensitivity, and the open rigidity family | `Foundation/RigidityReceiver.lean::motions_antitone` | `rigidity_receiver.rs::{RigidPairReading,RigidClusterReading,rigid_clusters,ConstraintSensitivity,RemovalSensitivity,removal_sensitivity,verify_removal_sensitivity,RigidityFamily,rigidity_family,rigidity_member,rigidity_jacobian_member}`; `physical_constraint_grading.rs::{ConstraintComplexFamily,OpenContactLaw}` | — | [established-bounded; implemented-exact; measured] A pair is *implied* when its row annihilates `ker J`; a rigid cluster is a maximal pairwise-implied set, clusters **overlap** at hinges, and every returned cluster is verified by checking that each kernel basis vector restricts to a rigid motion of the sub-configuration. Enumeration refuses above an explicit bound. Removal sensitivity is read off the self-stress support — a constraint is dependent on the others exactly when some element of `ker J^T` is supported at it — and a bounded sample is re-derived by actually deleting the row. The open class makes the constraint set itself plural, so the return is a family with both bounds constructed; on a worked presentation carrying one open contact the bounds differ, `dim ker J = 7` refusing against `6` admitting, so an `Open` contact is a rigidity question and not bookkeeping |
| The physicochemical receiver: typed units, declared tables, and readings a static structure can support | `Foundation/PhysicochemicalReceiver.lean::{Dim,Dim.add,Dim.sub,Dim.zero,United,United.sum?,United.contains,united_sum_requires_equal_dimension,united_sum_of_equal_dimension,united_sum_contains_sum,united_contains_of_narrower,ResidueClass,ResidueClass.rank,ClassPair,classPair,classPair_symm,Table,Admission,Presentation,composition,admittingComposition,openComposition,refusing_le_admitting,admitting_eq_refusing_of_no_undecided,admitting_eq_refusing_add_open,composition_additive_of_disjoint,composition_invariant_under_relabelling,derivedAdmit,derivedPresentation,composition_invariant_under_isometry,tables_change_the_reading,physicochemical_contract}` | `crates/holonic-engine/src/physicochemical_receiver.rs::{UnitedInterval,TableGround,ResidueClass,ClassPair,ResidueClassTable,FormalChargeTable,HydrogenBondTable,RadiusTable,Dielectric,ParameterTables,TableIdentity,TablePassage,ProtonationBasis,protonation_basis,ChemicalSite,SiteTable,PairPopulation,PairWorkBound,FamilyRole,CompositionReading,composition_under_declared_protonation,situated_composition,compare_across_tables,RigidMotion,worked_presentation,PhysicochemicalRefusal}`; composes `quantity.rs::{BaseUnits,Dimension,Quantity,Cast}`, `physical_constraint_complex.rs::DistanceAperture::classify` and `physical_occurrence.rs::{Environment,ExteriorDeclaration}` | — | [established-bounded; formal-checked; implemented-exact; measured] The **physicochemical half of B7** of `docs/plans/THE_BIOLOGICAL_ECOLOGY_INSTANTIATES_THE_CARRIER.md`. It founds no unit algebra: `quantity.rs` already owns the dimension group, and the one composition added is `UnitedInterval`, the *enclosure*-valued quantity `Quantity` is not. Residue classes, formal charges at a declared protonation, hydrogen-bond heavy-atom roles, van der Waals radii and the dielectric are each an exterior declaration with a named source and a stated scope, carried by value into every reading; a residue or element outside a table's scope is refused by name. Two readings under different tables are refused for comparison unless a `TablePassage` accounts for every divergent residue. A charged reading needs a declared acidity: `protonation_basis` returns `AcidityUndeclared` carrying the environment's own stated reason, and **all three M5 occurrences leave that axis undeclared**, so the refusal is the receiver's primary result there. Open contacts make the composition a *family* between `refusing_bound` and `admitting_bound`; compositions add over disjoint families; readings are invariant under a class-preserving relabelling and under any map preserving the pairwise separations. A **rotation of an interval coordinate box is refused by name** rather than re-enclosed, so the full rigid-motion law is stated at point configurations while translation invariance holds everywhere |
| Hydrogen-bond *candidates*, steric overlaps, a burial *proxy*, exact half-sphere exposure, and a declared finite electrostatic model as an enclosure | `Foundation/PhysicochemicalReceiver.lean::{Geometry,candidate,candidate_ignores_hydrogen_geometry,two_geometries_one_candidate_reading,invEnclosure_contains,invEnclosure_narrows,coulombTerm,coulombSum,coulombSum_additive,coulombSum_enclosed,ProtonationBasis,chargedReading,no_charged_reading_without_a_declared_acidity,chargedReading_of_declared,the_two_bases_are_distinct}` | `physicochemical_receiver.rs::{HydrogenBondWindow,HydrogenBondCandidate,HydrogenBondCandidateReading,hydrogen_bond_candidates,StericOverlap,StericOverlapReading,steric_overlaps,NeighbourCount,BurialProxyReading,neighbour_count_proxy,ReferenceDirections,side_chain_directions,HalfSphereCount,HalfSphereExposureReading,half_sphere_exposure,reciprocal_distance_enclosure,ElectrostaticReading,electrostatic_enclosure,electrostatic_approximations,declared_coulomb_cast}`; `exact_value.rs::AlgebraicRoot::reciprocal_square_root`; registered in `receiver_atlas.rs::ledger_entries` as `B7phys` | — | [established-bounded; formal-checked; implemented-exact; measured] There is **no `HydrogenBond` type**: a heavy-atom model carries no hydrogen, so `candidate_ignores_hydrogen_geometry` and `two_geometries_one_candidate_reading` prove the reading cannot certify a bond. There is **no solvent-accessible surface area**, because a rolling-probe area is a numerical integral: what is offered is an exact integer neighbour count named a proxy and an exact half-sphere exposure — the count of neighbours on each side of the plane through a site normal to a declared reference direction, with the undecidable side carried apart. `1/r` is irrational for a rational `r²` and is carried as an exact enclosure from a Sturm certificate, with the approximations of the model (point charges, one uniform declared permittivity, no solvent, no ions, no polarization, the declared population as the cutoff) travelling in the reading. On the M5 release the interface composition reads 64/59/45 admitted with 1/0/0 open and separates all three presentations, while salt-bridge candidates 3/3/3 and like-charge contacts 1/1/1 separate none of them; the electrostatic enclosures are pairwise disjoint and the designed structure's is four hundred times wider because its coordinates carry three decimals against the predictions' six. On the five-residue window where R3/R4/R5 are blind the composition is blind too and the Coulomb sum is exactly zero, and the **atom-grain burial proxy separates the two predictions**. Of the wave-1 stray terminal oxygens, the two-angstrom burial proxy surfaces three of four and the steric reading surfaces one and misattributes it |
| Acoustic receiver: a resonator bank consuming the colour receiver's own co-present mode population | `Foundation/AcousticReceiver.lean::{pole,cayley,gain,cayley_den_ne_zero,normSq_cayley_lt_one,normSq_cayley_eq_one,realBlock,trace_realBlock,det_realBlock,charpoly_realBlock,rateForm_realBlock,Bank,Bank.drive,Bank.drive_add,Bank.advance,Bank.run,Bank.run_causal,Bank.run_add,Bank.pressure,Bank.pressure_add,Bank.timbre,energy,energy_advance_le,energy_advance_lt,colour,colour_does_not_distribute,quantize,quantize_then_superpose_ne_superpose_then_quantize,metamer_sounds_different,unison_looks_different,population_change_moves_both}` | `crates/holonic-engine/src/acoustic_receiver.rs::{Resonator,RateProvenance,ModalCoupling,ResonatorBank,BankState,BankRun,AcousticReading,AudibleComponent,BandStability,JointReceiverReading,joint_reading,export_pcm16,wav_bytes}`; consumes `dimensional_wave.rs::{ExactReceiverPhasePopulation,ExactReceiverPrimaryDoctrine,ExactComplexWaveCurrent}` and `causal_chord.rs::{Linearization,ChordComponent,rate_form}` | — | [proved-derived; formal-checked, implemented-exact] The receiver **R2** of `docs/plans/THE_RECEIVER_ATLAS_SEPARATES_WHAT_ONE_FACE_CANNOT.md`. The continuous resonator's `exp(λt)` is not rational, so the declared law is the exact Cayley map `z=(2+hλ)/(2−hλ)` at a declared rational step, a Gaussian rational in the same carrier the colour receiver's currents use; `‖2+hλ‖²−‖2−hλ‖²=8h·Re λ` makes stability preservation exact, and `Re(2−hλ)=2+hγ≥2` makes the step total. The bank is R1's block-diagonal `Linearization`, so every audible component's source mode, excitation, transport path, pole factor, residue, approximation error and residual are `ChordComponent` itself rather than a second account. Superposition holds exactly through the bank; the pressure projection is linear and last; the quadratic colour response and the integer export face are the two places the ordering is law, each with a concrete counterexample. Colour and timbre are two maps out of one `ExactReceiverPhasePopulation` and neither refines the other: exchanging which mode carries which quadrature is a colour metamer the bank separates, and current on an uncoupled mode is an acoustic unison colour separates. `#print axioms` returns `[propext, Classical.choice, Quot.sound]` with no `sorryAx` |
| Receiver width and release: uncertainty as a structure, not a scalar | `Foundation/ReceiverRelease.lean::{Horizon,Horizon.Within,horizonWithin_is_not_total,ChainDistanceAtMost,chainDistance_symm,chainDistance_trans,chainDistance_orderDual,Comparable,TwoAxisReach,twoAxisWidth,twoAxisWidth_mono,twoAxisWidth_at_index_zero_is_the_longitudinal_width,coarseReach,fineFibre,looking_toward_the_coarse_is_determined_and_toward_the_fine_is_plural,width,width_nonneg,inf'_le_of_mem,le_sup'_of_mem,abs_sub_le_width,width_le_of_bounds,width_mono,width_eq_zero_iff,Releasable,releasable_at_every_tolerance_iff_width_zero,ReleaseReturn,ReleaseLaw,holdingLaw,wideningLaw,no_default_among_the_lawful_returns,ReleaseLaw.widenSound,every_lawful_return_other_than_hold_ask_or_no_continuation_is_inside_its_tolerance,doubling,Occurs,timeOfEvent,timeOfEvent_crosses,timeOfEvent_is_least,occursByThree,timingReading,compatibleFibre,future_stable_event_with_unstable_timing,release_is_receiver_relative_not_a_global_gate,NonExpansive,width_nonExpansive_factor,expansive_factor_increases_width,doubling_factor_not_nonExpansive,coarser_receiver_factors,timingInsufficiency,no_transformer_from_the_released_coarse_face}` | `crates/holonic-engine/src/receiver_release.rs::{ExactZonotope,Generator,GeneratorSource,horizon_image,advance,ExactFace,DiameterNorm,Reading,LinearReading,CompatibleFamily,ReceiverWidth,WidthWitness,width,width_enumerated,width_enclosed,Horizon,INDEX_HORIZON_CEILING,width_over_readings,ProbeDirection,ObservationProbe,AskProbe,narrowing_probe,narrowing_observation,CoarseningTower,CoarseningStep,CoarserRelease,release_coarser,FactorMap,FactoredReading,ReleaseReturn,LawfulOptions,DecisionLaw,release,ReleasedClaim,CoarserClaim,WidthRefusal,EXTENT_CEILING,MULTIPLY_WORK_CEILING,TOWER_STEP_CEILING,CANDIDATE_CEILING}`; consumes `causal_chord.rs::Linearization` and `exact_linear.rs::ExactRatMatrix` | — | [proved-derived; formal-checked; implemented-exact] The receiver **R6** of `docs/plans/THE_RECEIVER_ATLAS_SEPARATES_WHAT_ONE_FACE_CANNOT.md`. The width is the diameter of an actual image set, so width zero, constancy of `R ∘ Phi_h` on the fibre and releasability at every tolerance are one statement; the width is monotone under fibre inclusion; a coarser receiver narrows exactly when its factor map is non-expansive, and `q ↦ 2q` is the lawful coarsening that widens. The `h`-step map is `causal_chord`'s own `x_{t+1} = A x_t + B u_t` over `Q`, and the enclosure is an exact **zonotope** because the image of a box under an exact linear map is one; the sup-norm diameter is `2·max_i Σ_j \|G_ij\|`, exact and linear in the generator count, and the squared-Euclidean diameter is refused on an enclosure by name because its maximizer ranges over `2^k` vertices. `Ask` is computed — the generator whose exact observation reduces the diameter most, with its provenance — and `ReleaseCoarser` checks the factoring at every step of a declared tower. The decision law is the caller's, and every arm that **names** a tolerance sits inside it: `release` checks a `Released` return against the tolerance the law declared and a `ReleaseCoarser` return against the options' own, a `CoarserRelease` carries the tolerance it was searched under and `LawfulOptions::assemble` refuses one searched under another, and in Lean the `releaseCoarser` constructor carries `coarserWidth ≤ tolerance` so the arm cannot be formed outside tolerance at all. `ReceiverWidth`, `LawfulOptions` and `CoarserRelease` have private fields and validated wire routes. Two lawful laws return different arms on one reading. On the exact doubling recurrence over the fibre `{1/8, 1/4}` and horizon `3` the receiver "the event occurs by horizon 3" has width `0` and "the time of the event" has width `1`: **an event can be future-stable while its timing is not**, and no scalar on the fibre could produce both answers. `#print axioms` returns `[propext, Classical.choice, Quot.sound]` with no `sorryAx` |
| The separating atlas theorem, and the contracts that make an atlas credible | `Foundation/ReceiverAtlas.lean::{Separates,Separates.mono,Separates.of_refinement,SeparatingAtlas.markov,SeparatingAtlas.linearizationMarkov,SeparatingAtlas.identity_probe_determines_the_operator,SeparatingAtlas.probeChart,SeparatingAtlas.fullProbeAtlas,SeparatingAtlas.fullProbeAtlas_separates,SeparatingAtlas.pow_eq_aeval_modByMonic_charpoly,SeparatingAtlas.natDegree_modByMonic_charpoly_lt,SeparatingAtlas.markov_truncation,SeparatingAtlas.MarkovTwoNSuffices,SeparatingAtlas.SpectralTopologicalReading,SeparatingAtlas.readings_agree,SeparatingAtlas.spectralTopologicalInsufficiency,SeparatingAtlas.no_transformer_from_the_spectral_topological_reading,RebaseEquivariant,SourceAccountable,DeclarationIndependent,causalChord_is_rebase_equivariant}` | `crates/holonic-engine/src/receiver_atlas.rs::{ReceiverAtlas::admit_at,ReceiverAtlas::chart_time,ReceiverAtlas::separate,ReceiverAtlas::is_separating,Separator,SeparationOutcome,AtlasInsufficiency,SeparatingVerdict,DECLARED_CLASS_CEILING,AtlasContract,ContractStatus,Recomputed,StandingStatus,ContractEntry,ContractRow,contract_ledger,ledger_entries}`; consumes `causal_chord.rs::{Linearization,transfer_function,causal_chord,separate_under_probe}`, `hodge_receiver.rs::{HodgeOperator,hodge_reading,hodge_decomposition}`, `rigidity_receiver.rs::{RigidityJacobian,rigidity_reading,TrivialMotionReading,removal_sensitivity}` and `topological_receiver.rs::persistence` | — | [established-bounded; formal-checked; implemented-exact] The receiver **R7** of `docs/plans/THE_RECEIVER_ATLAS_SEPARATES_WHAT_ONE_FACE_CANNOT.md`, extending C6's atlas in place: a chart carries a declared **time**, `separate` returns the probe/receiver/time that parts two occurrences or the typed `IndistinguishableToThisAtlas` naming the charts it read and the charts the bound excluded, and `is_separating` scans a declared bounded class. **Positive:** the full coordinate probe atlas separates the exact linear systems of bounded dimension at every `n`, at **time one**, because with `B = C = I` the `k = 1` Markov parameter is `A` itself — the `Fin n` generalization of `CausalChord.full_atlas_determines_the_operator_fin_two`, which needs no cofactor expansion once the atlas is read through Markov parameters. The Cayley–Hamilton truncation `A^k = aeval A (X^k %ₘ charpoly)` gives agreement at every time from agreement below `n` **when the characteristic polynomials already agree**; the classical `2n` declared-port statement is the open `MarkovTwoNSuffices` and is not proved. **Negative:** the R3+R5 atlas is not separating for finite one-dimensional complexes — two Laplacian-cospectral non-isomorphic graphs share the grade-0 and grade-1 spectra, the integral Betti numbers, the torsion and the persistence diagram — returned as a `ReceiverInsufficiency` witness, with R1's driving-point response as the richer receiver that does separate them. The contract ledger's twenty-four rows over six contracts carry seven `Satisfied` rows, recomputed at every call against R1's, R3's, R4's and R5's own owners — enforced by type, because `Satisfied` carries a `Recomputed` token with no literal and a non-recomputed row carries a `StandingStatus` with no `Satisfied` arm. `DeclarationIndependence` is the sixth contract, the Lean `DeclarationIndependent` made present in Rust: R3 discharges it as the metric-freedom of the harmonic dimension and R5 **fails** it, because the declared coefficient field decides its reading. R5's rebase equivariance is `Failed` by the projected writhe; every gluing-with-interface-coupling row and every stability row is `Unproved` with the absent law named. `#print axioms` returns `[propext, Classical.choice, Quot.sound]` with no `sorryAx` |
| Root counting over `Q`: the gauge a sign-variation reading is blind to, the certified root enclosure, and the one half-plane counter | `Foundation/RootCount.lean::{PositivelyRescaled,theContinuingCountIsInvariantUnderPerEntryPositiveRescaling,theReadingIsInvariantUnderPerEntryPositiveRescaling,thePerEntryGaugeIsSharpAtItsPositivity,theContinuingCountIsInvariantUnderCommonNonzeroRescaling,theReadingIsInvariantUnderCommonNonzeroRescaling,theCommonPositiveRescalingIsAPerEntryOne,theSquarefreePartSharesItsRoots,theSquarefreePartAcquiresNoRoot}`, building on `Millennium/Sturm.lean::{variationFrom,variationCount,readingAt,remainderChain,TheReadingEqualsThePopulation}` | `exact_value.rs::{SturmChain,SturmChain::of,SturmChain::from_pair,SturmChain::polynomial,SturmChain::sign_variations,SturmChain::cauchy_index,STURM_DEGREE_CEILING,STURM_COEFFICIENT_BIT_CEILING,STURM_WORK_CEILING,check_declared_sturm_work,IntegerPolynomial::sturm_chain,AlgebraicRoot::isolate_against,AlgebraicRoot::isolate_with}`; `rational_polynomial.rs::{RealRootEnclosure,certified_real_root_enclosure,taylor_shifted,isolate_real_roots,isolate_against_chain,isolate_with_chain,ISOLATION_DEPTH_CEILING,SEPARATION_SPLITTING_DEPTH_CEILING,MONIC_COMPANION_BIT_CEILING,ROOT_BOUND_DOUBLING_CEILING,rational_root_census,rational_root_census_within,signed_primitive_integer_form,HalfPlaneCount,half_plane_count,axis_root_count,cauchy_index,distinct_real_root_count}`; consumed by `causal_chord.rs` (re-export), `hodge_receiver.rs`, `lattice_gauge.rs::exact_spectrum`, `winding_inertia.rs`, `certified_face.rs`, `implicit.rs`, `quintic_chart.rs` | — | [proved-derived; formal-checked; implemented-exact] **exact** over `Z`. The Sturm chain is a sign-tracked **subresultant** pseudo-remainder sequence (Collins/Brown–Traub), built **once per polynomial**, carrying the polynomial it belongs to so an isolation cannot be mispaired, and counted against many times, with signs read by integer Horner on the homogenized member — no rational arithmetic in a count. `STURM_WORK_CEILING` bounds degree and coefficient width **together**, which is what the two individual ceilings did not; `MONIC_COMPANION_BIT_CEILING` refuses the census's monic companion before it is formed. `IntegerPolynomial`, `SturmIsolationCertificate`, `AlgebraicRoot` and `RationalPolynomial` are closed at the wire with `serde(try_from)`, and the algebraic root's wire **re-runs its isolation**. Its correctness is the per-entry positive gauge, which is the boundary `Millennium/Sturm.lean` names open and `RootCount.lean` closes; `thePerEntryGaugeIsSharpAtItsPositivity` exhibits the negative counterexample the sign rule exists for. The root enclosure is a `k`-th root bound with the positive and negative sides counted separately, each returned only once a Taylor shift proves coefficientwise nonnegativity past it, and capped by the absolute Cauchy bound it replaced. `rational_root_census_within` takes a caller's enclosing interval as a first-class argument and **verifies** it. Sturm's theorem itself is a cited classical fact, not a `sorry`. `#print axioms` returns `[propext, Classical.choice, Quot.sound]` with no `sorryAx` |
| Causal chord: the exact transfer object of a linearization, and the half-plane census (R1) | `Foundation/CausalChord.lean::{spectrum_does_not_determine_response,full_atlas_determines_the_operator_fin_two,semisimple_imaginary_has_conserving_receiver,jordan_has_no_conserving_receiver,rebase_transfer}` | `causal_chord.rs::{Linearization,TransferFunction,transfer_function,PoleAtlas,ModeSupport,ResolventProbe,half_plane_count,ConservingReceiverSpace,CausalChord}` | — | **exact** over `Q`: poles are squarefree factors, residues are exact Laurent heads or elements of `Q[x]/(f)`. The half-plane counter itself now lives beside the Sturm owner in `rational_polynomial.rs` and is re-exported here at its old path with its old signature. Open: "`A` is `G`-skew for some positive definite `G` ⟹ `A` semisimple" is unformalized and named as such |
| Hodge and spectral receiver on an incidence complex, under a declared metric (R3) | `Foundation/HodgeReceiver.lean::{hodge_decomposition,hodge_decomposition_unique,finrank_harmonic_metric_free,harmonicEquivCohomology}` | `hodge_receiver.rs::{MetricDeclaration,BoundaryLaw,HodgeOperator,hodge_decomposition,hodge_reading,exact_hodge_spectrum,refine_spectral_gap,hodge_family}` over `physical_constraint_grading.rs`'s family | — | **exact**: `dim ker Δ_k` is refused unless it equals the integral Betti number, and an irrational eigenvalue is retained as a factor plus an isolating interval plus its Sturm certificate. The isolation is `rational_polynomial::isolate_against_chain`'s against one cached `SturmChain`; this module supplies the `[0, tr Δ_k]` enclosure positive semidefiniteness gives it and no longer carries its own descent |
| Persistence over the exact aperture filtration, and linking where an embedding stands (R5) | `Foundation/TopologicalReceiver.lean::{Filtration.toTower,PersistenceModule.persistentRank,multiplicity_eq,linking_is_an_integer,projectedWrithe_is_not_projection_invariant}` | `topological_receiver.rs::{ApertureFiltration,FiltrationOrder,persistence,persistence_with_grade_zero_agreement,grade_zero_agreement,integral_profile,linking_under_directions,contact_loops,presented_contact_loops}` | — | **exact** over `Q` and over a declared `F_p` decided within `MODULUS_BIT_CEILING`; an undecided order comparison stays a family, and projection invariance of linking is an open `Prop` tested over a declared candidate family. `persistence` runs `grade_zero_agreement` before returning, so the matrix reduction and the elder-rule union–find are required to agree on the production path; `ApertureFiltration`'s wire route is gated through a re-validating `TryFrom` |
| A contact family of **one** component: the intra-chain population and the cycle it closes (B1/B7) | `Foundation/AperturedGradedComplex.lean::{withinComponentPairs,withinComponentPairs_card,withinPairs_disjoint_crossPairs,loopEdges_card,backboneContactCycle_is_cycle,segment_cycles_are_multiples}` | `physical_constraint_complex.rs::{ContactFamilyKind,found_within_component_contact_family,within_component_pairs,within_component_pair_count}`; `physical_constraint_grading.rs::EdgeProvenance::is_covalent_backbone`; `topological_receiver.rs::presented_contact_loops`; `physical_intake.rs::enacted_within_component_classes` | — | **exact**: the population is `C(n − k + 1, 2)` at a caller-declared separation `k ≥ 2`, which is the declared **domain** of `within_component_pair_count` (`None` below it, because `withinComponentPairs n 0` carries the diagonal), and the covalent chain step is a one-cell of the presentation and never a contact. Several founded families are a family of complexes: the 2-cell law is asked of each founding's own contacts, `family_faces` and `two_chain_boundary_of_family` carry the per-family populations, a vertex triple carries one cell minted once, and the wire route is a coherence-re-checking `TryFrom` |
| Typed environment index, situated occurrence, and the status of one contact edge (B3/B4) | `Foundation/PhysicalOccurrence.lean::{Coordinate.no_third_state,compareHere,occurrence_contract,static_law_reaches_exactly_three,symmetrization_merges_a_distinguishable_pair}` | `physical_occurrence.rs::{Environment,Occurrence,EnvironmentPassage,EnvironmentTower,ContactStatus,static_status,DirectionalUncertainty,VerticalFamily,HorizontalFamily}` | — | **exact**: a reading is a claim at its own environment only, and `KineticallyInaccessible` carries an exterior declaration because no static structure derives it |
| Every Lean name a Rust owner cites is declared (library-level citation check) | every paired owner's Lean file under `ElementaryHolonics/` | `crates/holonic-engine/src/lean_citations.rs::{every_correspondence_table_cites_a_declared_lean_name,every_inline_lean_citation_names_a_declared_lean_name}` | — | [implemented-exact] Test-only. Reads each paired owner's Lean file, collects its declarations (keyword heads, inductive constructors, structure fields, with universe annotations stripped), and looks up every backticked name in the first column of a header correspondence table and every inline `Foundation/…lean::Name` citation in the engine's sources. A citation resolves by its final segment, because Lean writes a declaration unqualified inside its namespace. A stale citation fails `cargo test` rather than waiting for a review |
| Capability, local chart, and the atlas whose content is its transition maps (C6) | `Foundation/ReceiverAtlas.lean::{no_operation_from_the_carrier_alone,cocycle_iff_isEmpty_defect,atlasGluing_total}`, namespace `Soma.Holonics.Foundation.Atlas` | `receiver_atlas.rs::{Capability,CapabilityKind,SuppliedCapability,NoCapability,LocalChart,CocycleDefect,CoherentPlacement}` | — | **exact**: a carrier with no supplied capability admits no operation, by type rather than by check |
| Typed bridges between charts: eight statuses, one grade, composition at the meet (C7) | `Foundation/Bridge.lean::{BridgeStatus.entails,entails_antisymm,statusMeet,comp_at_meet,counterexample_blocks_promotion}`, namespace `Soma.Holonics.Foundation.Bridges` | `bridge.rs::{EpistemicGrade,SupportLevel,BridgeStatus,Bridge,ProposedBridge,BridgeComposition,ExactRebase}` (`ExactRebase` is the alias of `continuing_tower.rs::ExactShift`) | — | **exact**: a composite never carries a stronger status than its weakest leg, and a recorded counterexample blocks promotion |
| One typed scale between two occurrences: identity, continuation, isomorphism, receiver equality, equal potential, tolerance (T1) | `Foundation/RelationLadder.lean::{Rung,Rung.entails,rungMeet,entails_is_not_total,no_rung_below_identity_entails_identity,Situation,SituationAuto,ReceiverEqualAt,PresentAgreement,EqualPotential,WithinTolerance,situationAutoImpliesEqualPotential,SituationIso,situationIsoTransportsEqualPotential,noSituationIsoBetweenCarriersOfDifferentSize,equalPotentialAntitone,separatorRefutesEqualPotential,noRungBelowIdentityAscendsToIdentity,receiverEqualityWithoutEqualPotential,equalPotentialWithoutIsomorphism,isomorphismWithoutIdentity,continuationWithoutAnyEqualFace,lossyContinuationIsNotAnIsomorphism,receiverEquivarianceIsNecessary,generatorEquivarianceIsNecessary,toleranceIsNotTransitive,withinTolerance_of_width_le,situatedOccurrenceEqIff,passageDoesNotIdentify,shadowOfRelation,Expression,eval,Enactment,musicalFace,absoluteFace}`, namespace `Soma.Holonics.Foundation.RelationLadder` | `relation_ladder.rs::{Rung,rung_meet,Situation,NamedGenerator,NamedReceiver,SituationAutomorphism,EquivarianceReading,ToleranceReading,Separator,PotentialVerdict,ContinuationVerdict,Classification,ClassificationNote,LadderRefusal,Expression,eval,Enactment,musical_face,absolute_face,Admissible,enact}` | — | **exact**: rung 5 **is** `CausalRelevance.futureAgreement` and rung 6 is `ReceiverRelease.width` inside a declared tolerance; neither is redefined. The order is a genuine partial order and not total — a continuation is relative to a declared lineage and is incomparable with a face, so the Rust `Classification` reports it beside the identity chain. Rung 3 implies rung 5 only with the equivariance carried inside it, and each half is separately necessary with a witness. The Rust separator search is bounded by a declared history-length ceiling whose population is checked with checked arithmetic before any walk; exhausting it returns `NotSeparatedWithinBound`, which is never reported as equal potential |
| Standing is what a lineage retains, memory is generated now, and extinction is receiver-relative (T2) | `Foundation/Standing.lean::{causalSignature,causalSignature_eq_iff_futureAgreement,StandingLaw,StandingLaw.futureAgreement_of_retain_eq,StandingLaw.separating_future_refutes_the_standing,canonicalStanding,standingLaw_exists_iff_future_factors,two_histories_leave_one_standing,one_present_face_two_standings_separated_later,TimedFace,MemoryLaw,MemoryLaw.remember,the_remembered_face_is_a_new_occurrence,receiver_agreement_after_the_passage_is_not_recovery,one_standing_two_contexts_two_faces,one_original_two_standings_two_reconstructions,FaithfulAt,faithfulAt_iff_receiver_factors_through_standing,timbreIsNotRetained,the_unretained_receiver_is_reconstructed_confidently_and_wrongly,aperture,aperture_comp_of_le,aperture_chain_has_zero_changing_receiver_defect,the_available_face_changes_while_the_source_does_not,compatibleFibre,compatibleFibre_antitone,the_fibre_is_never_a_singleton_and_a_later_step_separates,apertureInsufficiency,no_transformer_from_the_coarser_aperture,the_chain_separates,a_non_separating_chain_never_reaches_a_singleton,Extinct,width_pair,extinct_iff_release_width_inside_tolerance,extinct_mono_tolerance,extinct_mono_receivers,extinct_mono_generators,fossilStep,fossilState,the_energy_is_the_square_of_the_amplitude,the_wave_is_extinct_at_horizon_three,the_medium_separates_what_the_wave_chart_declared_extinct,the_fossil_is_durable,the_inscription_stays_below_its_limit,the_energy_that_left_the_wave_is_accounted_for,the_per_step_ledger,rotate,planeEnergy,rotate_preserves_energy_along_every_word,no_horizon_releases_the_energy_receiver,nothing_nonzero_is_extinct_at_zero_tolerance,the_difference_is_redistributed_not_removed,lawful_silence_is_extinction_at_the_outward_receiver,causalSignature_eq_potential,retain_eq_establishes_equalPotential,extinct_iff_withinTolerance_after_every_history,extinct_at_zero_iff_equalPotential}`, namespace `Soma.Holonics.Foundation.Standing` | `crates/holonic-engine/src/standing.rs::{GeneratorFamily,SourcePopulation,StandingLaw,ReceiverReading,FutureObservation,SufficiencyVerdict,sufficiency,TimedFace,MemoryLaw,Fidelity,fidelity,ApertureChain,ContractionCertificate,ExtinctionVerdict,extinction,StandingRefusal,EXTENT_CEILING,GENERATOR_CEILING,WORD_LENGTH_CEILING,WORD_COUNT_CEILING,POPULATION_CEILING,OBSERVATION_CEILING,CHECK_WORK_CEILING,SEARCH_WORK_CEILING}`; consumes `receiver_release.rs::{CompatibleFamily,width_enumerated,ReceiverWidth,DiameterNorm,Reading}` and `exact_linear.rs::ExactRatMatrix` | — | **exact**: standing is proved to be *any* quotient through which every admitted future observation factors and no other (`standingLaw_exists_iff_future_factors`), so histories need not be recoverable and no event archive is owed. Memory is a generator: `TimedFace` carries the present time, the same standing under two contexts returns two faces, and the fidelity law is `receiverTransformer_exists_iff` at the retention map, with a constructed instance of a total reconstruction returning a definite face that is exactly wrong at an unretained receiver. Extinction **is** `ReceiverRelease.Releasable` on the two-point family `{T_w x, T_w 0}` (`extinct_iff_release_width_inside_tolerance`), monotone in tolerance, receivers and generators, and equal to the relation ladder's rung 6 after every history — coinciding with rung 5 exactly at `ε = 0`. The exact instance is a damped linear wave coupled to a medium: at horizon 3 the wave chart is extinct at `1/8` while the medium reads `21/64` at the **same** tolerance and never falls below it again, and the declared quadratic energy that left the wave chart is accounted for exactly in the medium plus dissipation. In the exact `3-4-5` rational rotation the declared energy is invariant, so nothing is ever extinct: the difference is redistributed, not removed. The Rust owner returns three values — a certificate-backed `Extinct`, a `Separated` witness `(w, ρ)`, and `NotDecidedWithinBound`; a bounded word search can only refute, and `Extinct` requires a *checked* invariant-chart contraction certificate with an exact reading gain |
| Release over an edited artifact family: the generation law as a tube, the revision cycle as a declared circuit, and the typed disposition (T3) | `Transport/ArtifactRelease.lean::{Artifact,Region,AgreeOn,Releasable,step,stepMulti,SupportedOff,SupportedOffMulti,release_preserved_by_edit_supported_off,release_preserved_by_swing_supported_off,an_edit_supported_off_a_released_region_cannot_change_its_face,reopening_from_outside_is_an_emptying,only_an_edit_touching_the_region_can_change_its_face,restriction_never_widens,stepMulti_singleton,coordinateReading,releasable_iff_every_coordinate_width_is_zero,role,genderAgrees,frameRoleHolds,Admitted,draft,familyZero,pronounRegion,nounEdit,nameEdit,pronounEdit,frameEdit,the_pronoun_is_released_in_the_initial_family,the_noun_edit_keeps_the_family_and_the_release,the_name_edit_empties_the_family,the_frame_edit_empties_the_family,the_correction_restores_the_family,commuting_does_not_separate_the_independent_from_the_entangled,maleDraft,nameSwing,the_swing_enlarges_the_family,width_is_not_monotone_along_edits,regionTower,positionwise,rotationTube,the_rotation_tube_carries_no_holonomy,shearFace,the_entangled_edit_is_not_chartwise,rotate0,shear,the_commutator_of_the_entangled_pair_is_a_translation,commutatorCircuit,the_commutator_circuit_has_a_defect,windingCircuit,the_two_edit_circuits_commute,the_composite_of_the_two_circuits_is_the_joint_one,three_turns_of_the_first_circuit_return_every_face,four_turns_of_the_second_circuit_return_every_face,one_turn_of_the_first_circuit_has_a_defect,indexDistance,the_whole_chart_is_at_index_zero,indexDistance_antitone,periplus,periplusResidual,residual_empty_iff_the_draft_returns,the_visible_face_returns_while_the_residual_is_nonzero,CircuitOutcome,no_defect_on_a_declared_face_is_not_cancellation,Disposition,dispositionOf,the_disposition_is_a_function_of_the_family_and_receivers,an_empty_family_is_a_refusal,the_entangled_step_returns_a_refusal,no_candidate_carries_the_disposition,EditableDraft,IrrevocableUtterance,a_correction_never_changes_the_committed_boundary,a_correction_only_appends,no_sequence_of_corrections_retracts,the_committed_boundary_is_a_prefix_of_every_later_emission,draftRung,progress_is_a_continuation_and_not_receiver_equality}`, namespace `Soma.Holonics.Transport.ArtifactRelease` | `crates/holonic-engine/src/artifact_release.rs::{Artifact,Region,RegionFace,RegionTower,EditAction,Edit,Swing,ConstraintRule,Constraint,EnumeratedFamily,EnclosedFamily,ArtifactFamily,RegionReading,step,restrict_only,StepOutcome,ReleasePreservation,release_after_step,CommutatorVerdict,commutator_verdict,RevisionCircuit,circuit_square,circuit_holonomy,CircuitRefusal,PeriplusResidual,periplus_residual,RouteClass,StopReplaying,CircuitOutcome,RevisionHistory,classify_traversal,draft_rung,DeclaredChart,RequiredChartChange,CausalObstruction,ArtifactDisposition,disposition,Traversal,ArtifactSquareVerdict,ArtifactHolonomyVerdict,BoxedTubeRefusal,EditableDraft,IrrevocableUtterance,EditTorus,ArtifactRefusal,POSITION_CEILING,ALPHABET_CEILING,FAMILY_CEILING,SWING_BRANCH_CEILING,EDIT_WORD_CEILING,CIRCUIT_CEILING,STEP_WORK_CEILING,ROUTE_HISTORY_CEILING,CONSTRAINT_RULE_CEILING,SUPPORT_PROBE_CEILING}`; consumes `receiver_release::{width_enumerated,width_over_readings,Horizon,CompatibleFamily,Reading,ExactFace,ReceiverWidth,LawfulOptions,DecisionLaw,release,ReleaseReturn}`, `continuing_tube::{StationedTower,check_commuting_square,check_circuit_holonomy,SquareVerdict,HolonomyVerdict}`, `continuing_tower::Tower` and `relation_ladder::Rung` | — | [established-bounded; formal-checked; implemented-exact] Item **T3**. The edited family is literally a tube: `RegionTower` is the transverse ladder with `π_B` its `restrict`, `RevisionCircuit` is the longitudinal axis, and the two-axis square is `continuing_tube`'s own, not a second one. Release is `receiver_release`'s width at a region and nothing is rebuilt; the two-axis `Horizon` T5 returned is adopted for the transverse coordinate, with the region's index distance the `k` it must declare. **Three findings.** (1) Over a *family*, an edit supported off a released region cannot change its face at all, so a reopening is always an **emptying** and never a silent rewrite — the resolution is an edit supported on the region, which is a correction. (2) "Independent versus entangled" is three different relations — commuting on the family, chartwise locality (the tube square) and constraint entanglement — and `dog -> cat` against `is -> why` is separated only by the third. (3) Width is not monotone along edits: a swing with sub-swings enlarges the family, and only pure restriction is monotone. Every declared extent is bounded with checked arithmetic before the work it sizes, tokens are exact integers and every width and tolerance an exact `Rat`; each of the 46 audited Lean declarations depends only on `propext`, `Classical.choice` and `Quot.sound`, four on none, and none on `sorryAx` |
| Knot friction and edit torque: free edits, the compensating family and its obstruction, entanglement as self-stress support, and the knot per scale (T4) | `Transport/EditRigidity.lean::{dual_is_dotProduct,mem_range_iff_annihilators_vanish,keptJacobian,the_kept_face_differential_is_the_rigidity_jacobian,kept_rank_nullity,kept_self_stress_dimension,linear_face_is_its_own_differential,mulVec_dotProduct_transpose,Free,free_iff_mem_ker,Compensating,compensating_iff_solves,compensating_family_is_affine,free_iff_zero_compensates,obstructed_iff_certificate,selfStressSpace,selfStressSpace_rigidity,Entangled,Dependent,DroppingLosesNothing,dropRow,transpose_mulVec_eq_sum,entangled_iff_dependent,dependent_iff_droppingLosesNothing,entanglement_is_self_stress_support,a_face_carrying_no_self_stress_forbids_an_edit_of_its_own,first_order_edits_always_commute,sqNorm,PosDef,sqNorm_eq_zero_iff,positive_definite_of_ldl,normal_equation_gives_the_unique_minimizer,work_zero_iff_free,torque,torque_zero_iff_residual_annihilates_image,residual_expansion,stationary_iff_torque_zero,torque_nonzero_has_nonzero_image,the_exact_descent_step,metric_adjoint_pairs_the_residual_with_the_generators,pairRow,pairRow_pairing,ImpliedBy,impliedBy_mono,impliedBy_annihilates_motions,AmbientKnot,ambient_knot_is_downward_closed,impliedBy_gives_ambient_pair,lineChain,the_outer_pair_is_implied_at_the_whole_region,the_outer_pair_is_not_implied_at_the_subregion,induced_implication_does_not_descend,coincidentFace,coincidentEdit,stationarity_is_not_cancellation}`, namespace `Soma.Holonics.Transport.EditRigidity` | `crates/holonic-engine/src/edit_rigidity.rs::{KeptFaceOrigin,KeptReceiverJacobian,KeptReading,kept_reading,identify_with_rigidity_reading,face_own_edit,verify_entanglement_by_dropping,EditDirection,AllowedCompensation,MetricCertificate,ExactMetric,ObstructionCertificate,EditVerdict,RethreadingReceipt,rethreading_work,PluralRethreading,plural_rethreading,GeneratorFamily,ReceiverResidual,EditTorque,edit_torque,ExactDescent,descent_step,pair_row,pair_is_implied_at,ScaleKnotReading,RegionLadder,ArtifactKnots,artifact_knots,scale_knot_readings,KnotAscent,knot_ascent,EditComparison,compare_edits,order_by_squared_work,EditRigidityRefusal,CHART_CEILING,KEPT_FACE_CEILING,ALLOWED_BASIS_CEILING,GENERATOR_CEILING,LADDER_SCALE_CEILING,SCALE_REGION_CEILING,ELIMINATION_WORK_CEILING,PROBE_CEILING}`; consumes `rigidity_receiver::{RigidityJacobian,RigidityReading,rigid_clusters,RigidClusterReading}`, `exact_linear::ExactRatMatrix::{rank,kernel_basis,cokernel_annihilator,preimage_fibre,preimage_obstruction,metric_adjoint,adjoint_defect}` and `presentation_cost::{CostReceipt,Counted}` | — | [established-bounded; formal-checked; implemented-exact] Item **T4**. The edit side of R4's one Jacobian; **no second Jacobian is founded** — `KeptReceiverJacobian::from_rigidity` takes `RigidityJacobian::matrix` unchanged and `identify_with_rigidity_reading` refuses by name if the rank, free dimension or self-stress dimension disagrees with `rigidity_receiver::rigidity_reading`. `ker J_keep` is the free edits, the compensating family is the affine solution set of one exact system, an empty one returns a **checked** left-null certificate (the Fredholm alternative `mem_range_iff_annihilators_vanish`, proved over the rationals here), and `ker J_keep^T` support is entanglement. **Four findings.** (1) R4's removal-sensitivity law — a face is redundant iff a self-stress is supported at it — was claimed in R4's Rust prose and **not** carried by its Lean owner; it is proved here in all three readings (`entanglement_is_self_stress_support`). (2) The knot's nesting direction is **ascending**: what a subregion's induced faces hold, the region's induced faces hold (`impliedBy_mono`); the descending direction is false and `induced_implication_does_not_descend` is the counterexample. Read at the ambient motions instead, knots nest **downward**, so the two readings are not interchangeable. (3) `W^2` is one receiver of the edit: two edits with equal `W^2` carry Pareto-incomparable receipts, and two positive-definite metrics order the same pair in opposite directions, so a ranking of edits by friction is a declared receiver. (4) Of T3's three relations this linear notion is the **third** and neither of the other two: in a linear chart every pair of first-order edits commutes, so commuting separates nothing at all here. `W^2` is an exact rational — never a root, never a float — and every declared metric carries a checked exact `LDL^T` certificate. **Measured**: on a 24-residue RBX1 window at an eight angstrom aperture, displacing one residue is obstructed when only its first contact shell may move in all three presentations, is still obstructed at the second shell in the two free ones and compensable in the CUL1-bound one (which carries no self-stress at all), and when every other residue may move the compensation is supported on 69 of the 72 coordinates. 24 tests run by default, one `#[ignore]`d CPU-only M5 measurement with pinned readings; each of the 37 audited Lean declarations depends only on `propext`, `Classical.choice` and `Quot.sound` and none on `sorryAx` |
| The Holonic Interaction unit — `\|source⟩`, standing medium `H_int`, dynamic `H_pert`, `⟨perspective\|` — and its chain of necks and media (the subject T6/T7/T8 and the contact composition construct) | `Millennium/HolonicInteractionExterior.lean::{exteriorSquareAction,cuspFluxTransport,cuspFluxDefect_cubicZero,cuspFluxDefect_squareNonzero,cuspFluxTransport_advancesOrbit,dualMonodromies_preserveOrientedVolume}`; `Millennium/HolonicSnellInteraction.lean::{PhaseOccurrence,interfaceCurrent_eq_sineDifference,snellCompatible_iff,finiteInterfaceChain_eq_exterior,tangentialConservation_retainsNormalRemainder}`; `Millennium/HolonicRankFourInteractionPlanes.lean::{latticeThreePlusOneEquiv,InteractionPlane}` | `holonic_interaction.rs` (unit, media, contact exchange), `neck.rs` and `jet_staircase.rs` (stations), `holonic_chain.rs` (the chain); they compose `junction_law`, `edit_rigidity`, `causal_chord`, `receiver_release`, `continuing_tube`, `rigidity_receiver` | none | [definition; source-inspected] Typed in the [Aug 14](../research/records/2026-08-14_THE_APERTURE_IS_AN_INTERACTION_HAMILTONIAN_AND_A_PHASE_OBJECT_IS_INVISIBLE_TO_THE_MAGNITUDE_FACE.md), [Aug 20](../research/records/2026-08-20_SHADOWS_OF_HOLONIC_INTERACTIONS.md) and [Aug 24](../research/records/2026-08-24_THE_HOLONIC_INTERACTION_IS_A_COVARIANT_STORAGE_FLUX_COMPLEX_AND_THE_CUSP_RAISES_DIFFERENCE_ORDER.md) records. Returned September 19 as the four rows below (issues #1–#4). Open: faces derived from `exact_contact`/`contact_gluing` geometry, the multi-neck chain, the time/entropy crossing (#5) |
| Media and contact exchange: `M_contact = Σ w_f J_fᵀ D_f J_f`, the clocked edit energy, port-Hamiltonian dynamics `A = (Ω − M)G` with `AᵀG + GA = −2GMG`, and the spectral licence (non-growth vs decay) | `Transport/HolonicInteraction.lean::{contactForm_nonneg,contactForm_kernel_iff,clockedEnergy_eq_duration_times_power,port_storage_rate,storage_rate_neg_of_active_slip,vanishing_rate_form_places_no_spectrum,interface_tangential_agreement_retains_normal_remainder}` | `holonic_interaction.rs::{ContactFace,ContactDissipation,Clock,compare_edit_metric,Medium,Perturbation,Perspective,HolonicInteraction,StorageRateReading,SpectralReading,InterfaceReading,classify_convergence}` | none | [proved-derived; formal-checked; implemented-exact] PSD certified at every assembly; linearization enters `causal_chord::Linearization`; `SpectralReading` is a structural placement with a bounded cross-check (`StructuralPlacement`, `SpectralLicence`, `FormSplit`, `ConservativeCore`): real nonpositive (`Ω=0`), on the axis (`M=0`), non-growth with the on-axis count equal to the largest `A`-invariant subspace inside `ker(MG)` and decay only when it is `{0}`, and the Pontryagin bound `min(p,q)` for indefinite `G` with a vanishing rate form; split and hand stated separately, windings from `winding_inertia.rs`; counts stop at 24 and the core at 64, above which the licence is returned alone. `A = (Ω − M)G` is `[agent-inferred]` (standard port-Hamiltonian form, `H = ½xᵀGx`). Open: edit torque at the clocked metric; elastic, surface-storage and radiation terms; the covariant storage–flux closure `L_A` |
| The jet staircase (T8): finite jet ladder with its compatible families, exact difference↔jet passage, encoding by repeated integration, computed grain/order pairs, the cusp instance | `Transport/JetStaircase.lean::{truncate_idem,truncate_fibre_is_the_top_coefficient,coefficient_eq_iteratedDelta_at_zero,iteratedDelta_ofCoefficients_eq_zero,delta_eq_difference,rebuild_iteratedDelta}`; cusp in `Millennium/HolonicInteractionExterior.lean` | `jet_staircase.rs::{JetChart,FiniteJet,JetLadder,PolynomialFamily,compatible_splines,DifferenceTable,encode_sparse_differences,GrainMap,coarse_grain_order,cusp_staircase,mahler_chart_agreement,omega_action_is_not_iterated_difference}` | none | [proved-derived; formal-checked; implemented-exact] No universal order drop: one source under two grains gives two answers. Mahler/Iwasawa comparison is `[interpretation]`, exact on polynomial functions, stopping at `ω_n ≠ Δ^{p^n}`. Open: Lean change of basis at an arbitrary rational step |
| The neck station (T6): the pinhole where flux converges and diverges; three typed widths; étendue through the focus | `Transport/Neck.lean::{density_of_constant_flux,speedup_of_narrowing,no_speedup_of_negative_flux,residual_eq_zero_iff,etendue_survives_the_focus,reducedEtendue_interface}` | `neck.rs::{TubeProfile,NeckReading,SpeedupReading,StationBalance,NeckTube,GeometricSection,ReceiverUncertaintyWidth,AnalyticWidth,ConstitutiveLink,RayTransfer,optical_train,NeckInvariants}` | none | [proved-derived; formal-checked; implemented-exact] A station of the existing tube (`StationedTower`); balance through `junction_law::check_junction`; analytic width is a squared rational, real quadratic roots decided by discriminant sign, degree above two left undecided; `analytic_width_of_real_spectrum` decides it from a structural placement (a real spectrum closes the strip) at any extent. Open: pinching-strip, p-adic, vortex and whip source maps (`interpretation`) |
| The chain: one interaction read as `\|source⟩ → medium → neck → medium → ⟨perspective\|`; neck = rank of `A₂₁`, flux = power current, Markov staircase = jet, reversal reading, hinge by minimal section | `Transport/HolonicChain.lean::{transfer_factors_through_the_neck,transfer_rank_le_neck_rank,markov_eq_zero_of_closed_neck,markov_is_the_separating_atlas_parameter,transfer_expansion_with_remainder,port_power_balance,neck_power_jump_is_twice_the_cross_dissipation,adjoint_is_the_reversed_structure,no_direction_undoes_the_dissipation,reciprocity_of_the_reversed_chain}` | `holonic_chain.rs::{HolonicChain,NeckCoupling,RankBoundReading,PowerStations,ChainTube,MarkovStaircase,ChainWidths,ReversalReading,HingeSearch,hinge_by_minimal_section,elastic_chain}` | none | [established-bounded; formal-checked; implemented-exact; measured] Consumer: the elastic network `Ω=0, G=I, M=JᵀJ` from `rigidity_receiver::RigidityJacobian`, every constraint row a contact face. The rank is determined by the Fiedler–Markham/Gustafson nullity theorem when the ports read the whole neck (`RankRoute`, `RankLicence`, `PortRanks`; `proved-standard`, a named hypothesis in `full_rank_ports_read_the_whole_neck`, Lean lift owed #34), with no resolvent inverse; a closed neck has no pole and no width (`AnalyticScope::TransferHasNoPole`). M5 RBX1 12-residue window: section 4 of widest 8, determined rank 4 in 0.34 s, residuals 0, whole reading 11 s (was 161 s); 72/120/180 coordinates in 7/168/568 s; 324 did not return; the 612-coordinate complex is refused by the contact-assembly ceiling. Read on measured, designed and predicted RBX1 windows: 30 chains, zero refusals. Open: more than two media; analytic width above 24 coordinates; incremental cut ranks |
| The junction law: the tangential part continuous, the normal part jumping, and the jump being the source on the joint (T7, junction half) | `Transport/JunctionLaw.lean::{CompatiblePairs,join_is_exactly_the_compatible_pairs,noSharedBoundary_isEmpty_join,IsJunction,SideSplit,restrictTo,sideNormalFlux,normalJump,normalJump_eq_divergence,Balanced,balanceResidual,balanced_iff_divergence,balanced_iff_residual_zero,jump,Glues,glues_iff_agree,descend,descend_restricts_left,descend_restricts_right,no_descent_of_nonzero_jump,indicator,discrete_gauss,interior_cell_carries_no_boundary_flux,tellegen,codiff_comp_zero,interfaceSource,vertex_closure,no_interface_flux_for_a_nonclosing_source,chi,chi_union_add_inter,chi_glue_along_zero,unionOf,GluedAlongZero,chi_unionOf,sum_of_unit_costs,each_junction_costs_one_euler,walkHolonomy,walkHolonomy_telescopes,closed_walk_holonomy_one,no_consistent_orientation_on_a_reversing_loop,reflection_circuit_determinant,orientation_reversing_iff_odd,innerQ,innerQ_eq_zero,three_conormals_balance_iff,four_conormals_balance_of_pairwise,four_conormals_pairwise_sum,burgersFlux,shockSpeed,rankine_hugoniot_speed,rankine_hugoniot_balance,lax_admissible_iff,expansion_shock_violates_lax,tangentialPart,tangentialPart_orthogonal,innerQ_decompose,innerQ_split,sineSquared,snell_squared,no_transmitted_covector_beyond_the_critical_angle,InterfaceOutcome,classifyInterface,totally_reflected_iff}`, namespace `Soma.Holonics.Transport.JunctionLaw` | `crates/holonic-engine/src/junction_law.rs::{JointUnits,JointOrder,Joint,Side,Interface,OrientationBit,reflection_circuit_determinant,JunctionLaw,JunctionField,JunctionVerdict,check_junction,TangentialVerdict,check_tangential,descend,VertexVerdict,check_vertex_closure,TellegenReceipt,tellegen,gauss_region,assemble_euler,ResistiveNetwork,NodalSolution,CirculatingReading,dot_exact,tangential_part,InterfaceOutcome,classify_interface,RefractionReading,refract,burgers_flux,shock_speed,ShockVerdict,ShockReading,rankine_hugoniot,StatedLaw,israel_junction_conditions,ThinSheetReading,newtonian_sheet,PlateauVerdict,plateau_line_balance,plateau_vertex_balance,Triangulation,SurfaceCertificate,SurfaceReading,GridIdentification,grid_triangulation,pair_of_pants,klein_bottle,mobius_shorts,torus,one_holed_torus,mobius_band,annulus,SurfaceComparison,compare_surfaces,JunctionRefusal,DECLARED_EXTENT_CEILING}`; consumes `hodge_receiver::{HodgeOperator,MetricDeclaration,BoundaryCondition,hodge_reading}`, `rebase_invariants::{rebase_invariants,boundary_matrix,smith_normal_form}`, `contact_gluing::{orient,OrientationReading}`, `quantity::{BaseUnits,Dimension,Quantity}` and `relation_ladder::{Rung,rung_meet}`; registers three recomputed T7 rows in `receiver_atlas::ledger_entries` | — | [established-bounded; formal-checked; implemented-exact] Item **T7**'s junction half. Both halves of the law are `d` and its metric adjoint at a declared codimension-1 interface: `[[i* u]] = 0` is exactness of the Mayer-Vietoris jump, so tangential continuity **is** being one cochain across the joint and not a constraint on one; `[[n . flux]] = sigma` **is** `delta f = sigma`, because splitting the flux cells splits the divergence. `discrete_gauss` is the summation-by-parts identity and `tellegen` is that identity at a balanced flux — the junction's exact energy ledger, cited from `Foundation/HodgeReceiver.lean::codiff0_adjoint` and not rebuilt. The codimension-2 compatibility is `delta . delta = 0` and a non-closing source returns its annihilating covector. **The declared metric is the constitutive law** — conductance, permittivity, unit sheet weight — which is why the receiver atlas's `GluingWithInterfaceCoupling` row for T7 is computed rather than named absent. Instances: a finite circuit (KCL as the divergence, KVL as exactness, `Delta_0 v = s`, circulating currents as the harmonic 1-cochains of dimension `b_1`, Tellegen exact); a finite Maxwell-type interface (Snell exact over Q in squares, total internal reflection a typed deficit and never a NaN, the normal displacement jumping by the surface charge at a node, and the tangential field jumping by a surface current along a crease — the same grade-parametric checker one grade up); Rankine-Hugoniot for the exact Burgers flux with the Lax condition as the junction's irreversibility; a Newtonian thin sheet with the coupling carried as a declared unit; Plateau's line law proved as an equivalence. The Israel junction conditions are stated, graded `proved-standard` and cited, with what a faithful instance would owe named — the existing `Millennium/HolonicCurvedArcEinstein.lean::{minkowskiMetric,flatLorentzVacuumDynamics}` and `Millennium/NavierStokesCurvedTransport.lean::EinsteinFluidDynamics` provide Lorentzian and conservation foundations; their composition with a four-dimensional hypersurface, extrinsic curvature and Gauss-Codazzi realization is open in this junction owner. Surfaces: the pair of pants and the Mobius shorts both cost one unit of Euler characteristic and agree at every homology receiver; the **one-holed torus** against the Mobius shorts is the pair separated by the orientation bit alone. 42 tests, 43 audited Lean declarations on `propext`, `Classical.choice` and `Quot.sound` only, none on `sorryAx`, no float anywhere |
| The fold: a reflection applied to one side of an exact rational crease, its side-bit residual, and the cut that changes homology where a fold cannot (T7, fold and cut half) | `Transport/Fold.lean::{dot,distSq,dot_pair,dot_shift,dot_self_eq_zero_iff,Crease,side,linReflect,reflect,fold,sideBit,reopen,side_reflect,reflect_involutive,reflect_eq_self_of_side_zero,reflect_sub,linReflect_dot,linReflect_fixes_tangential,linReflect_normal,reflect_distSq,fold_distSq_same_side,fold_distSq_opposite_side,fold_brings_the_two_sides_together,fold_mem_positive_side,fold_eq_self_iff,fold_fibre,reopen_apply_fold,foldTransition,segment,side_segment,crossing,crossing_side_eq_zero,fold_segment_before,fold_segment_after,bounce_direction,sideWord_card,k_folds_branch_dyadically,residual_injective_on_fibre,cmul,cconj,cnormSq,cscale,cnormSq_cmul,lineReflect,rotBy,lineReflect_involutive,lineReflect_cscale,lineReflect_fixes_direction,lineReflect_cnormSq,lineReflect_comp_eq_rotBy,rotBy_comp,rotBy_one,rotBy_eq_id_iff,kawasakiTurn,reflectPairs,CreasesNonzero,kawasakiTurn_ne_zero,reflectPairs_eq_rotBy,kawasaki_iff,plusVertex,skewVertex,plusVertexIsFlatFoldable,skewVertexIsNotFlatFoldable,mountains,valleys,MaekawaBalanced,maekawa_iff_mountain_count,maekawa_degree_four,shear,planeDistSq,shear_changes_a_squared_distance_at_every_point_and_scale,no_isometry_is_a_shear,inversion,distSq_expand,inversion_distSq,inversion_preserves_cross_ratio,ChainTwo,betti0,betti1,betti2,Relabelling,fold_preserves_betti,fold_preserves_intrinsic_length,foldPotential,foldForce,foldStiffness,foldEquilibria_iff,no_equilibrium_of_neg,foldEquilibria_two_of_pos,equilibrium_unique_at_zero,positiveRoot_is_stable,negativeRoot_is_unstable,creaseTorque,held_equilibrium_iff_load_le_threshold,rotCos,rotSin,rationalRotation_orthogonal,displacementSq,displacementSq_eq,displacementSq_scales_with_lever,displacementSq_eq_zero_iff,backboneBarCount,backboneCoordinateCount,backbone_internal_dof,cross,lagrange_identity,dihedralCosSq,dihedralCosSq_nonneg,dihedralCosSq_le_one,pivot_preserves_squared_lengths,pivot_fixes_the_axis}`, namespace `Soma.Holonics.Transport.Fold` | `crates/holonic-engine/src/fold.rs::{dot,squared_distance,cross_product,Side,FoldPoint,Crease,FoldTransition,FoldWord,BilliardUnfolding,billiard_unfolding,LayerContactReading,layer_contact,PlasticCrease,GaussianRational,line_reflection_matrix,RotationReading,rotation_from_two_reflections,shear_defect,Inversion,CellComplex,CellularFold,CellularFoldReading,read_cellular_fold,CutIdentification,CutReading,reglue,read_cut,severed_contact_family,CreasePattern,CreasePatternFraming,hinge_framework,KawasakiVertex,MaekawaAssignment,FlatFoldabilityVerdict,flat_foldability,FoldCatastrophe,EquilibriumReading,CreaseModel,mechanical_base,torque_dimension,rational_rotation,opening_displacement_squared,BackboneResidue,BackboneChain,BackboneAtom,BackboneFraming,ExactDihedral,exact_dihedral,RamachandranChart,RamachandranRegion,RamachandranReading,PivotMove,PivotReading,FoldRefusal,DIMENSION_CEILING,FOLD_WORD_CEILING,CELL_CEILING,PATTERN_VERTEX_CEILING,PANEL_EXTENT_CEILING,BACKBONE_RESIDUE_CEILING}`; consumes `rigidity_receiver::{ExactConfiguration,RigidityJacobian,rigidity_reading}`, `rebase_invariants::rebase_invariants`, `exact_value::SturmChain`, `continuing_tower::{Transition,ReversePassageReceipt}`, `physical_constraint_complex::{ContactClass,DistanceAperture}`, `physical_occurrence::ConstraintDelta`, `presentation_cost::{CostReceipt,code_bits}` and `quantity::{BaseUnits,Dimension,Quantity}` | — | [established-bounded; formal-checked; implemented-exact] Item **T7**'s fold and cut half. A fold is `reflect x = x − 2 σ(x) n / ⟨n,n⟩` applied to the negative side: exact over `Q`, with `⟨n,n⟩` the only divisor and no square root or angle anywhere in the owner. It is a **piecewise isometry and not an isometry** — each closed side keeps every squared distance and across the crease they change by exactly `4 σ(x) σ(y) / ⟨n,n⟩` — and it is two-to-one off the crease, so folding in half is the quotient onto the closed positive half-space with the side bit as remainder. `FoldTransition` is `continuing_tower::Transition` with that bit as residual and a total `reopen`; `k` folds carry `k` bits and `2^k` layers, addressed in `Z/2^k` and cited to `padicTube_crossSection_branching` at `p = 2`. A plastic crease is a **different type with no `Transition` implementation**, whose `reverse_passage` returns the two merged faces. Rotation is two reflections exactly in the Gaussian-rational chart, and Kawasaki's flat-foldability is one rational equation on the accumulated turn; Maekawa is the integer count law; `flat_foldability` never affirms, returning a named local refutation or `NotDecidedWithinBound` against the cited NP-hardness. A shear changes a squared distance at every point and scale, so it is not a composition of folds. A fold cannot move a Betti number (rank invariance under a cell relabelling); a cut can, and the worked instances are an annulus to a disc (`b₁ : 1 ↦ 0`) and a disc in two (`b₀ : 1 ↦ 2`), with the gluing residual twelve bits against a fold's one. Crease patterns reuse the R4 Jacobian with each panel braced as a rigid body, and **a coplanar panel of four or more vertices is infinitesimally flexible and self-stressed**, named in `planar_panels` rather than counted as a folding motion. The protein backbone is `7r − 4` bars on `9r` coordinates predicting `2(r − 1)` internal freedoms, measured on the M5 window; dihedrals are reported as an exact rational `cos²θ` with two signs and never a float, and a Ramachandran region is a declared partition with a ground and an explicit `OutsideTheChart`. Each of the 68 audited Lean declarations depends only on `propext`, `Classical.choice` and `Quot.sound`, one on `propext` alone, and none on `sorryAx` |
| Two presentations of one affine relation, and the rebase between them | `Foundation/RelationPresentation.lean::{Presentation,Presentation.span,Presentation.carrier,Presentation.origin_mem,Presentation.mem_carrier_iff,Rebase,Rebase.refl,Rebase.carrier_eq,Rebase.symm,Rebase.trans,Rebase.residual_empty,domain,image,fibre,Rebase.faces_eq,presentationsWithDifferentSpansDiffer}`, namespace `Soma.Holonics.Foundation.RelationPresentation` | `crates/holonic-engine/src/native_ecology/constitutive_fibre/field/material_transport/normal/direct/wave/family/receiver.rs::{NormalWaveFamily::read_prospective,NormalWaveFamily::read_point_word,NormalWaveFamilyReceiver::point_word,NormalWaveFamilyReceiver::affine_relation}` | `crates/holonic-engine/kernels/exact_resident_section.cu::{fibre_rebase_step,fibre_query,fibre_stage,fibre_gcd}`, `kernels/normal_family_receiver.cuh::{section_normal_point_word_seed,section_normal_point_word_step}` | **exact**: a prospective future is carried in either of two charts of the SAME relation — one expanded composite in the i64 affine wire, or the retained ordered generating word transported in the wide carrier from this family's own receiver point. `Rebase.carrier_eq`/`Rebase.faces_eq` are the law that the two read identically; the device test `rebase_and_expansion_read_the_same_future` is that law at the receiver. The word is exact exactly where no output coordinate is free at a fixed anchor, and refuses a genuine vertical fibre rather than standing in for the expansion. `fibre_rebase_step` is the same re-basing inside one elimination step: the row is content-reduced against its pivot BEFORE the products, which the following normalization would otherwise divide straight back out. Reported values are unchanged bit for bit; only the intermediates shrink, and the transient factor removed is what left a 128-bit carrier |
| Launch is a passage owing a receipt; exclusive device access is typed (D1/D2) | `Foundation/DeviceLaunchLaw.lean::{linear_thread_bijective,DisjointPartition.region_disjoint,DisjointPartition.raceFree,scatter_perm,scatterAdd_perm}` | `crates/holonic-mount/src/launch_law.rs::{LawfulLaunch,LaunchEvidence,LaunchReceipt,DeviceWriteSpan,PartitionedWrite}` | `crates/holonic-engine/kernels/` | **partial:** exact for the declared launch shape and the partitioned write scope; every production mouth now presents `LaunchEvidence::Device` and defers nothing, while `LaunchEvidence::Census` remains a caller declaration and not a measured device capability |
| Declared incidence generates the gather, residency and scatter of a section operator (D3) | `Foundation/SectionLayout.lean::{Incidence.adjoint,Incidence.assembled_apply,Incidence.assembled_sum_regions,Incidence.assembled_symm,Incidence.colour_schedule,Incidence.colour_fiber_single_region}` | `crates/holonic-mount/src/section_layout.rs::{IncidenceDeclaration,SectionLayout,ScatterReceipt,RegionColouring,SectionKernels}` | `accelerators/cuda-kernel/src/lib.rs::{section_gather,section_apply,section_scatter_store,section_scatter_add}` + `crates/holonic-abi/src/section_layout_cuda.rs` | **bounded:** one dense local operator per layout, one device ring `Z/(2^61-1)`; device result equals the exact CPU reference bit for bit on a 1D chain, a 2D 4-neighbour grid and an irregular contact incidence |
| The engine's hand-written enclosure scatter declared as that incidence, and cross-checked against it on the card (D3 engine side) | — (cites `Foundation/SectionLayout.lean` through `mount::section_layout`) | `crates/holonic-engine/src/section_layout_adoption.rs::{enclosure_scatter_incidence,window_placement_operator}` | `crates/holonic-engine/kernels/normal_enclosure_ports.cuh::section_normal_enclosure_scatter_section` | **bounded:** coordinates only, and only on words in `[0, 2^61-1)`; the enclosure radius is a second accumulating address with no device arm, and the triple must run in its own context |
...
| Exact physical contact incidence read as homology, Hodge and spectrum | `physical_constraint_complex.rs`; `physical_constraint_grading.rs` | The adapter into `GradedCausalComplex`, and through it `rebase_invariants` (Betti with torsion through Smith normal form), `sheaf_diffusion` (coboundary and the exact Hodge operator) and `lattice_gauge::exact_spectrum` — one reading per resolution of the open class |

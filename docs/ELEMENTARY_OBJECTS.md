...
| Navigator | `configure` (initial configuration: the key), `advance` (own clock, carry), `restrict` (scale square), `address` (source word), trace faces and the dynamical zeta, lock address, scale zeta and complex dimensions, `release` at tolerance | Lean `Holon/Generator`, `Foundation/{FractalPacking,FractalString,GeneratorInference}`, `Transport/{GeneratorTraceFaces,SourceMoment,ReflectiveContinuation}`; Rust `holonics::navigator` (`Navigator`, `Transport`, `Clock`, `PhaseLift`, `reflection`, `address`), `holonics::navigator::trace::{SiteFactor,Machine}`; `holonics::navigator::Transport::Map`, a finite-order port map on `ℤ/d` with `order` and `compose` (Lean `Holon/Generator.{map_pow_mod_order, map_turn_lossless, map_compose_order_pos, map_compose_order_dvd, mapRotor_order}`) | `holonics::navigator`; `Holonics.Navigator` |
...
`Transport/GeneratorTraceFaces` own: for `N` equal ratios `r` it is `det(1 − T·[N])` at `T = r^s`

...
`archive/reference/holobrochos-a07ff376/src/holobrochos/THEORY/54_THE_DISCRETE_MANIFOLD.md` §4 (2026-07-05),
...
`docs/canon/TABLET_THE_CHART.md` §10 states the first half — *a coordinate system is a receiver and the
...
`archive/reference/holobrochos-a07ff376/src/holobrochos/THEORY/32_THE_TRANSFORMER.md` §2, ratified
2026-07-04:
...
**One notation hazard.** `research/papers/source/synopsis/AUDIT.md` lists *"cross-ratio equals Euler
...
`docs/canon/TABLET_THE_FLOW.md` §7 holds the built gates: `analytic_field.rs` refuses construction unless
...
- **lensing is the metric bending the cone**, not a diaphragm. The body already has this and it was
  dismissed once as a proper name: `crates/holonic-body/src/manifold.rs:158`, *"the deposited circulation DRAGS
...
A device path per organ is the **cabinet-of-organs failure one level down**. `docs/canon/THE_HOLOBROCHOS_SPINE.md`
...
`hardware_cover::expand_front` (`crates/holonic-engine/src/hardware_cover.rs:1258`; the line has
drifted before — `grep -n "pub fn expand_front"` is the durable route) is the law: the
...
Proved identical at 1, 2, 3, 8 and 64 lanes over a branching front (`:1113-1146`), with a failing
cell surfacing from any lane (`:1150`).
...
|---|---|
| `causal_language` leader | **`expand_front`** — `crates/holonic-life/src/causal_language.rs:1038`, with the tip's extent supplied as `emitted.len() + 1`. Migrated. (Cited at `:482`, then `:820`; the line has moved twice in one day. Cite the construct — `grep -n "expand_front("` — and let the line be a hint.) |
...
corpus, which is the condition `docs/canon/THE_MEASURED_CAPABILITIES.md` already records — `4 → 55,098 ms`,

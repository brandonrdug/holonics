...
> [the repository guide](https://github.com/brandonrdug/holonics/blob/13f8c734/docs/REPOSITORY.md); the local equation atlas remains archived; current evidence is repository-local.
...
**Where this lives now.** `docs/canon/00_PURE_HOLONICS.md` and `docs/canon/07_CAUSAL_ALGEBRAIC_GEOMETRY.md`
...
**Where this lives now.** `docs/canon/02_INFORMATION_PHYSICS.md` and `CLAUDE.md` §4. The charter quote —
...
**Where this lives now.** `archive/plans/legacy/EROS_MATHEMATICS_PRODUCTION_FLOOR.md` is the standing floor.
Executable: `crates/holonic-life/src/lean_mathematics/` and `lean_mathematics.rs` inside `crates/holonic-life` (41,975
...
32 example filenames under `crates/holonic-life/examples/`. The energy premise ("intelligence does not cost
...
it. The Swing's geometry survives in `crates/holonic-body/src/carriage.rs` and `crates/relational-geometry/`
...
`crates/holonic-life/examples/eros_relampago_atmospheric_current.rs`.

Exact arithmetic is carried by `crates/holonic-body/src/num.rs` (no_std, zero deps), `crates/relational-geometry/src/exact.rs`
and `exact_analysis.rs`, and `crates/holonic-engine/src/exact_value.rs` / `exact_linear.rs`.

**One standing defect against this theme.** `crates/holonic-life/Cargo.toml` declares
`rug = { version = "1.30", features = ["float", "std"] }` alongside `ab_glyph` and `aho-corasick`;
all three are referenced **zero times** in `crates/holonic-life/src/` or `crates/holonic-life/examples/`. `rug` pulls
GMP/MPFR — an MPFR-float dependency standing in the manifest of the ecology crate. It should be
struck.

`archive/plans/CONTAMINATION_BANS.md` and `archive/plans/legacy/CPP_GPU_FOUNDATION.md` state the rules; the latter
...
**Where this lives now.** `docs/canon/03_CONDITIONING_AND_LEARNING.md`; `archive/plans/legacy/THE_GROWN_CIRCUIT.md`
(ratified 2026-08-07) is the only construction contract in the tree that treats growth as the object.
Executable: `crates/holonic-life/src/morphological_language/` and `morphological_language.rs`;
`crates/holonic-life/src/suffix_ecology.rs` (1,661 lines) realizes the emergent-tokenizer claim;
`crates/holonic-membrane/src/growing_carrier.rs`, `growing_ranked.rs`, `growing_sparse.rs` are the growth
carriers. `crates/holonic-life/src/resonance_ecology/` carries the leader/front material.
...
2026-08-07**: `docs/canon/04_GEOMETRY_NAVIGATION_AND_WEAVE.md:65-79` carries *ant integration* and
...
**Where this lives now.** `archive/plans/legacy/THE_GROWN_CIRCUIT.md` — ratified by Brandon 2026-08-07, and
...
deficit. `docs/canon/01_CAUSAL_CALCULUS.md` holds the causal-calculus doctrine.
...
exactly as `archive/plans/legacy/THE_GROWN_CIRCUIT.md` claimed.
...
unrecovered. Byrne's styling is not present in `research/papers/source/` as a reasoning mode. And the
...
`research/papers/source/research/papers/prime-archimedean-formulation-atlas/main.typ` carries the **Bowl of Integers**
...
**Where this lives now.** `docs/canon/07_CAUSAL_ALGEBRAIC_GEOMETRY.md` and `CLAUDE.md` §2/§3/§12.
...
**Where this lives now.** `docs/canon/05_ONTOLOGY.md`. `[measured]` "Localized inevitabilities" and
"localized truth" appear in no source file. The strongest executable expression of the position is
negative and real: `crates/holonic-life` conditions and generates with **no distribution, no corpus scan, and
no router** — see `crates/holonic-life/src/suffix_ecology.rs`, `text_material.rs` (1,686),
`current_world.rs` (1,954), and `docs/canon/06_ESTABLISHED_CAPABILITIES.md`.
...
files) is the dead tree, kept as a lesson at his instruction. `crates/holonic-life/src/dialogue_lineage.rs`
and `crates/holonic-membrane/src/recovery.rs` are the live lineage carriers. **The busy-loop argument is the
...
his direction, and `docs/canon/EPISTEMIC_GRADES.md` makes a falsification a first-class return. Live
...
**Where this lives now.** `docs/canon/THE_RECOVERED_LAW.md` (deposited 2026-08-07, 324 lines) is the
...
use tokenizers, statistics, clustering, search, labels**. `archive/plans/CONTAMINATION_BANS.md` carries
...
jurisdiction rule in `docs/canon/THE_RECOVERED_LAW.md` is the reconciliation, and `CLAUDE.md` does not
cite it.
...
- The transition commit message states "543 passed" against 15 targets. `--list` on each binary
  ...
  is wrong.**
- The one failure is real and unfixed: `crates/holonic-mount/src/bin/mount-scope-gate.rs:1436`,
...
`docs/canon/THE_MATHEMATICS_TABLET.md` §7 is the synopsis, including why the Millennium problem is
...
**Where this lives now.** `CLAUDE.md` §6, §7, §9, §10 and `docs/canon/THE_RECOVERED_LAW.md`. `[measured]`
...
`docs/ARCHITECTURE_MAP.md` and reviewed at the changed owner.
...
only copy being `archive/reference/engine-a07ff376/HOLONIC_DSA_BASELINE.tsv`, archive material), **and**
three of its five `PROTECTED_ROOTS` were laboratory paths — `src/crates/holonic-body/src`,
`src/crates/holonic-membrane/src`, `src/crates/holonic-life/src` — so **the ownership ratchet covered none of `soma/`**.
...
**Now:** the baseline is `tools/baselines/HOLONIC_DSA_BASELINE.tsv`, emitted from a detached worktree of the
...
*Status:* **does not exist.** `applications/record-index` is the nearest thing and
`[measured]` **it cannot find a root here** — `discover_laboratory_root` at
`soma/tools/record-index/src/lib.rs:192-203` required `src/soma/FORMULA.md` and `.agents/COMMUNE`,
...
*Status:* `crates/holonic-membrane` exists at 26,194 lines and is load-bearing, but it is a **contact and
...
and `crates/holonic-life/examples/eros_parent_on_open_substitution.rs`. **No owner evolves a codec for an
unseen format.**
...
*Status:* **does not exist.** `formal/elementary-holonics` is a Lean project and
`research/notebook/` is a personal Typst+Lean workbench; neither is the ordered derivation chain. Explicitly
...
carriers exist and are live: `crates/holonic-life/src/text_material.rs` (1,686) + `text_material/` +
`text_material_cuda.rs`, `crates/holonic-life/src/laboratory_language/` (1,951),
`crates/holonic-life/examples/eros_cohered_corpus.rs`, `eros_text_training.rs`. `soma/tools/genesis-chain.py`
...
`crates/holonic-life/src/synchronized_occurrence/`, `coupled_informant_current.rs`,
...
beyond repair. Carriers exist — `crates/holonic-life/src/agentic_language/`, `agentic_research.rs`,
...
*Status:* `docs/references/EXTERNAL_RESOURCES.md` exists (104 lines, accessed 2026-08-03) and
...
under `archive/cpp-engine/`. `research/notebook/check.sh:27` still does `cd
"${repository_root}/formal/elementary-holonics"` — **that path no longer exists** (it is
`formal/elementary-holonics`), so the notebook's Lean check is broken by the transition.
`archive/reference/engine-a07ff376/` is now **182 byte-identical duplicates** of live files, one differing
...
`archive/plans/legacy/THE_GROWN_CIRCUIT.md` records.
...
| MorphoHDL circuitry analysis; triangular hinge-like simplicial complex | §8 | `archive/plans/legacy/THE_GROWN_CIRCUIT.md` (contract only, no code) |
| "Relativistic calculus of information topology" | §8 | none |
| Lightning-leader integration; sphere-packing; self-similarity limit | §9 | `leader_quadrature.rs` (the self-similarity termination law, measured); `research/papers/source/research/papers/prime-archimedean-formulation-atlas/main.typ` (the Apollonian packing as an integer reflection orbit) — **row corrected 2026-08-13; it read "no code"** |
...
| ant integration; warp and weft | §7 | **`docs/canon/04_GEOMETRY_NAVIGATION_AND_WEAVE.md:65-79`**, ratified; the `(position ; winding)` datatype is not implemented |
...
| Holonic language; ownership discipline | §25 | `crates/holonic-language`; cross-language owners in `docs/ARCHITECTURE_MAP.md` |
| Holonic relational database | §25 | none; `applications/record-index` **broken** |
...
| External-resource sweep | §25 | `docs/references/EXTERNAL_RESOURCES.md` incomplete |
| `rug`/`ab_glyph`/`aho-corasick` declared and unused in `crates/holonic-life` | §6 | standing defect |
| `mount-scope-gate` failure at lane cohort four | §20 | standing defect, fifth fixture unreached |
| Governing documents describe an archived body | §26 | `README.md`, `CLAUDE.md`, `CONSTRUCTION_STATE.md`, `research/notebook/check.sh` |
...
`grep -n "project-postulate" docs/canon/TABLET_THE_UNIVERSALITY_MACHINE.md`,
`grep -c "rug\|ab_glyph\|aho-corasick" crates/holonic-life/Cargo.toml`,
`grep -n "architecture-lint" tools/gates.sh`.

- the Universality Machine definition is carried at `docs/canon/TABLET_THE_UNIVERSALITY_MACHINE.md:100`,
...
- `crates/holonic-life/Cargo.toml` declares none of the three dependencies;
- `architecture-lint` is a live gate with a committed baseline (`tools/gates.sh`,
  `tools/baselines/HOLONIC_DSA_BASELINE.tsv`), repaired 2026-08-10 — it is not broken.

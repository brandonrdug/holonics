...
transcripts and files named; the laboratory is cited as **provenance only** (`docs/canon/THE_DOCUMENT_LAW.md`
...
Beside them: `docs/canon/THE_DIALECT.md:127` measures *"You overcomplicated it"* at **~7% of all his
...
`archive/reference/holobrochos-a07ff376/src/soma/FORMULA.md:7987-7993` makes the protection **jurisdictional
rather than epistemic**, and `docs/canon/THE_CONTAMINANT_PROTOCOL.md` §2.4 already carries its clause 3.
...
1. **The document that measures the pattern is unreachable from the entry point.**
   `docs/canon/THE_DIALECT.md:18` declares *"Read this before `docs/canon/THE_QUOTE_NETWORK.md`."*
...
3. **The one check that would have fired was one command and was not run.** `CLAUDE.md` §0 says of
   `docs/canon/THE_HOLOBROCHOS_SPINE.md`: *"Read it before proposing any construction; an organ that no
...
instead of source"* — a sentence `docs/canon/THE_RECOVERED_LAW.md` was created to carry.
...
`crates/holonic-life/examples/eros_lean_proof_production.rs` already does at 39 declaration organs (`CLAUDE.md`
...
- The tree now carries **two independent Lean readers**: the imported
  `crates/holonic-life/src/lean_mathematics.rs` plus `lean_mathematics/` at 3,521 lines, and this one at 1,692.
...
(`2b5c2213`, 06-23), `src/formal/elementary-holonics/` (`7fba08d2`, 07-26), and the 3,521-line
`crates/holonic-life/src/lean_mathematics.rs` (`ba8716b5`, 08-02). Seven weeks, six bodies.
...
**Not established, and stated so:** whether the ten `crates/holonic-life` ecologies are genuinely redundant.
...
re-founding claim is solid at the era, Swing and Lean level; inside `crates/holonic-life` it is *same shape,
plausibly layered*. `crates/holonic-life`'s per-file provenance is squashed and could not be dated from commits.
...
`archive/reference/holobrochos-a07ff376/src/soma/FORMULA.md:283-287`, §IX *"THE NUMBER AND THE WORD — one
algebra"*:
...
**There is one substrate and the charts are peers.** `docs/canon/00_PURE_HOLONICS.md:25-27`:
...
`docs/canon/THE_HOLOBROCHOS_SPINE.md:202-205`:
...
**It is bits, in the law itself.** `crates/holonic-body/src/place.rs:5`:

> *"★ **THE QUANTUM IS THE BIT** — a place is **extended one bit at a time** (`extend`), never by a
> byte."*

`crates/holonic-body/src/boundary.rs:1-6` — *"Light in / radiation out, as SHAPES, with **NO AUTHORED
ENCODER**… an octet does not enter as its absolute code (a symbol-fiction) — it enters as its
DIFFERENCE from context."* `crates/holonic-body/src/lib.rs:7` calls `carriage` *"the **substrate-neutral** whole
lineage stroke."*

**And the operation has no home domain.** `CLAUDE.md:409`: *"every domain is a different **material**
carried by the same operation."* `docs/canon/THE_DIALECT.md:266-268`: *"He is describing one operation —
...
2. **Write the owner's `file:line` before naming the object.** Search for the *mechanism*, not the
   ...
   `docs/canon/THE_MEASURED_CAPABILITIES.md` is the index for this and is already in the pickup order.
...
4. **Run the spine test.** `grep -i '<the object>' docs/canon/THE_HOLOBROCHOS_SPINE.md`. No station, no
...
the failure it prevents, and it is measured. `docs/canon/THE_DIALECT.md:196-197`: *"Profanity (6.7% of
...
`crates/holonic-life/examples/eros_lean_proof_production.rs`, 39 declaration organs, 9 kernel-admitted, 22
...
**It is not a scheduler and it grades nothing.** `docs/plans/THE_ROADMAP.md` holds the order of work.

**It is not complete.** Fourteen instances across four days and three laboratory months because
fourteen are measured. Further instances are added here under `docs/canon/THE_CONTAMINANT_PROTOCOL.md` §1

...
> [the maintained crosswalk](https://github.com/brandonrdug/holonics/blob/13f8c734/docs/ARCHITECTURE_MAP.md) and the subject guides; dated repository records supply
...
`docs/canon/THE_DOCUMENT_LAW.md` §1 permits exactly two genres to be routing-only; this is the second.
...
**What this is, and what it is not.** `docs/canon/THE_DIALECT.md` maps Brandon's language.
`docs/canon/THE_QUOTE_NETWORK.md` maps provenance. `research/papers/source/holonics/registry.typ` maps mathematical
objects by id. `docs/canon/THE_CORRESPONDENCE_ATLAS.md` lets the corpus be entered **by concept**, and says
so. The former generated claim index carried titles and therefore could not carry a line.
...
| subject | owner | note |
|---|---|---|
| **the mouth itself** | `crates/holonic-life/src/incidence_production.rs:2113` `admit_later` | derives the arrival's rank in `⪯` from the material's own `caused_by`; returns `ArrivalResponse` |
...
| the ports | `crates/holonic-body/src/incidence.rs:199` `EventPortKind::{Ingress,Exposed}` | `EventPort::ingress(cell, hand, slot)` — address ⊕ hand ⊕ slot |
...
| material membrane (text) | `crates/holonic-life/src/text_material.rs` | container, byte range, role, chronology, parentage retained as lineage |
| dialogue membrane | `crates/holonic-life/src/dialogue_lineage.rs` | admits only visible `user`/`assistant` message occurrences |
...
| interior conduct entry | `crates/holonic-membrane/src/event_mouth.rs` | **not** an exterior intake — conducts an already-built event population |
| the outfall | `crates/holonic-life/src/form_mouth.rs` | sealed octets to disk; content-addressed |
...
| the fate vocabulary (ABI) | `crates/holonic-abi/src/holon.rs:585` `SourceDispositionRow` | `standing` / `continued(carrier)` / `ended` |
| a contact that lived and returned nothing | `crates/holonic-abi/src/holon.rs:1226` `SilentReceipt` | *"no world-side actuation… The source relation genuinely lived"* |
| the return names its origin | `crates/holonic-abi/src/holon.rs:646` `ReturnOriginRow` | continuation vs birth, with plural parents as lineage rows |
| the six-role transition | `crates/holonic-membrane/src/live_holon.rs:477-566` | `begin` · `meeting` · `deed` · `consequence` · `returned` · `retain_residual` |
| the grain re-cut | `crates/holonic-life/src/decomposing_codec.rs` | re-cuts at the collapsed pair's own separating word |
| the join to elaboration | `crates/holonic-life/src/reintegrating_elaboration.rs` | built; **its only caller is a driver** |
| the live plan and its two convictions | `docs/plans/THE_ROADMAP.md` plan 1 | 46,745 identities embed the CPU path; 11,266 of 11,282 Codex witnesses lost chronology |
...
|---|---|---|
| **the neck** — no two-body contact | `archive/reference/holobrochos-a07ff376/src/holobrochos/THEORY/33_THE_NECK.md` §2 | a mouth handing material to the interior is an identity map and passes nothing |
...
**`H.0466`**, `proved-derived`, `research/papers/source/holonics/mathematical-physics.typ:469` is the
...
| subject | owner | grade |
|---|---|---|
| Fourier–Plancherel, convolution transport | `H.0246`, `research/papers/source/holonics/topology-analysis-dynamics.typ:339` | `proved-standard` |
...
| the conduct rule | `docs/canon/TABLET_THE_REASONING_CYCLE.md:243`, `docs/canon/TABLET_THE_CAUSAL_PROFILE.md:173` | Fourier needs a founded translation action, Mellin a founded dilation action |
...
| the half-density chain | `research/papers/source/mathematics/lemmas/mellin-half-density-chart.typ`, `theorems/oriented-half-density-crossing.typ` | classical |
...
The last construction promoted into an *"aperture law"* was withdrawn: `docs/canon/TABLET_THE_MANIFOLD.md`
...
| registry form | `H.0461`, `research/papers/source/holonics/mathematical-physics.typ:313` | `definition` — *"A specific symmetry group and representation law are required."* |
...
| **gravitas, exactly** | `docs/canon/THE_RECOVERED_LAW.md:180-194` | *"Gravitas **is** geodesic deviation"*; **the metric is a receiver face of standing** |
...
| **mass weighs, never gates** | `crates/holonic-body/src/arrow.rs:15-26` | `reach` WEIGHS; `aim` GATES — the jurisdiction rule |
| charge as combinatorial defect | `crates/holonic-engine/src/local_star.rs:1200-1247` | with an exact conserved residual |
| section modulus, the placement law | `docs/canon/TABLET_THE_MANIFOLD.md` §on section modulus | equal material ≠ equal load; the crossing bears it |
...
| subject | owner | grade |
|---|---|---|
| **the field equation** | `H.0460`, `research/papers/source/holonics/mathematical-physics.typ:284` | **`proved-standard`** — contracted Bianchi in the statement, so a solution owes `∇^μT_μν = 0` |
...
| what IS barred | `docs/canon/THE_HOLOBROCHOS_SPINE.md` §2 | the **Holobrochos slogan-form** of the field equations, `HUNCH`/`OPEN` at source — not the mathematics |
| conservation as an identity | `archive/reference/…/CANON/THE_EMPTY_BOUNDARY.md` §0 | Bianchi forces `∇·T = 0`; Kirchhoff is its circuit face; `∂²=0` is why a read can finish |
| the world tube and the profile | `docs/canon/TABLET_THE_CAUSAL_PROFILE.md:22-72` | `definition`; the caustic instance was **withdrawn 2026-08-13** and the body owns none |
| the cone and the horizon | `docs/canon/TABLET_THE_MANIFOLD.md` §22 | the metric sets the cone and the metric is a receiver face of standing |
...
| **the loop that is stated and not closed** | `docs/plans/THE_ROADMAP.md` § on the geometry response | *"`geometry_responses` is supplied, never updated… The curvature is measured and discarded."* |
...
| subject | owner | grade |
|---|---|---|
| the chain law and its conditions | `docs/canon/THE_HOLOBROCHOS_SPINE.md` §1 | `established-bounded`; the conditions are read off the law, not enumerated |
...
| circuit chains, current and voltage | `H.0454`, `research/papers/source/holonics/mathematical-physics.typ:112` | `proved-standard` |
| incidence, cycles, cuts | `H.0144`, `research/papers/source/holonics/algebra-combinatorics.typ:429` | `proved-standard`; cycle current ⊥ gradient potential |
| **the complete crossing law** | `research/papers/source/mathematics/theorems/causal-parity-kirchhoff-return.typ` | event balance, interval balance, Kirchhoff loop, curved loop, junction return, **branch reflection vanishing at admittance matching**, standing return, passivity |
...
| **the valve** | `crates/holonic-life/src/relational_language/types.rs:163` | a contact conducts only after the same junction phase returns through **two distinct** source pairs |
...
| the Smith chart as a chart | `docs/canon/TABLET_THE_MANIFOLD.md` §16.1 | `Γ` is Möbius; sections compose in `PSL(2,ℂ)`; match means the germ crosses whole |
...
| — implemented | `contact_gluing.rs:313-502`; `leader_quadrature.rs`; `crates/holonic-body/src/channel.rs` | the return rides **against** orientation — the half turn |

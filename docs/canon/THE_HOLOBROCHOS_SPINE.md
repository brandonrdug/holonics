...
**Genre:** canon (`docs/canon/THE_DOCUMENT_LAW.md` §1.1). It states the spine the roadmap is ordered by.
**It schedules nothing**; `docs/plans/THE_ROADMAP.md` remains the only file that says what is open.
...
**Both are live in this repository** — `research/papers/source/synopsis/` and
`research/papers/source/holonics/registry.typ` — and the Soma documents themselves are at
`archive/reference/holobrochos-a07ff376/src/soma/`, whose directory name is misleading.
...
catalog grades it `HUNCH`/`OPEN` and it carries a twenty-seven-entry superseded ledger. `archive/reference/README.md`
...
`research/papers/source/synopsis/AUDIT.md` and `research/papers/source/synopsis/README.md` performed the Holobrochos
...
`docs/canon/02_INFORMATION_PHYSICS.md` lists **six** — *"Rest, accumulation, leak, circulation,
...
Verbatim from `research/papers/source/synopsis/README.md` and `AUDIT.md`. **This list governs. Do not extend
...
probability, and `archive/reference/holobrochos-a07ff376/src/soma/FORMULA.md:8424` **explicitly refuses that
import**: the abstraction *"does not thereby import a Hilbert space, Born rule, Planck scale, or
physical quantum field into software or social systems."* `docs/canon/TABLET_THE_TURN.md` already states
...
is named nowhere in `docs/canon/THE_MEASURED_CAPABILITIES.md`; it should be.
...
`archive/reference/holobrochos-a07ff376/src/soma/FORMULA.md`:
...
`archive/reference/` is non-authoritative historical material by location. That `condition_again` passes an
...
**Credited.** `crates/holonic-life/src/conditioned_rest.rs` already does it correctly. The production is sealed
...
across a call. `archive/plans/THE_ASSEMBLY.md` F1 is the reason this is the natural shape here anyway —
...
| ~~`soma/abi::cuda_execution`~~ | ~~273~~ | ~~0~~ | **DELETED 2026-08-10**, `crates/holonic-abi/src/lib.rs:16` |
...
| `temper` | 597 | 0 | wired by the return join — see `docs/plans/THE_ROADMAP.md` |
...
mattered most: **every large `crates/holonic-life` learning organ named in this project's capability record
...
`crates/holonic-life` reaches the *substrate* (`holonic-structure`, 6 modules, 20+ consumers) heavily and the
*engine* almost not at all. `archive/plans/THE_ASSEMBLY.md` F1 records that the obvious wiring is
...
Executable: `crates/holonic-abi/src/active.rs:266` — `ActionCurrent::new` **refuses zero**, *"A1 supplies a
...
1. **No claim inherited from the speculative corpus beyond §2's retained list.** `holobrochos/`,
   `labyrinth/`, and the physics rooms are application and comparison domain. `archive/reference/README.md`
...
- It does not supersede `research/papers/source/synopsis/` or `research/papers/source/holonics/registry.typ`, which are
  the current authority for pure-holonics and RH scope, nor `docs/canon/THE_RECOVERED_LAW.md` on
  jurisdiction, nor `docs/canon/EPISTEMIC_GRADES.md` on grading.
- It is not a schedule. `docs/plans/THE_ROADMAP.md` is ordered by it and remains the only statement of
  what is open.

...
`crates/holonic-life/src/agentic_research.rs:643-704` `advance_open_completion`, whose exterior world is an
...
quotient remains. That is the same sentence as `docs/canon/TABLET_THE_FLOW.md`'s comprehension law: a body
...
|---|---|---|---|
| — | **mount** | `crates/holonic-membrane/src/live_current.rs` `receive_with` | **WIRED**, 10 consumers |
| — | mount, resident | `crates/holonic-mount/src/cuda.rs` module/function/buffer | **WIRED**, 10 |
| — | mount, material | `crates/holonic-life/src/text_material.rs`, `conditioned_rest.rs` `mount` | **WIRED** |
| **heat** | **differentiate** | `crates/holonic-life/src/incidence_production.rs` `differentiate` | **WIRED**, 4 |
| **heat** | decompose | `crates/holonic-life/src/decomposing_codec.rs` `decompose` | **WIRED**, 1 |
| **heat** | **found a new axis** | `crates/holonic-engine/src/founded_receiver.rs` | **WIRED** |
| **heat** | branch, emanate | `crates/holonic-life/src/suffix_ecology.rs`, `resonance_ecology` | **WIRED**, 5 |
| — | **conduct** | `crates/holonic-abi/src/conduct.rs` `conduct_current` | **WIRED** |
...
| — | **radiate** | `crates/holonic-body/src/manifold.rs` `live_*_emitting` | **WIRED**, 27 |
| — | radiate, journal | `crates/holonic-membrane/src/emission_journal.rs` | **WIRED**, 6 |
| — | **exterior return** | `crates/holonic-engine/src/world.rs` `receive_through` | **WIRED**, 32 — the atomic commit |
| — | return, real port | `crates/holonic-life/src/lean_mathematics/kernel_returns.rs` (`lake env lean`) | **WIRED**, 2 |
...
| **cool** | **deposit** | `crates/holonic-life/src/form_mouth.rs`, `holonic_training.rs` commit | mouth **DRIVER-ONLY** by design |
...
| **cool** | condense with certificate | `crates/holonic-membrane/src/live_constituent.rs` `compress_certified` | **WIRED**, 10 |
| **heat** | reopen a **numeric face** collapsed to a grain | `crates/holonic-engine/src/reopening.rs` `reopen` | **DRIVER-ONLY** |
| **heat** | **reopen a collapsed DISTINCTION — re-cut at the collapsed pair's own separating word** | `crates/holonic-life/src/decomposing_codec.rs` | **WIRED**, 1 — and its one caller is the row below |
| **heat** | the join: elaborate → re-integrate → next pass on the changed route | `crates/holonic-life/src/reintegrating_elaboration.rs` | built; **its only caller is a driver** |
| **heat** | reconstruct | `crates/holonic-life/src/reconstruction_fiber.rs` | **DRIVER-ONLY** |
...
2. **The heating hand is built, composed, and called only from outside — CORRECTED 2026-08-13, the
   ...
   collapsed **distinction**. That organ is `crates/holonic-life/src/decomposing_codec.rs`, whose stated
...
`crates/holonic-life/src/incidence_production.rs:1843` implements the Information Chemistry law
...
`SourceDispositionRow::{standing, continued(carrier), ended}` (`crates/holonic-abi/src/holon.rs:585`) and
...
that reopens.** `crates/holonic-life/src/decomposing_codec.rs` already re-cuts at that word, and
...
`docs/plans/THE_ROADMAP.md` plan 1 is *"The material mouth — the purified corpus as one declared body
...
| kind | finding |
|---|---|
| **unused dependencies** | `crates/holonic-life` declares four it never uses: `rug` (arbitrary-precision float), `ab_glyph`, `aho-corasick`, and **`soma-surface`** — a whole wgpu crate linked and never invoked |
...
`grep -c "rug\|ab_glyph\|aho-corasick\|soma-surface" crates/holonic-life/Cargo.toml` returns **0** — the

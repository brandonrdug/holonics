...
facets and exact operators now live at the crate root, with structural carriers under
`structure` and receiver-relative geometry under `geometry`. HNN application and wire identifiers retain `hna`; the
...
| `holonics::membrane` | Live receiving, concrete standing storage, constituent and current runtime; `holonics-portable` supplies shared `no_std` carriers and section arithmetic/ABI, while `soma-abi` retains host/device wire records. |
...
remains. Main's `structure` and `geometry` modules now own their source. Advanced
...
| Normalized vector-current transport and learning differential | `holonics::ratio::exponentiated::NormalizedKernel::{apply,differential,pullback,fit_step}` | Exact log-rational kernel chart; binary sigmoid and complete value/potential returns. `connected_holonic_field` composes it into the reference field that drives the synopsis. It is exterior CPU execution. |

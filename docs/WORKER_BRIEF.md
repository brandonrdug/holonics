...
| An exact run that takes minutes is rebased, not waited on (roadmap rule 12) | report the method, certificate and actual consuming placement; use the [exact algebra contract](plans/THE_EXACT_DEVICE_LAW_IS_CONSTRUCTED.md#the-shared-exact-algebra-consumer), including kernel independence and reconstruction; a CPU reference does not impose a CPU-first production order |

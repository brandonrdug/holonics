...
fn extend_charts(charts: &mut Vec<PrimeChart>, wanted: usize) -> Result<(), PrimeImageRefusal> {
    ...
    while charts.len() < wanted {
        ...
        if crate::primality::is_prime(candidate) {
...
/// **The hoisted CRT lift, run over the first `charts` descending charts, for one cross-owner
/// check.**
///
/// [definition] Returns the chart moduli, the lifted residues, their joint modulus and the
/// window reconstruction of `values`. It exists so `holonic_engine`'s arithmetic-fibre pair law
/// (`chinese_remainder_pair`) can be held against the private accumulator this module lifts with;
/// it is not a reading API.
#[doc(hidden)]
pub fn hoisted_lift_probe(
    values: &[u64],
    charts: usize,
) -> Result<(Vec<u64>, Vec<BigInt>, BigInt, Option<Vec<Rat>>), PrimeImageRefusal> {
    let mut declared = Vec::with_capacity(charts);
    extend_charts(&mut declared, charts)?;
    let mut accumulator = CrtAccumulator::new(values.len());
    let mut moduli = Vec::with_capacity(declared.len());
    ...
    let reconstructed = accumulator.reconstruct();
    Ok((
        moduli,

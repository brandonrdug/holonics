...
//! plus the deposition work `½⟨x⁺, (Q⁺ − Q) x⁺⟩` (`Holon/Deposition.lean::commit_balance`,
//! `Holon/Deposition.lean::deposition_work`), realized by [`crate::law::ReferenceHolon::commit`]. The
//! learned power is the effort form of the rate form, `⟨x, (AᵀQ + QA) x⟩ = 2⟨Qx, L Qx⟩` for
//! `A = LQ` (`Holon/Deposition.lean::learned_rate_form`).
//!
//! [definition] **Passivity-preserving deposition** (`Holon/Deposition.lean::committed_energy_bound`,
//! from `Holon/Deposition.lean::energy_product_bound`): passive words and deposits
...
//! (`Foundation/Standing.lean` retention). The divergence witness
//! (`Holon/Deposition.lean::normal_law_divergence_witness`, `Holon/Deposition.lean::indefiniteBlock`,
//! `Holon/Deposition.lean::divergentState`) grows `×9` per commit.
//!
//! [definition; agent-inferred] **The passive projection, exactly over ℚ.** The Lean
//! `Holon/Deposition.lean::projectPassive` clips the positive eigenvalues of `sym L`
//! (`Holon/Deposition.lean::clipNeg`, spectral theorem over ℝ); those eigenvalues are in general
...
//! (`Holon/Deposition.lean::projectPassive_passive`) and `L' = L` for passive `L`
//! (`Holon/Deposition.lean::projectPassive_of_passive`) — so the bound is restored exactly as in
//! `Holon/Deposition.lean::projected_committed_energy_bound`. It removes a positive semidefinite
...
//! | `symPart`, `quad_symPart` | [`crate::scalar::symmetric_part`] |
...
use crate::scalar::{dot, form_matrix, matrix, matrix_form, symmetric_part};

/// Both sides of `Holon/Deposition.lean::learned_rate_form`: `⟨x, ((LQ)ᵀQ + Q(LQ)) x⟩` and
/// `2⟨Qx, L Qx⟩`.
...
/// [definition] **The deposit ledger** (`Holon/Deposition.lean::committed_energy_bound`): the
...
/// `diag(1, −1)`, the indefinite normal-law block (`Holon/Deposition.lean::indefiniteBlock`).
...
/// `(3ⁿ, 0)` (`Holon/Deposition.lean::divergentState`).
...
mod tests {
    ...
    /// `Holon/Deposition.lean::projectPassive_passive`, `Holon/Deposition.lean::projectPassive_of_passive`.
    #[test]
    fn the_projection_is_passive_and_fixes_passive_relations() {
        ...
        assert_eq!(
            ...
            ExactRatMatrix::from_diagonal(vec![int(0), int(-1)]).unwrap()
        );
    }

    /// `Holon/Deposition.lean::learned_rate_form` on the witness: the rate form `2W` is indefinite.
    #[test]
    ...
    fn a_deposit_beyond_its_declared_growth_is_refused() {
        ...
        assert!(DepositLedger::certify_deposit(&q, &grown, &int(1)).is_ok());
        assert!(matches!(
            DepositLedger::certify_deposit(&q, &grown, &rat(1, 2)),
            Err(HolonError::DepositExceedsBound { .. })
        ));
        assert_eq!(
            DepositLedger::certify_deposit(&q, &q, &int(-2)),

...
//! grain reading and the combined read of the count face and the wave through `R P_R^(τ_R)`
//! (Decision 27).
...
use crate::hnn::masses::{ClassMasses, CountFace, MassStep, Regions};
...
/// The read is `f = k(r)/L_R + R · P_R^(τ_R) v_R` (Decision 27, the combined face): the wave rotated
/// to the receiving ring's phase, plus the count face's grain logits at the window's region; each
/// class's real logit read at the grain, its imaginary logit (the wave's alone) halved into turns.
/// The count logits lie on the grain, so each cell is the wave's shifted by `k_c/L_R` exactly and
/// keeps the wave's fibre.
#[test]
fn the_read_rotates_the_anchor_to_the_receivers_phase() {
    let field = &chain();
    let mut medium = Medium::encoding(field, 12);
    // Masses a few reached comparisons deposited at the region of the preceding cell 2.
    let mut masses = ClassMasses::prior(Regions::PrecedingCell, field.alphabet());
    for class in [1usize, 1, 3, 0, 1] {
        masses
            .deposit(&MassStep {
                ring: 2,
                region: 3,
    ...
    medium.masses[2] = Some(masses.clone());
    ...
    .unwrap();
    let count = CountFace::read(&masses, 3, phases.grain()).unwrap();
    ...
    assert!(
        phases
            .read(field, &medium, &current, &anchor[..2], &count)
            .is_err()
    );
    // A count face of another grain is refused.
    let coarse = CountFace::read(&masses, 3, 8).unwrap();
    assert!(
        phases
            .read(field, &medium, &current, &anchor, &coarse)
            .is_err()
    );
}

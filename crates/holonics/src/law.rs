...
//!
//! [definition] The five statements (`Holon/Law.lean`): **advance** — one implicit-midpoint step
//! has zero balance residual (`Holon/Law.lean::advance_law`, from
//! `Holon/Element.lean::midpoint_balance`), and backward Euler carries the defect
//! `−½⟨Δq, QΔq⟩` (`Holon/Element.lean::backwardEuler_balance`); **receive** — the passive coholon
//! reads an effort by a linear functional with zero power (`Holon/Law.lean::passive_reading`), and
//! an active receiver through an interface conductance `G` satisfies `P_H + P_R = −D_Σ`,
//! `D_Σ = ⟨Δ, GΔ⟩ ≥ 0` for passive `G` (`Holon/Law.lean::active_receiver_law`); **interact** — a
//! Holon of Holons is a Holon (`Holon/Law.lean::PortHolon.interconnect`); **restrict** — the
//! pushforward (`Holon/Restriction.lean::pushforwardD_isDirac`), scale square and Kron reduction;
//! **pullback** — efforts move by the transpose and power is preserved
//! (`Holon/Law.lean::pullback_law`). The continuous balance is
//! `Holon/Element.lean::PortHolon.energy_balance`; with learning and deposition it is
//! `Holon/Deposition.lean::learned_energy_balance` and, per commit,
//! `Holon/Deposition.lean::commit_balance`.
//!
...
//! balance's residual is computed, not assumed. The five power readings of the survey
//! (`TellegenReceipt`, `StorageRateReading`, `ClockedEnergy`, `PowerStations`,
//! `DiffusionEnergyBalance`) become views of [`EnergyBalance`] in plan phase 3 integration.
//!
//! | Lean | Rust |
//! |---|---|
...
use crate::scalar::{at, dot, form_matrix, matrix, neg, quad, rat, sub};
...
/// [definition] **A passive coholon's reading** (`Holon/Law.lean::passive_reading`): the linear
...
pub trait HolonLaw {
    ...
    /// power: its bond is `(0, Qx)` (`Holon/Law.lean::passive_reading`).
    fn receive(
        &self,
        state: &HolonState,
    ...
    /// (`Holon/Restriction.lean::pushforwardD_isDirac`).
...
impl ReferenceHolon {
    ...
    /// (`Holon/Deposition.lean::commit_balance`): the returned balance is the word's plus the
    ...
    pub fn commit(
        ...
        let mut flow_constant = crate::scalar::zeros(n);
        let mut effort_constant = crate::scalar::zeros(n);
        let qx = q.apply(x)?;
        for i in 0..sigma {
        ...
        let target = neg(&crate::scalar::add(
        ...
        let flow = crate::scalar::add(&flow_coefficient.apply(&z)?, &flow_constant);
        let effort = crate::scalar::add(&effort_coefficient.apply(&z)?, &effort_constant);
...
/// [definition] **The active receiver interface** (`Holon/Law.lean::active_receiver_law`).
#[derive(Clone, Debug, PartialEq, Eq)]

...
impl SymmetricCirculant {
    ...
    /// a family of directions has no intrinsic order and its indexing is a reading convention — can
...
impl StarTable {
    pub fn found(extent: usize) -> Result<Self, WindingError> {
        ...
        for (step, interval) in isolated.iter_mut().enumerate() {
            let Some(value) = rational_star_value(step, extent) else {
                continue;
            };
...
///
/// Returns `None` when the exact value is irrational; no approximation or interval is produced.
///
...
pub fn rational_star_value(step: usize, extent: usize) -> Option<Rat> {

//! **Navigators `G`: transport with an initial configuration, its own clock and phase lift.**
//!
...
//! (`jumps_are_carries`, over `Geometry/PhaseCarry.winding_add`, `carry_cocycle`, `carry_le_one`);
//! a rational clock passage jumps losslessly (`clockPassage_jumps_lossless`).
//!
//! [definition; agent-inferred] Exact charts. The phase is a Cayley half-angle chart
//! ([`crate::geometry::screw::RationalPhase`], a point of the rational unit circle) and the winding
//! a `BigInt`; no real angle is formed. The clock's ticks run on an odometer
//! ([`crate::geometry::winding::Odometer`]) whose overflow is the counted jump population. The
//! transport is the flow `ẋ = A x + b` ([`Transport::Linear`], [`Transport::Affine`], or a screw
//! `ω × x + v` whose Lie generator is a [`crate::geometry::screw::ScrewGenerator`]); one clock tick
//! is its Cayley (implicit-midpoint) step `(I − hA/2) x⁺ = (I + hA/2) x + h b`, the exact rational
...
use crate::geometry::winding::{Odometer, carry, winding};
use crate::ratio::Rat;
...
/// [definition] **The transport of a navigator** as the flow `ẋ = A x + b`.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum Transport {
...
/// [definition] **A navigator**: transport, initial configuration, clock and phase lift.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// One navigator advance: the configuration reached and the jumps the clock counted. No
/// intermediate configuration is retained.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct NavigatorAdvance {
...
impl Navigator {
    ...
    /// A screw navigator: the Lie generator of a situated screw with its initial point.
    pub fn screw(
...
mod tests {
    ...
    use crate::ratio::linear::vector::{dot, ints};
    ...
    fn jumps_are_carries_and_compose_as_a_cocycle() {
        ...
        for a in 0u32..7 {
            for b in 0u32..7 {
                ...
                let mut clock = Clock::ring(integer(1), 5).unwrap();
    ...
    /// (`Geometry/PhaseCarry.value_digits`) and the elapsed duration is `h · ticks`.
    #[test]
    ...
    fn a_rotation_screw_ticks_losslessly_and_keeps_its_key() {
        ...
        let mut navigator =
            Navigator::screw(&screw, Clock::ring(rat(1, 3), 4).unwrap(), lift()).unwrap();
        assert_eq!(navigator.situated_screw(), Some(screw));
        let start = navigator.initial().to_vec();
        let reached = navigator.advance(&start, 9).unwrap();
        assert_eq!(
            dot(&reached.configuration, &reached.configuration),
            dot(&start, &start)
        );
        assert_eq!(reached.jumps, BigUint::from(2u32));
        assert_eq!(navigator.lift().winding(), &BigInt::from(2));
        // A different key is a different orbit under the same navigator.
        let other = navigator.tick(&ints(&[1, 0, 0])).unwrap();

...
//! (`Holon/Reaction.bilinear_reaction_workless_iff_zero`, from
//! `Holon/Reaction.eq_zero_of_herm_zero`). The power-neutral reaction is instead
...
//! `c` (`Holon/Reaction.skewReaction_workless`, `Holon/Reaction.re_herm_skew`). The
...
//!    (`Holon/Reaction.skewPart`, the Frobenius-orthogonal projection,
//!    `Holon/Reaction.skewPart_orthogonal`), rounded on the strict upper triangle and
...
//!    it fixes an already skew-Hermitian matrix (`Holon/Reaction.skewPart_of_skew`).
//! 2. [`passive_complex_at_grain`]: the certified congruence clip of
//!    [`crate::holon::deposition::project_passive`] (`Holon/Deposition.projectPassiveCongruence_passive`)
...
use crate::holon::deposition::project_passive;
use crate::ratio::linear::ExactRatMatrix;
use crate::ratio::linear::inertia::{Inertia, inertia};
use crate::ratio::linear::vector::{matrix, symmetric_part};
...
/// (`Holon/Reaction.realify`, up to the coordinate order).
...
pub fn passive_complex_at_grain(
    ...
    let r = crate::ratio::linear::vector::form_matrix(&clip.removed);
...
/// can remove a part far larger than the block, leaving a large dissipative block that the explicit incident word amplifies.
pub fn passive_shift_at_grain(
    ...
    let passive_at = |tau: &BigInt| -> Result<bool, HolonError> {
        let shifted = crate::ratio::linear::vector::form_matrix(&symmetric)
            .subtract(&ExactRatMatrix::identity(2 * n)?.scaled(&Rat::from_integer(tau.clone())))?;
        Ok(inertia(&crate::ratio::linear::vector::matrix_form(&shifted)?).positive == 0)
    };
...
mod tests {
    ...
    /// `Holon/Reaction.skewReaction_workless` at the grain: every modulated sum of exactly
    ...
    /// `Holon/Reaction.skewPart_of_skew`: a skew-Hermitian slice is fixed; the removed part
    ...
    /// `Holon/Deposition.projectPassiveCongruence_passive` / `_of_passive` in the complex

...
//! ([`crate::holon::restriction`]). [`Transition::residual`] is what
//! [`crate::holon::restriction::Descent`] retains when a square or a factoring does not descend.
//!
//! Lean `Foundation/ContinuingTower`; every public item below cites the declaration it realizes.
//!
...
//! `num_bigint::BigInt` or `crate::ratio::Rat`; comparison is `Eq` on those exact carriers.
//!
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ChartMissing { chart } => {
                write!(
                    formatter,
                    "the section carries no witness at chart {chart:?}"
                )
            }
...
///
/// Lean counterpart: the two fields of `Foundation/ContinuingTower.Tower`. Lean states them
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Tower.CompatibleSection.compatible`.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Transition.reopen_apply`.
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Tower`. Lean's `Face : Index → Type v` is a
...
pub type TowerOutcome<T, V> = Result<V, TowerRefusal<<T as Tower>::Index, <T as Tower>::Face>>;
...
pub(crate) type ReopenOutcome<T> = Result<
    ReopenReceipt,
    ReopenRefusal<
...
/// `Foundation/ContinuingTower.Tower`, which Lean carries as proofs. Here they are asked and
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Tower.CompatibleSection`. Lean's section is
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Tower.ObservationFibre`, and — by
/// `Tower.observationFibre_eq_preimageFibre`, which is `rfl` — also
/// `Foundation/Holon.Holon.PreimageFibre` of the restrict-to-chart receiver. The two are one
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Tower.MaterializedFace`. A face without its
...
///
/// Lean counterpart: `Foundation/ContinuingTower.shiftTower_obstructed` proves emptiness
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Tower.GluingResult`, whose totality is
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Tower.gluingResult_total`. Lean's trichotomy
...
///
/// Lean counterpart: `Foundation/ContinuingTower.ComputableTower`. Lean's
...
pub trait ComputableTower: Tower {
    ...
    fn materialize(&self, state: &Self::State, chart: &Self::Index) -> TowerFaceOutcome<Self>;
...
/// compatible section.
///
/// Lean counterpart: `Foundation/ContinuingTower.ComputableTower.section`, where
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Transition`. [`Transition::apply`] is what
...
pub trait Transition {
    ...
    fn check_reopen(&self, source: &Self::Source) -> ReopenOutcome<Self> {
    ...
    ///
    /// Lean counterpart: `Foundation/ContinuingTower.Transition.laterReceiverFactors` — the
    ...
    /// still separate them.
    ///
    /// Lean counterpart: `Foundation/ContinuingTower.Transition.residual_separates` and
...
///
/// Lean counterpart: the `PUnit` residual of `Foundation/ContinuingTower.Transition.ofEquiv`,
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Transition.comp`, and the equation
...
///
/// Lean counterpart: `Foundation/ContinuingTower.coarseGrain`. The composition law is
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Transition.ofEquiv` instantiated at an
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Tower.restrictTransition`, where the
...
pub trait TowerRestrictTransition: Tower {
    ...
    /// face exactly.
    ///
    /// Lean counterpart: `Foundation/ContinuingTower.Tower.section_witness_reopened`.
...
/// `Foundation/ContinuingTower.padicFibreEquiv` and `padicFibre_card`, which Lean discharges
...
pub enum SplittingRefusal {
    ...
    /// *before* any carrier of that width is allocated.
    CarrierAboveCeiling {
        /// The coarser chart.
        coarse: u32,
        /// The finer chart.
...
///
/// Lean counterpart: `Foundation/ContinuingTower.padicFibreEquiv`, the bijection between the
...
///
/// Lean counterpart: `Foundation/ContinuingTower.padicTower`, whose plurality is
...
/// and `Tower::restrict` returns the tower's face, exactly as `Foundation/ContinuingTower`
...
impl ResidueTower {
    ...
    pub(crate) fn modulus_bits(&self, level: u32) -> u128 {
    ...
    ///
    /// Lean counterpart: `Foundation/ContinuingTower.padicFibreEquiv` together with
...
///
/// Lean counterpart: `Foundation/ContinuingTower.shiftTower`, with `shiftTower_obstructed`
...
///
/// Lean counterpart: `Foundation/ContinuingTower.unitTower`, whose verdict is
...
impl Tower for UnitTower {
    ...
    fn restrict(&self, coarse: &u32, fine: &u32, _face: &()) -> Result<(), TowerRefusal<u32, ()>> {
        if !self.refines(coarse, fine) {
            return Err(TowerRefusal::NotARefinement {
                coarse: *coarse,
                fine: *fine,
...
/// `Foundation/ContinuingTower.ResidualMigration.square_either_route_reopens`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration` carries `index_mono` and
...
pub(crate) type MigrationSourceFace<M> = <<M as Migration>::Source as Tower>::Face;
...
pub(crate) type MigrationTargetFace<M> = <<M as Migration>::Target as Tower>::Face;

/// A migration's own typed refusal, in its own four index and face types.
pub(crate) type MigrationRefusalOf<M> = MigrationRefusal<
...
/// `Foundation/ContinuingTower.Migration`, which Lean carries as a proof for every refinement
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration`. [`Transition`] is the
...
/// `Foundation/ContinuingTower.Migration`. `faces` supplies `(fine target chart, source face
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration.carrySection`, whose compatibility
...
/// not transport.
///
/// Lean counterpart: `Foundation/ContinuingTower.ResidualMigration`. The residual is
...
/// face exactly.
///
/// Lean counterpart: `Foundation/ContinuingTower.ResidualMigration.square_either_route_reopens`
    ...
    M::Target: TowerRestrictTransition,
{
    ...
    let source_residual = source
        ...
        .map_err(MigrationRefusal::Source)?;
    let reopened_restricted = migration.reopen(coarse, &transported, &migration_residual_coarse)?;
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration.identity`, and — because its face
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration.comp`, with `comp_assoc`,
...
///
/// Lean counterpart: `Foundation/ContinuingTower.padicHalfMigration`, whose naturality is
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration.StaysAtItsChart`,
...
impl<TI> IndexRouteReceipt<TI> {
    ...
    pub fn follows_refinement_everywhere(&self) -> bool {
        self.routes.iter().all(|(_, route)| {
            matches!(route, ChartRoute::SameChart | ChartRoute::FollowsRefinement)
        })
    }
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration.FactorsThroughRefinement`, whose
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration.FactorsThroughRefinement` and
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration.ReversePassage`, together with
...
///
/// Lean counterpart: `Foundation/ContinuingTower.TwoCharts` with its discrete `Preorder`, and
...
///
/// Lean counterpart: `Foundation/ContinuingTower.twoChartTower`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
...
///
/// Lean counterpart: `Foundation/ContinuingTower.swapMigration`, with

...
//!
//! [definition] A restriction `π` ([`Transition`], `Foundation/ContinuingTower.Transition`)
//! transports [`Transition::apply`] and retains [`Transition::residual`], and the residual is
//! exactly the dropped part (`Foundation/ContinuingTower.Transition.reopen_apply`). Two
...
//! `Transport/ContinuingTube.grainSquare_defect_is_not_a_loss`); a factor break keeps the
//! two sources `π` merges, the residuals that still separate them
//! (`Foundation/ContinuingTower.Transition.residual_separates`) and the two readings, and
//! the factor defect retains every fibre `π` merges beside those separators
//! ([`crate::holon::restriction::fibre::FibreDefect`]). The
//! matrix defect `π A_fine − A_coarse π` of [`crate::holon::restriction::SquareDefect`]
//! (`Holon/Restriction.squareDefect`) is the operator of which every square break is one
//! column reading; the equality is tested on [`crate::holon::restriction::LinearRestriction`].
//!
//! [proved-derived; formal-checked] The Lean owner is `Holon/Restriction.Descent`, total by
//! `Holon/Restriction.descent_total` (every reading factors through a restriction or
//! exhibits a merged pair it separates); the pointwise square is the operator defect read at a
//! state (`Holon/Restriction.squareDefect_mulVec_eq_zero_iff`), propagating to every horizon
//! by `Holon/Restriction.scale_square_pow`. The factoring at a standing is
...
impl<S, T, R> SquareBreak<S, T, R> {
    ...
    /// the break is not a loss (`Foundation/ContinuingTower.Transition.reopen_apply`).
...
/// (`Holon/Restriction.scale_square_pow` at horizon one; `squareDefect` pointwise).
...
/// tower supplies (`Foundation/ContinuingTower.Tower.restrictTransition`: a tower's
...
/// (`Holon/Restriction.Descent.defect`).
...
/// `π x = π y`, `ρ x = ρ y` (`Foundation/Standing.standingLaw_exists_iff_future_factors`).

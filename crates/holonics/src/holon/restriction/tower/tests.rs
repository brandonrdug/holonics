...
use super::{
    ...
    ComputableTower, EnumerableTower, ExactShift, GluingResult, HalvingMigration,
    IdentityMigration, MaterializedFace, Migration, MigrationOutcome, MigrationRefusal, NoResidual,
    ObservationFibre, ResidualMigration, ResidueTower, ReversePassageReceipt, ShiftTower,
    SwapMigration, Tower, TowerRefusal, TowerRestrictTransition, Transition, TwoChartTower,
    TwoCharts, UnitTower, carry_section, check_factors_through_restriction, check_index_routes,
    check_migration_naturality, check_migration_square_reopen, check_restriction_laws,
    check_reverse_passage, computable_section, glue_chain,
};
...
fn a_broken_restrict_refl_is_a_typed_refusal() {
    ...
    assert!(matches!(refusal, TowerRefusal::RestrictReflFailed { .. }));
}
...
fn an_incompatible_witness_family_is_refused() {
    ...
    assert!(matches!(refusal, TowerRefusal::IncompatibleWitness { .. }));
}
...
fn shift_tower_is_obstructed_at_a_named_chart() {
    ...
    let verdict = glue_chain(&ShiftTower, &[0u32, 1, 2, 3], BigUint::zero())
        .expect("the shift tower is lawful");
    match verdict {
    ...
    let verdict =
        glue_chain(&ShiftTower, &[0u32, 1, 2, 3], nat(2)).expect("the shift tower is lawful");
    match verdict {
        GluingResult::Obstructed(obstruction) => {
...
fn a_later_receiver_is_reopened_from_the_residual() {
    ...
    for value in 0u32..40 {
        ...
        let recovered =
            grain.reopen_later_receiver(&grain.apply(&source), &grain.residual(&source), &finer);
...
fn the_splitting_agrees_with_the_enumerated_adjacent_preimage() {
    ...
    assert_eq!(splitting.enumerate(1_000).expect("25 cosets fit"), preimage);
}
...
fn the_halving_migration_carries_the_continuing_object_unchanged() {
    ...
    let state = nat(20);
    let saved =
        computable_section(&tower, &state, &[0u32, 2, 4]).expect("20 materializes lawfully");
    ...
    let direct =
        computable_section(&tower, &state, &[0u32, 1, 2]).expect("20 materializes lawfully");
...
fn composed_migrations_carry_sections_functorially() {
    ...
    let state = nat(5_000);
    let saved =
        computable_section(&tower, &state, &[0u32, 4, 8]).expect("it materializes lawfully");
...
fn the_residual_is_supplied_and_not_derived_from_the_face_map() {
    ...
    for value in 0u32..40 {
        ...
        exact
            .check_reopen(&source)
            .expect("the exact residual reopens");
        tautological
            .check_reopen(&source)
...
fn a_migration_can_connect_charts_the_tower_relates_in_no_way() {
    ...
    let faces: Vec<(TwoCharts, BigUint)> = (0u32..4)
        .flat_map(|value| {
            [
                (TwoCharts::Left, nat(value)),
                (TwoCharts::Right, nat(value)),
            ]
        })
        .collect();
    let receipt =

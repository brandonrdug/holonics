...
//! ([`crate::holon::restriction`]): the two-axis commuting square of a stationed tower, its circuit
//! holonomy and the wormhole receipt.
//!
//! [definition] **One square defect.** The per-face [`SquareDefect`] returned here is the
//! pointwise chart of the operator [`crate::holon::restriction::SquareDefect`] `π A_fine − A_coarse π`:
//! at a linear tube ([`crate::holon::restriction::LinearTube`]) the difference of its two routes at a
...
//! core [`crate::holon::restriction::Descent`].
//!
//! # The paired Lean owner
//!
//! Lean `Transport/ContinuingTube`; every public item below cites the declaration it realizes.
//!
...
//! table: `Foundation/ContinuingTower.Migration.ConnectsIncomparableCharts` (the condition
//! `Wormhole.crosses` carries).
//!
//! # What a tube is here
//!
//! A tube has two axes. The **transverse** axis is the [`crate::holon::restriction::tower::Tower`]
...
use crate::holon::restriction::Descent;

use crate::holon::restriction::tower::{
...
/// copy a face whole, so the second
...
pub(crate) const DECLARED_WORK_CEILING: usize = 1 << 22;

/// The declared work of a check, or `None` when the product overflows.
pub(crate) fn declared_work(factors: &[usize]) -> Option<usize> {
...
pub(crate) fn total_face_population<T: StationedTower>(
    tube: &T,
...
///
/// Lean counterpart: `Transport/ContinuingTube.Tube` carries `transport` as a field typed by
...
/// along it.
///
/// Lean counterpart: `Transport/ContinuingTube.Tube`. [`Self::section`] is Lean's `station`
...
pub trait StationedTower {
    ...
    /// the caller counted them in: a selection that clones a whole face map costs that map's
    /// population, and a declaration of "one face" says nothing about it. [`check_commuting_square`] and [`check_circuit_holonomy`] sum this over the
...
pub(crate) type TubeStation<T> = <T as StationedTower>::Station;
/// The chart index of a tube's transverse sections.
pub(crate) type TubeIndex<T> = <<T as StationedTower>::Section as Tower>::Index;
/// The face type of a tube's transverse sections.
pub(crate) type TubeFace<T> = <<T as StationedTower>::Section as Tower>::Face;
/// A tube's own typed refusal, in its own station, index and face types.
pub(crate) type TubeRefusalOf<T> = TubeRefusal<TubeStation<T>, TubeIndex<T>, TubeFace<T>>;
...
pub(crate) type TubeSquareVerdict<T> = SquareVerdict<TubeStation<T>, TubeIndex<T>, TubeFace<T>>;
/// A tube's own [`HolonomyVerdict`], in its own station, index and face types.
pub(crate) type TubeHolonomyVerdict<T> = HolonomyVerdict<TubeStation<T>, TubeIndex<T>, TubeFace<T>>;
...
///
/// Lean counterpart: `Transport/ContinuingTube.SquareVerdict`, with `squareVerdict_total`
...
impl<S, I, F> SquareVerdict<S, I, F> {
    ...
    /// defect retaining both routes (`Transport/ContinuingTube.squareVerdict_total`).
...
impl<S, I> SquareDefect<S, I, Vec<Rat>> {
    ...
    /// [`crate::holon::restriction::SquareDefect::at`] at the source face
    /// (`Holon/Restriction.squareDefect_mulVec_eq_zero_iff`; tested on
    /// [`crate::holon::restriction::LinearTube`]).
    pub fn route_difference(&self) -> Vec<Rat> {
        crate::ratio::linear::vector::sub(
            &self.transported_then_restricted,
            &self.restricted_then_transported,
        )
    }
...
///
/// Lean counterpart: `Transport/ContinuingTube.Tube.square`, which is
...
///
/// Lean counterpart: `Transport/ContinuingTube.tube_circuit_has_no_defect` for the
...
///
/// Lean counterpart: `Transport/ContinuingTube.Wormhole`, whose `crosses` field is
...
///
/// Lean counterpart: `Foundation/ContinuingTower.Migration.ConnectsIncomparableCharts` and
/// `Migration.not_factorsThroughRefinement_of_connectsIncomparableCharts`, which
/// `Transport/ContinuingTube.Wormhole.not_factorsThroughRefinement` applies. The
...
/// `tower::check_index_routes` rather than reclassifying the routes.
...
///
/// Lean counterpart: `Transport/ContinuingTube.constantTube`, and `padicTube` when the
/// section is `tower::ResidueTower`. It is the tube a tower already is when nothing
...
///
/// Lean counterpart: `Transport/ContinuingTube.flipCircuit`, with `flipCircuit_hasDefect` and
...
///
/// Lean counterpart: `Transport/ContinuingTube.lossySwapMigration`, with

...
//! (`Holon/Port.Bond`, `Holon/Port.flow`, `Holon/Port.effort`); its power is
//! `⟨e, f⟩` (`Holon/Port.power`) and the symmetric bond pairing
//! `⟨⟨(f₁,e₁),(f₂,e₂)⟩⟩ = ⟨e₁,f₂⟩ + ⟨e₂,f₁⟩` is `Holon/Port.bondForm`, symmetric
//! (`Holon/Port.bondForm_symm`), with `⟨⟨b,b⟩⟩ = 2·power b`
//! (`Holon/Port.bondForm_self`) and nondegenerate
//! (`Holon/Port.bondForm_nondegenerate`).
//!
//! [definition; agent-inferred] **Units.** [`Dimension`] is a typed exponent vector over named base
//! dimensions, and [`PortUnits`] checks `power = flow · effort` at construction.
//!
//! | Lean | Rust |
//! |---|---|
...
impl Bond {
    ...
    pub fn zero(ports: usize) -> Self {
        Self {
            flow: crate::ratio::linear::vector::zeros(ports),
            effort: crate::ratio::linear::vector::zeros(ports),
        }
    }
    ...
    /// [definition] The power `⟨e, f⟩` (`Holon/Port.power`).
    pub fn power(&self) -> Rat {
        dot(&self.effort, &self.flow)
    }

    /// [definition] The bond pairing `⟨e₁,f₂⟩ + ⟨e₂,f₁⟩` (`Holon/Port.bondForm`).
...
mod tests {
    ...
    use crate::ratio::linear::vector::ints;
    ...
    fn the_pairing_is_symmetric_and_reads_twice_the_power() {
        ...
        assert_eq!(b.pairing(&b).unwrap(), integer(2) * b.power());
        assert_eq!(b.power(), integer(1));
    }

    /// `Holon/Port.bondForm_separating`: a bond pairing to zero against every unit bond is
    ...
    fn a_nonzero_bond_is_seen_by_a_unit_bond() {
        ...
        assert_eq!(b.pairing(&unit_effort).unwrap(), integer(4));
    }

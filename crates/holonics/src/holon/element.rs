...
//! (`Holon/Element.Ports`, `Holon/Element.PortHolon`). Quadratic storage is
//! `E = ½⟨x, Qx⟩` (`Holon/Element.storageEnergy`) with effort `Qx` and flow `−ẋ`; resistance
...
//! `Q(τ)` on a clock, whose change is deposition work (`Holon/Element.PortHolon.energy_balance`,
//! `Holon/Conformance.lc_pump_work`).
//!
//! [definition] **Passivity is certified, never assumed** (`Holon/Element.PortHolon.passive`):
...
//! `⟨e, L e⟩ ≤ 0`, decided as `n₊(sym L) = 0` (`Holon/Deposition.quad_symPart`). The passive
//! projection of an active relation is [`crate::holon::deposition::project_passive`].
//!
//! | Lean | Rust |
//! |---|---|
...
/// `½⟨x, (Q' − Q) x⟩` (`Holon/Deposition.deposition_work`).
...
mod tests {
    ...
    fn an_active_resistance_is_refused_with_its_inertia() {
        ...
        assert_eq!(r.dissipation(&ints(&[1, 1])).unwrap(), integer(2));
    ...
    /// (`Holon/Conformance.lc_pump_witness`).
    #[test]

...
//! (`Holon/Restriction.pushforward`); it preserves power, `⟨e', P f⟩ = ⟨Pᵀ e', f⟩`
//! (`Holon/Restriction.power_pushforward`, the pullback law `Holon/Law.pullback_law`),
//! and a morphism lands the pushed structure inside the target
//! (`Holon/Restriction.IsMorphism`), composing (`Holon/Restriction.IsMorphism.comp`).
//! The pushforward of a Dirac structure along any port map is Dirac
//! (`Holon/Restriction.pushforwardD_isDirac`), computed by
//! [`crate::holon::dirac::DiracStructure::pushforward`].
//!
//! [definition] **The scale square** `π A_fine = A_coarse π` propagates to every horizon
//! (`Holon/Restriction.scale_square_pow`); a failed square is retained as its typed defect
//! `π A_fine − A_coarse π` (`Holon/Restriction.squareDefect`; witnesses
//! `Holon/Restriction.shift_has_no_coarse_generator`,
//! `Holon/Restriction.diagonal_square`). **Kron/Schur elimination** of a network's interior is
...
//! boundary value extends (`Holon/Restriction.kron_exact`,
//! `Holon/Restriction.boundaryBond`; witness `Holon/Restriction.series_path_restriction`).
//!
//! | Lean | Rust |
//! |---|---|
...
//!
//! [definition] **Restriction is one owner.** The transverse axis — towers, gluing, the
//! non-invertible [`tower::Transition`] with its residual, migrations — is [`tower`] (Lean
//! `Foundation/ContinuingTower`); the longitudinal axis — the stationed tower, its two-axis
//! commuting square, circuit holonomy and the wormhole receipt — is [`tube`] (Lean
//! `Transport/ContinuingTube`). [`Descent`] reads every restriction square or factoring as a
//! witness or a typed defect built on [`tower::Transition::residual`]; the tube's per-face
//! `tube::SquareDefect` is the pointwise chart of the operator [`SquareDefect`] here
//! ([`LinearTube`], `tube::SquareDefect::route_difference`). A Schur boundary transfer is a
//! [`KronReduction`] reading.
//!
//! [definition] **A defect retains its fibre and its separator.** A factor defect is a
//! [`FibreDefect`]: every fibre `π` merges ([`PreimageFibre`]) and every separated merged pair
//! ([`Separation`], the Lean `Descent.defect`). A linear preimage is an [`AffineFibre`], and a
//! pair merged now and separated by a later word is a [`ShortestSeparator`].
...
pub use fibre::{
    AffineFibre, AffineFibreDefect, FibreDefect, PreimageFibre, Separation, ShortestSeparator,
};
...
impl PortMap {
    ...
    /// `e = Pᵀ e'`: the learning covector travels on the effort side (`Holon/Law.pullback_law`).
    pub fn pull_effort(&self, effort: &[Rat]) -> Result<Vec<Rat>, HolonError> {
        Ok(self.map.transpose()?.apply(effort)?)
    }

    /// Both sides of `Holon/Restriction.power_pushforward`: `(⟨e', P f⟩, ⟨Pᵀ e', f⟩)`.
    ...
    /// The pushed Dirac structure (`Holon/Restriction.pushforwardD`).
    pub fn pushforward(&self, structure: &DiracStructure) -> Result<DiracStructure, HolonError> {
        structure.pushforward(&self.map)
    }

    /// `Holon/Restriction.IsMorphism`: the pushforward lies in `target`. Both are Dirac and
    ...
    /// `P' ∘ P` (`Holon/Restriction.IsMorphism.comp`).
...
/// Whether `π A_fineⁿ = A_coarseⁿ π` (`Holon/Restriction.scale_square_pow`).
...
impl KronReduction {
    ...
    /// (`Holon/Restriction.boundaryBond`).
    ...
    pub fn extend(&self, boundary_values: &[Rat]) -> Result<Vec<Rat>, HolonError> {
        ...
        let mut state = crate::ratio::linear::vector::zeros(self.network.rows());
    ...
    /// The three statements of `Holon/Restriction.kron_exact` at one interior-balanced state:
...
mod tests {
    ...
    /// `Holon/Restriction.shift_has_no_coarse_generator`, `Holon/Restriction.diagonal_square`.
    #[test]
    ...
    /// `Holon/Restriction.series_path_restriction` and `Holon/Restriction.kron_exact`.
    #[test]
    ...
    /// on the Lean witnesses `Holon/Restriction.shift_has_no_coarse_generator` and
    /// `Holon/Restriction.diagonal_square`.
    #[test]
    ...
    fn the_square_descent_retains_the_fine_motion_and_equals_the_operator_defect() {
        ...
        for piece in breaks.breaks() {
            let difference = crate::ratio::linear::vector::sub(
                piece.fine_then_restricted(),
                piece.restricted_then_coarse(),
            );
    ...
    fn a_merging_restriction_reopens_and_a_reading_factors_or_breaks() {
        ...
        assert_eq!(breaks.fibres()[0].members, vec![0, 1, 3]);

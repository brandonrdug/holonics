...
//! when a division comes out inexact. Nothing is evaluated at a point
...
mod root_separation;
pub(crate) use root_separation::{RootSeparation, RootSeparationBound};
...
impl RationalPolynomial {
    ...
    pub(crate) fn from_integers(coefficients: &[BigInt]) -> Self {
        Self::new(
            coefficients
                .iter()
                .cloned()
    ...
    pub(crate) fn from_integer_polynomial(polynomial: &IntegerPolynomial) -> Self {
    ...
    pub(crate) fn times_with_work(&self, other: &Self) -> (Self, ExactWork) {
    ...
    pub(crate) fn divided_by_with_work(
        &self,
        divisor: &Self,
    ...
    /// `Foundation/RootCount.theSquarefreePartSharesItsRoots`, and **it is conditional**: it
    ...
    pub(crate) fn primitive_integer_form(&self) -> Result<IntegerPolynomial, ExactPolynomialError> {
        if self.is_zero() {
            return Err(ExactPolynomialError::ZeroPolynomial);
        }
    ...
    /// `Foundation/RootCount`'s `theReadingIsInvariantUnderCommonNonzeroRescaling`. It is
    ...
    pub(crate) fn signed_primitive_integer_form(
        &self,
...
pub(crate) fn euclidean_resultant(
...
/// wrong.
pub(crate) fn integer_discriminant(
...
pub(crate) const ROOT_BOUND_DOUBLING_CEILING: u32 = 4096;
...
pub(crate) const HALF_INTEGER_DESCENT_CEILING: u32 = 4096;
...
pub(crate) fn taylor_shifted(coefficients: &[BigInt], shift: &BigInt) -> Vec<BigInt> {
...
pub(crate) fn certified_real_root_enclosure(
    polynomial: &IntegerPolynomial,
...
pub(crate) fn interior_split_schedule(degree: usize) -> Vec<Rat> {
...
pub(crate) fn worst_retained_fraction(schedule: &[Rat]) -> Result<Rat, ExactPolynomialError> {
...
pub(crate) fn squared_shrinking_steps(
    initial_squared: &Rat,
    retained: &Rat,
    target_squared: &Rat,
...
/// three-hundred-digit rationals per root.
///
...
/// built over it, and every exact evaluation in the half-integer descent is sized by it, so it is
/// checked before the companion is formed.
///
...
pub(crate) const MONIC_COMPANION_BIT_CEILING: u64 = 1 << 12;
...
fn descend_half_integer(
    ...
    let mut pending: Vec<(Rat, Rat, u32, u32)> = vec![(lower.clone(), upper.clone(), count, 0)];
...
pub(crate) const SEPARATION_SPLITTING_DEPTH_CEILING: u64 = 1 << 17;
...
/// [`crate::ratio::algebraic::SturmChain::distinct_root_count`] requires.
pub(crate) fn distinct_real_root_count(
    polynomial: &RationalPolynomial,
) -> Result<u32, ExactPolynomialError> {
...
pub(crate) fn real_root_count_with_multiplicity(
    polynomial: &RationalPolynomial,

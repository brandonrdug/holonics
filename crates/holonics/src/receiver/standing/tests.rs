...
//!
//! Every theorem of `Foundation/Standing` appears here under the name the module's
...
use crate::receiver::face::{DiameterNorm, ExactFace, width_over_readings};
...
fn medium_generators() -> NavigatorFamily {
    ...
    .expect("a declared navigator family")
}
...
fn a_quotient_is_a_standing_exactly_when_the_future_factors_through_it() {
    let navigators = medium_generators();
    ...
    let verdict =
        sufficiency(&standing, &population, &navigators, &observations).expect("the check runs");
    assert_eq!(
        verdict,
    ...
    let refuted = sufficiency(&standing, &population, &navigators, &state_observations)
...
fn sufficiency_is_the_core_factor_descent_through_the_standing() {
    ...
    let verdict = sufficiency(&standing, &population, &navigators, &medium).expect("runs");
    let descent = sufficiency_descent(&standing, &population, &navigators, &medium).expect("runs");
    ...
    let verdict = sufficiency(&standing, &population, &navigators, &state_future).expect("runs");
    let descent =
        sufficiency_descent(&standing, &population, &navigators, &state_future).expect("runs");
...
fn one_present_face_two_standings_separated_later() {
    let exchange = NavigatorFamily::declared(
        "exchange",
...
// ---------------------------------------------------------------------------------------------
// 2. Memory is a navigator, not a retrieval
// ---------------------------------------------------------------------------------------------
...
/// its own, which is the zero changing-receiver defect of `Transport/ChangingReceiver`.
#[test]
...
fn fossil_generators() -> NavigatorFamily {
    NavigatorFamily::declared(
...
fn the_wave_chart_is_extinct_at_horizon_three() {
    let navigators = fossil_generators();
...
fn the_fossil_is_durable() {
    ...
    for word in navigators.words_within(5).expect("the admitted words") {
        let advanced = navigators
...
fn a_search_without_a_certificate_returns_not_decided_rather_than_extinct() {
    ...
    match verdict {
        ExtinctionVerdict::NotDecidedWithinBound {
            ...
            assert_eq!(
                words, 4,
                "one navigator and a horizon of 3 names four words"
            );
            assert_eq!(receiver, "medium");
...
fn rotation_generators() -> NavigatorFamily {
    NavigatorFamily::declared(
...
fn the_rotation_conserves_the_declared_energy_exactly() {
    let navigators = rotation_generators();
    ...
    for word in navigators.words_within(8).expect("the admitted words") {
        let now = navigators
...
fn a_certificate_whose_chart_is_not_invariant_is_refused() {
    ...
    let leaking = NavigatorFamily::declared(
        "a leaking chart",
        vec!["settle".to_owned()],
        vec![matrix(&[
            &[(1, 2), (0, 1), (1, 1)],
...
/// **The standing's reading is the passive coholon** (`Holon/Law.passive_reading`).

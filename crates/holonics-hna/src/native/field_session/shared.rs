...
use super::*;
use holonic_engine::native_ecology::constitutive_fibre::{ResidentHeldSection, ResidentNormalInput};
...
impl FieldSessionSpec {
    pub(super) fn shared_extents(&self) -> Result<(usize,usize,usize)> {
        if self.region_offsets.is_empty() || !self.region_offsets.contains(&0) {
            return Err(invalid("shared regions require explicit incidence offsets including zero"));
        }
        let mut unique=self.region_offsets.clone();unique.sort_unstable();unique.dedup();
        if unique.len()!=self.region_offsets.len(){return Err(invalid("duplicate region incidence offset"));}
        let width=self.symbols.len().checked_add(2).and_then(|n|n.checked_mul(unique.len()))
            .ok_or_else(||invalid("local region extent"))?;
        let nodes=width.checked_add(2).ok_or_else(||invalid("local field extent"))?/3;
        let features=nodes.checked_mul(6).and_then(|n|n.checked_add(1)).ok_or_else(||invalid("shared feature extent"))?;
        Ok((nodes,1,features))
    }
}
...
impl<'c> NativeFieldSession<'c> {
    fn prepare_shared(&self, request:&FieldSectionRequest)->Result<SharedPreparation<'c>> {
        ...
        let partial=match &request.partial {
            Some(parts)=>parts.iter().map(|p|p.as_ref().map(|s|self.chart.alphabet().symbol_of(s)
                .ok_or_else(||invalid("partial region must name one declared codec symbol"))).transpose()).collect::<Result<Vec<_>>>()?,
            None=>self.spec.symbols_of(&self.chart,&request.text)?.into_iter().map(Some).collect(),
        };
        let output_symbols=request.output_symbols.unwrap_or(partial.len());
        if output_symbols==0 || output_symbols>self.spec.section_symbols || partial.len()>self.spec.section_symbols {
            return Err(invalid("source/receiver exceeds its declared section aperture"));
        }
        let context=request.context.iter().map(|s|self.spec.symbols_of(&self.chart,s)).collect::<Result<Vec<_>>>()?
            .into_iter().flatten().collect::<Vec<_>>();
        if context.len()>self.spec.context_symbols{return Err(invalid("context exceeds its declared source aperture"));}
        ...
        let center_slot=self.spec.region_offsets.iter().position(|x|*x==0).unwrap();
        let center_start=2*center_slot*(symbols+2);
        let center=center_start..center_start+2*symbols;
        let global_width=output_symbols.checked_mul(2).and_then(|n|n.checked_mul(symbols)).ok_or_else(||invalid("receiving extent"))?;
        let mut given=vec![(0i64,0i64);global_width];
        let mut held=vec![false;global_width/2];
        let mut destinations=Vec::new();
        for i in 0..output_symbols {
            if let Some(Some(symbol))=partial.get(i){given[2*(i*symbols+self.chart.coordinates()[symbol.0 as usize])]=(1,1);}
            let fixed=request.partial.is_some() && partial.get(i).is_some_and(Option::is_some);
            held[i*symbols..(i+1)*symbols].fill(fixed);
            if !fixed{destinations.push(i);}
        }
        let mut rows=vec![(0i64,0i64);local.checked_mul(destinations.len()).ok_or_else(||invalid("region packet extent"))?];
        for (row,destination) in destinations.iter().enumerate(){
            ...
            for (slot,offset) in self.spec.region_offsets.iter().enumerate(){
                ...
                if let Some(symbol)=observed{rows[start+2*self.chart.coordinates()[symbol.0 as usize]]=(1,1);}
                let a=i64::from(active);let o=i64::from(observed.is_some());
                rows[start+2*symbols]=(a,a);rows[start+2*(symbols+1)]=(o,o);
            }
        }
        let mount=|rows,width,values|self.surface.mount_section_rest(&ResidentSectionRest::found(rows,width,ResidentGrain(0),64,values).map_err(invalid)?).map_err(invalid);
        let given=mount(1,global_width,given)?;
        let (input,conditions)=if destinations.is_empty(){(None,None)}else{
            let count=destinations.len();
            (Some(mount(count,local,rows)?),Some(mount(count,3,(0..count).flat_map(|_|[(1,1),(0,0),(1,1)]).collect())?))
        };
        Ok(SharedPreparation{input,conditions,given,held,destinations,output_symbols,center})
    }
    ...
    pub(super) fn shared_request(&mut self,request:&FieldSectionRequest)->Result<Value>{
        ...
        let received=if let (Some(input),Some(conditions))=(&prepared.input,&prepared.conditions){
            let output=self.body.preview_field_rows(ResidentConstitutiveSection::integers(input)?,ResidentConstitutiveSection::rationals(conditions)?)?;
            let full=output.scatter_components(prepared.center.clone(),&prepared.destinations,prepared.output_symbols)?;
        ...
        let selections=received.view().read_basis_sections(&chart,prepared.output_symbols)?.selections()?;
        let symbols=selections.iter().map(|r|self.spec.symbols[r.selected].clone()).collect::<Vec<_>>();
        let (text,bytes,decode_error)=match self.spec.codec {
            FieldTextCodec::Utf8Nibbles=>{
                let codes=selections.iter().map(|r|r.selected as u8).collect::<Vec<_>>();
                if codes.len()%2!=0 {(None,None,Some("odd nibble receiving extent".to_owned()))}else{
                    let bytes=codes.chunks_exact(2).map(|p|(p[0]<<4)|p[1]).collect::<Vec<_>>();
    ...
    pub fn observe_source(&mut self,request:&FieldSectionRequest,text:&str,step_bits:u32)->Result<Value>{
        ...
        if prepared.destinations.is_empty(){return Ok(json!({"scope":"shared-field-source-observation","rows":0,"parameter_update":"zero: all receiving positions supplied","held_disagreements":held_disagreements,"anatomy":self.inspect()}));}
        let local=6*self.spec.shared_extents()?.0;
        let mut values=vec![(0i64,0i64);local*prepared.destinations.len()];
        for (row,i) in prepared.destinations.iter().enumerate(){
            values[row*local+prepared.center.start+2*self.chart.coordinates()[target[*i].0 as usize]]=(1,1);
        }
        let target=self.surface.mount_section_rest(&ResidentSectionRest::found(prepared.destinations.len(),local,ResidentGrain(0),64,values).map_err(invalid)?).map_err(invalid)?;
        let mut mask=vec![true;local/2];mask[prepared.center.start/2..prepared.center.end/2].fill(false);
        let returned=self.body.observe_field_rows(
            ...
            ResidentConstitutiveSection::integers(&target)?,&mask,step_bits)?;
        Ok(json!({"scope":"shared-field-source-observation","returned":returned,"held_disagreements":held_disagreements,"elapsed_us":start.elapsed().as_micros(),"anatomy":self.inspect()}))
    }
}
...
mod tests {
    ...
    fn spec()->FieldSessionSpec {
        FieldSessionSpec{symbols:(0..16).map(|n|format!("{n:x}")).collect(),section_symbols:4096,context_symbols:4096,
            region_offsets:vec![-1,0,1],source_chart:FieldSourceChart::SharedRegions,
            codec:FieldTextCodec::Utf8Nibbles,fractional_bits:48}
    }
    fn partial(text:&str,hidden:usize)->FieldSectionRequest {
        let mut parts=text.as_bytes().iter().flat_map(|b|[Some(format!("{:x}",b>>4)),Some(format!("{:x}",b&15))]).collect::<Vec<_>>();
        parts[2*hidden]=None;parts[2*hidden+1]=None;
        FieldSectionRequest{text:String::new(),partial:Some(parts),output_symbols:None,context:vec![],commit:false,retain_comparison:false}
    }
    #[test]
    fn coefficient_shape_is_independent_of_the_complete_source_aperture(){
        let mut s=spec();let small=s.extents().unwrap();s.section_symbols=1_000_000;s.context_symbols=2_000_000;
        assert_eq!(s.extents().unwrap(),small);assert_eq!(small,(18,1,109));
        s.region_offsets=vec![-1,1];assert!(s.chart().is_err());
        s.region_offsets=vec![0,0];assert!(s.chart().is_err());
    }
    #[test]
    ...
    fn shared_source_updates_and_reopens_without_a_position_sized_model(){
        let dir=tempfile::tempdir().unwrap();let path=dir.path().join("shared.session");
    ...
    fn completely_observed_source_reports_a_conflict_without_deposition(){
        with_field_session(&spec(),|s|{
            let request=FieldSectionRequest{text:String::new(),partial:Some(vec![Some("6".into()),Some("1".into())]),
                output_symbols:None,context:vec![],commit:false,retain_comparison:false};
            let before=s.inspect_current()?;
            ...
            assert_eq!(s.inspect_current()?,before);Ok(())
        }).unwrap();
    }
}

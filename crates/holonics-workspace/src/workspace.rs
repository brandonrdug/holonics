...
use holonics_hna::{addressed_ingress, AthenaAlphaApplication};
use holonic_engine::{
...
impl VariantWorkspace {
    ...
    pub fn conduct(
        ...
        let mut next = self.manifest.clone();
        let ordinal = next.allocate()?;
        let base = format!(".local/runs/run-{ordinal}");
    ...
    pub fn stage_world_return(
        ...
        let candidate_reference = artifact(
            ...
            format!(".local/runs/run-{}/candidate.json", run.ordinal),
        );
        let path = self
            .store
    ...
    pub fn commit(&mut self) -> Result<WorkspaceReturn<NativeMorphologyCommit>, ApplicationError> {
        ...
        let commit_reference = artifact(
            ArtifactKind::Commit,
            format!(".local/runs/run-{}/commit.json", run.ordinal),
        );
        let completed_reference = artifact(
            ArtifactKind::Run,
            format!(".local/runs/run-{}/completed.json", run.ordinal),
        );
    ...
    pub fn decline(&mut self) -> Result<WorkspaceReturn<NativeDeclineReceipt>, ApplicationError> {
        ...
        let decline_reference = artifact(
            ...
            format!(".local/runs/run-{}/decline.json", run.ordinal),
        );
        let completed_reference = artifact(
            ArtifactKind::Run,
            format!(".local/runs/run-{}/completed.json", run.ordinal),

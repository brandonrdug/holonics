...
//! | `deposit` | the successor's loci (the predecessor copied on the card, the moved words scattered; the receiving parametron's bound harmonic coordinate `h_R` among them, Decision 26), the arrived re-read | the normal laws' steps (the receiving map's exogenous step with its statistic `B`, the internal loci's prox steps), the harmonic step, the budgeted carry, the budget, the successor's operators, the code length | the moved words (16 octets each) and a word |

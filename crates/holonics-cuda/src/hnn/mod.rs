...
//! | `deposit` | the successor's loci (the predecessor copied on the card, the moved words scattered), the arrived re-read | the normal laws' prox steps, the class masses' deposit (Decision 27), the budgeted carry, the budget, the successor's operators, the code length | the moved words (16 octets each) and a word |

...
//! | `refine` | the counts frozen at the cut ([`MomentSnapshot`]); the charts' refinement (`store`: warm, cold and target phases, every Newton–Schulz step and certificate); the word (`execute`: the open `E_g M_g[c]` and the pair port, every tick with its splits and remainders, the receiving read of the wave `R P_R^(τ_R) v_R`) | the operators `I − ½K`, `m_a` and each step's decision from its certificate (a cold start's transpose and the exact fallback formed on the host, `n²` words); the wave's faces in `ℚ(θ)`; each tick's balance (its terms pass the 128-bit carrier) and the release, read exactly from the record | the plan and weights (≈ 5 kB), the moved operators and charts, 32 octets per certificate, the record and logits (≈ 55 kB) |
//! | `compare` | the word's return (`hnn_word_reverse`): every element's, transit's and junction's reverse step with their carried remainders | the landmark tree's face at each phase's causal address (Decision 28), read at the grain from the host's constitution and added to the card's logits (its comparison of `q^L` with `2^k` passes the 128-bit carrier, and no kernel reads the tree; timed as `WallTimes::tree_read`); the Holon ratio and its covector; the covector's pull through `Rᵀ` and its first split (it lives on `(1/W)ℤ`, not a lattice); the conductance's division by each ring's admittance sum (not dyadic); the composition onto the loci (`reference::compose`) | the carried reads (`8·A·2d_R` octets), the return's record (≈ 25 kB) |
//! | `deposit` | the successor's loci (the predecessor copied on the card, the moved words scattered), the arrived re-read | the normal laws' prox steps, the landmark tree's deposit (Decision 28), the budgeted carry, the budget, the successor's operators, the code length | the moved words (16 octets each) and a word |

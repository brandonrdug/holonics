...
//!
//! `crates/holonics-portable/src/arrow.rs` carries `Arrow { reach, aim, cross }` — the span, the cohere, the
//! gyration — with the note *"The whole arrow, never one scalar."* That organ is **two-dimensional**:
//! `Place` is a pair, so its `cross` is the scalar signed area, and `crates/holonics-portable` is `no_std` with zero
...
/// **WHICH WAY THE ARROW AIMS** — lifted unchanged from `crates/holonics-portable/src/arrow.rs:36-39`.
///
...
/// **THE CAUSAL CLASS** — lifted from `crates/holonics-portable/src/arrow.rs:69-75`, whose own note is the reason
...
mod tests {
    ...
    fn the_two_dimensional_even_part_is_the_rotor() {
        ...
        for (ar, ai, br, bi) in [(2i64, 0, 3, 0), (1, 0, 0, 1), (4, -1, 2, 5), (0, 0, 3, 7)] {
            ...
            let product = left.product(&right).expect("same signature");
            // `crates/holonics-portable/src/arrow.rs:119-120`: aim = ar*br + ai*bi, cross = ai*br - ar*bi.

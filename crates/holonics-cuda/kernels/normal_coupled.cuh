...
extern "C" __global__ void section_normal_source_plane(
    const int64_t *source,const int64_t *hi,uint32_t at,uint32_t den,uint32_t status,uint32_t n,uint32_t receiver,
    int64_t *out,int64_t *out_hi,uint32_t *slot,const uint32_t *census,const uint32_t *lineage,uint32_t lineage_count){
    if(blockIdx.x||threadIdx.x)return;if(upstream_refused(census,lineage,lineage_count,slot))return;
    wide denominator=fibre_current_denominator(source,hi,den,status,slot);if(*slot)return;
    if(!n||n>UINT32_MAX/6u||receiver>1u){atomicOr(slot,REFUSED_MALFORMED);return;}
    uint32_t r=2u*n;
    ...
    if(receiver){
        wide current=0,previous=0;
        for(uint32_t i=0;i<n;++i){
            current=add_checked(current,source[at+r+2u*i],slot);
            previous=add_checked(previous,source[at+2u*r+2u*i],slot);
        }
        if(*slot)return;
        if(current!=denominator||previous!=denominator){atomicOr(slot,REFUSED_MALFORMED);return;}
    }
    out[0]=out_hi[0]=1;
}

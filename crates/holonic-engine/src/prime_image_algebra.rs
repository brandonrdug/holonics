...
//!
//! # Why this owner exists, measured
//!
//! [established-bounded; source-inspected] [`crate::exact_linear::ExactRatMatrix::reduced_row_echelon`]
//! is textbook Gauss–Jordan over `Rat = BigRational` on one thread. Issue #50 records what that
//! costs on real RBX1 material: 72/120/180 coordinates in 7/168/568 s, 324 coordinates without a
//! return after 39 minutes. The growth is in the **coefficients**, not in the shape — a rational
...
//! [`IntegralPresentation`] and each owns its own output. That independence is declared to
//! [`crate::hardware_cover`] as a front of one cell per image and **checked** by
//! [`crate::hardware_cover::CoverDecomposition::independence`], not asserted.
//!
...
//! `m_i` that differs per row — so the operator is not shared — and is **bilinear in two data
//! words** (`m_i` and `a_kj` are both read from the field), where the generated apply is linear in
//! one. A modular elimination therefore cannot be inferred from the presence of a generated
...
//!
//! `mount::ModularWords` is nevertheless the ring this module computes in, and at
//! [`mount::ModularWords::DEVICE`]'s own modulus `2^61 − 1` its `add` and `mul` *are* the device's
//! — the same code `accelerators/cuda-kernel` compiles for nvptx. [`PrimeChart::device`] is that
//! chart, and it is the first chart every reconstruction takes, so the leading image of every
//! return is already computed in the card's own arithmetic.
//!
//! # Prior art this composes rather than re-founds
//!
//! ```text
//! [source-audit 2026-09-20 27c72825] prior-art 'modular rank|rational reconstruction|prime image|fraction.free|CRT|Chinese remainder'
//!   over formal/elementary-holonics/ElementaryHolonics crates docs research/records -> 73 files, 0 record titles
//! [source-audit 2026-09-20 27c72825] prior-art 'prime image algebra|prime_image|PrimeImage|modular image chart'
//!   over formal/elementary-holonics/ElementaryHolonics crates docs research/records -> 0 files, 0 record titles
//! [source-audit 2026-09-20 27c72825] prior-art 'Miller.Rabin|miller_rabin|primality|is_probable_prime|strong probable prime'
//!   over formal/elementary-holonics/ElementaryHolonics crates docs research/records -> 61 files, 0 record titles
//! ```
//!
//! The name `prime_image_algebra` is new (zero hits). The **material** is not, and none of it is
//! re-founded here:
//!
//! - **Rational reconstruction** is `crate::rational_polynomial::rational_reconstruction` — Wang's
//!   bounded form, already used by that owner's multi-modular polynomial lift. This module calls it.
//! - **Chinese remainder** is `crate::arithmetic_fiber::chinese_remainder_pair`, which owns the law
//!   and its Bézout receipt. This module's bulk lift is that law with the Bézout coefficient hoisted
//!   out of the per-entry loop (one modular inverse per round instead of one extended gcd per entry),
//!   and [`self::tests`] holds the two against each other on every fixture rather than on trust.
//! - **Primality** is `crate::topological_receiver::is_prime` — deterministic Miller–Rabin over
...
use mount::{ExactRing, ModularWords};

use crate::exact_linear::{ExactLinearError, ExactRatMatrix};
use crate::hardware_cover::{ChartId, CoverDecomposition, FrontCell, HardwareCover};

pub const PRIME_IMAGE_SCHEMA: &str = "holonic-engine.prime-image-algebra.v1";
...
pub const DECLARED_IMAGE_EXTENT_CEILING: usize = 1 << 26;
...
pub const DECLARED_PRIME_CEILING: usize = 1024;
...
pub const DECLARED_PRIME_FLOOR: u64 = 1 << 32;
...
pub const DECLARED_PRIME_MODULUS_CEILING: u64 = 1 << 62;
...
pub enum PrimeImageRefusal {
    ...
    #[error("a row scale of a cleared presentation is zero; a scale is invertible or it is not a scale")]
    ZeroRowScale,
    #[error("the exact linear carrier refused: {0}")]
    Carrier(#[from] ExactLinearError),
    #[error("the hardware cover refused the declared image front: {0}")]
    CoverBarrier(String),
}

fn bounded_words(what: &'static str, rows: usize, columns: usize) -> Result<usize, PrimeImageRefusal> {
...
pub struct IntegralPresentation {
    rows: usize,
    columns: usize,
    entries: Vec<BigInt>,
...
impl IntegralPresentation {
    ...
    pub fn of(matrix: &ExactRatMatrix) -> Result<Self, PrimeImageRefusal> {
    ...
    /// `B[row][column]`.
    pub fn entry(&self, row: usize, column: usize) -> Result<&BigInt, PrimeImageRefusal> {
        self.entries
    ...
    pub fn row_scale(&self, row: usize) -> Result<&Rat, PrimeImageRefusal> {
        self.row_scales
            .get(row)
            .ok_or(PrimeImageRefusal::ShapeDisagrees {
    ...
    pub fn decoded_row(&self, row: usize) -> Result<Vec<Rat>, PrimeImageRefusal> {
    ...
    pub fn scaled_target(&self, target: &[Rat]) -> Result<Vec<Rat>, PrimeImageRefusal> {
    ...
    /// Whether every cleared entry fits one machine word.
...
///
/// [definition] The modulus is decided prime by `crate::topological_receiver::is_prime` —
...
impl PrimeChart {
    ...
    pub fn declared(modulus: u64) -> Result<Self, PrimeImageRefusal> {
        ...
        if !crate::topological_receiver::is_prime(modulus) {
...
fn extend_charts(charts: &mut Vec<PrimeChart>, wanted: usize) -> Result<(), PrimeImageRefusal> {
    ...
    while charts.len() < wanted {
        ...
        if crate::topological_receiver::is_prime(candidate) {
...
///
/// [definition] The fields are public for the same reason [`crate::exact_work::ExactWork`]'s are:
...
///
/// No field here is a duration. `docs/canon/TABLET_THE_CHART.md` and
/// [`crate::exact_work::ExactWork`] hold the same law: *a cost is measured in work, never in
/// elapsed time. A clock may measure; it may never select.* A wall clock reading belongs in a
/// verification receipt beside the command that produced it, never inside the value.
...
/// **Where the independent images were placed, observable in the returned reading.**
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
pub struct ImagePlacement {
    /// One entry per occupied chart of the hardware cover, with how many images it carried.
    pub sections: Vec<(String, usize)>,
    /// The lanes actually used — never more than the chart declares about itself.
    pub lanes: usize,
    /// The mode identity the cover assigned to this decomposition.
    pub mode: String,
    /// Whether the cover's device chart carried any image. **False today, and the reason is
    /// named in [`crate::section_layout_adoption`]**: the generated tile boundary applies one
    /// shared linear local operator, and a pivot sweep is neither shared nor linear in one word.
    pub device_carried: bool,
}
...
impl KernelCertificate {
    ...
    pub fn minor(&self) -> (u64, &[usize], u64) {
        (self.minor_modulus, &self.minor_rows, self.minor_determinant)
    }

    /// The chart moduli whose images were lifted into this return.
    pub fn image_moduli(&self) -> &[u64] {
        &self.image_moduli
    }

    /// The charts computed and then refused, with the reason. Retained, never dropped.
    pub fn refused_moduli(&self) -> &[(u64, BadChart)] {
        &self.refused_moduli
    }

    pub fn cost(&self) -> &ImageCost {
        &self.cost
    }
    ...
    pub fn reduced_rows(&self) -> Vec<Vec<Rat>> {
    ...
    pub fn verify(&self, matrix: &ExactRatMatrix) -> Result<(), PrimeImageRefusal> {
        ...
        let mut spent = 0u64;
        verify_minor(&presentation, &chart, &self.minor_rows, &self.pivot_columns, &mut spent)?;
        verify_kernel(
            &presentation,
            self.rank,
            &self.pivot_columns,
...
///
/// The images are placed on the cpu chart of a cover this owner declares for itself. A caller
/// holding a device declaration passes it to [`certified_kernel_over`] so the placement is read
/// off the cover it actually holds.
pub fn certified_kernel(matrix: &ExactRatMatrix) -> Result<KernelCertificate, PrimeImageRefusal> {
    certified_kernel_over(matrix, &HardwareCover::cpu_only())
}

/// The certified reading, with the cover the caller holds.
pub fn certified_kernel_over(
    ...
    if columns == 0 || rows == 0 {
        let placement = declare_placement(cover, 0)?;
        return Ok(KernelCertificate {
            schema: PRIME_IMAGE_SCHEMA.to_owned(),
            rows,
            columns,
            rank: 0,
    ...
    let needed = usize::try_from(minor_bits.saturating_mul(2) / 60 + 4).unwrap_or(DECLARED_PRIME_CEILING);
    ...
    let lanes = cover
        .cpu()
        .resident_lanes()
        .min(u64::try_from(budget).unwrap_or(1))
        .max(1) as usize;
    ...
    while consumed < budget {
        ...
        // **Independent images**: each reads the same immutable presentation and owns its own
        // output. The decomposition is declared to the hardware cover and its independence is
        // **checked before the batch runs**, which is what licenses running them at once.
        declare_placement(cover, batch.len())?;
        let images = reduce_batch(&presentation, batch, lanes);
        consumed = batch_end;
        cost.images_reduced += images.len();

        for image in images {
            cost.image_multiplications = cost.image_multiplications.saturating_add(image.multiplications);
        ...
        let placement = declare_placement(cover, image_moduli.len())?;
        return Ok(KernelCertificate {
            schema: PRIME_IMAGE_SCHEMA.to_owned(),
            rows,
            columns,
...
pub fn certified_fibre(
    matrix: &ExactRatMatrix,
    target: &[Rat],
) -> Result<(Option<(Vec<Rat>, Vec<Vec<Rat>>)>, KernelCertificate), PrimeImageRefusal> {
    certified_fibre_over(matrix, target, &HardwareCover::cpu_only())
}

/// The certified fibre, with the cover the caller holds.
pub fn certified_fibre_over(
    matrix: &ExactRatMatrix,
    target: &[Rat],
    cover: &HardwareCover,
) -> Result<(Option<(Vec<Rat>, Vec<Vec<Rat>>)>, KernelCertificate), PrimeImageRefusal> {
    ...
    let certificate = certified_kernel_over(&augmented, cover)?;
...
/// Declare the image front to the hardware cover and read the placement back off it.
fn declare_placement(
    cover: &HardwareCover,
    images: usize,
) -> Result<ImagePlacement, PrimeImageRefusal> {
    let front: Vec<FrontCell> = (0..images)
        .map(|index| FrontCell {
            index,
            extent: 1,
        })
    ...
    let decomposition = CoverDecomposition::on_chart(
        cover,
        &front,
        "prime_image_algebra::certified_kernel",
        ChartId::Cpu,
    )
    .map_err(|barrier| PrimeImageRefusal::CoverBarrier(barrier.to_string()))?;
    decomposition
        .independence(&front)
        .map_err(|barriers| PrimeImageRefusal::CoverBarrier(format!("{barriers:?}")))?;
    let sections = decomposition
        .sections
        .iter()
        .map(|section| (section.chart.to_string(), section.cells.len()))
        .collect();
    let device_carried = decomposition
        .sections
        .iter()
        .any(|section| !matches!(section.chart, ChartId::Cpu) && !section.cells.is_empty());
    Ok(ImagePlacement {
        sections,
        lanes: cover.cpu().resident_lanes().min(images.max(1) as u64) as usize,
        mode: format!(
            "{}|{}|{}",
            decomposition.mode.kernel, decomposition.mode.arithmetic, decomposition.mode.apparatus
        ),
        device_carried,
    })
}
...
///
/// [definition] This is `crate::arithmetic_fiber::chinese_remainder_pair`'s law with the Bézout
/// coefficient hoisted: the pair law computes an extended gcd per combination, and every entry in
/// one round shares the same modulus pair, so the inverse of `M mod p` is computed once per round
/// rather than once per entry. `tests::the_hoisted_lift_agrees_with_the_pair_law` holds the two
/// against each other rather than asserting the identity.
#[derive(Clone, Debug)]
...
impl CrtAccumulator {
    ...
    fn absorb(&mut self, image: &[u64], modulus: u64) {
        ...
        for (at, residue) in self.residues.iter_mut().enumerate() {
            ...
            let difference = chart
                .add(wanted, modulus - current % modulus)
                .unwrap_or(0);
    ...
    fn reconstruct(&self) -> Option<Vec<Rat>> {
        ...
        for residue in &self.residues {
            let (numerator, denominator) = crate::rational_polynomial::rational_reconstruction(
                residue,
                &self.modulus,
                &bound,
                &bound,
...
// ===============================================================================================
// 8. the reading a consumer sees
// ===============================================================================================

/// A short account of one certified reading, for a receipt.
pub fn account(certificate: &KernelCertificate) -> BTreeMap<&'static str, String> {
    let cost = certificate.cost();
    BTreeMap::from([
        ("rank", certificate.rank().to_string()),
        ("nullity", certificate.kernel().len().to_string()),
        ("charts-lifted", certificate.image_moduli().len().to_string()),
        ("charts-refused", certificate.refused_moduli().len().to_string()),
        ("prepared-entries", cost.prepared_entries.to_string()),
        ("charts-decided", cost.charts_decided.to_string()),
        (
            "image-multiplications",
            cost.image_multiplications.to_string(),
        ),
        ("lifted-entries", cost.lifted_entries.to_string()),
        (
            "reconstruction-attempts",
            cost.reconstruction_attempts.to_string(),
        ),
        (
            "reconstructed-entries",
            cost.reconstructed_entries.to_string(),
        ),
        (
            "verification-multiplications",
            cost.verification_multiplications.to_string(),
        ),
        ("readout-entries", cost.readout_entries.to_string()),
        ("dependency-span", cost.dependency_span.to_string()),
        (
            "device-carried",
            certificate.placement().device_carried.to_string(),
        ),
    ])
}

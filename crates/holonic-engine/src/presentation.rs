...
/// Exact candidate-cover tile used only to avoid visiting empty terminal
/// cells while an implicit primitive is being located. The tile is a work
/// decomposition, not a sampling or a geometry approximation.
...
pub use terminal_surface::{
    CoordinateFieldContribution, DepthPhaseContribution, ExactCell, ExactCellSupport,
    ExactSurfaceCell, ExactSurfacePresentation, ExecutedPresentation, IntegralPhaseCell,
    OcclusionFace, OcclusionObstruction, PresentationAddress, PresentationBoundary,
    PresentationMatrix, PresentationMember, PresentationWork, PresentedPrimitiveKey,
    PrimitiveContribution, QuadraticPhaseContribution, SurfaceContribution, SurfaceCoverage,
    SurfaceDepth, SurfaceMemberId, TerminalMatrixSpec, classify_presentation_address,
    classify_receiver_address, exact_surface_presentation,
    exact_surface_presentation_with_executor,
};

...
/// The alpha carbons of the RBX1 window, exactly, through the mmCIF intake owner.
///
/// `physical_intake::mmcif` is the owner of an mmCIF presentation and of the statement that a
/// deposited decimal is a rational with a last place; nothing is re-parsed here.
fn rbx1_window(path: &Path) -> Result<Vec<Vec<Rat>>, String> {
    ...
    if window.len() != WINDOW {
        return Err(format!(
            "{} carries {} residues in its RBX1 chain, fewer than the declared window of {WINDOW}",
            path.display(),
            window.len()
        ));
    }
...
/// **The measured return on the authenticated M5 release: the exact Hodge reading.**
///
/// This is the cheap half — rational row reduction and integer Smith normal form, no
/// characteristic polynomial — and it runs by default. Absent the release it refuses. The laws
/// themselves are checked above on synthetic complexes that need no fixture.
#[test]
fn the_hodge_receiver_measures_the_m5_structures() {
    ...
    for (lineage, file) in M5_STRUCTURES {
        ...
        let founded = contact_complex(&window, true);
        let operator = HodgeOperator::found(lineage, &founded.complex, &unit(), &BoundaryCondition::Free)
            .expect("the operator founds");

        let readings = hodge_readings(&operator).expect("the readings return");
        for reading in &readings {
            assert_eq!(reading.harmonic_dimension, reading.betti);
            assert_eq!(
                reading.exact_dimension + reading.coexact_dimension + reading.harmonic_dimension,
                reading.cells
            );
        }
        assert_eq!(readings[0].harmonic_dimension, 1, "the window is one connected component");

        // The Hodge decomposition of a declared 1-cochain, with its three receipts.
        let ones = vec![Rat::one(); operator.extent(1)];
        let decomposition = hodge_decomposition(&operator, 1, &ones).expect("the decomposition");
        assert!(decomposition.pairings.iter().all(Zero::is_zero));

        let blocks = super::decoupled_blocks(&operator.laplacian(0).expect("the Laplacian"))
            .expect("the blocks return");

        println!(
            "hodge_receiver M5 | {lineage} | occurrences {} | one-cells {} | two-cells {} | \
             b0 {} | b1 {} | b2 {} | exact/coexact/harmonic at grade 1 {}/{}/{} | \
             decoupled blocks of Delta_0 {} | harmonic support at grade 0 {} | \
             harmonic participation at grade 0 {:?}",
            founded.vertices.len(),
            founded.edges.len(),
            founded.faces.len(),
            readings[0].betti,
            readings.get(1).map(|r| r.betti).unwrap_or(0),
            readings.get(2).map(|r| r.betti).unwrap_or(0),
            readings[1].exact_dimension,
            readings[1].coexact_dimension,
            readings[1].harmonic_dimension,
            blocks.len(),
            readings[0].harmonic_localization.support.len(),
            readings[0]
...
/// **The heavy measurement: the exact grade-zero spectrum of the M5 contact Laplacians.**
///
/// Marked and ignored by default: it is a measurement, not a law. The characteristic polynomial of a
/// `24 × 24` integer Laplacian is cheap and the Sturm isolation of its twenty-four roots is the
/// rest of the cost. Measured 2026-09-18 in release on the declared workstation, against the same
/// reading taken before `exact_value::SturmChain` replaced the naive Euclidean remainder sequence
/// over `ℚ` that was rebuilt at every count: isolation `3.72 / 4.05 / 3.14 s` against
/// `10.62 / 12.28 / 9.46 s`, and narrowing the gap to `10⁻⁵` `1.2 / 1.3 / 1.1 ms` against
/// `1.22 / 1.18 / 0.91 s`. The whole test in debug: `210 s` against `644 s`. Every printed reading
/// below is unchanged. Run it with
/// `cargo test -p holonic-engine --lib hodge_receiver::the_hodge_receiver_measures_the_m5_spectra
/// -- --ignored --nocapture`, and read the printed elapsed time as the measurement it is.
#[test]
#[ignore = "exact spectral isolation on a 24-cell Laplacian; measured cost is printed"]
fn the_hodge_receiver_measures_the_m5_spectra() {
    let root = require_structure_root();
    let mut gaps: Vec<(&str, ExactInterval)> = Vec::new();
    for (lineage, file) in M5_STRUCTURES {
        let window = rbx1_window(&root.join(file)).unwrap_or_else(|error| panic!("{lineage}: {error}"));
        let founded = contact_complex(&window, false);
        let operator = HodgeOperator::found(lineage, &founded.complex, &unit(), &BoundaryCondition::Free)
            .expect("the operator founds");
        ...
        let gap = spectrum.spectral_gap.clone().expect("a positive eigenvalue exists");
        assert!(gap.lower.is_positive());
        assert_eq!(spectrum.kernel_multiplicity, 1);
        let narrowed = refine_spectral_gap(&spectrum, &ratio(1, 100_000), DEFAULT_ISOLATION_DEPTH)
            .expect("the refinement returns");
        let refined = started.elapsed();
        // **The closed wire, measured on the material it has to carry.** A remount re-multiplies
        // the squarefree decomposition, re-derives the radical as its primitive integer form and
        // re-reads every multiplicity off it, so its cost is printed beside the isolation's rather
        // than assumed — and the measured spectrum comes back unchanged.
        let serialized = serde_json::to_string(&spectrum).expect("the spectrum serializes");
        let remount_started = std::time::Instant::now();
        let remounted: ExactHodgeSpectrum =
            serde_json::from_str(&serialized).expect("the closed wire accepts the measured spectrum");
        let remount = remount_started.elapsed();
        assert_eq!(remounted, spectrum);
        println!(
            "hodge_receiver M5 spectrum | {lineage} | extent {} | trace {} | distinct {} | \
             rational {} | isolated {} | completely rational {} | isolating gap [{}, {}] | \
             gap narrowed to 1e-5 [{}, {}] | isolation depth {} | isolation {:?} | \
             with refinement {:?} | wire remount {remount:?}",
            spectrum.extent,
            spectrum.trace,
            spectrum.distinct(),
            spectrum.rational_eigenvalues.len(),
            spectrum.isolated_eigenvalues.len(),
            spectrum.is_completely_rational(),
            gap.lower,
            gap.upper,
            narrowed.lower,
            narrowed.upper,
            spectrum.isolation_depth,
            elapsed,
            refined,
        );
        gaps.push((lineage, narrowed));
    }

    // **The separation, checked.** The three narrowed certificates are pairwise disjoint, so the
    // exact algebraic connectivity of the contact Laplacian orders the three presentations with no
    // float anywhere — including the two Protenix predictions, whose gaps differ by less than
    // five parts in ten thousand and are still told apart exactly.
    for left in 0..gaps.len() {
        for right in (left + 1)..gaps.len() {
            let (a, b) = (&gaps[left].1, &gaps[right].1);
            assert!(
                a.upper < b.lower || b.upper < a.lower,
                "{} and {} have overlapping gap certificates",
                gaps[left].0,
                gaps[right].0
            );
        }
    }
    let designed = &gaps[0].1;
    assert!(
        gaps[1].1.upper < designed.lower && gaps[2].1.upper < designed.lower,

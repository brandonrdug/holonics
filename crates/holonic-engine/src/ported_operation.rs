...
//!
//! **Quarantine, 2026-08-18.** The realization that had grown here — an operation enum, a program,
//! a carrier trait and a host `enact` loop — was lifted whole into [`crate::ported_reference`] as
//! serial reference machinery after the Phoenix audit found it running normalization, chronology,
//! contact, gating and re-entry on the CPU around contraction leaves. What remains here is exactly
//! the sentence below claims: the bridge. The resident realization is
//! [`crate::front_passage`] over [`crate::resident_section`].
//!
...
pub enum SourceTestimony {
    ...
    /// identifiers with an optional verbatim slice — `Class.method (the source line)` — and a

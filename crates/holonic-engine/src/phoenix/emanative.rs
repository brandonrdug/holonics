...
pub const EMANATIVE_REST_SCHEMA: &str = "holonic-engine.athena.emanative-continuation.v1";
...
fn occurrence_address(
    ...
    frame(
        &mut hasher,
        b"holonic-engine.athena.emanative-occurrence.v1",
    );
...
fn potential_digest(
    ...
    digest_json(&(
        "holonic-engine.athena.emanative-potential.v1",
        vocabulary_extent,
        grain,
        top_lower,
        plural,
...
fn semantic_receipt_digest(returned: &RuntimeReturn) -> Result<String, String> {
    ...
    digest_json(&(
        "holonic-engine.athena.emanative-semantic-receipt.v1",
        (
...
fn digest_native_ids(ids: &[u32]) -> String {
    let mut hasher = Sha256::new();
    frame(&mut hasher, b"holonic-engine.athena.native-word.v1");

...
use super::{
    foreign_section_descent::CarrierSectionRest, receiver_restricted_transport::NativeAnatomyRest,
};

const SCHEMA: &str = "holonic-engine.phoenix.foreign-potential-complex-rest.v1";
const NATIVE_SCHEMA: &str = "holonic-engine.phoenix.native-anatomical-potential-rest.v1";
...
/// Receiver-exact condensation testimony for one native anatomical class. The complete member
/// potentials remain in the separately retained cold foreign reconstruction rest.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct NativePotentialClassCertificate {
    pub native: NativeStateId,
    pub representative_coefficient_node: usize,
    pub coefficient_nodes: Vec<usize>,
    pub history_addresses: Vec<String>,
    pub selected_native_address: u32,
}

/// The hot terminal-order receiver compiled through Athena's native anatomy. It owns no foreign
/// hidden coordinate or factor; those remain cold reconstruction testimony under an exact digest.
#[derive(Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct NativeAnatomicalPotentialRest {
    pub schema: String,
    pub cold_foreign_reconstruction_sha256: String,
    pub cold_foreign_occurrence_population: usize,
    pub native_population: Vec<NativeStateId>,
    pub occurrence_to_native: Vec<NativeStateId>,
    pub receiver_rows: Vec<u32>,
...
impl NativeAnatomicalPotentialRest {
    /// Compile the declared terminal-order receiver through the native anatomical quotient.
    /// Distinct foreign potentials may condense only after every class member returns the same
    /// selected native face; the complete foreign potential remains separately reconstructable.
    pub fn seal_from_foreign(
        ...
        anatomy
            .validate()
            .map_err(|error| ForeignPotentialRestRefusal::Wire(error.to_string()))?;
        if anatomy.occurrences.len() != foreign.history_addresses.len()
            || anatomy.quotient.source_population.len() != foreign.history_addresses.len()
            || anatomy
                .occurrences
                .iter()
                .enumerate()
        ...
        let (foreign_lower, foreign_upper) = foreign.cultivated_receiver_incidence()?;
        let histories = foreign.history_addresses.len();
        let native_population = anatomy.quotient.native_population.clone();
        let occurrence_to_native = anatomy
            .quotient
            .quotient
            .iter()
            .map(|assignment| assignment.native)
            .collect::<Vec<_>>();
        if occurrence_to_native.len() != histories {
            return Err(ForeignPotentialRestRefusal::Shape);
        }
        ...
        for native in &native_population {
            ...
            for row in 0..foreign.receiver_rows.len() {
                lower_incidence.push(foreign_lower[row * histories + representative]);
                upper_incidence.push(foreign_upper[row * histories + representative]);
            }
            class_certificates.push(NativePotentialClassCertificate {
                native: *native,
                representative_coefficient_node: representative,
                coefficient_nodes: class.coefficient_nodes.clone(),
                history_addresses: class.occurrences.clone(),
                selected_native_address: class.emitted_native_address,
            });
        }
        // `ResidentIntervalPotentialReceiver` consumes row-major incidence. The loop above is
        // class-major, so transpose once at construction and never during native inference.
        ...
        let mut rest = Self {
            schema: NATIVE_SCHEMA.to_owned(),
            cold_foreign_reconstruction_sha256: foreign.identity_sha256.clone(),
            cold_foreign_occurrence_population: histories,
            native_population,
            occurrence_to_native,
            receiver_rows: foreign.receiver_rows.clone(),
            lower_incidence,
            upper_incidence,
            decoder: foreign.decoder.clone(),
            class_certificates,
            open_exterior: foreign.open_exterior.clone(),
            identity_sha256: String::new(),
        };
        rest.validate_shape()?;
    ...
    pub fn one_hot_for_native(
    ...
    ) -> Result<Vec<Rat>, ForeignPotentialRestRefusal> {
        let Some(at) = self
            .native_population
            .iter()
        ...
        Ok((0..self.native_population.len())
            .map(|candidate| Rat::from_integer(BigInt::from(if candidate == at { 1 } else { 0 })))
            .collect())
    }
    ...
    pub fn mount_receiver(
        &self,
    ) -> Result<ResidentNativeAnatomicalPotentialReceiver, ForeignPotentialRestRefusal> {
        let card = CudaRefineExecutor::new()
            .map_err(|error| ForeignPotentialRestRefusal::Apparatus(error.to_string()))?;
        self.mount_receiver_on(card)
    }
    ...
    pub fn mount_receiver_on(
        ...
        let resident = ResidentIntervalPotentialReceiver::mount(
            card,
            "athena-alpha/native-anatomical-potential-receiver",
            self.receiver_rows.len(),
    ...
    fn validate_shape(&self) -> Result<(), ForeignPotentialRestRefusal> {
        ...
        if self.schema != NATIVE_SCHEMA
            || self.cold_foreign_reconstruction_sha256.len() != 64
            || self.cold_foreign_occurrence_population == 0
            || natives.is_empty()
            || natives.len() != self.native_population.len()
            || self.occurrence_to_native.len() != self.cold_foreign_occurrence_population
            || self
                .occurrence_to_native
                .iter()
                .any(|native| !natives.contains(native))
            || rows.is_empty()
            ...
            || self
                .receiver_rows
                .iter()
                .any(|row| !self.decoder.contains_key(row))
        {
            return Err(ForeignPotentialRestRefusal::Shape);
        }
        ...
        for certificate in &self.class_certificates {
            if !natives.contains(&certificate.native)
                || certificate.coefficient_nodes.is_empty()
                || certificate.history_addresses.len() != certificate.coefficient_nodes.len()
                || certificate
                    .coefficient_nodes
                    .iter()
                    .any(|node| !nodes.insert(*node))
                || !self
                    .decoder
                    .contains_key(&certificate.selected_native_address)
            {
                return Err(ForeignPotentialRestRefusal::Shape);
            }
        }
        if nodes.len() != self.cold_foreign_occurrence_population {
            return Err(ForeignPotentialRestRefusal::Shape);
        }
        Ok(())
    }

    fn identity_body(&self) -> Result<Vec<u8>, ForeignPotentialRestRefusal> {
        serde_json::to_vec(&(
            &self.schema,
            &self.cold_foreign_reconstruction_sha256,
            self.cold_foreign_occurrence_population,
            &self.native_population,
            &self.occurrence_to_native,
            &self.receiver_rows,
            &self.lower_incidence,
            &self.upper_incidence,
            &self.decoder,

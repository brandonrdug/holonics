    ...
    fn inferred_body(id: &str) -> Body {
        if id.starts_with("held-") {
            match id.rsplit('-').next().and_then(|value| value.parse::<u8>().ok()) {
                ...
                .map(|(body_i, body)| {
                    ...
                    HeldOutBodyReturn {
                        ...
                        input_lineage_digest: digest(
                            format!("lineage-{case_i}-{body_i}").as_bytes(),
                        ),
    ...
    fn ablation(
        ...
        let (control_lineage, control_before, control_after, ablated) =
            if control_kind == AblationControlKind::CausalPrefix {
                ...
                (material.to_owned(), control.clone(), control, causal_ablated)
            } else {
        ...
        let control_ids = if control_kind == AblationControlKind::CausalPrefix {
            ...
            [format!("{id}-control-before"), format!("{id}-control-after")]
        };
    ...
    fn valid() -> FourBodyInput {
        ...
        let absent_overlay_operation_ids =
            vec!["overlay-op-u".to_owned(), "overlay-op-v".to_owned()];
        let absent_factor_populations = vec!["factor-u".to_owned(), "factor-v".to_owned()];
        let held_out_difference_digest = digest(
            &serde_json::to_vec(&(baseline.digest.clone(), recombined.digest.clone())).unwrap(),
        );
        ...
        let recombination = NativeRecombinationReceipt {
            ...
            recombined_deed: bound(ExecutionKind::NativeRuntime, "recombined", &recombined.digest),
            cause_ablations,
    ...
    fn causal_prefix_control_kind_cannot_be_swapped() {
        let mut input = valid();
        input.recombination.cause_ablations[0].control_kind =
            AblationControlKind::SubjectDisjoint;
        assert!(grade(input).is_err());
    }
    #[test]

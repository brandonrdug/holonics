...
use crate::streamed_standing::StagedRegion;
...
impl<'a> NativeMaterialSource<'a> {
    ...
    ) -> Result<NativeMaterialSource<'static>, String> {
        let rest = MountedNativeRest::open(path).map_err(|error| error.to_string())?;
        Ok(NativeMaterialSource {
            rest: RestHandle::Owned(rest),
        })
    }
    ...
    fn extent(
        &self,
        population: &str,
    ) -> Result<crate::native_rest::MountedPopulationExtent, String> {
        self.rest

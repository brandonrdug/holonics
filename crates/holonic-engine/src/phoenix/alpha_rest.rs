//! Canonical source-detached standing for the receiver-history cultivated Athena alpha.
//!
...
pub const ATHENA_ALPHA_REST_SCHEMA: &str =
    "holonic-engine.phoenix.athena-alpha-native-rest.v1";

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AthenaAlphaDecoderReceipt {
...
#[serde(deny_unknown_fields)]
pub struct AthenaAlphaNativeRest {
    pub schema: String,
    ...
    pub decoder: AthenaAlphaDecoderReceipt,
    pub product_identity_sha256: String,
    pub open_exterior: Vec<String>,
}

/// The source-detached rest consumed into one non-cloned productive owner.
pub struct MountedAthenaAlpha {
    session: ProductSession,
    receiver_history: ReceiverHistoryCompression,
    product_identity_sha256: String,
}

impl AthenaAlphaNativeRest {
    pub fn seal(
        session: &ProductSession,
        receiver_history: ReceiverHistoryCompression,
        open_exterior: Vec<String>,
    ) -> Result<Self, AthenaAlphaRestRefusal> {
        receiver_history
            .validate()
            .map_err(|error| AthenaAlphaRestRefusal::ReceiverHistory(error.to_string()))?;
        let body = session.body_identity().map_err(AthenaAlphaRestRefusal::Product)?;
        let cultivation_continuation = session
            .continuation_identity()
            .map_err(AthenaAlphaRestRefusal::Product)?;
        let factor_complex = session
            .factor_complex_rest()
            .map_err(AthenaAlphaRestRefusal::Product)?;
        let decoder = AthenaAlphaDecoderReceipt {
        ...
        let rest = Self {
            schema: ATHENA_ALPHA_REST_SCHEMA.to_owned(),
            body,
    ...
    pub fn read(bytes: &[u8]) -> Result<Self, AthenaAlphaRestRefusal> {
        let rest: Self = serde_json::from_slice(bytes)
            .map_err(|error| AthenaAlphaRestRefusal::Wire(error.to_string()))?;
        rest.validate()?;
        Ok(rest)
    }

    pub fn canonical_bytes(&self) -> Result<Vec<u8>, AthenaAlphaRestRefusal> {
        self.validate()?;
        serde_json::to_vec(self).map_err(|error| AthenaAlphaRestRefusal::Wire(error.to_string()))
    }

    pub fn validate(&self) -> Result<(), AthenaAlphaRestRefusal> {
        if self.schema != ATHENA_ALPHA_REST_SCHEMA
        ...
        {
            return Err(AthenaAlphaRestRefusal::Identity);
        }
    ...
    ) -> Result<MountedAthenaAlpha, AthenaAlphaRestRefusal> {
        self.validate()?;
        let expected_factor_identity = self.factor_complex.identity().clone();
        let mut session = ProductSession::open_with_continuation(
            product_directory,
            continuation_directory,
        )
        .map_err(AthenaAlphaRestRefusal::Product)?;
        if session.body_identity().map_err(AthenaAlphaRestRefusal::Product)? != self.body
            || session
                .continuation_identity()
                .map_err(AthenaAlphaRestRefusal::Product)?
...
fn product_identity(
    ...
    decoder: &AthenaAlphaDecoderReceipt,
) -> Result<String, AthenaAlphaRestRefusal> {
    let bytes = serde_json::to_vec(&(
        ATHENA_ALPHA_REST_SCHEMA,
        body,
        continuation,
        receiver_history,
        factor,
...
pub enum AthenaAlphaRestRefusal {
    #[error("the Athena-alpha rest wire refused: {0}")]
    Wire(String),
    #[error("the receiver-history rest refused: {0}")]
    ReceiverHistory(String),
    #[error("the mounted Phoenix product refused: {0}")]
    Product(String),
    #[error("the Athena-alpha rest identity does not reconstruct")]
    Identity,
}

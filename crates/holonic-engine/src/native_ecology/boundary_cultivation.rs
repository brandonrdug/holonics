...
pub const BOUNDARY_CULTIVATION_SCHEMA: &str = "holonics.e3.returned-boundary-cultivation.v1";
...
impl ReturnedBoundaryCultivationRest {
    ...
    pub fn canonical_identity(&self) -> Result<String, BoundaryCultivationRefusal> {
        digest_json(&(rest_identity(&self.predecessor)?, &self.standing))
    }
...
fn validate_candidate(
    ...
    if candidate.schema != PROVISIONAL_SCHEMA
        ...
        || candidate.returned_boundary_contacts.iter().any(|contact| {
        ...
        || rest_identity(predecessor)?.complete_sha256 != candidate.predecessor_occurrence_sha256
    {
        return Err(BoundaryCultivationRefusal::Candidate);
    }
    Ok(())
...
mod tests {
    use super::*;
    use crate::native_ecology::heterogeneous_fusion::{
    ...
    fn returned_candidate_rests_and_withdraws_to_the_exact_predecessor() {
        ...
        let cultivated = ReturnedBoundaryCultivationRest::cultivate(base, &candidate, returning())
            .expect("cultivated");
        assert_ne!(
        ...
        let remounted = ReturnedBoundaryCultivationRest::read(&standing, &decoder, &fibres, &bytes)
            .expect("remounted");

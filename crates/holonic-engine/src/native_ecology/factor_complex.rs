//! Same-body deposition of returned factor covers into the native projection morphology.
//!
...
pub struct NativeFactorRealization {
    pub address: String,
    pub founding_factor_addresses: Vec<String>,
    pub target_row: u32,
...
const FACTOR_COMPLEX_REST_SCHEMA: &str =
    "holonic-engine.native-ecology.session-factor-complex-rest.v1";
...
pub struct SessionFactorComplexRest {
    ...
    realizations: Vec<NativeFactorRealization>,
...
impl SessionFactorComplex {
    pub fn found(
        cover: DerivedFactorCover,
        predecessor: AlignedFactor,
        mut realizations: Vec<NativeFactorRealization>,
        open_factor_addresses: Vec<String>,
    ) -> Result<Self, SessionFactorRefusal> {
        cover.validate().map_err(|_| SessionFactorRefusal::Shape)?;
        ...
        for realization in &realizations {
            if realization.address.is_empty()
                ...
                || realization.founding_factor_addresses.is_empty()
                ...
                || realization.founding_factor_addresses.iter().any(|address| {
    ...
    pub fn realizations(&self) -> &[NativeFactorRealization] {
        &self.realizations
    }
...
/// Conduct one exact local section on the same resident owner used by the native session.  The
        ...
        let mut complex = SessionFactorComplex::found(
            ...
            vec![
                NativeFactorRealization {
                    ...
                    founding_factor_addresses: vec![addresses[1].clone()],
                    target_row: 2,
                ...
                NativeFactorRealization {
                    ...
                    founding_factor_addresses: vec![addresses[0].clone()],
                    target_row: 1,

...
#[test]
#[ignore = "requires CUDA; projection erases imaginary receiving covectors"]
fn affine_geometry_projection_returns_real_coordinates_and_masks_imaginary_pullback() {
    let readout = ResidentReadout::new().unwrap();
    let surface = ResidentSurface::on(&readout).unwrap();
    let unit = 1_i128 << 16;
    let source = Rc::new(balls(
        &surface,
        &[(vec![unit, 0, 0, unit, 2 * unit, -unit], unit / 256)],
        AFFINE_GEOMETRY_COMPONENTS,
    ));
    let map = cayley_map();
    let coefficients =
        ResidentNormalEnclosureSection::affine_coefficients(&surface, &[map], ResidentGrain(16))
            .unwrap();
    let action = source
        .clone()
        .affine_geometry(&[0], coefficients, true)
        .unwrap();
    let projected = action.output().row(0).unwrap().inspect().unwrap();
    assert!(
        projected
            .center
            .iter()
            .all(|coordinate| coordinate.imaginary.is_zero())
    );
    assert!(projected.contains(&[wave(13, 0, 5), wave(-1, 0, 5), wave(4, 0, 1),]));

    let imaginary_only = balls(
        &surface,
        &[(vec![0, 7 * unit, 0, 11 * unit, 0, 13 * unit], 0)],
        AFFINE_GEOMETRY_COMPONENTS,
    );
    let back = action.pull_back(&imaginary_only).unwrap();
    let zero = vec![wave(0, 0, 1), wave(0, 0, 1), wave(0, 0, 1)];
    assert!(
        back.source()
            .row(0)
            .unwrap()
    ...
    assert_eq!(
        back.source().row(0).unwrap().inspect().unwrap().radius,
        Rat::zero()
    );
}

#[test]
#[ignore = "requires CUDA; exact dyadic affine declaration at grain 96"]
fn affine_coefficients_accept_a_high_grain_dyadic_map() {
    let readout = ResidentReadout::new().unwrap();
    let surface = ResidentSurface::on(&readout).unwrap();
    let grain = ResidentGrain(96);
    let unit = 1_i128 << grain.0;
    let source = Rc::new(balls_at_grain(
        &surface,
        &[(vec![unit, 0, 0, unit, 2 * unit, -unit], 1)],
        AFFINE_GEOMETRY_COMPONENTS,
        grain,
    ));
    let map = AffineMap3 {
    ...
    let coefficients =
        ResidentNormalEnclosureSection::affine_coefficients(&surface, &[map], grain).unwrap();
    let action = source.affine_geometry(&[0], coefficients, false).unwrap();
    assert!(
        action
            .output()
            .row(0)
            ...
            .inspect()
            .unwrap()
            .contains(&[wave(2, 0, 1), wave(-2, 1, 1), wave(5, -1, 1),])
    );
}

#[test]
#[ignore = "requires CUDA; realification roundtrip, adjoint pairing and whole-ball radius"]
fn realification_roundtrip_and_adjoint_preserve_the_declared_ball() {
    let readout = ResidentReadout::new().unwrap();
    let surface = ResidentSurface::on(&readout).unwrap();
    let unit = 1_i128 << 16;
    let source = Rc::new(balls(
        &surface,
        &[(
            vec![unit, 2 * unit, 3 * unit, 4 * unit, 5 * unit, 6 * unit],
            unit / 64,
        )],
        AFFINE_GEOMETRY_COMPONENTS,
    ));
    let source_ball = source.row(0).unwrap().inspect().unwrap();
    let encoded = source.clone().realify().unwrap();
    let encoded_ball = encoded.output().row(0).unwrap().inspect().unwrap();
    assert!(encoded_ball.contains(&[
        wave(1, 0, 1),
        wave(2, 0, 1),
        wave(3, 0, 1),
        wave(4, 0, 1),
    ...
    assert_eq!(encoded_ball.radius, source_ball.radius);

    let decoded = encoded.output_handle().decode_realification().unwrap();
    let decoded_ball = decoded.output().row(0).unwrap().inspect().unwrap();
    assert!(decoded_ball.contains(&[wave(1, 2, 1), wave(3, 4, 1), wave(5, 6, 1),]));
    assert_eq!(decoded_ball.radius, source_ball.radius);
    ...
    let gy = balls(
        ...
        2 * AFFINE_GEOMETRY_COMPONENTS,
    );
    let back = encoded.pull_back(&gy).unwrap();
    let back_ball = back.source().row(0).unwrap().inspect().unwrap();
    assert!(back_ball.contains(&[wave(2, 4, 1), wave(6, 8, 1), wave(10, 12, 1),]));
    assert_eq!(
        back_ball.radius,
        gy.row(0).unwrap().inspect().unwrap().radius
    );
    let encoded_exact = vec![
        wave(1, 0, 1),
        wave(2, 0, 1),
        wave(3, 0, 1),
        wave(4, 0, 1),
    ...
    let source_exact = vec![wave(1, 2, 1), wave(3, 4, 1), wave(5, 6, 1)];
    let source_covector = vec![wave(2, 4, 1), wave(6, 8, 1), wave(10, 12, 1)];
    assert_eq!(
        dot(&encoded_exact, &gy_exact),
        dot(&source_exact, &source_covector)
    );

    let projected = source.clone().project_real().unwrap();
    let projected_ball = projected.output().row(0).unwrap().inspect().unwrap();
    assert!(projected_ball.contains(&[wave(1, 0, 1), wave(3, 0, 1), wave(5, 0, 1),]));
    assert_eq!(projected_ball.radius, source_ball.radius);
    let projected_back = projected
        .pull_back(&gy_for_projection(&surface, unit))
        .unwrap();
    assert!(
        projected_back
            .source()
            ...
            .inspect()
            .unwrap()
            .contains(&[wave(2, 0, 1), wave(4, 0, 1), wave(6, 0, 1),])
    );
}

fn gy_for_projection<'c>(
    surface: &'c ResidentSurface<'c>,
    unit: i128,
) -> ResidentNormalEnclosureSection<'c> {
    balls(
        surface,
        &[(
            vec![2 * unit, 7 * unit, 4 * unit, 9 * unit, 6 * unit, 11 * unit],
            0,
        )],
        AFFINE_GEOMETRY_COMPONENTS,
    )

...
#[cfg(target_os = "macos")]
#[test]
#[ignore = "requires Metal; image scratch uses twenty-byte carriers before evidence allocation"]
fn metal_image_aperture_refuses_before_allocating_derived_evidence() {
    let r = ResidentReadout::new().unwrap();
    let s = ResidentSurface::on(&r).unwrap();
    let available = s.declaration().max_sectiond_bytes as usize;
    // One complex source and two complex outputs: preimage needs 16*n+18 wide
    // carriers, image 16*n+26. Find an admitted preimage whose image crosses the limit.
    let n = (1..available)
        .find(|n| {
            ResidentSurface::constitutive_wide_scratch(16 * n + 18).unwrap() <= available
                && ResidentSurface::constitutive_wide_scratch(16 * n + 26).unwrap() > available
        })
        .expect("Metal aperture separates these adjacent exact layouts");
    let body = ResidentConstitutiveFibre::found_bilinear_contact(&s, 1, n, 2).unwrap();
    let source = points(&s, &[0, 0]);
    let observed = points(&s, &[0, 0, 0, 0]);
    let preimage = body
        .read_condition_preimage(current(&source), current(&observed))
        .unwrap();
    let before = s.census();
    assert!(
        matches!(body.read_condition_image(current(&source),&preimage),
        Err(ConstitutiveFibreError::ScratchAperture{required,..}) if required>available)
    );
...
fn partial_and_empty_domains_keep_their_witnesses_and_block_unqualified_use() {
    ...
    let cut = body.occurrences();
    assert!(body
        .advance_bilinear_contact(image.current(), current(&one), None)
        .is_err());
...
fn invalid_operands_and_word_overflow_preserve_the_input_family() {
    ...
    assert!(body
        .read_condition_image(ResidentConstitutiveCurrent::rational(&bad).unwrap(), &f)
        .is_err());
    let big_condition = body
        .read_condition_preimage(current(&one), current(&huge))
        .unwrap();
    assert!(body
        .read_condition_image(current(&huge), &big_condition)
        .is_err());

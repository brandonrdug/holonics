//! The addressed junction which makes the admitted native rests one inference ecology.
//!
...
mod tests {
    ...
    use crate::native_ecology::{
        heterogeneous_fusion::{PortDeclaration, SharedWorldGenerator, SourcePortResponse},
        recurrent_condensation::{
            CONDENSED_DECODER_SCHEMA, CONDENSED_FIBRES_SCHEMA, CONDENSED_STANDING_SCHEMA,
            CompactBoundaryFace, CondensedDecoder, CondensedFibre, CondensedFibreMember,
            CondensedFibres, CondensedSeparator, CondensedStanding,
        },
    };
    ...
    fn heterogeneous() -> HeterogeneousFusionRest {
        ...
        let responses = (0..2)
            .flat_map(|family| {
                [CODEWORD, OPTICAL].into_iter().enumerate().flat_map(
                    move |(boundary_at, boundary)| {
                        (0..2).map(move |state| {

//! An addressed cultivation continuation over one authenticated native product.
//!
...
const DIRECTORY_SCHEMA: &str =
    "holonic-engine.native-ecology.cultivation-continuation-directory.v1";
...
impl AddressedReturnLineage {
    fn validate(&self) -> Result<(), ContinuationRefusal> {
        for (name, value) in [
            ...
            ("native admission", &self.admission_sha256),
...
mod tests {
    ...
    fn body() -> CultivatedBodyIdentity {
        CultivatedBodyIdentity::new(
            ...
            Vec::new(),
            &RuntimeLawReceipt {
                schema: "holonic-engine.native-ecology.runtime-law.v1".to_owned(),
                grain: 48,
                series_aperture: 1,
                band_terms: 1,
                vocabulary_extent: 4,

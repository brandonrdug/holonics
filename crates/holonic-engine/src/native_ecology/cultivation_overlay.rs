//! A resident rank-one cultivation overlay for the native cultivated body.
//!
...
const FACTORIZED_LAW: &str = "native-ecology.overlay.factorized-contract";
const RE_ENTRY_LAW: &str = "native-ecology.overlay.re-entry";
const STANDING_LAW: &str = "native-ecology.overlay.standing";
...
pub fn canonical_rank_derivation_bytes(receipt: &RankDerivationReceipt) -> Vec<u8> {
    ...
    frame(
        &mut bytes,
        b"holonic-engine.native-ecology.w3-rank-derivation.v1",
    );
    ...
    match receipt {
        ...
        FactorDerivationReceipt::Derived(receipt) => {
            let bytes = serde_json::to_vec(&(
                "holonic-engine.native-ecology.derived-rank-derivation.v1",
                &receipt.defect,
...
impl FactorizedCandidate {
    pub fn new(
        ...
        Self {
            ...
            testimony,
            ports: OverlayPorts {
                input: "native-ecology.overlay/input".to_owned(),
                predecessor: "native-ecology.overlay/w2-predecessor".to_owned(),
                predecessor_output: "native-ecology.overlay/w2-predecessor-output".to_owned(),
                delta_output: "native-ecology.overlay/delta-output".to_owned(),
                output: "native-ecology.overlay/output".to_owned(),
            },
        }
    }
    ...
    pub fn complex(
        ...
        let mut complex =
            PortedOperationComplex::new("native-ecology.factorized-cultivation-overlay");
        let input = complex.port(self.ports.input.clone());
        let predecessor_output = complex.port(self.ports.predecessor_output.clone());
        let terminal_input = complex.port("native-ecology.overlay/terminal-input");
        ...
        let input_standing = complex
            .bind_operation(
                "native-ecology.overlay.standing.input",
        ...
        let predecessor_standing = complex
            .bind_operation(
                "native-ecology.overlay.standing.predecessor",
                OperationSpecies::Construction,
                vec![],
...
#[cfg(test)]
#[path = "cultivation_overlay_tests.rs"]

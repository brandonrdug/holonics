...
use crate::native_ecology::factor_complex::NativeFactorRealization;

const SCHEMA: &str = "holonic-engine.soulkiller.receiver-restricted-factor-descent.v1";
const EXTERIOR_ANATOMY_SCHEMA: &str =
    "holonic-engine.soulkiller.exterior-native-anatomy-witness.v1";

/// One factor realization and the complete source occurrences which founded it.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct HistorySupportedFactor {
    pub realization: NativeFactorRealization,
...
/// Exterior construction coordinates for one native class.  The independently mountable native
/// rest retains only the class address, receiver consequence and local incidence.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct ExteriorNativeAnatomyClassWitness {
    pub native: NativeStateId,
    pub occurrences: Vec<String>,
...
/// Cold exterior reconstruction testimony.  This object is sealed separately and is not a field
/// of [`NativeAnatomyRest`].
#[derive(Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ExteriorNativeAnatomyWitness {
    pub schema: String,
    ...
    pub classes: Vec<ExteriorNativeAnatomyClassWitness>,
...
/// The one-way construction return.  The two objects must be written, moved and remounted
/// separately; Athena accepts only `native`.
#[derive(Debug, PartialEq, Eq)]
pub struct NativeAnatomyDescentReturn {
    pub native: NativeAnatomyRest,
    pub exterior: ExteriorNativeAnatomyWitness,
}
...
pub enum ReceiverRestrictedFactorRefusal {
    ...
    #[error("native anatomy refused: {0}")]
    Native(#[from] NativeAnatomyRefusal),
...
impl NativeAnatomyDescentReturn {
    /// Found the coarsest exact Athena receiver/history anatomy on the resident quotient
    /// apparatus and split the result into a source-neutral native core and cold exterior witness.
    /// The CPU reading is an admission reference taken before the resident deed; it is never a
    /// semantic replay after the device return.
    pub fn found_on_device(
        ...
        let mut native_classes = Vec::with_capacity(quotient.native_population.len());
        ...
        for fibre in &quotient.reconstruction_fibres {
            ...
            native_classes.push(NativeAnatomyClass {
                native: fibre.native,
                emitted_native_address: first.emitted_native_address,
                predecessor_classes: predecessor_classes.clone(),
                successor_classes: successor_classes.clone(),
            });
            exterior_classes.push(ExteriorNativeAnatomyClassWitness {
                native: fibre.native,
                occurrences: members
                    .iter()
        ...
        let native = NativeAnatomyRest::new(quotient.native_population.clone(), native_classes)?;
        let exterior = ExteriorNativeAnatomyWitness {
            schema: EXTERIOR_ANATOMY_SCHEMA.to_owned(),
            native_identity_sha256: native_identity(&native)?,
            quotient,
            occurrences: system.occurrences,
            receivers: system.receivers,
            classes: exterior_classes,
            foreign_to_native,
        ...
        let result = Self { native, exterior };
        result.validate()?;
        Ok(result)
    }
    ...
    pub fn validate(&self) -> Result<(), ReceiverRestrictedFactorRefusal> {
        ...
        self.exterior.validate(&self.native)
    }
}

impl ExteriorNativeAnatomyWitness {
    pub fn read(
        bytes: &[u8],
        native: &NativeAnatomyRest,
    ) -> Result<Self, ReceiverRestrictedFactorRefusal> {
        let witness: Self = serde_json::from_slice(bytes)
            .map_err(|error| ReceiverRestrictedFactorRefusal::Wire(error.to_string()))?;
        witness.validate(native)?;
        Ok(witness)
    }
    ...
    pub fn canonical_bytes(
        &self,
        native: &NativeAnatomyRest,
    ) -> Result<Vec<u8>, ReceiverRestrictedFactorRefusal> {
    ...
    pub fn validate(
        &self,
        native: &NativeAnatomyRest,
    ) -> Result<(), ReceiverRestrictedFactorRefusal> {
        native.validate()?;
        if self.schema != EXTERIOR_ANATOMY_SCHEMA
            || self.native_identity_sha256 != native_identity(native)?
        ...
        if self.quotient.source_population.len() != self.occurrences.len()
            || self.quotient.native_population != native.native_population
            || self.classes.len() != native.classes.len()
        {
            return Err(ReceiverRestrictedFactorRefusal::InvalidSupport);
        }
        ...
        let native_classes = native
            .classes
            .iter()
            .map(|class| (class.native, class))
            .collect::<BTreeMap<_, _>>();
        let mut native_by_occurrence = BTreeMap::new();
        for class in &self.classes {
            let Some(native_class) = native_classes.get(&class.native) else {
                return Err(ReceiverRestrictedFactorRefusal::InvalidSupport);
            };
            if class.occurrences.is_empty()
                || class.coefficient_nodes.len() != class.occurrences.len()
                || class.inherited_history_states.len() > class.occurrences.len()
                || class.foreign_receiver_classes.is_empty()
                || class.realization_addresses.is_empty()
                || class.emitted_native_address != native_class.emitted_native_address
                || class.predecessor_classes != native_class.predecessor_classes
                || class.successor_classes != native_class.successor_classes
            {
                return Err(ReceiverRestrictedFactorRefusal::InvalidSupport);
            }
            for occurrence in &class.occurrences {
                if native_by_occurrence
                    .insert(occurrence.as_str(), class.native)
                    .is_some()
                {
                    return Err(ReceiverRestrictedFactorRefusal::InvalidSupport);
                }
        ...
        for distinction in &self.foreign_only_distinctions {
            if distinction.left_occurrence == distinction.right_occurrence
                || native_by_occurrence.get(distinction.left_occurrence.as_str())
                    != Some(&distinction.native)
                || native_by_occurrence.get(distinction.right_occurrence.as_str())
                    != Some(&distinction.native)
            {
                return Err(ReceiverRestrictedFactorRefusal::InvalidSupport);
            }
        }
        for analogous in &self.analogous_sections {
            if analogous.left_occurrence == analogous.right_occurrence
                || native_by_occurrence.get(analogous.left_occurrence.as_str())
                    != Some(&analogous.native)
                || native_by_occurrence.get(analogous.right_occurrence.as_str())
                    != Some(&analogous.native)
            {
                return Err(ReceiverRestrictedFactorRefusal::InvalidSupport);
            }
        }
        for homologous in &self.homologous_sections {
            if homologous.left_occurrence == homologous.right_occurrence
                || homologous.common_realization_addresses.is_empty()
                || native_by_occurrence.get(homologous.left_occurrence.as_str())
                    != Some(&homologous.native)
                || native_by_occurrence.get(homologous.right_occurrence.as_str())
                    != Some(&homologous.native)
            {
                return Err(ReceiverRestrictedFactorRefusal::InvalidSupport);
            }
...
fn native_identity(native: &NativeAnatomyRest) -> Result<String, ReceiverRestrictedFactorRefusal> {
    let bytes = native
        .canonical_bytes()
        .map_err(ReceiverRestrictedFactorRefusal::Native)?;
...
mod tests {
    ...
    fn realization() -> NativeFactorRealization {
        NativeFactorRealization {
            address: "factor-a".to_owned(),
            founding_factor_addresses: vec!["returned-a".to_owned()],
            target_row: 7,

...
/// The three presentations of one deposited `_atom_site` table, with every byte count measured.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5Presentations {
    /// What the receipts are of, written out.
    pub object: String,
    /// The deposited mmCIF text of the `_atom_site` block.
    pub mmcif_text: CostReceipt,
    /// The scaled-integer wire at the resident decimal grain, carrying every atom.
    pub scaled_wire: CostReceipt,
    /// The alpha-carbon restriction on that wire, with the dropped atoms as its retained residual.
    pub alpha_carbon: CostReceipt,
}

impl M5Presentations {
    /// The three as a labelled family, in the order above.
    pub fn family(&self) -> Vec<ParetoPoint> {
        vec![
            ParetoPoint::new("mmcif-text", self.mmcif_text.clone()),
            ParetoPoint::new("scaled-integer-wire", self.scaled_wire.clone()),
            ParetoPoint::new("alpha-carbon-restriction", self.alpha_carbon.clone()),
        ]
    }
}

/// Produce the three receipts from a deposited mmCIF text at a declared resident decimal grain.
///
/// The object presented is stated exactly: the
/// `(label_asym_id, label_seq_id, label_comp_id, label_atom_id, x, y, z)` table of the deposit,
/// with each coordinate its exact written decimal token. All three presentations are presentations
/// of that one table.
///
/// * `mmcif-text` is the deposit's own block. It carries **more** than the object — twenty columns
///   where the object has seven — which is not a residual but a cost in bytes, and it owes no
///   residual because the object is recoverable from it exactly.
/// * `scaled-integer-wire` is `physical_intake::mmcif::AtomOccurrence::projected_wire` at
///   `resident_places`, six `i64` interval words per atom, plus the identifiers. Its residual is
///   **measured**: the code size of the fibres the projection actually merges on this data.
/// * `alpha-carbon-restriction` is `grain_tower::GrainSelection` on that wire — the `CA` atom of
///   every residue — and its residual is the dropped atom records, in bits, which is exactly what
///   `Transition::reopen` needs to return the source.
pub fn m5_presentations(
    source_lineage: impl Into<String>,
    text: &str,
    resident_places: u32,
) -> Result<M5Presentations, CostRefusal> {
    let source_lineage = source_lineage.into();
    let measurement = measure_atom_site_block(source_lineage.clone(), text)?;
    let structure = StructurePresentation::parse(source_lineage.clone(), text)
        .map_err(|error| CostRefusal::IntakeRefused {
            source_lineage: source_lineage.clone(),
            detail: error.to_string(),
        })?;

    let instrument = format!(
        "presentation_cost::m5_presentations over {source_lineage} \
         (measure_atom_site_block + physical_intake::mmcif::StructurePresentation::parse)"
    );

    // Measured atom-level counts.
    let mut atoms: u64 = 0;
    let mut alpha_carbons: u64 = 0;
    let mut identifier_bytes: u64 = 0;
    let mut alpha_identifier_bytes: u64 = 0;
    let mut first_record_identifier_bytes: Option<u64> = None;
    let mut first_alpha_identifier_bytes: Option<u64> = None;
    // The residual the wire projection actually owes, measured: how many distinct atom rows of the
    // object land on one wire row.
    let mut wire_fibres: BTreeMap<Vec<u8>, BTreeMap<Vec<u8>, u64>> = BTreeMap::new();

    for chain in &structure.chains {
        for residue in &chain.residues {
            for atom in &residue.atoms {
                atoms += 1;
                let identifier = (chain.label_asym_id.len()
                    + residue.monomer.len()
                    + atom.label.len()) as u64;
                identifier_bytes += identifier;
                if first_record_identifier_bytes.is_none() {
                    first_record_identifier_bytes = Some(identifier);
                }
                if atom.label == "CA" {
                    alpha_carbons += 1;
                    alpha_identifier_bytes += identifier;
                    if first_alpha_identifier_bytes.is_none() {
                        first_alpha_identifier_bytes = Some(identifier);
                    }
                }
                let (lower, upper) = atom.projected_wire(resident_places).map_err(|error| {
                    CostRefusal::IntakeRefused {
                        source_lineage: source_lineage.clone(),
                        detail: error.to_string(),
                    }
                })?;
                let mut wire_face = Vec::with_capacity(64);
                wire_face.extend_from_slice(chain.label_asym_id.as_bytes());
                wire_face.extend_from_slice(&residue.source_ordinal.to_le_bytes());
                wire_face.extend_from_slice(atom.label.as_bytes());
                for word in lower.iter().chain(upper.iter()) {
                    wire_face.extend_from_slice(&word.to_le_bytes());
                }
                let mut source_face = Vec::with_capacity(64);
                source_face.extend_from_slice(chain.label_asym_id.as_bytes());
                source_face.extend_from_slice(&residue.source_ordinal.to_le_bytes());
                source_face.extend_from_slice(residue.monomer.as_bytes());
                source_face.extend_from_slice(atom.label.as_bytes());
                source_face.extend_from_slice(atom.x.token.as_bytes());
                source_face.push(b'|');
                source_face.extend_from_slice(atom.y.token.as_bytes());
                source_face.push(b'|');
                source_face.extend_from_slice(atom.z.token.as_bytes());
                *wire_fibres
                    .entry(wire_face)
                    .or_default()
                    .entry(source_face)
    ...
    if atoms != measurement.rows {
        return Err(CostRefusal::RowsDisagreeWithIntake {
            scanned_rows: measurement.rows,
            decoded_atoms: atoms,
        });
    }

    // The measured residual of the wire: the bits needed to say which source row a wire row came
    // from, summed over the wire rows. Zero exactly when the projection is injective on this data.
    let wire_residual_bits = wire_fibres
        .values()
        .fold(BigUint::zero(), |total, fibre| {
            total + code_bits(&BigUint::from(fibre.len() as u64))
        });

    let dropped_atoms = atoms - alpha_carbons;
    let dropped_identifier_bytes = identifier_bytes - alpha_identifier_bytes;

    let record_bytes = WIRE_COORDINATE_BYTES + WIRE_ORDINAL_BYTES;
    let wire_bytes = record_bytes * atoms + identifier_bytes;
    let wire_decode = WIRE_FIXED_FIELDS * atoms + identifier_bytes;
    let alpha_bytes = record_bytes * alpha_carbons + alpha_identifier_bytes;
    let alpha_decode = WIRE_FIXED_FIELDS * alpha_carbons + alpha_identifier_bytes;
    let alpha_residual_bits =
        8 * (record_bytes * dropped_atoms + dropped_identifier_bytes);

    let object = format!(
        "the (label_asym_id, label_seq_id, label_comp_id, label_atom_id, x, y, z) table of \
         {source_lineage}: {atoms} atom rows, {alpha_carbons} of them alpha carbons"
    );

    let mmcif_text = CostReceipt {
        presentation: format!("{source_lineage}: the deposited mmCIF _atom_site block, as text"),
        bytes: Counted::measured(measurement.block_bytes, instrument.clone()),
        decode_work: Counted::derived(
            measurement.block_bytes,
            "one scan step per byte of the block: the records are variable width, so no field is \
             reachable without scanning every byte before it",
        ),
        update_work: Counted::derived(
            measurement.row_bytes,
            "the admitted change replaces the first data row; text rows are variable width, so \
             every byte of the rows region is rewritten",
        ),
        certificate_work: Counted::derived(
            measurement.block_bytes + OBJECT_FIELDS_PER_ATOM * atoms,
            "decode_work, then one comparison per object field (7 per atom row)",
        ),
        residual: Counted::derived(
            0u64,
            "the presentation is the deposit itself; the transition to it is invertible, and \
             ContinuingTower.Transition.ofEquiv_residual_subsingleton makes its residual code in \
             zero bits",
        ),
    };

    let scaled_wire = CostReceipt {
        presentation: format!(
            "{source_lineage}: physical_intake::mmcif::AtomOccurrence::projected_wire at 10^{resident_places}, every atom"
        ),
        bytes: Counted::derived(
            wire_bytes,
            "(48 interval bytes + 4 ordinal bytes) per atom, plus the measured identifier bytes \
             (chain + monomer + atom label)",
        ),
        decode_work: Counted::derived(
            wire_decode,
            "7 fixed-width field reads per atom, plus one step per identifier byte",
        ),
        update_work: Counted::derived(
            record_bytes + first_record_identifier_bytes.unwrap_or(0),
            "the admitted change replaces the first atom record; the records are fixed width and \
             randomly addressable, so only that record is rewritten",
        ),
        certificate_work: Counted::derived(
            wire_decode + OBJECT_FIELDS_PER_ATOM * atoms,
            "decode_work, then one comparison per object field (7 per atom row)",
        ),
        residual: Counted::measured(
            wire_residual_bits,
            format!(
                "{instrument}: sum over distinct wire rows of code_bits(number of distinct object \
                 rows landing on it)"
            ),
        ),
    };

    let alpha_carbon = CostReceipt {
        presentation: format!(
            "{source_lineage}: grain_tower::GrainSelection (label_atom_id == CA) on that wire"
        ),
        bytes: Counted::derived(
            alpha_bytes,
            "the wire rule, over the measured alpha-carbon rows only",
        ),
        decode_work: Counted::derived(
            alpha_decode,
            "the wire rule, over the measured alpha-carbon rows only",
        ...
        update_work: Counted::derived(
            record_bytes + first_alpha_identifier_bytes.unwrap_or(0),
            "the admitted change replaces the first carried record; fixed width, randomly \
             addressable",
        ),
        certificate_work: Counted::derived(
            alpha_decode + OBJECT_FIELDS_PER_ATOM * alpha_carbons,
            "decode_work, then one comparison per carried object field; the object rows this \
             presentation does not carry are the residual coordinate, not the certificate",
        ),
        residual: Counted::derived(
            alpha_residual_bits,
            "8 bits per byte of the dropped atom records — exactly what grain_tower's \
             SelectionResidual retains so that ContinuingTower.Transition::reopen returns the \
             source",
        ),
    };

...
extern "C" __global__ __launch_bounds__(512) void section_normal_wave_receive(
    ...
    direct_normal_predict(old,frame,n,n,grain,out+NORMAL_SOURCE_FORWARD*stride,work,false,slot);
    if(*slot)return;
    ...
    if(*slot)return;
    direct_normal_predict(next,frame,n,n,grain,out,work,false,slot);
    if(*slot)return;
    if(!threadIdx.x){

...
extern "C" __global__ void section_constitutive_wave_relation(
    ...
    uint32_t n,uint32_t kc,int64_t *graph,int64_t *graph_hi,
    int64_t *derived,int64_t *derived_hi,int64_t *fixed,int64_t *fixed_hi,
    ...
    if(!n||!kc||g64>UINT32_MAX||2u*q64>UINT32_MAX){atomicOr(slot,REFUSED_MALFORMED);return;}
    ...
    extern __shared__ wide scratch[];wide *residual=scratch,*row=residual+l;
    ...
    for(uint32_t col=0;col<u;++col){
        ...
        if(col<2u){
            ...
            for(uint32_t j=0;j<kc;++j){
                ...
                residual[source_width+2u*j]=col? -hi:hr;
                residual[source_width+2u*j+1u]=col?hr:hi;
            }
        }else if(col>=z_at&&col<eta_at){
            uint32_t j=(col-z_at)%r;
            bool is_current=col>=z_at+r;
            residual[j]=is_current?hd:-hd;
            residual[(is_current?r:2u*r)+j]=hd;
        }else if(col>=eta_at)residual[f+col-eta_at]=hd;
        ...
        if(*slot)return;
        wide den=hd;uint32_t ignored=0,rank=0;

...
__device__ size_t contextual_balls(uint32_t n){return 22u*(2u*(size_t)n+1u);}
__device__ size_t contextual_beta(uint32_t n){return contextual_balls(n);}
__device__ size_t contextual_profile_words(uint32_t n){return 16u*(size_t)n+12u;}
__device__ size_t contextual_output_visible(uint32_t n){return contextual_beta(n)+8u*n;}
__device__ size_t contextual_input_visible(uint32_t n){return contextual_output_visible(n)+contextual_profile_words(n);}
__device__ size_t contextual_context(uint32_t n){return contextual_input_visible(n)+contextual_profile_words(n);}
__device__ size_t contextual_raw(uint32_t n){return contextual_context(n)+36u*n+22u;}
__device__ size_t contextual_meta(uint32_t n){return contextual_raw(n)+30u*n;}
__device__ size_t contextual_extra(uint32_t n){return contextual_meta(n)+4u;}
...
__device__ void contextual_query_parts(const int64_t *table,const int64_t *now,uint32_t n,uint32_t at,uint32_t query,
    ...
    if(query==0){*visible=now+contextual_output_visible(n);*context=now+contextual_context(n);return;}
    if(source<0 || !at){atomicOr(slot,REFUSED_MALFORMED);return;}
    if(query==1){*visible=now+contextual_input_visible(n);*context=contextual_at(table,at-1u)+contextual_context(n);return;}
    if(query==2){const int64_t *old=contextual_at(table,(uint32_t)source);*visible=old+contextual_output_visible(n);*context=old+contextual_context(n);return;}
    if(reference<0 || reference>=(int64_t)at){atomicOr(slot,REFUSED_MALFORMED);return;}
    *visible=contextual_at(table,(uint32_t)reference)+contextual_input_visible(n);
    *context=contextual_at(table,(uint32_t)reference-1u)+contextual_context(n);
}
...
__device__ void contextual_material_prepare(
    ...
    wide S=(wide)((uwide)1u<<grain);int64_t *meta=out+contextual_meta(n);wide *extra=(wide *)(out+contextual_extra(n));
    if(threadIdx.x==0){
        ...
        if(linked && !*slot){
            for(size_t j=0;j<contextual_profile_words(n);++j)out[contextual_input_visible(n)+j]=original[contextual_output_visible(n)+j];
            ...
            for(uint32_t i=1;i<at;++i){
                const int64_t *old=contextual_at(table,i),*m=old+contextual_meta(n);
                if(m[1]<0 || m[2]!=(int64_t)i)continue;
                bool same=true;const wide *a=(const wide *)(out+contextual_input_visible(n)),*b=(const wide *)(old+contextual_input_visible(n));
    ...
    for(uint32_t i=threadIdx.x;i<=at;i+=blockDim.x){
        const int64_t *entry=i==at?out:contextual_at(table,i);bool valid=entry[contextual_meta(n)+1u]>=0;
    ...
    for(uint32_t coordinate=threadIdx.x;coordinate<4u*R;coordinate+=blockDim.x){
        ...
        if(q==0 || (linked && (q<3 || contrasted))){
            for(uint32_t i=0;i<at;++i){
                const int64_t *old=contextual_at(table,i),*m=old+contextual_meta(n);if(m[1]<0)continue;
                const wide *b=(const wide *)(old+contextual_beta(n));
    ...
    if(threadIdx.x==0){
        ...
        if(linked && !*slot){
            const int64_t *si=out+contextual_input_visible(n),*ci=contextual_at(table,at-1u)+contextual_context(n);
            ...
            wide old_error=contextual_source_error(original+contextual_output_visible(n),original+contextual_context(n),n,grain,version,slot);
            ...
            if(contrasted && !*slot){
                ...
                reference_error=contextual_source_error(ref+contextual_input_visible(n),contextual_at(table,(uint32_t)meta[2]-1u)+contextual_context(n),n,grain,version,slot);
    ...
    for(uint32_t j=threadIdx.x;j<R;j+=blockDim.x){
        ...
        raw=out+contextual_raw(n);history_write_integer(sum,raw+5u*j,out_hi+contextual_raw(n)+5u*j,slot);
        history_write_integer(pos,raw+5u*(R+j),out_hi+contextual_raw(n)+5u*(R+j),slot);
        history_write_integer(neg,raw+5u*(2u*R+j),out_hi+contextual_raw(n)+5u*(2u*R+j),slot);
    }
    __syncthreads();if(*slot)return;
    if(threadIdx.x==0){
        wide rounds=0,error=moment_read_evaluation(out+contextual_raw(n),R,grain,ball,&rounds,slot);extra[10]=error;
        wide ex=contextual_source_error(out+contextual_output_visible(n),out+contextual_context(n),n,grain,version,slot);
        ball[R]=add_checked(extra[0],add_checked(ft_ceil_product(extra[1],ex,grain,slot),error,slot),slot);
        if(*slot)return;
        for(size_t i=0;i<contextual_extra(n)+24u;++i)out_hi[i]=out[i];
    }
}

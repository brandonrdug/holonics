...
// foreign arm's: the arrays are the native rest's own populations, and the rest declares the law
// each of these realizes in its own metadata.
//
// **The predecessor's shape is not extended.** The quarantined `athena_walk` opened `if (flat !=
// 0) return;` under a maximum-legal-block flat grid: one lane worked and the rest of the launch
// existed to be discarded. Both laws below are covered by their EXTENT.
//
//   walk    — a chain in T, so the parallel extent is the PROMPT POPULATION and, inside one step,
//             the transport ROW. One warp owns one prompt and searches each row 32-ARY: 32 probes
//             per round narrow the row by a factor of 32, so the dependency span of a row search
//             is ceil(log32 |row|) rounds rather than the serial ceil(log2 |row|).
//   future  — positions x vocabulary, every cell an independent climb, so it is the parallel deed.
//             A block owns a tile of positions and a tile of germs; each position's suffix chain is
//             STAGED ONCE into shared and read by every germ lane of that position, so the climb
//             crosses global memory once per (block, position) instead of once per (position, germ).
// ---------------------------------------------------------------------------------------------

#define ATHENA_NO_SLOT 0xFFFFFFFFu

// One lane, one germ: the row is sorted, so the serial law is a binary search.
__device__ __forceinline__ uint32_t athena_row_find(const uint32_t *indptr, const uint32_t *germ, uint32_t state, uint32_t g) {
    ...
    while (lo < hi) {
        ...
        if (at == g) return mid;
        if (at < g) lo = mid + 1u; else hi = mid;
    }
    return ATHENA_NO_SLOT;
}

// **One warp, one germ, 32 probes per round.** The row is sorted, so the 32 probe values are
// ascending across the lanes and `__ballot_sync` of `value <= g` is a PREFIX mask whose population
// count names the lane the germ sits at or after. Every lane computes the same `lo`/`hi` from the
// same ballots, so the loop is warp-uniform and the whole warp is always converged at the ballot.
__device__ __forceinline__ uint32_t athena_row_find_warp(const uint32_t *indptr, const uint32_t *germ, uint32_t state, uint32_t g, uint32_t lane) {
    uint32_t lo = indptr[state], hi = indptr[state + 1];
    if (lo >= hi) return ATHENA_NO_SLOT;
    while (hi - lo > 32u) {
        const uint32_t n = hi - lo;
        const uint32_t at = lo + (uint32_t)(((unsigned long long)n * (unsigned long long)lane) >> 5);
        const uint32_t v = germ[at];
        const uint32_t eq = __ballot_sync(0xFFFFFFFFu, v == g);
        if (eq != 0u) {
            const uint32_t l = (uint32_t)__ffs((int)eq) - 1u;
            return lo + (uint32_t)(((unsigned long long)n * (unsigned long long)l) >> 5);
        }
        const uint32_t le = __ballot_sync(0xFFFFFFFFu, v <= g);
        if (le == 0u) return ATHENA_NO_SLOT;  // g sits below the row's first germ
        const uint32_t k = (uint32_t)__popc(le);
        const uint32_t next_lo = lo + (uint32_t)(((unsigned long long)n * (unsigned long long)(k - 1u)) >> 5);
        const uint32_t next_hi = (k < 32u) ? (lo + (uint32_t)(((unsigned long long)n * (unsigned long long)k) >> 5)) : hi;
        lo = next_lo;
        hi = next_hi;
    }
    const uint32_t at = lo + lane;
    const uint32_t hit = (at < hi && germ[at] == g) ? 1u : 0u;
    const uint32_t mask = __ballot_sync(0xFFFFFFFFu, hit);
    if (mask != 0u) return lo + (uint32_t)__ffs((int)mask) - 1u;
    return ATHENA_NO_SLOT;
}

// **The walk.** One warp per prompt; the prompts arrive concatenated with their offsets, so a
// grid of `ceil(prompts / warps_per_block)` blocks covers the population by extent. Out is
// `positions x 2`: `[class, mark]`, mark 1 forward, 0 arc (the germ was found at a shorter
// context), 2 unseen (the germ is not in the rest's vocabulary and the walk returns to the root).
extern "C" __global__ void __launch_bounds__(512) athena_walk_cooperative(
    const uint32_t *indptr, const uint32_t *germ, const uint32_t *target, const uint32_t *suffix,
    const uint32_t *prompt, const uint32_t *offsets, uint32_t prompts, uint32_t positions,
    uint32_t classes, uint32_t vocabulary,
    int64_t *out_lo, int64_t *out_hi,
    uint32_t *refused, const uint32_t *census, const uint32_t *lineage, uint32_t lineage_count
) {
    if (upstream_refused(census, lineage, lineage_count, refused)) return;
    ...
    const uint32_t warp = threadIdx.x >> 5;
    const uint32_t lane = threadIdx.x & 31u;
    const uint32_t p = blockIdx.x * warps + warp;
    if (p >= prompts) return;
    const uint32_t begin = offsets[p], end = offsets[p + 1];
    // The rest's arrays are the deed's only semantic input, so an atlas that addresses a class it
    // does not have, or a prompt whose span leaves its own buffer, is MALFORMED and the occurrence
    // refuses rather than reading whatever sits at the address. Every lane of the warp reads the
    // same words and takes the same branch.
    if (begin > end || end > positions) {
        atomicOr(refused, REFUSED_MALFORMED);
        return;
    }
    uint32_t state = 0u;
    for (uint32_t t = begin; t < end; ++t) {
        const uint32_t g = prompt[t];
        int64_t mark;
        if (g >= vocabulary) {
            state = 0u;
            mark = 2;
            ...
            mark = 1;
            for (;;) {
                if (at >= classes) { atomicOr(refused, REFUSED_MALFORMED); return; }
                const uint32_t slot = athena_row_find_warp(indptr, germ, at, g, lane);
                if (slot != ATHENA_NO_SLOT) {
                    state = target[slot];
                    if (state >= classes) { atomicOr(refused, REFUSED_MALFORMED); return; }
                    break;
                }
                const uint32_t parent = suffix[at];
                if (parent == at) { state = 0u; mark = 0; break; }
                at = parent;
                mark = 0;
            }
        }
        if (lane == 0u) {
            out_lo[(size_t)t * 2] = (int64_t)state;      out_hi[(size_t)t * 2] = (int64_t)state;
            out_lo[(size_t)t * 2 + 1] = mark;            out_hi[(size_t)t * 2 + 1] = mark;
        }
    }
}

// **The future section.** For every position and every vocabulary germ, the standing of the class
// the germ reaches from the landed class's suffix chain (`depth_face` 0, zero where no class in the
// chain offers it), or how far up the chain it was found (`depth_face` 1, minus one where
// unreached). In `rows x 2` (the walk); out `rows x vocabulary`.
//
// The block owns `positions_per_block` positions and `lanes * germs_per_lane` germs. The staging
// depth is the caller's declared aperture: `chain_stage` classes of each position's suffix chain
// cross into shared once and are read by every germ lane; past it the climb continues in global
// memory from the staged tail, so the aperture changes the residency and never the returned word.
extern "C" __global__ void __launch_bounds__(512) athena_future_staged(
    const uint32_t *indptr, const uint32_t *germ, const uint32_t *target, const uint32_t *standing, const uint32_t *suffix,
    const int64_t *walk_lo, uint32_t rows, uint32_t classes, uint32_t vocabulary,
    uint32_t positions_per_block, uint32_t germs_per_lane, uint32_t chain_stage, uint32_t depth_face,
    int64_t *out_lo, int64_t *out_hi,
    uint32_t *refused, const uint32_t *census, const uint32_t *lineage, uint32_t lineage_count
) {
    extern __shared__ uint32_t athena_chain_raw[];
    __shared__ int32_t athena_stop;
    if (threadIdx.x == 0) athena_stop = upstream_refused(census, lineage, lineage_count, refused) ? 1 : 0;
    __syncthreads();
    if (athena_stop) return;

    const uint32_t lanes = blockDim.x / positions_per_block;
    const uint32_t germ_tile = lanes * germs_per_lane;
    const uint32_t tiles_v = (vocabulary + germ_tile - 1u) / germ_tile;
    const uint32_t v_tile = blockIdx.x % tiles_v;
    const uint32_t t_tile = blockIdx.x / tiles_v;
    const uint32_t group = threadIdx.x / lanes;
    const uint32_t lane = threadIdx.x % lanes;
    const uint32_t t = t_tile * positions_per_block + group;
    const uint32_t v0 = v_tile * germ_tile;

    uint32_t *chain = athena_chain_raw;
    uint32_t *staged = athena_chain_raw + (size_t)positions_per_block * (size_t)chain_stage;
    uint32_t *tail = staged + positions_per_block;

    // (a) STAGE: one lane per position climbs the chain once and writes it where every germ lane
    //     of that position reads it.
    if (lane == 0u) {
        uint32_t depth = 0u;
        uint32_t at = ATHENA_NO_SLOT;
        if (t < rows) {
            const int64_t landed = walk_lo[(size_t)t * 2];
            // The landed class arrives from the walk's own section; a class the rest does not have
            // is MALFORMED and this occurrence refuses rather than addressing whatever is there.
            if (landed < 0 || (unsigned long long)landed >= (unsigned long long)classes) {
                atomicOr(refused, REFUSED_MALFORMED);
            } else {
                at = (uint32_t)landed;
                while (depth < chain_stage) {
                    chain[(size_t)group * (size_t)chain_stage + depth] = at;
                    depth++;
                    const uint32_t parent = suffix[at];
                    if (parent >= classes) { atomicOr(refused, REFUSED_MALFORMED); at = ATHENA_NO_SLOT; break; }
                    if (parent == at) { at = ATHENA_NO_SLOT; break; }
                    at = parent;
                }
            }
        }
    ...
    const uint32_t depth = staged[group];
    for (uint32_t j = 0; j < germs_per_lane; ++j) {
        const uint32_t v = v0 + j * lanes + lane;
        if (v >= vocabulary) continue;
        int64_t value = depth_face ? -1 : 0;
        int found = 0;
        // (b) the staged prefix of the chain, from shared
        for (uint32_t d = 0; d < depth; ++d) {
            const uint32_t slot = athena_row_find(indptr, germ, chain[(size_t)group * (size_t)chain_stage + d], v);
            if (slot != ATHENA_NO_SLOT) {
                const uint32_t reached = target[slot];
                if (reached >= classes) { atomicOr(refused, REFUSED_MALFORMED); found = 1; break; }
                value = depth_face ? (int64_t)d : (int64_t)standing[reached];
                found = 1;
                break;
            }
        }
        // (c) the remainder of the chain, from global, from the staged tail
        if (!found) {
            uint32_t at = tail[group];
            uint32_t d = depth;
            while (at != ATHENA_NO_SLOT) {
                const uint32_t slot = athena_row_find(indptr, germ, at, v);
                if (slot != ATHENA_NO_SLOT) {
                    const uint32_t reached = target[slot];
                    if (reached >= classes) { atomicOr(refused, REFUSED_MALFORMED); break; }
                    value = depth_face ? (int64_t)d : (int64_t)standing[reached];
                    break;
                }
                const uint32_t parent = suffix[at];
                if (parent >= classes) { atomicOr(refused, REFUSED_MALFORMED); break; }
                if (parent == at) break;
                at = parent;
                d++;
            }
        }
        const size_t flat = (size_t)t * (size_t)vocabulary + (size_t)v;
        out_lo[flat] = value;
        out_hi[flat] = value;
    }
}

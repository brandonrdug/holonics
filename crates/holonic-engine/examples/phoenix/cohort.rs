...
impl TowerDeclaration {
    pub fn sharing(name: &str, site: Site, intervention: Intervention) -> Self {
        Self { name: name.to_owned(), site, intervention, enters_at: site.first_layer(), shares_prefix: true }
    }
    ...
    pub fn whole(name: &str, site: Site, intervention: Intervention) -> Self {
        Self { name: name.to_owned(), site, intervention, enters_at: 0, shares_prefix: false }
    }
}
...
impl TowerReturn {
    pub fn stood(&self) -> bool {
        self.obstructions.is_empty()
            && self.layers.values().all(|l| l.lineage_empty && l.a_priori_held)
    ...
    complex
        ...
        .map(|o| {
            ...
            let typed = complex.operations.get(&o.law).is_some_and(|op| {
                !op.testimony.is_empty() && op.testimony.iter().all(|t| matches!(t, holonic_engine::ported_operation::SourceTestimony::Intervention { .. }))
            });
            (name, typed)
        })
        .collect()
...
#[derive(Default)]
struct Live {
    carried: Option<(Rc<ResidentSection<'static>>, u32)>,
    shared: BTreeMap<&'static str, (Rc<ResidentSection<'static>>, u32)>,
    entered: bool,
    launches: u64,
    work: ExactWork,
}
...
fn key_of(
    ...
    if residency >= Residency::Whole {
        ...
        if let Some(positions) = &material.positions {
            addressed.push(("the mounted positions".to_owned(), positions.resident_address(), positions.rows(), 0));
        }
    }
    addressed.sort();
    GraphKey {
...
pub fn circulate_cohort(
    ...
    if let Some(species) = identity_site_species {
        ...
        material.bands.insert(tower::IDENTITY_BANDS.to_owned(), (surface.mount_bands(&identity, tower::BAND_GRAIN).map_err(|e| e.to_string())?, (tokens.len() - 1) as u32));
    }
    let forward: Vec<u32> = (0..tokens.len() as u32).collect();
    let backward: Vec<u32> = (0..tokens.len() as u32).rev().collect();
    material.positions = Some(surface.mount_positions(&forward).map_err(|e| e.to_string())?);
    ...
    let mut base_entry: Vec<Option<(Rc<ResidentSection<'static>>, u32)>> = vec![None; tower::LAYERS + 1];
    let mut base_shared: BTreeMap<&'static str, (Rc<ResidentSection<'static>>, u32)> = BTreeMap::new();
    ...
    let snapshot_layer = declarations
        .iter()
        .filter(|d| d.shares_prefix && matches!(d.intervention, Intervention::WithdrawSharedKvFamily { .. }))
        .map(|d| d.enters_at)
        .min()
    ...
    for layer in 0..tower::LAYERS {
        ...
        for (index, declaration) in declarations.iter().enumerate() {
            ...
            if let Some((section, bound)) = &live[index].carried {
                material.standings.insert(tower::CARRIED_STANDING.to_owned(), (Rc::clone(section), *bound));
            } else if layer != 0 {
                return Err(format!("{}: no carried standing at layer {layer}", declaration.name));
            }
            if role == KvRole::Shared {
                ...
                let (k, kb) = live[index].shared.get(k_name).ok_or_else(|| format!("{}: no shared K standing at layer {layer}", declaration.name))?;
                let (v, vb) = live[index].shared.get(v_name).ok_or_else(|| format!("{}: no shared V standing at layer {layer}", declaration.name))?;
            ...
            if reversed {
                let spare = spare_positions.take().ok_or("the reversed positions are already mounted")?;
                spare_positions = material.positions.replace(spare);
            }

            let entry = if layer == 0 { Entry::Rows } else { Entry::Carried };
            let mut founded = tower::found_layer(layer, entry, chart, &scales, terms, layer_scalar, applied, tokens.len())?;
            ...
            let declared: BTreeSet<EventId> = [founded.returns[tower::PLE_SECTION], founded.returns[tower::CONTACT], founded.returns[tower::LAYER_ENCLOSURE]].into_iter().collect();
            let (fusable, refused) = factored_seals(&founded.complex, &founded.realization, terminal, &declared);
            ...
            let boundary = format!("the layer return, with {} declared faces (per-layer input, contact, layer enclosure)", declared.len());
            for (level, ledger) in [
            ...
            let census_at_launch = bound
                ...
                .map_err(|o| format!("{} layer {layer} refused at launch: {}", declaration.name, describe(&o)))?;
            ...
            if reversed {
                let spare = spare_positions.take().ok_or("the forward positions are already mounted")?;
                spare_positions = material.positions.replace(spare);
            }
            ...
            if role == KvRole::OwnAndStore {
                ...
                live[index].shared.insert(k_name, (Rc::new(k), kb));
                live[index].shared.insert(v_name, (Rc::new(v), vb));
                if index == 0 {
                    base_shared = live[index].shared.clone();
                }
            }
            let (out, ob) = bound.release_section(terminal).ok_or("layer return")?;
            let out = (Rc::new(out), ob);
        ...
        let boundary = layer + 1 == tower::LAYERS || (layer + 1) % band == 0 || layer + 1 == snapshot_layer;
        if boundary {
            ...
            if layer + 1 == snapshot_layer {
                for (name, (section, _)) in &base_shared {
                    shared_before.insert((*name).to_owned(), surface.read_out(section).map_err(|e| e.to_string())?);
                }
            }
            band_first = layer + 1;
    ...
    for (index, declaration) in declarations.iter().enumerate() {
        let applied: &Intervention = if declaration.site == Site::Final { &declaration.intervention } else { &none };
        if !live[index].entered {
        ...
        material.standings.clear();
        let (section, bound_octaves) = live[index].carried.clone().ok_or_else(|| format!("{}: no carried standing at the final boundary", declaration.name))?;
        ...
        let declared: BTreeSet<EventId> = [founded.returns[tower::FINAL_NORMED]].into_iter().collect();
        let (fusable, refused) = factored_seals(&founded.complex, &founded.realization, terminal, &declared);
        for quotient in &fusable {
            founded.realization.bind(*quotient, SealedMidpointQuotient);
        }
        ...
        let census_at_launch = bound
            .launch_on(&surface.mode(), circulation.conducting())
            .map_err(|o| format!("{} refused at the final launch: {}", declaration.name, describe(&o)))?;
    ...
    let (base_named, base_key, base_label) = base_final.ok_or("the base tower declared no final boundary")?;
    ...
    if let Some(bound) = base_bound {
        ...
        match instantiations.offer(&format!("{base_label} · relaunched"), &base_key) {
            Instantiation::Reused { index, .. } => {
                let census_at_launch = bound.launch_on(&surface.mode(), circulation.conducting()).map_err(|o| format!("the reuse relaunch refused: {}", describe(&o)))?;
                circulation.conducted(last.slot).map_err(|e| e.to_string())?;
                deed_launches += 1;
                circulation.terminal().map_err(|e| e.to_string())?;
                let returned = bound.returned(census_at_launch).map_err(|o| format!("the relaunched deed did not return: {}", describe(&o)))?;
                let second_final_normed = bound.read_section(&returned, base_named[tower::FINAL_NORMED]).map_err(|o| describe(&o))?;
                let second_potential = bound.read_terminal(&returned).map_err(|o| describe(&o))?;
                reuse = Some(ReuseReceipt { label: base_label, instantiation: index, first_potential, second_potential, first_final_normed, second_final_normed });
            }
...
fn drain_keeping(pending: &mut Vec<Pending>, returns: &mut [TowerReturn], keep: usize) -> Result<Option<CompiledPassage<'static>>, String> {
    let mut kept = None;
    for deed in pending.drain(..) {
        read_into(&deed, returns)?;
...
fn read_into(deed: &Pending, returns: &mut [TowerReturn]) -> Result<(), String> {
    let returned = deed.bound.returned(deed.census.clone()).map_err(|o| format!("{} did not return: {}", deed.label, describe(&o)))?;
    let stands = returned.stands();
    let a_priori = returned.measured_octaves.iter().all(|(port, measured)| deed.bound.octave_field.get(port).copied().unwrap_or(0) >= *measured);
    if let Err(obstruction) = deed.bound.standing(&returned) {
        returns[deed.tower].obstructions.push((deed.layer.unwrap_or(usize::MAX), format!("{}: {}", deed.label, describe(&obstruction))));
        return Ok(());
    }
    match deed.layer {
        Some(layer) => {
            let terminal = deed.bound.read_section(&returned, deed.returns[tower::LAYER_ENCLOSURE]).map_err(|o| format!("{}: {}", deed.label, describe(&o)))?;
            let contact = deed.bound.read_section(&returned, deed.returns[tower::CONTACT]).map_err(|o| format!("{}: {}", deed.label, describe(&o)))?;
            let ple = deed.bound.read_section(&returned, deed.returns[tower::PLE_SECTION]).map_err(|o| format!("{}: {}", deed.label, describe(&o)))?;
        ...
        None => {
            let final_normed = deed.bound.read_section(&returned, deed.returns[tower::FINAL_NORMED]).map_err(|o| format!("{}: {}", deed.label, describe(&o)))?;
            let potential = deed.bound.read_terminal(&returned).map_err(|o| format!("{}: {}", deed.label, describe(&o)))?;

...
pub const IMPLEMENTATION_PACKAGE_INIT: &str =
    "/home/b/scratch/huggingface/.venv/lib/python3.13/site-packages/transformers/__init__.py";
pub const IMPLEMENTATION_SHA256: &str =
    "64ecac478c7d11b9a6993ea194bdf98ce867d0e6d361295d976140e0ded53933";
...
pub fn implementation(symbol: &str) -> SourceTestimony {
    SourceTestimony::Implementation {
        locator: IMPLEMENTATION.to_owned(),
...
pub fn mount<'chart>(
    ...
    receipt.regions.insert(
        named("layer_scalar"),
        source.region(&named("layer_scalar"), Some(digest_words(&scalar)))?,
    );
...
pub fn enter(
    source: &mut Source,
    tokens: &[usize],
    material: &mut ResidentMaterial<'_>,
) -> Result<(), String> {
...
pub fn implementation_version() -> Option<String> {
    ...
    text.lines().find_map(|line| {
        line.strip_prefix("__version__ = ")
            .map(|v| format!("transformers {}", v.trim().trim_matches('"')))
    })
}
...
pub fn source_occurrence(
...
) -> Result<SourceOccurrence, String> {
    let implementation =
        AuthenticatedText::read(IMPLEMENTATION, implementation_version().as_deref())
            .map_err(|e| e.to_string())?;
    if implementation.sha256 != IMPLEMENTATION_SHA256 {
        return Err(format!(
            "the implementation at {IMPLEMENTATION} hashes to {} and this site's bindings were read against {IMPLEMENTATION_SHA256}",
            implementation.sha256
        ));
    }
    let configuration =
        AuthenticatedText::read(&format!("{root}/config.json"), None).map_err(|e| e.to_string())?;
    let locator = format!("{root}/model.safetensors");
    let (octets, header_octets, header_sha256) =
        AuthenticatedContainer::read_header(&locator).map_err(|e| e.to_string())?;
    ...
    Ok(SourceOccurrence {
        ...
        container: AuthenticatedContainer {
            identity: holonic_engine::foreign_map::FileIdentity::at(&locator).ok(),
            locator,
            octets,
            header_octets,
            header_sha256,
...
pub fn found_deed(
    ...
    let mut complex = PortedOperationComplex::new(
        "Gemma-4-E4B layer 0 with its per-layer input, sliding attention",
    );
    ...
    realization.bind(
        token,
        Enter {
            population: PLE_ENTERING.to_owned(),
    ...
    realization.bind(
        ...
        RmsRebase {
            group: HIDDEN,
            gain: Some(named("input_layernorm.weight")),
            eps,
        },
    );
    ...
    let h = if sibling == Sibling::CollapseAfterInputRebase {
        let collapsed = law(
            &mut complex,
            "input rebase collapsed (control)",
            OperationSpecies::Quotient,
            vec![standing],
            vec![standing],
            None,
            vec![intervention(
                "control: the remainder removed at this site; the enclosure collapsed to its midpoint; not a source law",
            )],
        )?;
    ...
    realization.bind(
        ...
        RmsRebase {
            group: HEAD_WIDTH,
            gain: Some(named("self_attn.q_norm.weight")),
            eps,
        },
    );
    ...
    let kn = law(
        ...
        vec![families],
        vec![families],
        Some(named("self_attn.k_norm.weight")),
        {
            let mut t = rebase_testimony(
                "Gemma4TextAttention.forward (key_states = self.k_norm(key_states))",
                &named("self_attn.k_norm.weight"),
                HEAD_WIDTH,
            );
    ...
    realization.bind(
        ...
        RmsRebase {
            group: HEAD_WIDTH,
            gain: Some(named("self_attn.k_norm.weight")),
            eps,
        },
    );
    ...
    let chronology_testimony = |site: &str| {
        vec![
            ...
            implementation(
                "Gemma4TextRotaryEmbedding.compute_default_rope_parameters (base ** (torch.arange(0, dim, 2, dtype=torch.int64))",
            ),
            implementation(
    ...
    let o = law(
        ...
        vec![receivers],
        vec![standing],
        Some(named("self_attn.o_proj.weight")),
        vec![
    ...
    realization.bind(
        ...
        RmsRebase {
            group: HIDDEN,
            gain: Some(named("post_attention_layernorm.weight")),
            eps,
        },
    );
    ...
    let h2 = law(
        ...
        rebase_testimony(
            ...
            &named("pre_feedforward_layernorm.weight"),
            HIDDEN,
        ),
    )?;
    realization.bind(
        ...
        RmsRebase {
            group: HIDDEN,
            gain: Some(named("pre_feedforward_layernorm.weight")),
            eps,
        },
    );
    ...
    let dn = law(
        ...
        rebase_testimony(
            ...
            &named("post_feedforward_layernorm.weight"),
            HIDDEN,
        ),
    )?;
    realization.bind(
        ...
        RmsRebase {
            group: HIDDEN,
            gain: Some(named("post_feedforward_layernorm.weight")),
            eps,
        },
    );
    ...
    let pg = law(
        ...
        vec![standing],
        vec![ple],
        Some(named("per_layer_input_gate.weight")),
        vec![
    ...
    realization.bind(
        pg,
        Contract {
            population: named("per_layer_input_gate.weight"),
        },
    );
    ...
    let pp = law(
        ...
        vec![ple],
        vec![standing],
        Some(named("per_layer_projection.weight")),
        vec![
            implementation(&format!(
    ...
    realization.bind(
        pp,
        Contract {
            population: named("per_layer_projection.weight"),
        },
    );
    ...
    realization.bind(
        ...
        RmsRebase {
            group: HIDDEN,
            gain: Some(named("post_per_layer_input_norm.weight")),
            eps,
        },
    );

...
impl Serial {
    ...
    fn floor_of(&self, value: &Rat) -> BigInt {
        (value * Rat::from_integer(pow2(self.grain))).floor().to_integer()
    }

    fn ceil_of(&self, value: &Rat) -> BigInt {
        (value * Rat::from_integer(pow2(self.grain))).ceil().to_integer()
    }
    ...
    pub fn contract(&self, x: &[Iv], rows: usize, inner: usize, map: &[u16], out_width: usize) -> Result<Vec<Iv>, String> {
        ...
        for t in 0..rows {
            for o in 0..out_width {
                ...
                out.push((shift_floor(&lo, i64::from(lowest)), shift_ceil(&hi, i64::from(lowest))));
            }
        }
        Ok(out)
    }
    ...
    pub fn rms(&self, x: &[Iv], group: usize, gain: Option<&[u16]>, eps: Dyadic) -> Result<Vec<Iv>, String> {
        ...
        for chunk in x.chunks(group) {
            ...
            let r_hi = AlgebraicRoot::reciprocal_square_root(&mean_lo, 100).map_err(|e| format!("{e:?}"))?;
            let r_lo = AlgebraicRoot::reciprocal_square_root(&mean_hi, 100).map_err(|e| format!("{e:?}"))?;
            let r = ExactInterval::new(r_lo.enclosure().lower.clone(), r_hi.enclosure().upper.clone()).map_err(|e| format!("{e:?}"))?;
            ...
            for (i, (a, b)) in chunk.iter().enumerate() {
                ...
                if let Some(gains) = &gains {
                    ...
                    let (ql, qh) = if m.is_negative() { (&yh * &m, &yl * &m) } else { (&yl * &m, &yh * &m) };
    ...
    pub fn chronology(&self, x: &[Iv], rows: usize, heads: usize, head_width: usize, theta: u64, terms: usize) -> Result<Vec<Iv>, String> {
        let bands = head_width / 2;
        let ratio = AlgebraicRoot::nth_root(&Rat::from_integer(BigInt::from(theta)), bands as u32, 64)
            .map_err(|e| format!("{e:?}"))?
            .enclosure()
            .reciprocal()
            .map_err(|e| format!("{e:?}"))?;
        ...
        for _ in 0..bands {
            ...
            angle = angle.times(&ratio).map_err(|e| format!("{e:?}"))?.round_out(80).map_err(|e| format!("{e:?}"))?;
        }
        let mut out = x.to_vec();
        for t in 0..rows {
            let p = Rat::from_integer(BigInt::from(t as u64));
            for band in 0..bands {
                let arc = ExactInterval::new(&angles[band].lower * &p, &angles[band].upper * &p).map_err(|e| format!("{e:?}"))?;
                ...
                let (c_lo_s, s_lo_s) = CertifiedSeries::circular_series(&arc.lower, terms).map_err(|e| format!("{e:?}"))?;
                let (c_hi_s, s_hi_s) = CertifiedSeries::circular_series(&arc.upper, terms).map_err(|e| format!("{e:?}"))?;
                ...
                let (half_pi_low, half_pi_high) = (Rat::new(BigInt::from(15707), BigInt::from(10000)), Rat::new(BigInt::from(15709), BigInt::from(10000)));
                let (pi_low, pi_high) = (Rat::new(BigInt::from(31415), BigInt::from(10000)), Rat::new(BigInt::from(31417), BigInt::from(10000)));
                ...
                if arc.upper > Rat::from_integer(BigInt::from(4)) {
                    return Err("the serial reference encloses the chronology only up to an arc of 4".to_owned());
                }
                ...
                for head in 0..heads {
                    ...
                    out[first] = (shift_floor(&(xc_l - ys_h), g), shift_ceil(&(xc_h - ys_l), g));
                    out[second] = (shift_floor(&(xs_l + yc_l), g), shift_ceil(&(xs_h + yc_h), g));
                }
            }
        }
        Ok(out)
    ...
    pub fn contact(&self, q: &[Iv], k: &[Iv], v: &[Iv], rows: usize, heads: usize, kv_heads: usize, head_width: usize, window: usize, terms: usize) -> Result<Vec<Iv>, String> {
            ...
            for h in 0..heads {
                ...
                for j in start..=t {
                    ...
                    for d in 0..head_width {
                        let (l, u) = corners(&q[q_base + d].0, &q[q_base + d].1, &k[k_base + d].0, &k[k_base + d].1);
                        lo += l;
                        hi += u;
                    }
                    scores.push((self.rat_of(&shift_floor(&lo, -i64::from(self.grain))), self.rat_of(&shift_ceil(&hi, -i64::from(self.grain)))));
                }
                ...
                let weights: Vec<ExactInterval> = scores
                    ...
                    .map(|(l, h)| {
                        ...
                        let e_lo = CertifiedSeries::exponential_enclosure(&lo_arg, terms).map_err(|e| format!("{e:?}"))?;
                        let e_hi = CertifiedSeries::exponential_enclosure(&hi_arg, terms).map_err(|e| format!("{e:?}"))?;
                        ExactInterval::new(e_lo.lower.clone(), e_hi.upper.clone()).map_err(|e| format!("{e:?}"))
                    })
                    .collect::<Result<_, _>>()?;
                ...
                for d in 0..head_width {
                    ...
                    for (r, j) in (start..=t).enumerate() {
                        ...
                        let w = &weights[r];
                        let products = [&w.lower * &vl, &w.lower * &vh, &w.upper * &vl, &w.upper * &vh];
                    ...
                    let q_lo = if num_lo.is_negative() { &num_lo / &total_lo } else { &num_lo / &total_hi };
                    let q_hi = if num_hi.is_negative() { &num_hi / &total_hi } else { &num_hi / &total_lo };
    ...
    pub fn gelu(&self, x: &[Iv], c1: Dyadic, c2: Dyadic, terms: usize) -> Result<Vec<Iv>, String> {
        ...
        x.iter()
            .map(|(a, b)| {
                ...
                let t_lo = CertifiedSeries::hyperbolic_tangent_enclosure(&u_lo, terms).map_err(|e| format!("{e:?}"))?;
                let t_hi = CertifiedSeries::hyperbolic_tangent_enclosure(&u_hi, terms).map_err(|e| format!("{e:?}"))?;
    ...
    pub fn hadamard(&self, a: &[Iv], b: &[Iv]) -> Vec<Iv> {
        a.iter()
            ...
            .map(|((al, ah), (bl, bh))| {
                ...
                (shift_floor(&l, -i64::from(self.grain)), shift_ceil(&h, -i64::from(self.grain)))
            })
            .collect()
    }

    pub fn re_entry(&self, a: &[Iv], b: &[Iv]) -> Vec<Iv> {
        a.iter().zip(b).map(|((al, ah), (bl, bh))| (al + bl, ah + bh)).collect()
    }
    ...
    pub fn scale(&self, x: &[Iv], by: DyadicEnclosure) -> Vec<Iv> {
        ...
        x.iter()
            .map(|(a, b)| {
                ...
                (shift_floor(&l, -i64::from(by.grain)), shift_ceil(&h, -i64::from(by.grain)))
            })
            .collect()
    }

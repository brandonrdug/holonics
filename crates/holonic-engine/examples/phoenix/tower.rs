...
pub use super::resident_layer::{
    bond, configuration, implementation, intervention, law, point_enclosure, rebase_testimony, shape, Founded, Source, EMBED, EMBED_SCALE,
    EPS_BITS, FFN, GAINS, GELU_CUBIC_BITS, GELU_SCALE_BITS, HEADS, HIDDEN, KV_HEADS, PLE_EMBED, PLE_EMBED_SCALE, PLE_MODEL_PROJECTION,
...
impl Species {
    pub fn of(layer: usize) -> Self {
        if FULL_LAYERS.contains(&layer) { Self::Full } else { Self::Sliding }
    }
...
pub fn populations(layer: usize) -> Vec<String> {
    ...
    for suffix in POPULATIONS.iter().chain(GAINS.iter()) {
        let shared_only = matches!(*suffix, "self_attn.k_proj.weight" | "self_attn.v_proj.weight" | "self_attn.k_norm.weight");
...
pub fn unbound_populations(layer: usize) -> Vec<String> {
    ...
    ["self_attn.k_proj.weight", "self_attn.v_proj.weight", "self_attn.k_norm.weight"].iter().map(|s| named(layer, s)).collect()
}
...
pub fn mount_layer<'chart>(source: &mut Source, readout: &'chart holonic_engine::embedding_fiber::ResidentReadout, material: &mut holonic_engine::front_passage::ResidentMaterial<'chart>, layer: usize) -> Result<LayerMount, String> {
    ...
    receipt.regions.insert(scalar_name.clone(), source.region(&scalar_name, Some(digest_words(&scalar)))?);
...
pub fn mount_final<'chart>(source: &mut Source, readout: &'chart holonic_engine::embedding_fiber::ResidentReadout, material: &mut holonic_engine::front_passage::ResidentMaterial<'chart>) -> Result<LayerMount, String> {
    use holonic_engine::front_passage::MountedPopulation;
    use super::resident_layer::digest_words;
    let mut receipt = LayerMount::default();
    for name in [FINAL_NORM, EMBED] {
        let (words, shape) = source.whole(name)?;
        let dim = *shape.last().ok_or_else(|| format!("{name} has no shape"))?;
        receipt.regions.insert(name.to_owned(), source.region(name, Some(digest_words(&words)))?);
        let mounted = readout.mount_bfloat16(&words, dim).map_err(|error| format!("{error:?}"))?;
        let masses = mounted.absolute_row_mass().map_err(|error| format!("{error:?}"))?;
        let widest = masses.iter().map(|m| m.unsigned_abs()).max().unwrap_or(0);
        let mass_octaves = i64::from(128 - widest.leading_zeros()) + i64::from(mounted.exponent());
        ...
        receipt.stored_octets += (words.len() * 2) as u64;
        receipt.resident_octets += mounted.resident_octets() as u64;
        material.populations.insert(name.to_owned(), MountedPopulation { readout: mounted, mass_value_octaves: u32::try_from(mass_octaves.max(0)).unwrap_or(0) });
    }
    Ok(receipt)
}
...
pub fn enter(source: &mut Source, tokens: &[usize], layer: usize, material: &mut holonic_engine::front_passage::ResidentMaterial<'_>) -> Result<(), String> {
    ...
    material.entering.insert(super::resident_layer::PLE_ENTERING.to_owned(), EnteringRows { words: per_layer, rows: tokens.len(), width: PLE_WIDTH });
    Ok(())
}
...
pub fn material_plan(source: &Source, layer: usize, tokens: usize) -> Result<MaterialPlan, String> {
    ...
    Ok(MaterialPlan { maps, band_elements: bands, positions: tokens })
}
...
pub fn final_material_plan(source: &Source) -> Result<MaterialPlan, String> {
    let embed = source.container.tensor(EMBED).map_err(|error| error.to_string())?.shape.clone();
    let norm = source.container.tensor(FINAL_NORM).map_err(|error| error.to_string())?.shape.clone();
    Ok(MaterialPlan { maps: vec![(EMBED.to_owned(), embed[0], embed[1]), (FINAL_NORM.to_owned(), 1, norm[0])], band_elements: 0, positions: 0 })
}
...
pub fn found_bands(species: Species, terms: usize) -> Result<Vec<((i64, i64), (i64, i64))>, String> {
...
pub fn found_layer(layer: usize, entry: Entry, chart: Chart, scales: &(DyadicEnclosure, DyadicEnclosure), terms: SeriesAperture, layer_scalar: Dyadic, sibling: &Intervention, tokens: usize) -> Result<Founded, String> {
    ...
    let mut complex = PortedOperationComplex::new(&format!("Gemma-4-E4B layer {layer} with its per-layer input, {} attention, K/V {role:?}", species.layer_type()));
    ...
    let residual_in = match entry {
        Entry::Rows => x0,
        Entry::Carried => {
            let carried = law(&mut complex, "carried standing", OperationSpecies::Construction, vec![], vec![standing], None, vec![
                implementation("Gemma4TextModel.forward (hidden_states = decoder_layer()"),
                implementation("Gemma4TextModel.forward (for i, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):)"),
            ])?;
            realization.bind(carried, Standing { name: CARRIED_STANDING.to_owned() });
            carried
        }
    };
    ...
    realization.bind(token, Enter { population: super::resident_layer::PLE_ENTERING.to_owned(), scale: PLE_EMBED_SCALE });
    ...
    let q = law(&mut complex, "receiver projection", OperationSpecies::Transport, vec![standing], vec![receivers], Some(n("self_attn.q_proj.weight")), vec![
        implementation("Gemma4TextAttention.forward (query_states = self.q_proj(hidden_states).view(hidden_shape))"),
    ...
    let chronology_testimony = |site: &str| {
        ...
        match species {
            Species::Sliding => {
                ...
                t.push(configuration("rope_parameters.sliding_attention.rope_type", "default"));
                t.push(configuration("rope_parameters.sliding_attention.rope_theta", "10000.0"));
            }
            Species::Full => {
                ...
                t.push(configuration("rope_parameters.full_attention.rope_type", "proportional"));
                t.push(configuration("rope_parameters.full_attention.rope_theta", "1000000.0"));
                t.push(configuration("rope_parameters.full_attention.partial_rotary_factor", "0.25"));
            }
        }
        t
    };
    ...
    let (k_in, v_in) = match role {
        KvRole::Own | KvRole::OwnAndStore => {
            let k = law(&mut complex, "presented projection", OperationSpecies::Transport, vec![standing], vec![families], Some(n("self_attn.k_proj.weight")), vec![
                implementation("Gemma4TextAttention.forward (key_states = self.k_proj(hidden_states).view(hidden_shape))"),
        ...
        KvRole::Shared => {
            ...
            let k = law(&mut complex, "shared presented standing", OperationSpecies::Construction, vec![], vec![families], None, vec![
                implementation("Gemma4TextAttention.forward (key_states, value_states = shared_kv_states[self.layer_type])"),
                implementation("Gemma4TextAttention.__init__ (self.is_kv_shared_layer = layer_idx >= first_kv_shared_layer_idx >= 0)"),
                implementation("Gemma4TextAttention.__init__ (first_kv_shared_layer_idx = self.config.num_hidden_layers - getattr(self.config, \"num_kv_shared_layers\", 0))"),
                configuration("num_kv_shared_layers", "18"),
                configuration("num_hidden_layers", "42"),
                configuration(&layer_type_field, species.layer_type()),
            ])?;
            realization.bind(k, Standing { name: k_name.to_owned() });
            let v = law(&mut complex, "shared carried standing", OperationSpecies::Construction, vec![], vec![families], None, vec![
                implementation("Gemma4TextAttention.forward (key_states, value_states = shared_kv_states[self.layer_type])"),
                implementation("Gemma4TextAttention.forward (if self.store_full_length_kv:)"),
                implementation("Gemma4TextAttention.forward (shared_kv_states[self.layer_type] = key_states, value_states)"),
                configuration("num_kv_shared_layers", "18"),
                configuration(&layer_type_field, species.layer_type()),
            ])?;
            realization.bind(v, Standing { name: v_name.to_owned() });
            (k, v)
        }
    };
    ...
    let contact = law(&mut complex, "contact and carried construction", OperationSpecies::Construction, vec![receivers, families, families], vec![receivers], None, contact_testimony)?;
    realization.bind(contact, Contact { heads: HEADS, kv_heads: KV_HEADS, head_width: head, window, terms });
...
pub fn found_final(chart: Chart, sibling: &Intervention, scales_unused: &(DyadicEnclosure, DyadicEnclosure)) -> Result<Founded, String> {
    ...
    realization.bind(carried, Standing { name: CARRIED_STANDING.to_owned() });
    let normed = law(&mut complex, "final rebase", OperationSpecies::Transport, vec![standing], vec![standing], Some(FINAL_NORM.to_owned()),
    ...
    realization.bind(potential, Contract { population: EMBED.to_owned() });
    bond(&mut complex, "tied boundary", standing, normed, potential, 0)?;

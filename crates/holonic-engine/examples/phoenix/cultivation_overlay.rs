...
use holonic_engine::resident_law::MountedPopulation;
use holonic_engine::resident_section::{ResidentGrain, ResidentSection, ResidentSurface};
...
impl RankOneCandidate {
    ...
    pub fn bind<'chart>(
        ...
        let (complex, events) = self.complex(receipt).map_err(|reason| {
            FrontPassageObstruction::Compile(holonic_engine::front_passage::CompileRefusal::Shape(
                reason.to_string(),
            ))
        })?;

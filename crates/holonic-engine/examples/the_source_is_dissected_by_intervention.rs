...
use conduct::{describe, Conducted, Site};
...
use holonic_engine::resident_section::{word_value, Dyadic, ResidentGrain, ResidentSurface, SeriesAperture};
...
fn compare(base: &[(i64, i64)], sibling: &[(i64, i64)], grain: ResidentGrain, width: usize) -> FaceComparison {
    ...
    positions.dedup();
    FaceComparison { entries, identical, separated, widest_gap, widest_shift, positions_separated: positions }
}
...
fn compare_family(base: &Conducted, sibling: &Conducted, grain: ResidentGrain) -> Vec<(Face, FaceComparison)> {
    let mut out = Vec::new();
    for (b, s) in base.layers.iter().zip(&sibling.layers) {
        let head = if b.species == tower::Species::Full { tower::FULL_HEAD } else { tower::SLIDING_HEAD };
        out.push((Face::Ple(b.layer), compare(&b.ple, &s.ple, grain, tower::PLE_WIDTH)));
        out.push((Face::Contact(b.layer), compare(&b.contact, &s.contact, grain, tower::HEADS * head)));
        out.push((Face::Return(b.layer), compare(&b.terminal, &s.terminal, grain, tower::HIDDEN)));
    }
...
fn vision_projection(
    ...
    use holonic_engine::front_passage::{Contract, Enter, EnteringRows, MountedPopulation, ResidentRealization, RmsRebase, WithdrawColumns};
    ...
    let entered = if let Some((from, span)) = withdraw {
        let w = resident_layer::law(&mut complex, "soft-token columns withdrawn (intervention)", OperationSpecies::Quotient, vec![modality_port], vec![modality_port], None,
            vec![resident_layer::intervention(&format!("matched sibling: vision soft-token columns {from}..{} withdrawn before the rebase; the caller's intervention, not a source law", from + span))])?;
        realization.bind(w, WithdrawColumns { from, span });
        resident_layer::bond(&mut complex, "soft tokens withdrawn", modality_port, enter, w, 0)?;
        w
    } else {
        enter
    };
    ...
    resident_layer::bond(&mut complex, "soft tokens rebase", modality_port, entered, rebase, 0)?;
...
fn main() {
    ...
    // ------------------------------------------------------------------------------------------
    println!("\nTHE BASE DEED (fixed predecessor) — {} tokens, chart {:?}, grain 2^-{}, {} series terms", tokens.len(), chart, grain.0, terms.0);
    let base = match conduct::conduct(surface, readout, &mut source, &args.root, &header_regions, &content_sha256, &tokens, grain, terms, chart, Site::Nowhere, &Intervention::None) {
        Ok(c) => c,
        Err(error) => {
    ...
    println!("  {} layers + final in {:.1} s · launches {} · widest charge {} octets · every lineage empty {} · every a-priori bound held {}",
        base.layers.len(), base.wall_s, base.deed_launches, base.peak_charged_octets, base.layers.iter().all(|l| l.lineage_empty), base.layers.iter().all(|l| l.a_priori_held));
    let (base_plural, base_top) = order_face(&base.potential, tokens.len(), grain, 16);
    println!("  order face at the last position: plural top {:?} · top-16 by lower bound {:?}", base_plural, base_top);
    // the base's own determinism: a second base deed must be bit-identical on every face
    let replay = match conduct::conduct(surface, readout, &mut source, &args.root, &header_regions, &content_sha256, &tokens, grain, terms, chart, Site::Nowhere, &Intervention::None) {
        Ok(c) => c,
        Err(error) => {
    ...
    println!("  the replayed base deed: every face bit-identical {replay_identical} ({} faces)", replay_family.len());
    ...
    for taxon in panel(tokens.len()) {
        ...
        let sibling = conduct::conduct(surface, readout, &mut source, &args.root, &header_regions, &content_sha256, &tokens, grain, terms, chart, taxon.site, &taxon.intervention);
        let mut outcome = Outcome { taxon, stood: false, refusal: None, matched: false, intervention_occurrences: 0, controls_held: Vec::new(), first: None, faces_separated: 0, faces_total: 0, potential: None, family: Vec::new(), order_moved: false, plural: Vec::new(), top: Vec::new(), wall_s: 0.0 };
        match sibling {
            Err(error) => {
            ...
            Ok(sibling) => {
                ...
                outcome.potential = family.iter().find(|(f, _)| *f == Face::Potential).map(|(_, c)| c.clone());
                ...
                for control in &outcome.taxon.controls {
                    let (name, held, evidence) = match control {
                        Control::LayersBeforeIdentical(l) => {
                            ...
                            (format!("every face of layers before {l} bit-identical"), held, if held { format!("{} faces, every entry identical", faces.len()) } else { format!("moved: {}", moved.join("; ")) })
                        }
                        Control::EveryLayerIdentical => {
                            let faces: Vec<&(Face, FaceComparison)> = family.iter().filter(|(f, _)| f.layer().is_some()).collect();
                            let held = faces.iter().all(|(_, c)| c.identical == c.entries);
                            (format!("every face of every layer bit-identical"), held, format!("{} layer faces; final normed identical {}", faces.len(), family.iter().find(|(f, _)| *f == Face::FinalNormed).is_some_and(|(_, c)| c.identical == c.entries)))
                        }
                        Control::PositionZeroUnseparated(l) => {
                            ...
                            (format!("layer {l} return at position 0 not separated"), c.separated == 0, format!("{} of {} separated, {} identical, widest shift {:.3e}", c.separated, c.entries, c.identical, rat_f64(&c.widest_shift)))
                        }
                        Control::PositionZeroIdentical(l) => {
                            let (b, s) = (&base.layers[*l], &sibling.layers[*l]);
                            let c = compare(&b.terminal[..tower::HIDDEN], &s.terminal[..tower::HIDDEN], grain, tower::HIDDEN);
                            (format!("layer {l} return at position 0 bit-identical"), c.identical == c.entries, format!("{} of {} identical, {} separated", c.identical, c.entries, c.separated))
                        }
                        Control::OrderFaceUnmoved => {
                            let (plural, top) = order_face(&sibling.potential, tokens.len(), grain, 16);
                            let held = plural == base_plural && top == base_top;
                            ("the order face at the output unmoved".to_owned(), held, format!("plural {:?} top-16 {}", plural, if held { "identical".to_owned() } else { format!("{top:?}") }))
                        }
                    };
                    outcome.controls_held.push((name, held, evidence));
                }
                println!("  stood in {:.1} s · matched sibling {} ({} intervention occurrences) · {} of {} faces separated", sibling.wall_s, outcome.matched, occurrences, outcome.faces_separated, outcome.faces_total);
                match &outcome.first {
                    Some((face, c)) => println!("  shortest separating history: {} — {} of {} separated at positions {:?}, widest gap {:.3e}, widest shift {:.3e}", face.name(), c.separated, c.entries, c.positions_separated, rat_f64(&c.widest_gap), rat_f64(&c.widest_shift)),
                    None => println!("  no face separated"),
                }
                if let Some(c) = &outcome.potential {
                    println!("  potential: {} of {} separated, widest gap {:.3e} · order face moved {} · plural {:?} · top-16 {:?}", c.separated, c.entries, rat_f64(&c.widest_gap), outcome.order_moved, outcome.plural, outcome.top);
                }
                for (name, held, evidence) in &outcome.controls_held {
                    println!("  control: {name} — {} ({evidence})", if *held { "HELD" } else { "MOVED" });
                }
    ...
    if args.vision {
        ...
        let base_v = vision_projection(surface, readout, &mut source, &args.root, &header_regions, &content_sha256, &args.python, &args.out, grain, chart, None);
        let sib_v = vision_projection(surface, readout, &mut source, &args.root, &header_regions, &content_sha256, &args.python, &args.out, grain, chart, Some((0, 256)));
        match (base_v, sib_v) {
            (Ok((bn, rows, vgrain, bnames)), Ok((sn, _, _, snames))) => {
                let c = compare(&bn, &sn, vgrain, tower::HIDDEN);
                let (m, extra, _) = matched(&bnames, &snames);
                vision_line = format!("vision: {rows} soft tokens × 768 → 2560 at grain 2^-{}: sibling matched {m} (+{extra} intervention occurrence); projected rows separated {} of {} entries at {} of {rows} positions, widest gap {:.3e}, {} identical", vgrain.0, c.separated, c.entries, c.positions_separated.len(), rat_f64(&c.widest_gap), c.identical);
    ...
    let caused: Vec<&Outcome> = outcomes.iter().filter(|o| o.stood && o.taxon.dissects != "normalization (gauge)").collect();
    ...
    let controls_all = outcomes.iter().filter(|o| o.stood).flat_map(|o| o.controls_held.iter()).collect::<Vec<_>>();
    ...
    let (gauge_pass, gauge_evidence) = match (gauge, after) {
        (Some(g), Some(a)) if g.stood && a.stood => {
            ...
            for ((fg, cg), (_, ca)) in g.family.iter().zip(&a.family) {
                if cg.widest_gap > widest_g { widest_g = cg.widest_gap.clone(); }
                if ca.widest_gap > widest_a { widest_a = ca.widest_gap.clone(); }
                if cg.separated > 0 {
                    faces_g += 1;
                    if !(ca.separated > 0 && ca.widest_gap > cg.widest_gap) {
                        dominated = false;
                        if worst.is_none() { worst = Some(format!("{}: gauge gap {:.3e} vs after {:.3e}", fg.name(), rat_f64(&cg.widest_gap), rat_f64(&ca.widest_gap))); }
                    }
                }
            }
            let ratio = if widest_g > zero() { rat_f64(&widest_a) / rat_f64(&widest_g) } else { f64::INFINITY };
            let order_held = g.controls_held.iter().filter(|(n, _, _)| n.starts_with("the order face")).all(|(_, h, _)| *h);
            (dominated && order_held,
             format!("gauge (before the rebase): {faces_g} of {} faces separated, widest gap anywhere {:.3e}, order face {}; after the rebase: widest gap anywhere {:.3e} (×{ratio:.2e} wider), first separation at {}; domination at every separated face {dominated}{}", g.family.len(), rat_f64(&widest_g), if order_held { "unmoved" } else { "MOVED" }, rat_f64(&widest_a), a.first.as_ref().map(|(f, c)| format!("{} ({} of {})", f.name(), c.separated, c.entries)).unwrap_or_default(), worst.map(|w| format!("; first failure {w}")).unwrap_or_default()))
        }
    ...
    let reuse_separated = match (k22, k24) {
        (Some(a), Some(b)) => a.stood && b.stood && a.first.as_ref().is_some_and(|(f, _)| f.layer() == Some(22)) && b.first.as_ref().is_some_and(|(f, _)| f.layer() == Some(24)) && b.controls_held.iter().all(|(_, h, _)| *h),
        _ => false,
    };
    ...
    let order: Vec<String> = outcomes.iter().filter(|o| o.stood).map(|o| format!("{} → {}", o.taxon.dissects, if o.order_moved { "moved" } else { "unmoved" })).collect();
    ...
    let shortest: Vec<String> = outcomes.iter().filter(|o| o.stood).map(|o| match &o.first { Some((f, c)) => format!("{} → {} at positions {:?}", o.taxon.dissects, f.name(), c.positions_separated), None => format!("{} → none", o.taxon.dissects) }).collect();
    ...
    // ------------------------------------------------------------------------------------------
    for o in &outcomes {
        form.push(format!("TAXON {} — dissects {} — site {:?} — intervention {:?}", o.taxon.name, o.taxon.dissects, o.taxon.site, o.taxon.intervention));
        form.push(format!("  stood {} · wall {:.1} s · matched sibling {} · intervention occurrences {}{}", o.stood, o.wall_s, o.matched, o.intervention_occurrences, o.refusal.as_ref().map(|r| format!(" · refusal {r}")).unwrap_or_default()));
        form.push(format!("  faces separated {} of {}", o.faces_separated, o.faces_total));
        match &o.first {
        ...
        for (name, held, evidence) in &o.controls_held {
            form.push(format!("  control {} — {name}: {evidence}", if *held { "HELD" } else { "MOVED" }));
        }
    ...
    let path = format!("{}/dissection-{}-tokens-grain-{}-terms-{}.form", args.out, tokens.len(), grain.0, terms.0);
    let mut file = std::fs::File::create(&path).expect("artifact");
    for line in &form {
        writeln!(file, "{line}").expect("write");
    }
    println!("\n  artifact {path}");
    println!("\n{} of {} falsifiers PASS; {} OPEN", verdicts.lines.len() - verdicts.failed, verdicts.lines.len(), verdicts.failed);

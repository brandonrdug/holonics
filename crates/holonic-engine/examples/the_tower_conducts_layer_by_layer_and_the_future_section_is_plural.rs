...
impl Vocabulary {
    fn read(root: &str) -> Result<Self, String> {
        let text =
            std::fs::read_to_string(format!("{root}/tokenizer.json")).map_err(|e| e.to_string())?;
        let json: serde_json::Value = serde_json::from_str(&text).map_err(|e| e.to_string())?;
        let vocab = json
            .get("model")
            .and_then(|m| m.get("vocab"))
            .and_then(|v| v.as_object())
            .ok_or("no model.vocab")?;
...
fn source_face(
    ...
    for line in text.lines() {
        ...
        let position: usize = position
            .parse()
            .map_err(|_| format!("bad position in {line}"))?;
        if let Some(k) = kind.strip_prefix("hidden") {
            let k: usize = k
                .parse()
                .map_err(|_| format!("bad hidden index in {line}"))?;
            let bits = u64::from_str_radix(payload.trim(), 16)
    ...
    Ok(SourceFace {
        hidden,
        logits_last: logits.into_iter().map(|(_, v)| v).collect(),
        surfaces,
    })
}
...
fn same_codeword(enclosure: &[(i64, i64)], grain: ResidentGrain, face: &[Rat]) -> (usize, usize) {
    ...
    for ((lo, hi), value) in enclosure.iter().zip(face) {
        let mid =
            (word_value(*lo, grain) + word_value(*hi, grain)) / Rat::from_integer(BigInt::from(2));
        if let (Ok((native, _)), Ok((theirs, _))) =
            (round_into_bfloat16(&mid), round_into_bfloat16(value))
        {
            total += 1;
            if native == theirs {
                same += 1;
...
fn conduct_tower_inner(
    ...
    for layer in 0..tower::LAYERS.min(limit) {
        ...
        let material_reconciled =
            reconciled.iter().all(|(_, p, m)| p == m) && reconciled.len() == plan.maps.len();
        for (name, region) in mount.regions {
            regions.insert(name, region);
        }
        let identity_held = FileIdentity::at(&format!("{root}/model.safetensors"))
            .ok()
            .is_some_and(|now| {
                header_regions.is_empty()
                    || Some(&now)
                        == FileIdentity::at(&format!("{root}/model.safetensors"))
                            .ok()
                            .as_ref()
            });
        ...
        let every_front_certified = bound
            .fronts()
            .iter()
        ...
        layers.push(LayerReceipt {
            ...
            material_reconciled,
            semantic_bounded: bound
                .admission
                .semantic
                .iter()
                .filter(|c| c.is_bounded())
                .count(),
            semantic_unbounded: bound
                .admission
                .semantic
                .iter()
                .filter(|c| !c.is_bounded())
                .count(),
            apparatus_bounded: bound
                .admission
                .apparatus
                .iter()
                .filter(|c| c.is_bounded())
                .count(),
            apparatus_unbounded: bound
                .admission
                .apparatus
                .iter()
                .filter(|c| !c.is_bounded())
...
fn vision_projection_control(
    ...
    for position in 0..rows {
        ...
        for (i, value) in row {
            ...
            let lo = word_value(lo, grain);
            let hi = word_value(hi, grain);
            if num_traits::Signed::abs(&value) > max_abs {
                max_abs = num_traits::Signed::abs(&value);
            }
            if lo <= value && value <= hi {
                inside += 1;
            } else {
    ...
    for position in 0..rows {
        ...
        let (s, t) = same_codeword(
            &native[position * tower::HIDDEN..(position + 1) * tower::HIDDEN],
            grain,
            &face,
        );
        same += s;
...
fn main() {
    let args = parse_args();
    println!(
        "THE TOWER CONDUCTS LAYER BY LAYER AND THE FUTURE SECTION IS PLURAL — {}",
        args.root
    );
    ...
    if inputs.is_empty() {
        println!(
            "REFUSED: no input — give --text or --tokens; the material is supplied at runtime and compiled into nothing"
        );
        std::process::exit(2);
    }
    ...
    for (text, ids, _) in &inputs {
        ...
        match conduct_tower(
            ...
            Ok(returned) => {
                println!(
                    "  {} layer deeds + the final deed in {:.1} s · deed launches {} · tower semantic work {:?} · widest charge of one deed {} octets · the surface's own peak {} octets",
                    returned.layers.len(),
                    returned.wall_s,
                    returned.deed_launches,
                    returned
                        .tower_work
                        .coordinates()
                        .iter()
                        .map(|(n, v)| format!("{n}={v}"))
                        .collect::<Vec<_>>(),
                    returned.peak_charged_octets,
                    returned.peak_resident_octets
                );
                for l in returned.layers.iter().filter(|l| {
                    l.layer == 0 || l.layer == 22 || l.layer == 23 || l.layer == 24 || l.layer == 41
                }) {
                    println!(
                        "    layer {:>2} {:?} {:?}: {} operations · {} fronts · material {} octets (charged {}, reconciled {}) · graph {}/{} · launches {} · certified {} · a-priori held {} (slack {}) · lineage empty {} · widest enclosure {:.3e} · mount {:.2} s bind {:.3} s",
                        l.layer,
                        l.species,
                        l.role,
                        l.operations,
                        l.fronts,
                        l.material_resident_octets,
                        l.material_charged_octets,
                        l.material_reconciled,
                        l.graph_nodes,
                        l.graph_edges,
                        l.captured_launches,
                        l.every_front_certified,
                        l.a_priori_held,
                        l.widest_slack,
                        l.lineage_empty,
                        rat_f64(&l.widest_enclosure),
                        l.mount_wall_s,
                        l.bind_wall_s
                    );
                }
                let future = future_section(
                    &returned.potential,
                ...
                println!(
                    "  THE PLURAL FUTURE SECTION at position {}: top lower bound {:.4} · {} candidates not separated from the top · {} separated",
                    future.position,
                    rat_f64(&future.top_lower),
                    future.plural.len() + 0,
                    future.separated
                );
    ...
    if let Some(first) = returns.first() {
        println!(
            "  collapsed population under the {:?} chart (layer: quotients, sum of widths in grains, widest, nonzero): {}",
            args.chart,
            first
                .layers
                .iter()
                .filter(|l| l.layer % 7 == 0 || l.layer == 41)
                .map(|l| format!(
                    "{}: {} q · Σ {} · max {} · nz {}",
                    l.layer,
                    l.quotients,
                    l.collapsed_width_sum,
                    l.collapsed_width_max,
                    l.collapsed_nonzero
                ))
                .collect::<Vec<_>>()
                .join(" | ")
    ...
    if !args.skip_interval_control {
        ...
        match conduct_tower(
            ...
            Ok(returned) => {
                println!(
                    "  the interval chart carried every layer (unexpected at this grain); widest terminal enclosures: {:?}",
                    returned
                        .layers
                        .iter()
                        .map(|l| rat_f64(&l.widest_enclosure))
                        .collect::<Vec<_>>()
                );
                interval_control = Some((
                    "carried".to_owned(),
                    returned.layers[0].widest_enclosure.clone(),
                    returned
                        .layers
                        .last()
                        .map(|l| l.widest_enclosure.clone())
                        .unwrap_or_else(|| Rat::from_integer(BigInt::from(0))),
                    returned.layers.len(),
                ));
            }
            Err((error, partial)) => {
                ...
                let layer0 = partial
                    ...
                    .map(|l| l.widest_enclosure.clone())
                    .unwrap_or_else(|| Rat::from_integer(BigInt::from(0)));
                let amplification = if layer0 > Rat::from_integer(BigInt::from(0)) {
                    &layer0 / &entering
                } else {
                    Rat::from_integer(BigInt::from(0))
                };
                println!(
                    "  the interval chart refused after {} layer(s): {}",
                    partial.len(),
                    error.chars().take(300).collect::<String>()
                );
                println!(
                    "  layer 0's widest certified enclosure {:.3e} against the entering grain {:.3e}: amplification ≈ 2^{:.1}",
                    rat_f64(&layer0),
                    rat_f64(&entering),
                    rat_f64(&amplification).log2()
                );
    ...
    let every_layer_whole = returns.iter().all(|r| {
        r.layers.len() == tower::LAYERS
            && r.layers.iter().all(|l| {
                l.lineage_empty
                    && l.every_front_certified
                    && l.a_priori_held
                    && l.material_reconciled
                    && l.semantic_unbounded + l.semantic_bounded == 9
                    && l.apparatus_bounded >= 5
            })
    });
    ...
    let species_seen: BTreeSet<String> = first
        .layers
        .iter()
        .map(|l| format!("{:?}", l.species))
        .collect();
    let roles_seen: BTreeSet<String> = first
        .layers
        .iter()
        .map(|l| format!("{:?}", l.role))
        .collect();
    let shared_ops = first
        .layers
        .iter()
        .filter(|l| l.role == KvRole::Shared)
        .map(|l| l.operations)
        .collect::<BTreeSet<_>>();
    let own_ops = first
        .layers
    ...
    let read_octets: u64 = returns
        .iter()
        .map(|r| {
            r.layers
                .iter()
                .map(|l| (2 * l.terminal.len() * 8) as u64)
                .sum::<u64>()
                + (2 * r.final_normed.len() * 8) as u64
                + (2 * r.potential.len() * 8) as u64
        })
        .sum();
    let egress: u64 = returns
        .iter()
        .map(|r| r.census_after.egress_section_octets - r.census_before.egress_section_octets)
        .sum();
    let d2d: u64 = returns
        .iter()
        .map(|r| r.census_after.device_to_device_octets - r.census_before.device_to_device_octets)
        .sum();
    ...
    let futures: Vec<FutureSection> = returns
        .iter()
        .map(|r| future_section(&r.potential, r.positions - 1, r.vocabulary, grain, args.top))
        .collect();
    let distinct_final: BTreeSet<Vec<(i64, i64)>> =
        returns.iter().map(|r| r.final_normed.clone()).collect();
            ...
            Ok(output) => {
                ...
                verdicts.record(
                    ...
                    stdout.contains("REFUSED")
                        && !stdout.contains("THE PLURAL FUTURE")
        ...
        for (r, (text, ..)) in returns.iter().zip(&inputs) {
            ...
            match source_face(&args.python, &r.tokens, &emit, args.top) {
                Ok(face) => {
                    ...
                    for l in &r.layers {
                        ...
                        if let Some(rows) = face.hidden.get(&(l.layer + 1)) {
                            if let Some(row) = rows.get(last) {
                                ...
                                let (inside, outside, worst) =
                                    defect(&l.terminal[from..from + tower::HIDDEN], grain, row);
                                let (same, total) = same_codeword(
                                    &l.terminal[from..from + tower::HIDDEN],
                                    grain,
                                    row,
                                );
                    ...
                    let normed_vs_last_hidden = face
                        .hidden
                        .get(&tower::LAYERS)
                        .and_then(|rows| rows.get(last))
                        .map(|row| {
                            defect(
                                &r.final_normed[last * tower::HIDDEN..(last + 1) * tower::HIDDEN],
                                grain,
                                row,
                            )
                        });
                    let normed_same = face
                        .hidden
                        .get(&tower::LAYERS)
                        .and_then(|rows| rows.get(last))
                        .map(|row| {
                    ...
                    let codeword_agreement: Vec<String> = per_layer
                        .iter()
                        .filter(|(l, ..)| *l == 0 || *l == 10 || *l == 20 || *l == 30 || *l == 41)
                        .map(|(l, _, _, _, same, total)| format!("layer {l}: {same}/{total}"))
                        .collect();
                    ...
                    let source_top = face
                        ...
                        .map(|(i, _)| i);
                    let source_top_in_plural =
                        source_top.is_some_and(|t| future.plural.iter().any(|(id, ..)| *id == t));
                    let source_top_surface = face
                        .surfaces
                        .first()
                    ...
                    let inside_plural = top_ids
                        .iter()
                        .filter(|id| future.plural.iter().any(|(p, ..)| p == *id))
                        .count();
                    let line = format!(
                        "{text:?}: per-layer (layer, inside, outside, worst gap) first {:?} · layer 41 {:?} · final normed vs hidden42 {:?} · same bf16 codeword as the source at the last position: {} · final normed {:?} · source top {source_top_surface} in the plural section {source_top_in_plural} · {inside_plural} of the source's top {} inside the plural section ({} candidates) · face in {:.1} s",
                        per_layer
                            .iter()
                            .take(2)
                            .map(|(l, i, o, w, ..)| (*l, *i, *o, rat_f64(w)))
                            .collect::<Vec<_>>(),
                        per_layer
                            .last()
                            .map(|(l, i, o, w, ..)| (*l, *i, *o, rat_f64(w))),
                        normed_vs_last_hidden
                            .as_ref()
                            .map(|(i, o, w)| (*i, *o, rat_f64(w))),
                        codeword_agreement.join(", "),
                        normed_same,
                        top_ids.len(),
                        future.plural.len(),
                        face_clock.elapsed().as_secs_f64()
                    );
    ...
    match &interval_control {
        ...
        None => verdicts.open(
            11,
            "the certified interval enclosure's divergence",
            "not run (--no-interval-control)",
        ),
    }
    ...
    if args.vision_control {
        ...
        verdicts.open(
            13,
            "one real non-text projection as the bounded portability control",
            "not run (--no-vision-control)",
        );
    }
    ...
    for name in [
        tower::EMBED,
        resident_layer::PLE_EMBED,
        resident_layer::PLE_MODEL_PROJECTION,
        resident_layer::PLE_PROJECTION_NORM,
        tower::FINAL_NORM,
    ] {
    ...
    let text_populations: Vec<String> = source
        .container
        .names()
        .into_iter()
        ...
        .map(str::to_owned)
        .collect();
    let crossed = text_populations
        .iter()
        .filter(|n| stimulated.contains(*n))
        .count();
    let unbound_present = text_populations
        .iter()
        .filter(|n| unbound.contains(*n))
        .count();
    let not_crossed: Vec<&String> = text_populations
        .iter()
        .filter(|n| !stimulated.contains(*n) && !unbound.contains(*n))
        .collect();
    let excited_rows: BTreeSet<usize> = inputs
        .iter()
        .flat_map(|(_, ids, _)| ids.iter().copied())
        .collect();
    let coverage_line = format!(
        "text-tower populations {}: stimulated (caused material crossed them) {crossed} · unbound by the source realization {unbound_present} · not crossed {} {:?} · vocabulary rows excited by these inputs {} of {} ({} unexcited — a measured class, not a condensation)",
        text_populations.len(),
        not_crossed.len(),
        not_crossed.iter().take(6).collect::<Vec<_>>(),
        excited_rows.len(),
        tower::VOCABULARY,
        tower::VOCABULARY - excited_rows.len()
    );
    verdicts.record(14, "every text population carries a coverage status after the deeds — stimulated, unbound by the source, or not crossed — and the vocabulary rows the inputs excited are counted against the unexcited",
        ...
        coverage_line.clone());
    for (number, name) in [
        (15u32, "native rest, runtime and frozen inference"),
        (16, "condensation"),
        (17, "cultivation"),
        (18, "reborn dissection"),
    ] {
    ...
    writeln!(
        file,
        "THE TOWER CONDUCTS LAYER BY LAYER AND THE FUTURE SECTION IS PLURAL — receipt"
    )
    .unwrap();
    writeln!(
        file,
        "root {} · grain 2^-{} · terms {} · chart {:?} · device {} · mode {:?}",
        args.root,
        args.grain,
        args.terms,
        args.chart,
    ...
    for (r, (text, ids, pieces)) in returns.iter().zip(&inputs) {
        ...
        writeln!(
            file,
            "  collapsed population per layer (quotients, Σ widths, widest, nonzero): {:?}",
            r.layers
                .iter()
                .map(|l| (
                    l.layer,

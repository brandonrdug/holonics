...
fn main() {
    ...
    // ---------------------------------------------------------------------------------------
    println!();
    println!("-- repetition against breadth: what one walk can reach and what a family can --");
    // Repeating one loop returns hol(g)^n, so repetition explores at most the CYCLIC SUBGROUP
    // generated by that one element and saturates at its order. A family of walks generates the
    // group its holonomies generate. The gap between them is exactly the non-commutativity, which
    // is why the abelian control below must show NO gap.
    let family = twisted.plaquette_holonomies().expect("the walks close");
    let breadth = StructureGroup::close(family.clone(), 64).expect("the family closes");
    println!(
        "  the family carries {} walk holonomies and closes at order {}",
        family.len(),
        breadth.order()
    );
    let mut widest = 1usize;
    for (at, single) in family.iter().enumerate() {
        let repeated =
            StructureGroup::close([single.clone()], 64).expect("one holonomy closes cyclically");
        if repeated.order() > widest {
            widest = repeated.order();
        }
        println!(
            "  walk {at} repeated forever reaches order {:<3} abelian {}  ({})",
            repeated.order(),
            repeated.is_abelian(),
            written(single)
        );
    ...
    println!(
        "  best repetition reaches {widest}; breadth reaches {}",
        breadth.order()
    );
    assert!(
        breadth.order() >= widest,
        "breadth can never reach less than the best single loop"
    );
    if breadth.is_abelian() {
        println!("  the family is ABELIAN, so breadth buys nothing the generators did not carry");
    } else {
        assert!(
            breadth.order() > widest,
            "on a non-abelian family breadth must strictly exceed every repetition"
        );
        println!(
            "  the family is NON-ABELIAN, and breadth strictly exceeds every repetition by {}",
            breadth.order() - widest
        );
    }

    // The control, and it is the one that stops the instrument measuring itself: on an abelian
    // connection the two must COINCIDE. A run reporting breadth as a gain here would be reading
    // its own construction rather than the material.
    println!();
    println!("-- control 5: breadth on an abelian connection buys nothing --");
    let abelian_family = abelian.plaquette_holonomies().expect("the walks close");
    let abelian_breadth =
        StructureGroup::close(abelian_family.clone(), 64).expect("the abelian family closes");
    let abelian_widest = abelian_family
        .iter()
        .map(|single| {
            StructureGroup::close([single.clone()], 64)
                .expect("one holonomy closes")
                .order()
        })
        .max()
        .unwrap_or(1);
    println!(
        "  abelian: breadth {} against best repetition {}",
        abelian_breadth.order(),
        abelian_widest
    );
    assert!(
        abelian_breadth.is_abelian(),
        "the control's own family must be abelian or it is not a control"
    );
    assert_eq!(
        abelian_breadth.order(),
        abelian_widest,

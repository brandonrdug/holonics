...
fn mount_l3(root: &Path) -> Result<NativeTerrainRest, String> {
    let directory = root.join(
        ".local/artifacts/the_athena_holonics_mathematics_ecology_works_from_its_own_rested_terrain/native-rest",
    );
    mount_written(
...
fn run_lean(
    ...
    let returned = Command::new("lake")
        .args(["env", "lean"])
        .arg(proof)
        .current_dir(root.join("formal/elementary-holonics"))
        .output()
        .map_err(|error| error.to_string())?;
    if !returned.status.success() {
        return Err(format!(
...
fn complete_cost(
    ...
    let l1 = root.join(
        ".local/artifacts/returned_theorem_families_cultivate_the_continuing_laboratory_rest/native-rest",
    );
    ...
    let inherited = root.join(".local/artifacts/the_whole_foreign_map_crosses_into_native_rest");
    Ok(json!({
...
fn purity_receipt(root: &Path) -> Result<Value, String> {
    let files = [
        root.join("crates/holonic-life/src/mathematical_particle/production_aperture/native_terrain.rs"),
        root.join(
            "crates/holonic-life/src/mathematical_particle/production_aperture/native_terrain_types.rs",
        ),

...
pub fn emit(returned: &PhysicalFoldReturn) -> Result<ArtifactReturn, String> {
    ...
    let output = repository.join(".local/artifacts/the_physical_fold_returns_as_an_exact_constraint_complex");
...
fn lean_return(repository: &Path) -> Result<LeanReturn, String> {
    let formal = repository.join("formal/elementary-holonics");

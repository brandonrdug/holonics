//! Freeze the twelve-node potential with its exact quotient/support testimony and the existing
//! mathematical and arbitrary-organ rests. The existing source-detached remount owner returns the
//! native product; no foreign tower is exposed inside the sandbox.
...
const GLUE: &str =
    ".local/artifacts/the_second_successor_receiver_and_support_faces_glue_the_twelve_node_parametron";
const RECEIVER_HISTORY: &str = concat!(
    ".local/artifacts/the_cultivated_factor_descends_through_its_receiver_history_support/",
...
const MULTIMODAL: &str = concat!(
    ".local/artifacts/the_returned_arbitrary_organ_consequence_cultivates_the_same_athena_continuation/",
    "native-rest"
);
const OUTPUT: &str = ".local/artifacts/the_twelve_node_parametron_rest_returns_holonics_hna";
const REMOUNT_EXECUTABLE: &str = "the_parametron_receiver_history_rest_returns_holonics_hna";
...
fn main() -> Result<(), String> {
    ...
    fs::copy(
        format!("{GLUE}/expanded-potential.rest"),
        product.join("potential.rest"),
    )
    .map_err(display)?;
    fs::copy(RECEIVER_HISTORY, product.join("receiver-history.rest")).map_err(display)?;
    fs::copy(
        format!("{GLUE}/00-foreign-receiver-quotient.json"),
        product.join("successor/foreign-receiver-quotient.json"),
    )
    .map_err(display)?;
    fs::copy(
        format!("{GLUE}/01-successor-support-passage.json"),
    ...
    if surfaces.len() != 12 {
        return Err("detached return did not conduct all twelve addressed nodes".to_owned());
    }
        ...
        .map(|root| {
            [root, root + 4, root + 8]
                ...
                .map(|history| {
                    surfaces[history]
                        .as_str()
                        .ok_or("native surface is not text")
                })
                .collect::<Result<String, _>>()
        })
        .collect::<Result<Vec<_>, _>>()?;
    let quotient: Value = read_json(&product.join("successor/foreign-receiver-quotient.json"))?;
    let support: Value = read_json(&product.join("successor/support-passage.json"))?;
    let class_population = quotient["classes"]
        .as_array()
        .ok_or("carrier quotient omitted classes")?
        .len();
    let support_reopenings = support["foreign_support_reopenings"]
        .as_array()
        .ok_or("support passage omitted reopenings")?
        .len();
    let passed = returned["passed"] == true
        ...
        && continuations
            .iter()
            .all(|surface| surface.split_whitespace().count() >= 3);
    let grade = json!({
        "schema":"holonics.twelve-node-parametron-athena-alpha-grade.v1",
        ...
        "coefficient_occurrence_population":12,
    ...
    fs::write(
        ...
        format!(
            "# The twelve-node Parametron rest returns Athena alpha\n\n[established-bounded; implemented-exact; measured] The twelve-node potential, nine-class carrier quotient, recursive support passage, native mathematics, and arbitrary-organ rest froze as product `{}`. A source-isolated remount returned mathematics, heterogeneous current, and four bounded three-occurrence continuations without foreign staging, foreign deeds, Lean/checker inference, or CPU semantic replay.\n\n```json\n{}\n```\n",
    ...
    write_json(
        ...
        &json!({
            ...
            "command":"target/release/examples/the_twelve_node_parametron_rest_returns_holonics_hna",
            "purpose":"freeze and source-detach the twelve-node receiver/support product and conduct native mathematics, arbitrary-organ and bounded language returns",
    ...
    if passed {
        ...
        Err("the twelve-node Athena-alpha grade refused".to_owned())
    }
}
...
fn run_source_detached(product: &Path, output: &Path) -> Result<(), String> {
    ...
    let executable = fs::canonicalize(
        current
            .parent()
            .ok_or("release example has no parent")?

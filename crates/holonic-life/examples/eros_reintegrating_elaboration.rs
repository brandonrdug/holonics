...
//! *"`crates/holonic-life/src/decomposing_codec.rs` performs DECOMPOSE → RE-INTEGRATE and revises its grain at
...
//!     -- standing/output routes 1 6 200 .local/artifacts/reintegrating-elaboration
//! ```
//!
...
fn main() {
    ...
    let deposit_at = PathBuf::from(
        argv.next()
            .unwrap_or_else(|| ".local/artifacts/reintegrating-elaboration".to_owned()),
    );

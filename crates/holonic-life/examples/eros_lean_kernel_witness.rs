...
//! `formal/kernel-witness` — the one formal project in this tree that requires no package —
...
//! the one named above.
//!
//! The toolchain is recorded beside the verdict. `formal/kernel-witness/lean-toolchain` pins
...
fn main() {
    ...
    let scratch_root = root.join(".local/artifacts/lean-kernel-witness");

...
fn main() -> Result<(), String> {
    ...
    write_json(
        ...
        &json!({
            ...
            "lean_source": "formal/elementary-holonics/ElementaryHolonics/Millennium/AthenaReceiverHistory.lean",
...
fn arguments() -> Result<(PathBuf, PathBuf), String> {
    let mut aperture = PathBuf::from(
        ".local/artifacts/the_complete_laboratory_exchange_returns_for_athena_alpha/04-continuation-aperture.json",
    );
    let mut output =
        PathBuf::from(".local/artifacts/the_receiver_history_congruence_replaces_the_trigram_table");

...
//!    `research/records`, `research/papers/source`, `soma`, `crates`. A symlink fixture maps live material
...
//!
//! The laboratory read `.local/runs/symbolic-language-agent/TEXT_REST.json`, written by
//! `eros_symbolic_language_agent`. `.local/runs/` was never tracked — zero files exist under it at any
...
/// `MorphologicalGenerationSpec` (`crates/holonic-life/src/morphological_language.rs:173-176`) has exactly one
...
const DECLARED_WORLD_RUST: &[&str] = &[
    "crates/holonic-life/src/relational_language/types.rs",
    "crates/holonic-life/src/relational_language/transport.rs",
    "crates/holonic-life/src/agentic_research/open_completion.rs",
    "crates/holonic-life/src/dialogue_lineage.rs",
    "crates/holonic-life/src/research_intelligence.rs",
];
...
fn build_declared_fixture(
    ...
    for (named, at) in DECLARED_WORLD_THEORY
        ...
        .chain(
            DECLARED_WORLD_RUST
                .iter()
                .map(|named| (*named, "crates/holonic-life/src")),
        )
    {
...
fn repository_root() -> PathBuf {
    // `crates/holonic-life` -> the repository root is two levels up, not three: the laboratory kept its life
    // crate at `src/crates/holonic-life`.

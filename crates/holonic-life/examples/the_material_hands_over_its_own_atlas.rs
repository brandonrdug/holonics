...
fn run() -> Result<(), String> {
    ...
    println!(
        "  nothing in `crates/holonic-life/src/incidence_production.rs` is edited; the constructor lives \
...
fn lean_sweep(workspace: &Path) -> Result<Swept, String> {
    let root = workspace.join("formal");
    let mut paths = Vec::new();
    collect(&root, "lean", &mut paths)?;
    paths.sort();
    println!("LEAN — {} containers under formal", paths.len());
...
fn workspace_root() -> Result<PathBuf, String> {
    ...
    manifest
        ...
        .ok_or_else(|| "the workspace root is not above crates/holonic-life".to_owned())
}

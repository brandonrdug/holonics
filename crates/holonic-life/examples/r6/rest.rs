...
pub fn found(root: &Path) -> Result<ProductionEcologyRest, String> {
    let i5 = root.join(".local/artifacts/the_athena_gemma_ecology_infers_returns_and_remounts/native-rest");
    ...
    let morphology = DynamicMorphologyRest::read(&read(root.join(
        ".local/artifacts/the_returned_constraints_found_dynamic_local_morphology/04-source-detached-dynamic-morphology-rest.json",
    ))?)
    .map_err(|error| error.to_string())?;
    let r4 = root
        .join(".local/artifacts/the_retained_causal_boundary_carries_the_long_horizon_inquiry/native-rest");
    ...
    .map_err(|error| error.to_string())?;
    let r5 =
        root.join(".local/artifacts/the_mathematical_and_physical_faces_share_native_transport/native-rest");

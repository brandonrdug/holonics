...
//!
//! `LeanMathematicsEcology` (`crates/holonic-life/src/lean_mathematics.rs`, `.../lean_mathematics/ecology.rs`)
...
//!
//! ```text
//!   formal/**.lean                    real repository Lean sources
...
//! package cache is not materialized under `formal`, so no verdict is available for a theorem
...
//!
//! The kernel loop below still runs in `formal/kernel-witness` because it is fast and offline,
...
//! `formal/elementary-holonics` to `LeanKernelWorld::new`, which has always taken the project
...
fn main() {
    ...
    let corpus_root = root.join("formal");
    let project_root = root.join("formal/kernel-witness");
    let scratch_root = root.join(".local/artifacts/lean-proof-production");
    ...
    if let Some(candidates) = mathlib_candidates.as_ref() {
        ...
        let mathlib_project = root.join("formal/rh-source-transport");
        ...
        let core_frame = LeanKernelWorld::new(
            &project_root,
            root.join(".local/artifacts/lean-proof-production-mathlib-core-frame"),
            mathlib_worker_aperture,
        )
        .expect("the core-frame kernel world constructs");
        let mathlib_frame = LeanKernelWorld::new(
            &mathlib_project,
            root.join(".local/artifacts/lean-proof-production-mathlib-frame"),
            mathlib_worker_aperture,
        )
    ...
    // geometry lemmas and the verdicts are about geometry.
    {
        use life::holonic_training::FiberAdmission;
        use life::lean_mathematics::kernel_returns::motion_admissions;
        let geometry_project = root.join("formal/elementary-holonics");
        ...
        let world = LeanKernelWorld::new(
            &geometry_project,
            root.join(".local/artifacts/lean-proof-production-swing"),
            4,
        );
        match world {

...
pub const DEFAULT_OUT: &str =
    ".local/artifacts/the_material_derivation_recurs_until_the_requested_receiver_returns_or_obstructs";
const R1_PRODUCT: &str = ".local/artifacts/the_rich_intake_returns_one_codec_neutral_operation_world_tube";
const R0_RECEIVERS: &str = ".local/artifacts/the_rich_mathematical_inquiry_returns_the_i5_baseline_boundary/02-receiver-basis-and-controls.json";
...
pub fn construct(root: &Path, out: &str) -> Result<(), String> {
    ...
    let lean_return =
        artifact::run_lean(&root.join("formal/elementary-holonics"), &lean_path)?;

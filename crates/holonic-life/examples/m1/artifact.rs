...
fn resolve_output(root: &Path, default_out: &str) -> Result<PathBuf, String> {
    ...
    {
        return Err(
            "the M1 output must be .local/artifacts/the_mathematical_particle_is_an_addressed_passage"
                .to_owned(),
        );
    }
    Ok(out)
...
fn history_reading(reading: &life::causal_section::CausalSectionReading) -> Value {
    json!({
        ...
        "shortest_separators": reading.shortest_separators.iter().map(|separator| json!({
            ...
            "interventions": separator.distinguishing_word,
...
fn closure(root: &Path, artifacts: &[ArtifactIdentity]) -> Result<Vec<Value>, String> {
    let mut paths = BTreeSet::from([
        ...
        "Cargo.toml".to_owned(),
        M0_RETURN.to_owned(),
        "crates/holonic-life/Cargo.toml".to_owned(),
        "crates/holonic-engine/Cargo.toml".to_owned(),
        "crates/holonic-engine/build.rs".to_owned(),
        "crates/holonic-life/examples/the_affine_swing_is_a_typed_mathematical_particle.rs".to_owned(),
    ]);
    collect_extension(root, "crates/holonic-life/examples/m1", "rs", &mut paths)?;
    collect_extension(root, "crates/holonic-life/src", "rs", &mut paths)?;

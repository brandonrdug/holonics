...
pub fn mount_l2(root: &Path) -> Result<NativeHexisRest, String> {
    let directory =
        root.join(".local/artifacts/recurring_laboratory_transport_condenses_into_native_hexis/native-rest");
...
pub fn write(
    ...
    if reopened
        ...
        != rest
            ...
            .map_err(|error| error.to_string())?
    {
        return Err("the L3 rest did not reopen as the same ecology".to_owned());
    }

...
pub fn manifest(directory: &Path) -> Result<Value, String> {
    ...
    for path in files {
        ...
        roll.update(relative.as_bytes());
        roll.update((bytes.len() as u64).to_le_bytes());
        roll.update(sha256.as_bytes());
        identities.push(json!({"path": relative, "octets": bytes.len(), "sha256": sha256}));
    }
    let returned = json!({
        "schema": "holonics.l3.product-manifest.v1",
        "truth_status": "measured",
        "files": identities,
        "rolled_sha256": roll.finalize().iter().map(|octet| format!("{octet:02x}")).collect::<String>(),
    });

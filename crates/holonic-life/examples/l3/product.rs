...
pub fn construct(root: &Path) -> Result<(), String> {
    ...
    let l2_source = read_json(
        &root.join(".local/artifacts/recurring_laboratory_transport_condenses_into_native_hexis/00-matched-cultivated-source-controls.json"),
    )?;
    let l2_condensed = read_json(
        &root.join(".local/artifacts/recurring_laboratory_transport_condenses_into_native_hexis/heldout-return/00-return.json"),
    )?;
    let anatomy = json!({

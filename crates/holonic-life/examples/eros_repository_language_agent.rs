...
//! `ba8716b5:src/crates/holonic-life/examples/eros_repository_language_agent.rs` (1,450 lines). Deleted at
//! `a07ff376`; never imported by `06518c3` ("Transition to Rust").
//!
//! ## Ported changes, each named
//!
//! 1. **Repository root.** The laboratory kept its life crate at `src/crates/holonic-life`, so the driver
//!    resolved the root as `CARGO_MANIFEST_DIR/../../..`. Here it is `crates/holonic-life`, so the root is
//!    `../..`.
...
//! `crates/holonic-life/src/laboratory_language/repository.rs`, surviving as a driver-local copy. It is kept
...
fn main() -> Result<(), String> {
    ...
    let output = root.join(".local/runs/agentic-language");

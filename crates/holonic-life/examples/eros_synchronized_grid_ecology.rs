...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let workspace = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../..")
        .canonicalize()?;
    let dataset = workspace.join(".local/runs/information-flow-datasets/source/GRID");
    let output =
        workspace.join(".local/runs/synchronized-grid-factorized-structural-fiber-information-flow");
    ...
    // still reported; this helper retains the native rest as a content-addressed form artifact.
    // The ERST plate reader was retired in R2.

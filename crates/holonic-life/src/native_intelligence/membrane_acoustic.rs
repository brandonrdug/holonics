...
use super::{
    GranularMembraneStanding, LaboratoryCellAffineSection, LaboratoryFactorCycleCorrespondence,
    MembraneStanding, NativeGranularPotential, NativeOpenWorldTubeReceipt,
    NativeOpenWorldTubeTerminal, NativeOutwardPortReturn, ReceiverHistoryRealizationPassage,
    RecurrentGranularRelationalCellWithdrawal, RecurrentGranularReturnedAffineEcologyRest,
...
const ACOUSTIC_PRODUCT_REST_SCHEMA: &str = "soma-life.acoustic-product-rest.v1";
...
#[derive(Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AcousticProductRest {
    schema: String,
    body: RecurrentGranularReturnedAffineEcologyRest,
...
impl AcousticProductRest {
    pub fn cultivate(
        ...
        let mut rest = Self {
            schema: ACOUSTIC_PRODUCT_REST_SCHEMA.to_owned(),
            body,
            production,
            identity_sha256: String::new(),
        };
    ...
    pub fn body(&self) -> &RecurrentGranularReturnedAffineEcologyRest {
        &self.body
    }
    ...
    pub fn withdraw_production(
        ...
        (
            RecurrentGranularReturnedAffineEcologyRest,
    ...
    pub fn validate(&self) -> Result<(), NativeAcousticError> {
        ...
        if self.schema != ACOUSTIC_PRODUCT_REST_SCHEMA
            || self.identity_sha256 != self.rederived_identity()?
        {
            return Err(NativeAcousticError::Lineage);
        }
...
impl MembraneStanding for AcousticProductRest {
...
impl GranularMembraneStanding for AcousticProductRest {
    fn membrane_granular_potential(&self) -> &NativeGranularPotential {
        self.body.granular_potential()
    }
}

impl MembraneDifferenceStanding for AcousticProductRest {
    type Successor = Self;

...
//!
//! ```text
//!   a driver  ──seal──▶  octets  ──▶  .local/artifacts/<driver>/<name>-<sha256>.form  ──▶  holon-plate deposit
//!                          │                          ▲
...
//!
//! `THE_ASSEMBLY.md:72` gives the shape `.local/artifacts/<driver>/<name>.form`; `THE_ASSEMBLY.md:108`
...
//!
//! ```text
//!   .local/artifacts/<driver>/<name>-<sha256 of the octets>.form
...
pub const DEPOSIT_ROOT: &str = ".local/artifacts";
...
mod tests {
    ...
    /// test writes into the repository's real `.local/artifacts/`.
    ///
    ...
    // `applications/holon-plate/tests/plate_mouth.rs`, from a working directory it owns.
    ...
    fn the_production_entry_point_composes_under_the_deposit_root() {
        ...
        match refusal {
            FormMouthRefusal::FormEmpty { under, name } => {
                ...
                assert_eq!(under, PathBuf::from(".local/artifacts/eros_text_training"));
        ...
        assert_eq!(
            ...
            PathBuf::from(format!(
                ".local/artifacts/eros_text_training/machine-rest-{ABC_ADDRESS}.form"
            ))
        );
    }
    ...
    fn the_message_carrying_entry_point_refuses_what_the_law_refuses() {
        let message = deposit_form_or_message("eros_text_training", "machine-rest", b"")
            .expect_err("zero octets are not a form");
        assert!(
            message.contains(".local/artifacts/eros_text_training"),
        ...
        // founded — `..` here would otherwise compose `.local/artifacts/../rest-<address>.form`
    ...
    fn a_read_back_refusal_renders_both_lengths_and_its_path() {
        let rendered = FormMouthRefusal::NotReadBackIdentical {
            path: PathBuf::from(".local/artifacts/d/r-0123.form"),
            written: 900,
            read_back: 128,
        }
        .to_string();
        ...
        assert!(rendered.contains(".local/artifacts/d/r-0123.form"), "{rendered}");
    ...
    fn an_empty_form_refusal_renders_the_site_and_the_directory() {
        let rendered = FormMouthRefusal::FormEmpty {
            under: PathBuf::from(".local/artifacts/eros_text_training"),
            name: "machine-rest".to_owned(),
        }
        .to_string();
        assert!(rendered.contains(".local/artifacts/eros_text_training"), "{rendered}");

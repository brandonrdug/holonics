...
//!
//! `crates/holonic-life/src/incidence_production.rs` founds `L_t = (K_t, ∂_t, o_t, ⪯_t, Γ_t)` from
...
/// denoted-value equality, which is exactly what `docs/canon/THE_HOLOBROCHOS_SPINE.md` §2 retains.
...
/// `crates/holonic-life/src/incidence_production.rs` has said since it was written that this is not its job:
///
...
/// `grep -n "receiver_exact_compression\|ReceiverExactCompression\|compress(" crates/holonic-life/src/material_incidence.rs`,

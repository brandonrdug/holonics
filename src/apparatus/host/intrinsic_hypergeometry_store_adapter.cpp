...
[[nodiscard]] intrinsic_hypergeometry_store_status read_exact(
  ...
  return closed && trailing == 0 ? intrinsic_hypergeometry_store_status::returned :
      intrinsic_hypergeometry_store_status::size_refused;
}

template<class Record>
[[nodiscard]] intrinsic_hypergeometry_store_status write_exact(
  ...
  return ::close(descriptor) == 0 ? intrinsic_hypergeometry_store_status::returned :
      intrinsic_hypergeometry_store_status::transfer_refused;
}

}  // namespace

intrinsic_hypergeometry_store_receipt read_causal_linear_handoff(
    const char* path, event::causal_linear_rest_record& record) noexcept {
  intrinsic_hypergeometry_store_receipt receipt{}; receipt.state = read_exact(path, record);
  ...
  if (!receipt.integrity_exact) {
    receipt.state = intrinsic_hypergeometry_store_status::integrity_refused; return receipt;
  }
  receipt.bytes = exact::word{sizeof(record)}; receipt.transfer_calls = exact::word{1};
  return receipt;
}

intrinsic_hypergeometry_store_receipt write_intrinsic_hypergeometry_handoff(
    const char* path, const event::intrinsic_hypergeometry_rest_record& record) noexcept {
  intrinsic_hypergeometry_store_receipt receipt{};
  ...
  if (receipt.state == intrinsic_hypergeometry_store_status::returned) {
    receipt.bytes = exact::word{sizeof(record)}; receipt.transfer_calls = exact::word{1};
  }
  return receipt;
}

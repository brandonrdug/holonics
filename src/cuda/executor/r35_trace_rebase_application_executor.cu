...
struct storage final {
  trace_rebase_application_mount *mount{};
  unsigned char *resident{};
  event::heldout_trace_rebase_observation *observation{};
  event::heldout_trace_rebase_observation *returned{};
...
[[nodiscard]] bool initialize(const trace_rebase_application_mount &mount,
                              storage &s) noexcept {
...
trace_rebase_executor_receipt execute_trace_rebase_application(
    const trace_rebase_application_mount &mount,
  ...
  if (mount.inherited.integrity !=
      event::trace_rebase_rest_integrity(mount.inherited))
    return receipt;
  ...
  if (!initialize(mount, s) ||
      launch_trace_rebase_heldout_mount(s.mount, s.resident, s.observation) !=
          cudaSuccess ||
  ...
  trace_rebase_heldout_currents currents{};
  bool exact = open_trace_rebase_heldout_currents(currents);
  exact = exact && launch_trace_rebase_heldout_source(
                       s.mount, s.secret, currents.source) == cudaSuccess;
  exact = exact && cudaStreamSynchronize(currents.source) == cudaSuccess;
  exact = exact && launch_trace_rebase_heldout_expose(
                       s.secret, s.observation, currents.organ) == cudaSuccess;
  exact = exact && launch_trace_rebase_heldout_predict(
                       reinterpret_cast<event::resident_trace_rebase *>(
                           s.resident),
                       s.mount, s.observation, currents.organ) == cudaSuccess;
  exact = exact && cudaStreamSynchronize(currents.organ) == cudaSuccess;
  close_trace_rebase_heldout_currents(currents);
  if (!exact ||
      launch_trace_rebase_heldout_compare(
          reinterpret_cast<event::resident_trace_rebase *>(s.resident),
          s.secret, s.observation) != cudaSuccess ||
      launch_trace_rebase_heldout_form(
          reinterpret_cast<event::resident_trace_rebase *>(s.resident),

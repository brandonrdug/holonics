...
__global__ void enclosure_deed_kernel(
  ...
  if (slot < count) {
    outputs[slot] = exact::execute_enclosure_deed(inputs[slot]);
  }
}
...
exact_executor_receipt execute_enclosure_deeds(enclosure_deed_batch batch) noexcept {
  exact_executor_receipt receipt{};
  if (batch.inputs == nullptr || batch.outputs == nullptr || batch.count == 0) {
    return receipt;
  }
  constexpr std::size_t input_width = sizeof(exact::enclosure_deed_input);
  constexpr std::size_t output_width = sizeof(exact::enclosure_deed_output);
  ...
  exact::enclosure_deed_input* device_inputs = nullptr;
  exact::enclosure_deed_output* device_outputs = nullptr;
  ...
  enclosure_deed_kernel<<<static_cast<unsigned>(grid_width), static_cast<unsigned>(block_width)>>>(

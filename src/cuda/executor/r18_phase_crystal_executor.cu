...
[[nodiscard]] bool allocate(phase_crystal_storage& storage) noexcept {
  return cudaMalloc(&storage.mount, sizeof(phase_crystal_mount)) == cudaSuccess &&
      cudaMalloc(&storage.production, sizeof(event::resident_phase_crystal)) == cudaSuccess &&
      cudaMalloc(&storage.rest, sizeof(event::phase_crystal_rest_record)) == cudaSuccess &&
      cudaMalloc(&storage.handoff, sizeof(event::phase_crystal_rest_record)) == cudaSuccess &&
      cudaMalloc(&storage.resident, sizeof(event::phase_crystal_observation)) == cudaSuccess &&
      cudaMalloc(&storage.returned, sizeof(event::phase_crystal_observation)) == cudaSuccess &&
...
phase_crystal_executor_receipt execute_phase_crystal(const phase_crystal_mount& mount,
  ...
  phase_crystal_executor_receipt receipt{};
  if (!organ::valid_phase_crystal_foundation(mount.foundation) ||
      mount.question.identity.value() == 0 || mount.question.receiver.value() == 0 ||
      mount.question.material.value() == 0 ||
      mount.inherited.integrity != event::geometry_inquiry_rest_integrity(mount.inherited)) {
    return receipt;
  }
  int device_count = 0;
  if (cudaGetDeviceCount(&device_count) != cudaSuccess || device_count <= 0 ||
      cudaSetDevice(0) != cudaSuccess) {
    receipt.state = phase_crystal_executor_status::device_unavailable;
    return receipt;
  }
  int major = 0;
  int minor = 0;
  ...
  phase_crystal_storage storage{};
  if (!allocate(storage)) {
    release(storage);
    receipt.state = phase_crystal_executor_status::allocation_refused;
    return receipt;
  }
  if (cudaMemset(storage.production, 0, sizeof(event::resident_phase_crystal)) != cudaSuccess ||
      cudaMemset(storage.rest, 0, sizeof(event::phase_crystal_rest_record)) != cudaSuccess ||
      cudaMemset(storage.handoff, 0, sizeof(event::phase_crystal_rest_record)) != cudaSuccess ||
      cudaMemset(storage.resident, 0, sizeof(event::phase_crystal_observation)) != cudaSuccess ||
      cudaMemset(storage.returned, 0, sizeof(event::phase_crystal_observation)) != cudaSuccess ||
    ...
    release(storage);
    receipt.state = phase_crystal_executor_status::transfer_refused;
    return receipt;
  }
  if (launch_phase_crystal_mount(storage.mount, storage.production, storage.resident) !=
          cudaSuccess || launch_phase_crystal_cases(storage.mount, storage.resident) !=
          cudaSuccess || cudaDeviceSynchronize() != cudaSuccess) {
    release(storage);
    receipt.state = phase_crystal_executor_status::probe_refused;
    return receipt;
  }
  if (launch_phase_crystal_form(storage.mount, storage.production, storage.resident) !=
          cudaSuccess || cudaDeviceSynchronize() != cudaSuccess ||
    ...
    release(storage);
    receipt.state = phase_crystal_executor_status::formation_refused;
    return receipt;
  }
  event::checker_raw_return raw{};
  ...
  if (!receipt.process.returned()) {
    release(storage);
    receipt.state = phase_crystal_executor_status::process_refused;
    return receipt;
  }
  if (cudaMemcpy(storage.raw, &raw, sizeof(raw), cudaMemcpyHostToDevice) != cudaSuccess ||
      launch_phase_crystal_resume(storage.raw, storage.production, storage.resident) !=
          cudaSuccess || cudaDeviceSynchronize() != cudaSuccess) {
    release(storage);
    receipt.state = phase_crystal_executor_status::return_refused;
    return receipt;
  }
  if (launch_phase_crystal_rest_remount(storage.mount, storage.production, storage.rest,
          storage.handoff, storage.resident) != cudaSuccess ||
      launch_phase_crystal_observe(storage.resident, storage.returned) != cudaSuccess ||
      ...
      cudaMemcpy(&handoff, storage.handoff, sizeof(handoff),
          cudaMemcpyDeviceToHost) != cudaSuccess) {
    release(storage);
    receipt.state = phase_crystal_executor_status::rest_refused;
    return receipt;
  }
  release(storage);
  if (observation.typed.state != event::checker_return_status::accepted ||
      !observation.rest.returned || !observation.remount.same_body ||
      !observation.remount.atlas_preserved || !observation.handoff.returned ||
      !observation.final_can_continue) {
    receipt.state = phase_crystal_executor_status::rest_refused;
    return receipt;
  }
  const std::uint64_t resident_bytes = sizeof(phase_crystal_mount) +
      sizeof(event::resident_phase_crystal) + sizeof(event::phase_crystal_rest_record) * 2U +
      sizeof(event::phase_crystal_observation) * 2U + sizeof(event::checker_raw_return);
  receipt.state = phase_crystal_executor_status::returned;

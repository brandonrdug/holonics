...
HOLONICS_CALLABLE inline expression_geometry_rest_receipt resident_expression_geometry::rest(
    expression_geometry_rest_record& record) noexcept {
  expression_geometry_rest_receipt receipt{};
  if (!source_detached_ || !first_.accepted || !second_.accepted || !geometry_.accepted ||
      ...
      !expression_geometry_.accepted || pending_live_ || stage_ != passage_stage::returned) {
    return receipt;
  }
  ...
  record.integrity = expression_geometry_rest_integrity(record); receipt.theory = expression_geometry_.identity;
  receipt.integrity = exact::word{record.integrity}; receipt.prior_returns_preserved =
      first_.accepted && second_.accepted && geometry_.accepted && phase_crystal_.accepted &&
      characteristic_.accepted && regular_singular_.accepted && code_reconstruction_.accepted &&
      moment_reconstruction_.accepted && cm_incidence_.accepted && toric_cycle_.accepted &&
      algebraic_variation_.accepted && causal_linear_.accepted && intrinsic_hypergeometry_.accepted;

...
HOLONICS_CALLABLE inline arithmetic_spectral_rest_receipt resident_arithmetic_spectral::rest(
    arithmetic_spectral_rest_record& record) noexcept {
  arithmetic_spectral_rest_receipt receipt{};
  if (!source_detached_ || !first_.accepted || !second_.accepted || !geometry_.accepted ||
      ...
      !hodge_realization_.accepted || !arithmetic_spectral_.accepted || pending_live_ ||
  ...
  record.arithmetic_spectral_morphology = arithmetic_spectral_morphology_;
  record.integrity = arithmetic_spectral_rest_integrity(record);
  receipt.theory = arithmetic_spectral_.identity; receipt.integrity = exact::word{record.integrity};
  receipt.prior_returns_preserved = first_.accepted && second_.accepted && geometry_.accepted &&
      phase_crystal_.accepted && characteristic_.accepted && regular_singular_.accepted &&
      code_reconstruction_.accepted && moment_reconstruction_.accepted && cm_incidence_.accepted &&
      toric_cycle_.accepted && algebraic_variation_.accepted && causal_linear_.accepted &&
      intrinsic_hypergeometry_.accepted && expression_geometry_.accepted && hodge_realization_.accepted;

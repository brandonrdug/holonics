...
#include <holonics/event/regular_singular_return.hpp>

namespace holonics::event::regular_singular_checker_detail {
...
HOLONICS_CALLABLE constexpr void normalize(const checker_raw_return& raw,
    const codec::regular_singular_face& face, checker_typed_return& typed) noexcept {
  constexpr char declaration[] = "Soma.Holonics.R20.generated_connection_and_loop";
  constexpr char source[] = "theorem generated_connection_and_loop";
  constexpr char unsolved[] = "unsolved goals";
  typed.produced_declarations = count(raw.standard_output, raw.stdout_bytes, declaration);
  ...
  typed.source_span_end = static_cast<std::uint16_t>(
      typed.source_span_begin + sizeof(source) - 1U);
  ...
  else if (typed.remaining_goal_count != 0) {
    typed.state = checker_return_status::remaining_goals;
  } else if (typed.kernel_boundary_crossed) {
    typed.state = checker_return_status::accepted;

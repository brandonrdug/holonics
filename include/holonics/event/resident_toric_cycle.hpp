...
#include <holonics/codec/toric_cycle_renderer.hpp>
#include <holonics/codec/toric_explanation_renderer.hpp>
#include <holonics/event/checker_pending_deed.hpp>
#include <holonics/event/cm_checker_normalization_law.hpp>
#include <holonics/event/toric_cycle_return.hpp>
#include <holonics/organ/toric_cycle_law.hpp>

namespace holonics::event {
class resident_toric_cycle final {
 public:
  resident_toric_cycle() = delete;
  resident_toric_cycle(const resident_toric_cycle&) = delete;
  resident_toric_cycle& operator=(const resident_toric_cycle&) = delete;
  resident_toric_cycle(resident_toric_cycle&&) = delete;
  resident_toric_cycle& operator=(resident_toric_cycle&&) = delete;

  HOLONICS_CALLABLE resident_toric_cycle(const organ::toric_cycle_foundation& foundation,
      const cm_incidence_rest_record& record,
      cm_incidence_remount_receipt& receipt) noexcept;
  HOLONICS_CALLABLE resident_toric_cycle(const toric_cycle_rest_record& record,
      toric_cycle_remount_receipt& receipt) noexcept;
  ...
  [[nodiscard]] HOLONICS_CALLABLE bool form(const organ::toric_cycle_question& question,
      toric_cycle_observation& observation) noexcept;
  [[nodiscard]] HOLONICS_CALLABLE bool resume(const checker_raw_return& raw,
      toric_cycle_observation& observation) noexcept;
  [[nodiscard]] HOLONICS_CALLABLE toric_cycle_rest_receipt rest(
 ...
 private:
  ...
  organ::toric_cycle_foundation foundation_{};
...
static_assert(std::is_trivially_destructible_v<resident_toric_cycle>);

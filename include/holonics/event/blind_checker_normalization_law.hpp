...
#include <holonics/event/blind_reconstruction_return.hpp>

namespace holonics::event::blind_checker_detail {
...
HOLONICS_CALLABLE constexpr void normalize(const checker_raw_return& raw,
    const codec::blind_formal_face<Capacity>& face, const char (&declaration)[Declaration],
    const char (&source)[Source], checker_typed_return& typed) noexcept {

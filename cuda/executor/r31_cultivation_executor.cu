...
namespace {
struct cultivation_storage final {
  cultivation_mount *mount{}; unsigned char *resident{};
  event::cultivation_observation *observation{}; event::cultivation_observation *returned{};
  organ::cultivation_workspace *workspace{}; event::checker_raw_return *raw{};
...
void release(cultivation_storage &s) noexcept {
  release_one(s.handoff); release_one(s.rest); release_one(s.raw); release_one(s.workspace);
  release_one(s.returned); release_one(s.observation); release_one(s.resident); release_one(s.mount); s = {};
}
[[nodiscard]] bool allocate(cultivation_storage &s) noexcept {
...
[[nodiscard]] bool initialize(const cultivation_mount &mount, cultivation_storage &s) noexcept {
...
cultivated_executor_receipt execute_organ_cultivation(const cultivation_mount &mount,
    const lean_process_configuration &process, event::cultivation_observation &observation,
    organ::cultivation_workspace &workspace, event::cultivated_organ_rest_record &handoff) noexcept {
  cultivated_executor_receipt receipt{};
  if (mount.inherited.integrity != event::rederivation_rest_integrity(mount.inherited)) return receipt;
  if (!device(receipt)) { receipt.state = cultivated_executor_status::device_unavailable; return receipt; }
  cultivation_storage s{};
  if (!allocate(s)) { release(s); receipt.state = cultivated_executor_status::allocation_refused; return receipt; }
  if (!initialize(mount, s) || launch_cultivation_mount(s.mount, s.resident, s.observation) != cudaSuccess ||
  ...
  for (std::uint8_t i = 0; exact && i < organ::cultivation_family_count; ++i)
    exact = launch_cultivation_family(s.mount, s.observation, s.workspace, i, currents.currents[i]) == cudaSuccess;
  exact = exact && join_cultivated_currents(currents); close_cultivated_currents(currents);
  if (!exact || launch_cultivation_close(s.observation, s.workspace) != cudaSuccess ||
      launch_cultivation_form(reinterpret_cast<event::resident_cultivated_organs *>(s.resident), s.observation) != cudaSuccess ||
  ...
  if (cudaMemcpy(s.raw, &raw, sizeof(raw), cudaMemcpyHostToDevice) != cudaSuccess ||
      launch_cultivation_resume(s.raw, reinterpret_cast<event::resident_cultivated_organs *>(s.resident), s.observation) != cudaSuccess ||
      launch_cultivation_rest(reinterpret_cast<event::resident_cultivated_organs *>(s.resident), s.rest, s.handoff, s.observation) != cudaSuccess ||
      launch_cultivation_observe(s.observation, s.returned) != cudaSuccess || cudaDeviceSynchronize() != cudaSuccess ||
  ...
  const std::uint64_t bytes = sizeof(mount) + sizeof(event::resident_cultivated_organs) +
      2U*sizeof(observation) + sizeof(workspace) + sizeof(raw) + 2U*sizeof(handoff); release(s);
  if (observation.passage.typed.state != event::checker_return_status::accepted ||
      !observation.rest.returned || !observation.remount.organs_preserved || !observation.handoff.returned)
    { receipt.state = cultivated_executor_status::rest_refused; return receipt; }
  receipt.state = cultivated_executor_status::returned; receipt.bytes_to_device = exact::word{sizeof(mount)+sizeof(raw)};

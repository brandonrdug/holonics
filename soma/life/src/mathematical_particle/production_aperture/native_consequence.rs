...
mod tests {
    ...
    fn rest() -> NativeHexisAthenaRest {
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
        let native = root
            .join("output/recurring_laboratory_transport_condenses_into_native_hexis/native-rest");
        NativeHexisAthenaRest::read(
            &fs::read(native.join("standing.bin")).expect("standing"),
            &fs::read(native.join("decoder.bin")).expect("decoder"),
            &fs::read(native.join("fibres.bin")).expect("fibres"),
        )
        .expect("native rest")
    }

    #[test]
    fn native_inquiries_are_input_addressed_and_have_no_codec_receiver() {
        let rest = rest();
        let inquiry = rest
            .found_native_mathematical_inquiry(
                vec![
                    "n0/material/integer".to_owned(),
                    "n0/material/f2".to_owned(),
                ],
                vec![vec![11, -11, 0], vec![1, 1, 0]],
                vec![NativeSuccessorHistory::FixedSection],
                vec!["l2/history/fixed-section".to_owned()],
                vec!["nonlinear continuation".to_owned()],
            )
            .expect("native inquiry");
        assert_eq!(inquiry.receiver_family, receiver_family());
        assert_eq!(inquiry.receiver_family.len(), 8);

        let distinct = rest
            .found_native_mathematical_inquiry(
                vec![
                    "n0/material/separator/integer".to_owned(),
                    "n0/material/separator/f2".to_owned(),
                ],
                vec![vec![1, 1, 0], vec![1, 1, 0]],
                vec![NativeSuccessorHistory::CarrierRebase {
                    source: 0,
                    target: 1,
                }],
                vec!["l2/history/carrier-rebase".to_owned()],
                vec!["richer successor words".to_owned()],
            )
            .expect("distinct inquiry");
        assert_ne!(inquiry.occurrence, distinct.occurrence);
        assert_ne!(inquiry.sections, distinct.sections);
    }
}

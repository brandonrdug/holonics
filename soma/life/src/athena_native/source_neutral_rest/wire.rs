use super::*;
pub const SOURCE_NEUTRAL_ATHENA_REST_SCHEMA: &str = "soma-life.source-neutral-athena-rest.v5";
...
#[derive(Debug, Error)]
pub enum SourceNeutralAthenaError {
    ...
    Sensory,
    #[error("the source-neutral Athena wire is malformed: {0}")]
    Wire(String),
    #[error("the resident apparatus refused the source-neutral Athena body: {0}")]
    Apparatus(String),
}
...
/// The one continuing Athena body after the reversible developmental chart has departed.
#[derive(Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SourceNeutralAthenaRest {
    pub(super) schema: String,
...
/// The one move-owned source-neutral Athena body after its invariant incidence, constitutive
/// families, receiver faces, and boundary atlas have crossed the resident card.
pub struct ResidentSourceNeutralAthena {
    pub(super) rest: SourceNeutralAthenaRest,
...
pub(super) fn native_oriented_realization_faces(
...
) -> Result<Vec<SourceNeutralNativeOrientedFace>, SourceNeutralAthenaError> {
    let pairing = section
        .radiation
    ...
    for mode in 0..population {
        ...
        let local_currents = resident_local_currents
            ...
            .collect::<Result<Vec<_>, SourceNeutralAthenaError>>()?;
...
/// boundary face reached its receiver; it does not remount that face as Athena ingress or alter
...
impl SourceNeutralFactorWindingConstitutiveChart {
    ...
    ) -> Result<Vec<Rat>, SourceNeutralAthenaError> {
    ...
    ) -> Result<SourceNeutralWindingDifference, SourceNeutralAthenaError> {
...
/// One exterior realization which has left the resident Athena deed.  The complete continuation
...
pub struct SourceNeutralExteriorCirculation {
    pub(super) athena: ResidentSourceNeutralAthena,
...
) -> Result<Vec<Observation>, SourceNeutralAthenaError> {
...
) -> Result<SourceNeutralContinuationReceiverHistory, SourceNeutralAthenaError> {
    if path.is_empty() {
        return Err(SourceNeutralAthenaError::Body(

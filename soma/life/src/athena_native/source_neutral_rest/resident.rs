use super::*;
impl ResidentSourceNeutralAthena {
    pub fn rest(&self) -> &SourceNeutralAthenaRest {
        &self.rest
    }

    pub fn into_rest(self) -> SourceNeutralAthenaRest {
        self.rest
    }
    ...
    pub fn factor_winding_constitutive_chart(
        &self,
    ) -> Result<SourceNeutralFactorWindingConstitutiveChart, SourceNeutralAthenaError> {
        // `ResidentSourceNeutralAthena` is obtainable only by consuming a validated rest through
    ...
    ) -> Result<SourceNeutralFactoredRadiationSection, SourceNeutralAthenaError> {
        self.resident
    ...
    pub fn condition_native(
        &mut self,
        projective: &GranularNativeProjectiveCurrent,
    ) -> Result<SourceNeutralResidentRadiationSection, SourceNeutralAthenaError> {
        self.resident
            .begin_factored_current_occurrence()
            .map_err(|error| SourceNeutralAthenaError::Apparatus(error.to_string()))?;
        let (emanation, crossed_structural_ports) = self
    ...
    pub fn begin_native_exterior(
        mut self,
        projective: GranularNativeProjectiveCurrent,
    ) -> Result<SourceNeutralExteriorCirculation, SourceNeutralAthenaError> {
        self.resident
            .begin_factored_current_occurrence()
            .map_err(|error| SourceNeutralAthenaError::Apparatus(error.to_string()))?;
        let (emanation, crossed_structural_ports) = self
    ...
    > {
        ...
        // factorization is Athena's resident state; an upper-triangular factor-pair expansion is
        ...
        let universal_ports = visible_ports
            .iter()
            .map(|port| {
                u32::try_from(crate::athena_native::granular_potential::port_index(port)).map_err(
                    |_| {
                        SourceNeutralAthenaError::Granular(

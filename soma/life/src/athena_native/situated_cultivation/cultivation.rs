use super::*;
use crate::athena_native::laboratory_cultivation::{
    LaboratoryCellAffineSection, LaboratoryFactorCycleCorrespondence,
};
use crate::athena_native::membrane_transport::AthenaMembraneStanding;
...
impl SituatedCultivatedAthenaRest {
    ...
    pub fn mount(self) -> Result<ResidentSituatedCultivatedAthena, SituatedCultivationError> {
        self.validate()?;
        ...
        .map_err(|error| SituatedCultivationError::Apparatus(error.to_string()))?;
        Ok(ResidentSituatedCultivatedAthena {
            rest: self,
            factors,
            expected,
            resident,
    ...
    pub fn validate(&self) -> Result<(), SituatedCultivationError> {
        ...
        if self.schema != SITUATED_CULTIVATED_ATHENA_REST_SCHEMA
    ...
    fn rederived_identity(&self) -> Result<String, SituatedCultivationError> {
        super::resident::digest_json(&(
            SITUATED_CULTIVATED_ATHENA_REST_SCHEMA,
            &self.predecessor_wire_sha256,
            self.ecology
                .identity_sha256()
                .map_err(|error| SituatedCultivationError::Ecology(error.to_string()))?,
    ...
    pub fn deposit_returned_difference(
    ...
    ) -> Result<LaboratoryCultivatedAthenaRest, SituatedCultivationError> {
        self.validate()?;
        difference
            .validate()
        ...
        let realization = ReceiverHistoryRealizationPassage::found(ecology.native())
            .map_err(|error| SituatedCultivationError::Ecology(error.to_string()))?;
        let mut rest = LaboratoryCultivatedAthenaRest {
...
impl AthenaMembraneStanding for SituatedCultivatedAthenaRest {

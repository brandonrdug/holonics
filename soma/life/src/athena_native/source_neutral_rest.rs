...
use crate::athena_native::granular_potential::{
...
use crate::athena_native::returned_difference_deposit::derive_returned_difference_deposit_from_history;
use crate::athena_native::situated_difference::{
...
) -> Result<i8, SourceNeutralAthenaError> {
...
) -> Result<Vec<BigInt>, SourceNeutralAthenaError> {
...
fn source_neutral_situated_difference_from_world_return(
...
) -> Result<SourceNeutralSituatedDifferenceBody, SourceNeutralAthenaError> {
    ...
    let latest_event = rest
        ...
        .max()
        .ok_or_else(|| {
            SourceNeutralAthenaError::Body("the standing spool has no event chronology".to_owned())
        })?;
...
fn unique_largest_rational_magnitude(
    ...
    plural_reason: &str,
) -> Result<usize, SourceNeutralAthenaError> {
    ...
    if anchors.len() != 1 {
        return Err(SourceNeutralAthenaError::Body(plural_reason.to_owned()));
    }
    Ok(anchors[0])
}
...
fn source_neutral_family_orientation(
    rest: &SourceNeutralAthenaRest,
) -> Result<Vec<i8>, SourceNeutralAthenaError> {
    ...
    for family in rest.body.ecology().mixed_constitutive_families() {
        let left = thread_sections.get(&family.left_thread).ok_or_else(|| {
            SourceNeutralAthenaError::Body("a left winding section is absent".to_owned())
        })?;
        let right = thread_sections.get(&family.right_thread).ok_or_else(|| {
            SourceNeutralAthenaError::Body("a right winding section is absent".to_owned())
        })?;
...
fn source_neutral_factor_hands(
    rest: &SourceNeutralAthenaRest,
) -> Result<BTreeMap<(u32, usize), i8>, SourceNeutralAthenaError> {
    let fibres = rest.body.ecology().exact_reconstruction_fibres();
...
fn source_neutral_emitting_thread_current(
    ...
    thread_address: &str,
) -> Result<ExactComplexWaveCurrent, SourceNeutralAthenaError> {
    let thread = ecology
        .native()
        .spools
        .iter()
...
fn quadratic_realization_current(
    projective: &GranularNativeProjectiveCurrent,
) -> Result<BigUint, SourceNeutralAthenaError> {

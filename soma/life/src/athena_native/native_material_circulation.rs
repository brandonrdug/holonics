...
const IDENTITY_SCHEMA: &[u8] = b"soma-life.athena-native-circulation-identity.v11";
...
pub struct NativeCirculationPassage {
    ...
    pub native_passages: Vec<CultivatedAthenaPassage>,
...
pub struct NativeCirculationRest {
    cultivated: CultivatedAthenaRest,
...
pub struct ResidentNativeCirculation {
    ...
    potential_identity: [u8; 32],
    codec_identity: [u8; 32],
    cultivated: ResidentCultivatedAthena,
...
impl NativeCirculationRest {
    ...
    pub fn read(bytes: &[u8]) -> Result<Self, NativeCirculationError> {
        ...
        let cultivated = CultivatedAthenaRest::read(read_blob(bytes, &mut cursor)?)
    ...
    pub fn cultivated(&self) -> &CultivatedAthenaRest {
        &self.cultivated
    }
    ...
    pub fn ablate(
        self,
        factor_address: &str,
    ) -> Result<(Self, WithdrawnNativeFactor, AthenaCultivationMutation), NativeCirculationError>
    {
        self.validate()?;
        let Self {
            cultivated,
    ...
    ) -> Result<(Self, AthenaCultivationMutation), NativeCirculationError> {
        self.validate()?;
        let Self {
            cultivated,
            potential,
        ...
        for section in &self.ingress_sections {
            for receiver in &self.receiver_family {
                let passage = match self
                    ...
                    .map_err(|error| NativeCirculationError::Cultivated(error.to_string()))?
                {
                    CultivatedAthenaConsequence::Returned(passage) => passage,

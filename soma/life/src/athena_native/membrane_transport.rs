...
use holonic_engine::{
    ...
    receiver_history_compression::{
        AddressedFactoredIntegralReceiverComplex, membrane_addressed_factored_receiver_complex,
    },
};
...
use super::{
    AddressedMaterialOccurrence, AffineLaboratoryCultivatedAthenaRest, LaboratoryCellAffineSection,
...
pub const ATHENA_MEMBRANE_CROSSING_SCHEMA: &str = "soma-life.athena-membrane-crossing.v1";
...
pub trait AthenaMembraneStanding {
...
pub struct AdmittedAthenaMembraneStanding<Standing> {
    standing: Standing,
    admitted_identity_sha256: String,
    complete_validation_receipt_sha256: String,
}

impl<Standing: AthenaMembraneStanding> AdmittedAthenaMembraneStanding<Standing> {
...
impl<Standing: AthenaMembraneStanding> AthenaMembraneStanding
...
pub trait GranularAthenaMembraneStanding: AthenaMembraneStanding {
...
pub struct AthenaMembraneOccurrence {
...
impl fmt::Debug for AthenaMembraneOccurrence {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AthenaMembraneOccurrence")
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AthenaMembraneCrossingReceipt {
    pub schema: String,
    pub rested_identity_sha256: String,
...
pub struct AthenaMembraneReturn {
    pub occurrence: AthenaMembraneOccurrence,
    pub receipt: AthenaMembraneCrossingReceipt,
}

impl fmt::Debug for AthenaMembraneReturn {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AthenaMembraneReturn")
...
#[derive(Debug)]
pub enum AthenaMembraneConsequence {
    Returned(AthenaMembraneReturn),
...
pub struct AthenaCausalMembrane<Standing = AffineLaboratoryCultivatedAthenaRest> {
    rest: Standing,
...
impl<Standing: AthenaMembraneStanding> AthenaCausalMembrane<Standing> {
    ...
    pub fn constitute_interior(mut self) -> Result<Self, MembraneInteriorError> {
        ...
        if holonic_engine::cuda_refine::trace_configuration().holonics_phase_trace {
            eprintln!("athena-membrane constitute-interior {:?}", began.elapsed());
        }
        Ok(self)
    }
    ...
    pub fn mount_resident_interior(mut self) -> Result<Self, MembraneInteriorError> {
        ...
        if holonic_engine::cuda_refine::trace_configuration().holonics_phase_trace {
            eprintln!(
                "athena-membrane mount-resident-interior {:?}",
                began.elapsed()
            );
        }
        Ok(self)
    ...
    fn cross(&self, occurrence: AthenaMembraneOccurrence) -> AthenaMembraneConsequence {
        ...
        open_exterior.sort();
        open_exterior.dedup();
        let receipt = AthenaMembraneCrossingReceipt {
            schema: ATHENA_MEMBRANE_CROSSING_SCHEMA.to_owned(),
        ...
        AthenaMembraneConsequence::Returned(AthenaMembraneReturn {
            occurrence,
            receipt,
        })
    }
...
impl<Standing: GranularAthenaMembraneStanding> AthenaCausalMembrane<Standing> {
    ...
    pub fn mount_resident_factor_receiver_faces(mut self) -> Result<Self, MembraneInteriorError> {
        ...
        if holonic_engine::cuda_refine::trace_configuration().holonics_phase_trace {
            eprintln!(
                "athena-membrane mount-factor-receiver-faces {:?}",
                began.elapsed()
            );
        }
        Ok(self)
...
impl<Standing: AthenaMembraneStanding> CausalMembrane for AthenaCausalMembrane<Standing> {
    type Standing = Standing;
    type Occurrence<'a>
        = AthenaMembraneOccurrence
    where
        Standing: 'a;
    type Return = AthenaMembraneConsequence;
    type Error = AthenaMembraneError;
...
#[derive(Clone, Debug, Error, PartialEq, Eq)]
pub enum AthenaMembraneError {
    #[error("the Athena membrane invariant failed: {0}")]
    Invariant(String),
}

fn insufficient(
    occurrence: AthenaMembraneOccurrence,
    defect: NativeMembraneDefect,
) -> AthenaMembraneConsequence {
    AthenaMembraneConsequence::Insufficient(NativeMembraneInsufficiency { defect, occurrence })
}
...
fn native_section_matches(
    rest: &impl AthenaMembraneStanding,
    native: &NativeConductedSection,
) -> bool {
...
fn native_contact_view<'a>(
    rest: &'a impl AthenaMembraneStanding,

...
use super::{
    AcousticAthenaRest, AcousticRelationalCellWithdrawal, AthenaMembraneStanding,
    GranularAthenaMembraneStanding, LaboratoryCellAffineSection,
    LaboratoryFactorCycleCorrespondence, NativeAcousticRadiationInput, NativeGranularPotential,
    ReceiverHistoryRealizationPassage, SituatedCultivationBranch, SituatedDifferenceSection,
};
...
const OPTICAL_ATHENA_REST_SCHEMA: &str = "soma-life.optical-athena-rest.v1";
...
#[derive(Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct OpticalAthenaRest {
    schema: String,
    body: AcousticAthenaRest,
...
impl OpticalAthenaRest {
    pub fn cultivate(
        ...
        let mut rest = Self {
            schema: OPTICAL_ATHENA_REST_SCHEMA.to_owned(),
            body,
            production,
            identity_sha256: String::new(),
        };
    ...
    pub fn body(&self) -> &AcousticAthenaRest {
        &self.body
    }
    ...
    pub fn validate(&self) -> Result<(), NativeOpticalError> {
        ...
        if self.schema != OPTICAL_ATHENA_REST_SCHEMA
            || self.identity_sha256 != self.rederived_identity()?
        {
            return Err(NativeOpticalError::Lineage);
        }
...
impl AthenaMembraneStanding for OpticalAthenaRest {
...
impl GranularAthenaMembraneStanding for OpticalAthenaRest {
    fn membrane_granular_potential(&self) -> &NativeGranularPotential {
        self.body.membrane_granular_potential()
    }
}

impl MembraneDifferenceStanding for OpticalAthenaRest {
    type Successor = Self;

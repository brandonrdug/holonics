...
use super::{
    AthenaMembraneStanding, LaboratoryCellAffineSection, LaboratoryFactorCycleCorrespondence,
};

pub const MEMBRANE_INTERIOR_SCHEMA: &str = "soma-life.morphology-derived-athena-interior.v1";
...
/// by the rest inside [`super::AthenaCausalMembrane`].
...
impl MorphologyDerivedInterior {
    ...
    pub fn derive(rest: &impl AthenaMembraneStanding) -> Result<Self, MembraneInteriorError> {
        ...
        for correspondence in rest.membrane_correspondences() {
            ...
            let cell = complex
                .found_cell(
                    format!("athena-factor/{}", correspondence.factor_address),
                    source_events,
                    0,
                    CausalChain::default(),
                )
        ...
        for (family_at, family) in families.iter().enumerate() {
            ...
            let upper = complex
                .found_cell(
                    format!("athena-constitutive/{}", family.address),
                    source_events,
                    1,
                    boundary,
                )

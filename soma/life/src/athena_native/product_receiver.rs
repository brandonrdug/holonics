...
use holonic_engine::{
    ...
    cuda_refine::{
        CudaRefineExecutor, ResidentAthenaIntegratedFront, ResidentSituatedCurrentCausalFrontReturn,
    },
};
...
use super::{
    ...
    MaterialNativeFactorization, OpticalAthenaRest, SituatedCultivatedConductReturn,
};
...
pub struct ResidentAthenaProduct {
    rest: OpticalAthenaRest,
...
impl OpticalAthenaRest {
    ...
    pub fn mount_product(self) -> Result<ResidentAthenaProduct, LaboratoryCultivationError> {
        self.validate()
        ...
        let resident = ResidentAthenaIntegratedFront::mount(
            CudaRefineExecutor::new()
                .map_err(|error| LaboratoryCultivationError::Relational(error.to_string()))?,
            format!("athena-alpha-product/{}", self.identity()),
            incidence,
            front,
            interactions,
            &participant_subject,
...
impl MaterialFactorizationStanding for OpticalAthenaRest {
...
impl ResidentAthenaProduct {
    ...
    pub fn into_rest(self) -> OpticalAthenaRest {
        self.rest
    }
}

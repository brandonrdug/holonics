...
use super::{
    CompleteExchangeCultivationCover, NativeConductConsequence, NativeConductPassage,
    NativeCultivationMorphology, NativeEcologyError, NativeEcologyRest, NativeFactorDeposit,
    NativeFactorReturnedLimb, NativeSectionAddress,
};

const CULTIVATED_ECOLOGY_REST_SCHEMA: &str = "soma-life.cultivated-ecology-rest.v2";
...
#[serde(deny_unknown_fields)]
pub struct CultivatedEcologyRest {
    schema: String,
...
#[serde(deny_unknown_fields)]
pub struct CultivationMutation {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CultivationWithdrawal {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct CultivatedConductPassage {
...
pub enum CultivatedConductConsequence {
    Returned(CultivatedConductPassage),
    Insufficient(ReceiverInsufficiency),
}

pub struct ResidentCultivatedEcology {
    rest: CultivatedEcologyRest,
    ingress_sections: Vec<NativeSectionAddress>,
    morphology_current: ResidentComplexIncidence,
}

#[derive(Debug, Error)]
pub enum CultivatedEcologyError {
...
impl CultivatedEcologyRest {
    pub fn cultivate(
        predecessor: NativeEcologyRest,
        reconstruction: CompleteExchangeCultivationCover,
    ) -> Result<Self, CultivatedEcologyError> {
        predecessor
            .validate()
            .map_err(|error| CultivatedEcologyError::Predecessor(error.to_string()))?;
        reconstruction
            .validate()
            .map_err(CultivatedEcologyError::Wire)?;
        let morphology = NativeCultivationMorphology::derive(&predecessor, &reconstruction)
            .map_err(|error| CultivatedEcologyError::Wire(error.to_string()))?;
        ...
        let mut rest = Self {
            schema: CULTIVATED_ECOLOGY_REST_SCHEMA.to_owned(),
            predecessor,
            morphology,
    ...
    pub fn read(bytes: &[u8]) -> Result<Self, CultivatedEcologyError> {
    ...
    pub fn canonical_bytes(&self) -> Result<Vec<u8>, CultivatedEcologyError> {
        self.validate()?;
        serde_json::to_vec(self).map_err(|error| CultivatedEcologyError::Wire(error.to_string()))
    }
    ...
    pub fn mount(self) -> Result<ResidentCultivatedEcology, CultivatedEcologyError> {
        self.validate()?;
    ...
    pub fn ablate(
        mut self,
        factor_address: &str,
    ) -> Result<(Self, WithdrawnNativeFactor, CultivationMutation), CultivatedEcologyError> {
        self.validate()?;
        if self.morphology.deposits.len() < 2 {
            return Err(CultivatedEcologyError::Factor(factor_address.to_owned()));
        }
        let at = self
            .morphology
            .deposits
            ...
            .ok_or_else(|| CultivatedEcologyError::Factor(factor_address.to_owned()))?;
        ...
        self.validate()?;
        let mutation = CultivationMutation {
    ...
    pub fn restore(
    ...
    ) -> Result<(Self, CultivationMutation), CultivatedEcologyError> {
        self.validate()?;
        withdrawn.validate()?;
        ...
        {
            return Err(CultivatedEcologyError::Factor(
    ...
    pub fn withdraw(
        mut self,
    ) -> Result<(NativeEcologyRest, CultivationWithdrawal), CultivatedEcologyError> {
        self.validate()?;
    ...
    pub fn validate(&self) -> Result<(), CultivatedEcologyError> {
        self.predecessor
            .validate()
            .map_err(|error| CultivatedEcologyError::Predecessor(error.to_string()))?;
        self.morphology
            .validate(&self.predecessor)
            .map_err(|error| CultivatedEcologyError::Wire(error.to_string()))?;
        if self.schema != CULTIVATED_ECOLOGY_REST_SCHEMA
            || self.identity_sha256 != self.rederived_identity()?
        {
            return Err(CultivatedEcologyError::Standing);
        }
    ...
    fn rederived_identity(&self) -> Result<String, CultivatedEcologyError> {
        digest_json(&(
            CULTIVATED_ECOLOGY_REST_SCHEMA,
            self.predecessor
                .wire_sha256()
                .map_err(|error| CultivatedEcologyError::Predecessor(error.to_string()))?,
            &self.morphology,
...
impl WithdrawnNativeFactor {
    ...
    fn validate(&self) -> Result<(), CultivatedEcologyError> {
...
impl ResidentCultivatedEcology {
    pub fn identity(&self) -> &str {
        self.rest.identity()
    }
    ...
    pub fn conduct(
        ...
        receiver: ReceiverId,
    ) -> Result<CultivatedConductConsequence, CultivatedEcologyError> {
            ...
            .map_err(|error| CultivatedEcologyError::Predecessor(error.to_string()))?
        {
            ...
            NativeConductConsequence::Insufficient(insufficiency) => {
                return Ok(CultivatedConductConsequence::Insufficient(insufficiency));
            }
        };
        ...
        let responses = resident_return
            .sections
            .first()
            .ok_or(CultivatedEcologyError::Standing)?;
        ...
        Ok(CultivatedConductConsequence::Returned(
            CultivatedConductPassage {
                predecessor,
    ...
    pub fn into_rest(self) -> CultivatedEcologyRest {
        self.rest
    }
}
...
) -> Result<Vec<i64>, CultivatedEcologyError> {
    ...
    for deposit in &morphology.deposits {
        for section in ingress_sections {
            ...
            if coefficient.denom() != &BigInt::from(1) {
                return Err(CultivatedEcologyError::Apparatus(
            ...
            incidence.push(
                coefficient
                    ...
                    .parse::<i64>()
                    .map_err(|_| {
                        CultivatedEcologyError::Apparatus(
...
fn exact_inverse_checks(
    deposit: &NativeFactorDeposit,
) -> Result<(bool, bool), CultivatedEcologyError> {
...
fn digest_json(value: &impl Serialize) -> Result<String, CultivatedEcologyError> {
...
impl From<NativeEcologyError> for CultivatedEcologyError {

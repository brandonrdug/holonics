...
//! exterior testimony only: the successor [`SourceNeutralEcologyRest`] contains no pointer back to
...
use super::{
    ...
    NativeOpticalProductionMorphology, SourceNeutralAcousticMorphology,
    SourceNeutralCultivationBranch, SourceNeutralEcologyError, SourceNeutralEcologyRest,
...
/// accepted by [`SourceNeutralEcologyRest`].
...
pub fn sever_source_bearing_ecology(
    predecessor_wire: &[u8],
) -> Result<
    (
        SourceNeutralEcologyRest,
...
> {
    ...
    .map_err(|error| SourceNeutralSeveringError::Successor(error.to_string()))?;
    let successor =
        SourceNeutralEcologyRest::found(body, granular, relational, realization, acoustic, optical)
            .map_err(|error: SourceNeutralEcologyError| {

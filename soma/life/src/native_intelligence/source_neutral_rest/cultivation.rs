...
impl SourceNeutralReturnedEcology {
    ...
    ) -> Result<(Self, SourceNeutralReturnedDeposit), SourceNeutralEcologyError> {
        difference
            .validate()
    ...
    pub fn validate(&self) -> Result<(), SourceNeutralEcologyError> {
        self.ecology
            .validate()
            .map_err(|error| SourceNeutralEcologyError::Body(error.to_string()))?;
        self.predecessor_deposit_receipt
        ...
        let primary_population = branch_rank
            ...
            .ok_or_else(|| SourceNeutralEcologyError::Body("primary extent overflow".to_owned()))?;
...
impl SourceNeutralAcousticMorphology {
    ...
    fn validate(&self) -> Result<(), SourceNeutralEcologyError> {
        ...
        if self.schema != SOURCE_NEUTRAL_ACOUSTIC_MORPHOLOGY_SCHEMA
            ...
            || self.identity_sha256 != self.rederived_identity()
        {
            return Err(SourceNeutralEcologyError::Sensory);
        }
        Ok(())
    }
...
impl SourceNeutralEcologyRest {
    pub(crate) fn found(
        ...
        let mut rest = Self {
            schema: SOURCE_NEUTRAL_ECOLOGY_REST_SCHEMA.to_owned(),
            body,
            granular,
            relational,
            realization,
    ...
    pub fn read(bytes: &[u8]) -> Result<Self, SourceNeutralEcologyError> {
        let rest: Self = serde_json::from_slice(bytes)
            .map_err(|error| SourceNeutralEcologyError::Wire(error.to_string()))?;
        rest.validate()?;
        Ok(rest)
    }

    pub fn canonical_bytes(&self) -> Result<Vec<u8>, SourceNeutralEcologyError> {
        self.validate()?;
        serde_json::to_vec(self).map_err(|error| SourceNeutralEcologyError::Wire(error.to_string()))
    }
    ...
    pub fn validate(&self) -> Result<(), SourceNeutralEcologyError> {
        ...
        if self.schema != SOURCE_NEUTRAL_ECOLOGY_REST_SCHEMA
    ...
    /// Re-found the relational/realization organs from the complete exchange using only
    /// proposal-relative source factors and visible assistant thought/answer returns. Tool
    /// apparatus remains exterior; no global continuation partition decides cultivation.
    pub fn recultivate_completed_exchange(
        self,
        visible: &VisibleMessageProjection,
        aperture: &ContinuationAperture,
        congruence: &ReceiverHistoryCongruence,
    ) -> Result<(Self, SourceNeutralRelationalRecultivationReceipt), SourceNeutralEcologyError>
    {
        self.validate()?;
        congruence
            .validate()
            .map_err(SourceNeutralEcologyError::Body)?;
        if visible.source_occurrence_sha256 != aperture.source_occurrence_sha256
            || congruence.source_occurrence_sha256 != aperture.source_occurrence_sha256.render()
            || aperture.families.is_empty()
            || congruence.sections.len() != aperture.families.len()
        {
            return Err(SourceNeutralEcologyError::Body(
                "the completed exchange charts do not share one source occurrence".to_owned(),
            ));
        }
        let predecessor_rest_identity_sha256 = self.identity_sha256.clone();
        let native_states = congruence
            .native
            .native_population
            .iter()
            .copied()
            .enumerate()
            .map(|(factor, state)| (state, factor))
            .collect::<BTreeMap<_, _>>();
        if native_states.len() != self.granular.factor_faces().len() {
            return Err(SourceNeutralEcologyError::Body(
                "the exchange native factors do not span the continuing granular base".to_owned(),
            ));
        }
        let factor_by_family = congruence
            .sections
            .iter()
            .map(|section| {
                let source = ItemId(section.source_item);
                let native = congruence
                    .native
                    .encode(source)
                    .map_err(|error| SourceNeutralEcologyError::Body(error.to_string()))?;
                let factor = native_states.get(&native).copied().ok_or_else(|| {
                    SourceNeutralEcologyError::Body(
                        "a receiver-history source escaped the native factor base".to_owned(),
                    )
                })?;
                Ok((section.predecessor_family_address.as_str(), factor))
            })
            .collect::<Result<BTreeMap<_, _>, SourceNeutralEcologyError>>()?;
        let factor_addresses = congruence
            .native
            .native_population
            .iter()
            .map(|state| format!("exchange/native-factor/{}", state.0))
            .collect::<Vec<_>>();
        ...
        let mut ingress_occurrence_population = 0_usize;
        let mut emanation_occurrence_population = 0_usize;
        let mut return_occurrence_population = 0_usize;
        for family in &families {
            let factor = *factor_by_family
                .get(family.occurrence.as_str())
                .ok_or_else(|| {
                    SourceNeutralEcologyError::Body(
                        "an exchange family escaped its proposal-relative factor".to_owned(),
                    )
                })?;
            if let Some(prior) = prior_by_container.insert(family.prompt.container, factor) {
                if prior != factor {
                    builder
                        .join_factors(prior, factor)
                        .map_err(|error| SourceNeutralEcologyError::Body(error.to_string()))?;
                }
            }
            let message = visible
                .messages
                .get(family.prompt.visible_index as usize)
                .filter(|message| {
                    message.occurrence == family.prompt.occurrence
                        && message.text_sha256 == family.prompt.content_sha256
                        && message.speaker_face == "user"
                })
                .ok_or_else(|| {
                    SourceNeutralEcologyError::Body(
                        "a proposal prompt changed across the exchange charts".to_owned(),
                    )
                })?;
            builder
                .receive(
                ...
                .map_err(|error| SourceNeutralEcologyError::Body(error.to_string()))?;
            ingress_occurrence_population += 1;
            for address in &family.response {
                let message = visible
                    .messages
                    .get(address.visible_index as usize)
                    .filter(|message| {
                        message.occurrence == address.occurrence
                            && message.text_sha256 == address.content_sha256
                            && message.speaker_face == "assistant"
                    })
                    .ok_or_else(|| {
                        SourceNeutralEcologyError::Body(
                            "a sibling response changed across the exchange charts".to_owned(),
                        )
                    })?;
                if message.phase_face != "final-answer" && message.phase_face != "commentary" {
                    continue;
                }
                builder
                    .receive(
                    ...
                    .map_err(|error| SourceNeutralEcologyError::Body(error.to_string()))?;
                emanation_occurrence_population += 1;
            }
            if let Some(address) = &family.later_operator_return {
                let message = visible
                    .messages
                    .get(address.visible_index as usize)
                    .filter(|message| {
                    ...
                    .ok_or_else(|| {
                        SourceNeutralEcologyError::Body(
                            "a later return changed across the exchange charts".to_owned(),
                        )
                    })?;
                builder
                    .receive(
                    ...
                    .map_err(|error| SourceNeutralEcologyError::Body(error.to_string()))?;
                return_occurrence_population += 1;
            }
        }
        if emanation_occurrence_population == 0 {
            return Err(SourceNeutralEcologyError::Body(
                "the completed exchange returned no assistant emanation".to_owned(),
            ));
        }
        let (codec, potential, condition) = builder
            .finish()
            .map_err(|error| SourceNeutralEcologyError::Body(error.to_string()))?;
        let (relational, realization, _) = source_neutralize_native_relational(&potential, &codec)
            .map_err(|error| SourceNeutralEcologyError::Body(error.to_string()))?;
        drop(codec);
        drop(potential);
        let successor = Self::found(
            self.body,
            self.granular,
            relational,
            realization,
        ...
        let receipt = SourceNeutralRelationalRecultivationReceipt {
            predecessor_rest_identity_sha256,
            successor_rest_identity_sha256: successor.identity_sha256.clone(),
            exchange_family_population: families.len(),
            ingress_occurrence_population,
            emanation_occurrence_population,
            return_occurrence_population,
            native_factor_population: native_states.len(),
            native_face_population: condition.native_face_population,
            native_cell_population: condition.native_cell_population,
            realization_presentation_population: successor.realization.presentation_roots.len(),
            source_occurrence_ledger_retained: false,
            exterior_surface_paths_are_native_identity: false,
        };
    ...
    pub fn deposit_returned_difference(
        self,
        difference: SituatedDifferenceSection,
    ) -> Result<(Self, SourceNeutralReturnedDifferenceReceipt), SourceNeutralEcologyError> {
        self.validate()?;
        let predecessor_rest_identity_sha256 = self.identity_sha256.clone();
        let SourceNeutralEcologyRest {
            body,
            granular,
            relational,
            realization,
    ...
    pub fn withdraw_latest_returned_difference(
        self,
    ) -> Result<(Self, SourceNeutralReturnedDifferenceWithdrawal), SourceNeutralEcologyError> {
        self.validate()?;
    ...
    pub fn withdraw_inherited_thread(
        ...
        spool_address: &str,
        thread_address: &str,
    ) -> Result<SourceNeutralInheritedThreadAblation, SourceNeutralEcologyError> {
        self.validate()?;
        if !self
    ...
    ) -> Result<SourceNeutralRadicalDirectionAblation, SourceNeutralEcologyError> {
        self.validate()?;
        let original_rest_identity_sha256 = self.identity_sha256.clone();
        let fibre = self
            .body
    ...
    /// their construction.
    pub fn mount_resident(self) -> Result<ResidentSourceNeutralEcology, SourceNeutralEcologyError> {
        self.validate()?;
        ...
        Ok(ResidentSourceNeutralEcology {
            rest: self,
            resident,
...
impl SourceNeutralInheritedThreadAblation {
    ...
    ) -> Result<SourceNeutralNativeRadiationObstruction, SourceNeutralEcologyError> {
    ...
    /// rested identity.
    pub fn restore(self) -> Result<SourceNeutralEcologyRest, SourceNeutralEcologyError> {
        let ecology = self
            .ecology
        ...
        let rest = SourceNeutralEcologyRest::found(
            body,
            self.granular,
            self.relational,
            self.realization,
        ...
        if rest.identity() != self.original_rest_identity_sha256 {
            return Err(SourceNeutralEcologyError::Body(
                "the inherited carrier did not restore the exact ecology rest".to_owned(),
            ));
        }
        Ok(rest)
    }
...
impl SourceNeutralRadicalDirectionAblation {
    ...
    ) -> Result<SourceNeutralRadicalConductPreservationReceipt, SourceNeutralEcologyError> {
        if projective.standing_potential_identity_sha256 != self.granular.identity()

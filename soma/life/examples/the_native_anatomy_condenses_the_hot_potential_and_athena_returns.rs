...
use holonic_engine::phoenix::{
    foreign_potential_rest::{ForeignPotentialComplexRest, NativeAnatomicalPotentialRest},
    receiver_restricted_transport::NativeAnatomyRest,
};
...
const ANATOMY: &str = concat!(
    "output/the_foreign_receiver_descends_into_athena_native_anatomy/",
    "native-anatomy.rest"
);
const OUTPUT: &str = "output/the_native_anatomy_condenses_the_hot_potential_and_athena_returns";
...
fn construct(output: &Path) -> Result<(), String> {
    ...
    let native =
        NativeAnatomicalPotentialRest::seal_from_foreign(&foreign, &anatomy).map_err(display)?;
    let native_bytes = native.canonical_bytes().map_err(display)?;
    if native.cold_foreign_reconstruction_sha256 != foreign.identity_sha256 {
        return Err("native hot rest does not bind the complete cold reconstruction".to_owned());
    }

    let product = output.join("product");
    fs::create_dir_all(product.join("cold-reconstruction")).map_err(display)?;
    ...
    fs::write(
        product.join("cold-reconstruction/foreign-potential.rest"),
        &foreign_bytes,
    )
    .map_err(display)?;
...
fn remount(potential: &Path, anatomy: &Path, output: &Path) -> Result<(), String> {
    ...
    if native.occurrence_to_native.len() != anatomy.occurrences.len()
        || native.occurrence_to_native
            != anatomy
                .quotient
                .quotient
                .iter()
    ...
    write_json(
        ...
        &json!({
            ...
            "receiver":returned,
            "source_accessed_paths":["/native-potential.rest","/native-anatomy.rest"],

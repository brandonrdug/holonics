...
const M1_PASSAGE: &str =
    "output/the_mathematical_particle_is_an_addressed_passage/addressed-passages.form";
const M6_PRODUCT: &str = "output/the_total_descent_route_cultivates_the_first_athena_variant/product/19-athena-product.json";
const M6_ROUTE_FIBRE: &str = "output/the_total_descent_route_cultivates_the_first_athena_variant/product/18-exact-route-work-and-fibre.json";
const EQUATION_ATLAS: [&str; 3] = [
    "research/equation-atlas/equations.jsonl",
    "research/equation-atlas/relations.jsonl",
    "research/equation-atlas/manifest.json",
];
...
pub fn mount(root: &Path) -> Result<SourceReturn, String> {
    ...
    let formal_root = root.join("soma/formal/elementary-holonics");
    let formal_source_closure = FORMAL_CLOSURE
        ...
        .map(|(relative, blob, expected)| {
            ...
            accessed.push(format!("soma/formal/elementary-holonics/{relative}"));
    ...
    accessed.dedup();
    let excluded =
        "soma/formal/elementary-holonics/ElementaryHolonics/Millennium/FamilyMordell.lean";

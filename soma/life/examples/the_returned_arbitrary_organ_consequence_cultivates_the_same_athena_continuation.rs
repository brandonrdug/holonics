...
const E2_ROOT: &str = "output/the_complete_inherited_organs_cross_native_potential_complexes/native-rest";
...
fn produce(output: &Path) -> Result<(), String> {
    ...
    fs::write(
        ...
        cultivated.standing_bytes().map_err(|error| error.to_string())?,
    )
    .map_err(|error| error.to_string())?;
    fs::write(
...
fn world_return(output: &Path) -> Result<(), String> {
    ...
    let joint = card
        ...
        .map_err(|error| error.to_string())?;
    let requested = [BoundaryId(101), BoundaryId(103), provisional.parent_boundary];
    ...
    for boundary in requested {
        let consequence = rest
            ...
            .consequences
            .iter()
            .find(|consequence| consequence.family == 1 && consequence.state == 1 && consequence.boundary == boundary)
...
fn read_e2() -> Result<(Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>, E2Continuation), String> {
    ...
    let provisional = fs::read(root.join("provisional-codec.json")).map_err(|error| error.to_string())?;
    let continuation_bytes = fs::read(root.join("continuation.json")).map_err(|error| error.to_string())?;
    let continuation =

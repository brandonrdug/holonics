...
fn complete_cost(
    ...
    let i5: Value = read_json(&root.join(
        "output/the_athena_gemma_ecology_infers_returns_and_remounts/01-complete-product-cost.json",
    ))?;
    let r4: Value = read_json(&root.join(
        "output/the_retained_causal_boundary_carries_the_long_horizon_inquiry/10-complete-product-descent.json",
    ))?;
    let r5: Value = read_json(&root.join(
        "output/the_mathematical_and_physical_faces_share_native_transport/06-complete-product-descent.json",
    ))?;

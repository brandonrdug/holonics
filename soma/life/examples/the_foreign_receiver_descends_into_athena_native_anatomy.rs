...
use holonic_engine::{
    ...
    phoenix::{
        ...
        receiver_restricted_transport::{AnatomicalOccurrence, NativeAnatomyRest},
    },
};
...
const OUTPUT: &str = "output/the_foreign_receiver_descends_into_athena_native_anatomy";
...
fn main() -> Result<(), String> {
    ...
    let anatomy =
        NativeAnatomyRest::found_on_device(occurrences, &foreign, &mut card).map_err(display)?;
    ...
    let repetition_caustics = anatomy
        .classes
        .iter()
    ...
    let anatomy_atlas = json!({
        ...
        "foreign_to_native":anatomy.foreign_to_native,
        "foreign_only_distinctions":anatomy.foreign_only_distinctions,
        "analogous_sections":anatomy.analogous_sections,
        "homologous_sections":anatomy.homologous_sections,
        "receivers":anatomy.receivers,
        "partial_successor_squares":anatomy.quotient.generators,
        "pathology":{
            "foreign_identity_overreach":anatomy.foreign_only_distinctions,
        ...
        "apparatus":anatomy.apparatus,
    });
    let native_classes = anatomy.quotient.native_population.len();
    let foreign_classes = foreign.classes.len();
    let exact_foreign_factor = anatomy.foreign_to_native.len() == foreign_classes;
    let passed = native_classes < foreign_classes
        && exact_foreign_factor
        && !anatomy.foreign_only_distinctions.is_empty()
        && anatomy
            .classes
            .iter()
            .all(|class| !class.occurrences.is_empty())
        && anatomy.apparatus.cpu_device_partition_equal
        && !anatomy.apparatus.cpu_semantic_replay_after_device
    ...
    let grade = json!({
        ...
        "truth_status":"established-bounded; implemented-exact; measured",
        "passed":passed,
        "addressed_occurrence_population":anatomy.occurrences.len(),
        ...
        "foreign_only_distinction_population":anatomy.foreign_only_distinctions.len(),
        "complete_native_occurrence_fibres":anatomy.classes.iter().map(|class| class.occurrences.len()).collect::<Vec<_>>(),
        "partial_successor_generator_population":anatomy.quotient.generators.len(),
        "analogous_pair_population":anatomy.analogous_sections.len(),
        "homologous_pair_population":anatomy.homologous_sections.len(),
        "repetition_caustic_population":repetition_caustics.len(),
        "resident_device":anatomy.apparatus.device,
        "resident_quotient_launches":anatomy.apparatus.launches,

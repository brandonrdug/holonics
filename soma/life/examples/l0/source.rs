...
pub const PREDECESSOR_COMMIT: &str = "980cb86f937b87ca7b50232dd0bbfdcfb249b5fd";
pub const PREFIX_COMMIT: &str = "4ba0cb0af51bcad9f5e45c7ef3f7c7ac6f7db0e3";
pub const PREFIX_TREE: &str = "f0329739fcd83e45b8cd966f598fd46a7c619c7f";
...
pub fn mount(root: &Path) -> Result<LaboratoryChronology, String> {
    ...
    if revisions.len() != 7 {
        return Err(format!(
            "the frozen L0 chronology expected seven addressed commits and returned {}",
            revisions.len()
        ));
    }
    ...
    for revision in revisions {
        ...
        for line in changes_text.lines() {
            ...
            let address = format!("{revision}:{lineage_path}");
            let bytes = git(root, &["show", &address])?;
            let blob_sha256 = artifact::digest(&bytes);
            mounted_octets = mounted_octets
                .checked_add(bytes.len() as u64)
                .ok_or("incremental source extent overflow")?;
    ...
    let address = |needle: &str| -> Result<String, String> {
        occurrences
            ...
            .ok_or_else(|| format!("frozen source occurrence {needle} absent"))
    };
    let partitions = vec![
        LaboratoryPartition {
            kind: LaboratoryPartitionKind::Development,
            occurrences: vec![address("FunctionalEquationParity.lean")?],
        },
        LaboratoryPartition {
            kind: LaboratoryPartitionKind::HeldOutSuccessor,
            occurrences: vec![address("PositiveForm.lean")?],
        },
        LaboratoryPartition {
            kind: LaboratoryPartitionKind::CodecNotationLayoutRebase,
            occurrences: vec![address("SelmerCalculus.lean")?],
        },
        LaboratoryPartition {
            kind: LaboratoryPartitionKind::PhysicalApparatusPerturbation,
            occurrences: vec![address(
                "THE_HOLONIC_INFERENCE_CIRCULATION_RETAINS_THE_CAUSAL_BOUNDARY_AND_HETEROGENEOUS_TRANSPORT_FUSES.md",
            )?],
        },
        LaboratoryPartition {
            kind: LaboratoryPartitionKind::EqualAnswerDifferentRoute,
            occurrences: vec![address("AdviceBar.lean")?],
        },
        LaboratoryPartition {
            kind: LaboratoryPartitionKind::SubjectPortDisjointControl,
            occurrences: vec![address("UniformityBar.lean")?],
        },
        LaboratoryPartition {

...
//! source below `soma/formal`.  The seal is the text body's own exact native rest.  A detached
...
fn build(workspace: &Path, codex_root: &Path, claude_root: &Path) -> Result<(), String> {
    ...
    let (lean_documents, lean_receipt) = lean_documents(&workspace.join("soma/formal"))?;
...
fn lean_documents(root: &Path) -> Result<(Vec<ParsedTextDocument>, LeanReadingReceipt), String> {
    ...
    for path in paths {
        ...
        documents.push(ParsedTextDocument {
            identity,
            source: format!("soma/formal/{relative}"),
            sections: vec![text],
        });
    }

...
# ⊕ the CUDA-only §XXXII-c REGISTER entry ⊕ contemporary live-current and resident text mouths.
...
for e in link_grain link_sum link_finish link_founded_grain link_register_grain link_register_sum link_register_finish link_founded_sum chart_mark chart_register_mark chart_count chart_recast register_own_recast register_own_recast_finish register_carrier_rebase scope_felt scope_founded scope_register scope_register_surface regional_contacts lineage_event lineage_event_population text_incidence_select text_section_restrict; do

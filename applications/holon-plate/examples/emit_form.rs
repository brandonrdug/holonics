...
//! naming: every `crates/holonic-life/examples/*` driver that reaches a rest **hashes** its form into a JSON
//! receipt and then drops the octets. `grep -n encode_native_bytes crates/holonic-life/examples/` returns
...
//! `crates/holonic-life/examples/eros_mathematics_instance_rest.rs`, opens its own mouth and writes a real
//! 989 KB conditioned rest — a morphology founded on fourteen declared documents over a
//! 103-artifact standing — to `.local/artifacts/eros_mathematics_instance_rest/`. The seed below exists so

...
cut.  This owner adds the dynamic law: after every ordered word of source transports, every
declared receiver still factors through the quotient.

Generator-wise transport equivariance proves the whole successor family at once, with noncommuting
order retained.  Endpoint order blindness, when present, does not erase occurrence lineage; the
complete word remains part of the declared receiver/history family.

The finite control shows why the extension is necessary: two occurrences can agree under a static
receiver and quotient, then separate after one successor transport.  Every theorem is discharged.

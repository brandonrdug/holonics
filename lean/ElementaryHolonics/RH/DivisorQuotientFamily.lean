...
/-!
# Quotient-family re-indexing of the divisor-source transport

[proved-derived; formal-checked] For a positive receiver `m` and an old divisor `n ∣ m`, the
quotient `k = m / n` is the relative axis between them; the descendants of `n` are `n * ℓ`. At
scale `x`, writing `s = m / x`, the receiver band `y < (n * ℓ) / x ≤ 1` becomes
`y * k / s < ℓ ≤ k / s`, and the old-cell test `n / x < y` becomes `s / k < y`. The lemmas below
state both in cross-multiplied integer form against an exact rational seam `yNum / yDen`, so no
quotient is taken (`belowSeam_iff_quotient`, `positiveBand_iff_quotientBand`).

[proved-derived] The re-indexing separates the geometric variables `(s, k, ℓ)` from the
arithmetic coefficient `c_ω(n ℓ) / c_ω(n) = ℓ^ω ∏_(p ∣ ℓ, p ∤ n)(1 − p^(−2ω))`, which is what
the proportional obstruction in `RH/DivisorSourceTransport` consumes; that coefficient identity is
derived in the history note, not checked here. This module certifies only the exact integer
incidence; it makes no claim about the kernel inequality.

Ported from the side project `formal/rh-source-transport/SomaRHSourceTransport/QuotientFamily.lean`
at `13f8c734` (namespace `Soma.RHSourceTransport`). No `axiom`, no `sorry`.
-/

namespace Soma.Holonics.RH.DivisorQuotientFamily
...
end Soma.Holonics.RH.DivisorQuotientFamily

...
[definition] The spine lives in `Holonics/`; paths below are relative to it.
`Foundation/Holon.lean` starts with an occurrence population, oriented source and target ports,
and a receiver. `Holon.Interaction` is an actual pullback join. Serial composition
...
```sh
# From the repository root: the foundation library (the pinned Lake default).
bash tools/lean_check.sh

# A subject or one changed owner.
bash tools/lean_check.sh Holonics.Framework.Geometry
bash tools/lean_check.sh Holonics.Physics.ConstitutiveModulation

# Every research module, when a change reaches that scope.
bash tools/lean_check.sh HolonicsResearch
```

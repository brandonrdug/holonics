...
occurrence fibre. -/
def logarithmicWindingHolon : Holonics.Holon ℤ ℤ ℂ where
  Occurrence := ℤ
  source n := n
  target n := n + 1
...
def iPowerIChartHolon : Holonics.Holon ℤ ℤ IPowerReceiverFace where
  Occurrence := ℤ
  source n := n
  target n := n + 1
  receive n :=
...
def iPowerIHolon : Holonics.Holon ℤ ℤ ℂ where
  Occurrence := ℤ
  source n := n
  target n := n + 1

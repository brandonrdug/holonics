...
of branch coordinates. Transporting a drive with the incidence preserves their work pairing;
mutual constitutive coefficients transform by the corresponding two-sided sign action. Equality
at one state alone does not imply that these chart transports occurred.  Hence winding orientation as a chart choice is distinct from changing a physical
...
/-- Transporting the two-index constitutive table with both reoriented branch charts preserves
the mutual response. This is a sufficient covariance law, not a converse inferred from one face. -/
...
def complexParametronHolon [Fintype Node] [DecidableEq Node]
    ...
    Holonics.Holon Node Node ParametronReceiverFace where
  Occurrence := Branch
  source := source
  target := target
  receive branch :=
...
def mutualParametronHolon [Fintype Node] [DecidableEq Node]
    (source target : Branch → Node) (coupling : Branch → Branch → ℝ)
    (state : Node → ℝ) : Holonics.Holon (Node × Node) (Node × Node) ℝ where

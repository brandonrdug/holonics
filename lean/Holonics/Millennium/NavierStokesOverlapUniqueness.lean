...
namespace Holonics.Millennium.NavierStokesOverlapUniqueness

open Holonics.Millennium.NavierStokes
open Holonics.Millennium.NavierStokesFiniteTime
...
theorem divergence_velocityDifference
  ...
  have hfd : fderiv ℝ (fun y => u₁ y - u₂ y) x =
    ...
    change fderiv ℝ (u₁ - u₂) x = fderiv ℝ u₁ x - fderiv ℝ u₂ x
    exact fderiv_sub h₁ h₂
  rw [hfd]
...
theorem laplacian_sub_at
  ...
  have hnegBase : Δ (-u₂) x = -Δ u₂ x := by
    simpa only [neg_one_smul] using laplacian_smul (-1 : ℝ) h₂
  have hadd := h₁.laplacian_add h₂.neg
  have hnegfun : (fun y => -u₂ y) = -u₂ := by
    funext y
    rfl
  rw [hnegfun] at hadd
  have hsum : (fun y => u₁ y - u₂ y) = u₁ + -u₂ := by
    funext y
    rfl
  rw [hsum, hadd]
  rw [hnegBase]
...
theorem eulerianTimeJet_velocityDifference
  ...
  have hfun : Function.uncurry (fun x t => velocity₁ x t - velocity₂ x t) =
      Function.uncurry velocity₁ - Function.uncurry velocity₂ := by
    funext z
    rfl
  rw [hfun]
...
theorem periodicSolutionOn_integral_differenceTimeWork_eq
  ...
  have hdiffSmooth : ContDiffOn ℝ ∞
      ...
      (Holonics.Millennium.NavierStokesFiniteTimeVorticity.openSpaceTimeSlab T) := by
...
theorem periodicSolutionOn_differenceEnergy_continuousOn
  ...
  have hdensity : Continuous (Function.uncurry
    ...
    exact (continuous_const.mul ((hv₁.sub hv₂).norm.pow 2)).congr
      (fun z => by rfl)
  have hcubeCompact : IsCompact unitCube := by
    unfold unitCube
    exact (EuclideanSpace.equiv (Fin 3) ℝ).toHomeomorph.isCompact_preimage.mpr
  ...
  exact hintegral.congr (fun t => by rfl)
...
end Holonics.Millennium.NavierStokesOverlapUniqueness

...
import HolonicsResearch.Millennium.HodgeConjecture
...
namespace Holonics.Millennium.MillenniumDifferenceAtlas

open Holonics.Foundation.MeasuredDifferenceReceiver
...
theorem criticalSeamDifference_eq_zero_iff_mem (s : ℂ) :
    criticalSeamDifference s = 0 ↔
      s ∈ Holonics.Millennium.Seam.theSelfConjugateSeam := by
  rw [criticalSeamDifference, sub_eq_zero]
  exact (Holonics.Millennium.Seam.mem_seam_iff s).symm
...
open Holonics.Millennium.HodgeConjecture
...
open Holonics.Millennium.MassGap
...
end Holonics.Millennium.MillenniumDifferenceAtlas

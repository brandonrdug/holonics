...
namespace Holonics.Millennium.FiveWitness

open Real Complex
open Holonics.Millennium.FiveTheta
open Holonics.Millennium.FiveDerivative
open Holonics.Millennium.FivePositivity
open Holonics.Millennium.BirchSwinnertonDyer
open Holonics.Millennium.GaussCoefficient
open Holonics.Millennium.HeckeTheta
open Holonics.Millennium.HeckeWitness
...
lemma fiveCoeff_mul {a b : ℕ} (hab : Nat.Coprime a b) :
  ...
  unfold fiveCoeff
  rw [chi5_natCast_mul, Holonics.Millennium.HeckeEuler.heckeCoeff_mul hab]
  ring
...
lemma fiveCoeff_prime_pow {p : ℕ} [Fact p.Prime] (hp2 : p ≠ 2) (hp5 : p ≠ 5) (k : ℕ) :
  ...
  have hrec := Holonics.Millennium.HeckeEuler.heckeCoeff_prime_pow_recursion
    (p := p) hp2 k
...
theorem fiveCoeff_prime (p : ℕ) [Fact p.Prime] (hp2 : p ≠ 2) (hp5 : p ≠ 5) :
  ...
  have htwist := Holonics.Millennium.FiveTwist.theTraceTwistLaw (p := p) hp2 hp5
...
private lemma theta5_hasSum_exp {t : ℝ} (ht : t ∈ Set.Ioi (0 : ℝ)) :
    HasSum (fun m : ℕ => ((fiveCoeff m : ℤ) : ℂ) *
      rexp (-(Holonics.Millennium.FivePositivity.al * m) * t))
      ((theta5 t : ℝ) : ℂ) := by
  ...
  have hfun : (fun m : ℕ => ((fiveCoeff m : ℤ) : ℂ) *
      ...
      = fun m : ℕ => ((fiveCoeff m : ℤ) : ℂ) *
          rexp (-(Holonics.Millennium.FivePositivity.al * m) * t) := by
    funext m
    congr 2
    unfold Holonics.Millennium.FivePositivity.al
    push_cast
    ring
  rwa [hfun] at h
...
private lemma dirichlet_summable5 {s : ℂ} (hs : 3 < s.re) :
    Summable fun m : ℕ => ‖((fiveCoeff m : ℤ) : ℂ)‖ /
      (Holonics.Millennium.FivePositivity.al * m) ^ s.re := by
  have hC := Holonics.Millennium.FivePositivity.al_pos
  set σ : ℝ := s.re with hσ
  set C : ℝ := Holonics.Millennium.FivePositivity.al with hC_def
...
end Holonics.Millennium.FiveWitness

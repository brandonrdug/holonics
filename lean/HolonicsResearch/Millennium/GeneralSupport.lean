...
namespace Holonics.Millennium.GeneralSupport
...
theorem theSlotValuationsAreEvenOffTheDiscriminant {ℓ : ℕ} [Fact ℓ.Prime]
  ...
  have hva : padicValRat ℓ ((a : ℚ)) = 0 := by
    have := Holonics.Millennium.FamilyGenocchi.int_val_zero (p := ℓ) hda
    push_cast at this ⊢
    exact this
  have hvb : padicValRat ℓ ((b : ℚ)) = 0 := by
    have := Holonics.Millennium.FamilyGenocchi.int_val_zero (p := ℓ) hdb
    push_cast at this ⊢
    exact this
  have hvab : padicValRat ℓ (((a : ℚ)) - ((b : ℚ))) = 0 := by
    have := Holonics.Millennium.FamilyGenocchi.int_val_zero (p := ℓ) hdab
    push_cast at this ⊢
    exact this
  have haq : ((a : ℚ)) ≠ 0 := by
    intro hc
  ...
  · -- below zero the three agree
    have h1 : Va = V := by
      ...
      rw [hVa, he]
      refine Holonics.Millennium.FamilySupport.val_add_left hx0 (by rw [← he]; exact hxa) ?_
      rw [padicValRat.neg, hva]
      omega
    have h2 : Vb = V := by
      have he : x - (b : ℚ) = x + (-(b : ℚ)) := by ring
      rw [hVb, he]
      refine Holonics.Millennium.FamilySupport.val_add_left hx0 (by rw [← he]; exact hxb) ?_
  ...
  · push_neg at hneg
    ...
    have hVa0 : 0 ≤ Va := by
      ...
      have := Holonics.Millennium.FamilySupport.val_add_left (ℓ := ℓ) hxa (by rw [← he]; exact hx0)
        (by rw [hva]; omega)
      rw [← he] at this
      omega
    have hVb0 : 0 ≤ Vb := by
      ...
      have := Holonics.Millennium.FamilySupport.val_add_left (ℓ := ℓ) hxb (by rw [← he]; exact hx0)
        (by rw [hvb]; omega)
      rw [← he] at this
      omega
...
end Holonics.Millennium.GeneralSupport

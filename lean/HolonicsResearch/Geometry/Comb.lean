...
namespace Holonics.Geometry.Comb
...
/-- **THE CAVEAT, RECORDED AGAINST THIS TREE'S OWN WORK.**  `Zeta.Xi.theFinitePlacePairingIsNonnegative`
is a **term-by-term** statement — each `Λ(n)·g(n)` is nonnegative because `Λ ≥ 0` — and
`Zeta.WeilVector.theWeilFunctionalOnOneTestVector` evaluates one vector exactly.  **Neither is Weil

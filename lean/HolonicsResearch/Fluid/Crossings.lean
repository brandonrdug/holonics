...
`formal/elementary-holonics/.lake/packages/mathlib`: `grep -rli "linking number" Mathlib
...
because of them.  The Rust side of the same gap is recorded in `docs/canon/TABLET_THE_TURN.md` §13.3 —
...
namespace Holonics.Fluid.Crossings
...
missing piece is exactly an adapter from a crossing to a move — the absence `docs/canon/TABLET_THE_TURN.md`

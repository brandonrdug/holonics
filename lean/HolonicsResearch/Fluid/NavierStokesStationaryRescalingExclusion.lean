...
namespace Holonics.Fluid.NavierStokesStationaryRescalingExclusion
...
theorem OpenPeriodicSolutionOn.stationaryProfile_eq_zero
  ...
  have hconstant :=
    Holonics.Fluid.NavierStokesRescalingPeriodObstruction.OpenPeriodicSolutionOn.stationaryProfile_constant_exponential
  ...
  have hperiod : ∀ y i, P (y + (length s)⁻¹ • EuclideanSpace.single i 1) = P y :=
    Holonics.Fluid.NavierStokesDynamicRescaling.OpenPeriodicSolutionOn.rescaled_pressure_period
  ...
  have hgrad : ∀ y, gradient P y = (amplitudeJet / amplitude s) • profile 0 := by
    intro y
    have hm :=
      Holonics.Fluid.NavierStokesDynamicRescaling.OpenSmoothSolutionOn.rescaled_momentum
...
end Holonics.Fluid.NavierStokesStationaryRescalingExclusion

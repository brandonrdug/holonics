...
* **the two frames disagree, and the disagreement is the reading.**  `Geometry/Ricci.lean`'s
...
namespace Holonics.Fluid.Caloric
...
carrier of `Geometry/Ricci.lean`.  Read it as a statement about the carrier: the exponential of a
...
three-cycle of `Geometry/Ricci.lean` the roundness reading obeys `W' = (1 − 3τ)²·W`, so the

...
The zero divisor of `Λ₀` is already standing here (`Zeta.Xi`): discrete support, a finite index at
...
`Zeta.RectangleArgumentPrinciple.rectIntegral_mul_logDeriv`, which returns the multiplicity-weighted
zero population inside an admissible rectangle.  `Zeta.WeightedArgumentPrinciple.hasWeightedArgumentPrinciple`

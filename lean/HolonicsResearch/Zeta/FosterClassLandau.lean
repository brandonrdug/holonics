...
namespace Holonics.Zeta.FosterClassLandau
...
open Holonics.Zeta.FosterTanks
...
theorem norm_le_of_growth [hf : FosterClass f A B σ] {r : ℝ} (hr : 0 < r) {z : ℂ}
  ...
  have hnz : ‖z‖ ≤ 1 / 2 + r := by
    ...
    calc ‖z‖ = ‖(z - 1 / 2) + 1 / 2‖ := by ring_nf
      ...
      _ ≤ 1 / 2 + r := by rw [Holonics.Zeta.FosterCount.norm_half]; linarith
...
end Holonics.Zeta.FosterClassLandau

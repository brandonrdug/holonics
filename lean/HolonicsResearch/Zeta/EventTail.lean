...
namespace Holonics.Zeta.EventTail
...
theorem norm_f_two_le {t : ℝ} (ht : 0 < t) (w : ℂ) (y' : ℝ) :
  ...
  calc Real.exp (((2 - w.re) ^ 2 - (y' - w.im) ^ 2) / (4 * t)) * ‖Holonics.Zeta.FlowedGammaContour.γ₁ (2 + Complex.I * y')‖
...
theorem differentiableAt_f_of_re {t : ℝ} (w : ℂ) {z : ℂ} (hz : -2 < z.re) :
  ...
  have : f t w = fun z => Complex.exp ((z - w) ^ 2 / (4 * t)) *
      Holonics.Zeta.FlowedGammaContour.γ₁ z := funext (f_apply t w)
  rw [this]
  exact (((differentiableAt_id.sub_const w).pow 2).div_const _).cexp.mul
    (Holonics.Zeta.FlowedGammaContour.differentiableAt_γ₁ hz)

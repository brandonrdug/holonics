...
import HolonicsResearch.Zeta.TransverseCurrentBound
...
noncomputable def reflectedFiniteSurplus (f : ℂ → ℂ) (A B σ : ℝ)
  ...
  ∑ ρ ∈ ((FosterClassSplit.Zfac (f := f) hR).zeros.erase z₀).erase
      (Holonics.Zeta.PairPopulation.reflect z₀),
...
theorem comb'_eq_reflected_pair_add_surplus {z₀ : ℂ} {R : ℝ}
    (hR : 0 < R)
    (hreflectmem : Holonics.Zeta.PairPopulation.reflect z₀ ∈
      (FosterClassSplit.Zfac (f := f) hR).zeros)
    (hreflect_ne : Holonics.Zeta.PairPopulation.reflect z₀ ≠ z₀)
    (hmreflect : (FosterClassSplit.Zfac (f := f) hR).mult
      (Holonics.Zeta.PairPopulation.reflect z₀) = 1) :
    (comb' (f := f) hR z₀ z₀) =
      ((1 : ℂ) / (z₀ - Holonics.Zeta.PairPopulation.reflect z₀)) +
        reflectedFiniteSurplus f A B σ hR z₀ := by
  unfold comb' reflectedFiniteSurplus
  have hmemErase : Holonics.Zeta.PairPopulation.reflect z₀ ∈
...
end Holonics.Zeta.FiniteZeroCurrent

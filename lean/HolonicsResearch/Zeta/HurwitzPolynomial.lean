...
open Holonics.Zeta.LandauLemma
open Holonics.Zeta.RectangleCauchy
open Holonics.Zeta.RectangleArgumentPrinciple
open Holonics.EllipticCurve.BirchSwinnertonDyerWinding
open Holonics.Zeta.ExplicitFormulaLimit

namespace Holonics.Zeta.HurwitzPolynomial
...
theorem nonreal_le_of_tendsto {q : ℕ → ℝ[X]} {p : ℝ[X]} {n : ℕ}
    ...
    (hcoeff : ∀ k, Tendsto (fun N => (q N).coeff k) atTop (𝓝 (p.coeff k))) (hp0 : p ≠ 0) :
    ∀ᶠ N in atTop,
      Holonics.Zeta.PolyaStep.nonreal p ≤ Holonics.Zeta.PolyaStep.nonreal (q N) := by
  classical
  ...
  have hleft : Holonics.Zeta.PolyaStep.nonreal p = ∑ z ∈ S, rootMultiplicity z P := by
    rw [Holonics.Zeta.PolyaStep.nonreal_eq_card_filter, card_filter_im_eq]
    unfold rootCount
    ...
    intro x _
    split_ifs <;> rfl
  have hright : Holonics.Zeta.PolyaStep.nonreal (q N) =
...
end Holonics.Zeta.HurwitzPolynomial

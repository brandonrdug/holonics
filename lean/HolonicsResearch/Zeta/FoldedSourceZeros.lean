import HolonicsResearch.Zeta.FoldedSourceTail
import HolonicsResearch.Zeta.FoldedSourceDerivative
...
namespace Holonics.Zeta.FoldedSourceZeros

open Complex Metric Filter Topology Set MeasureTheory
open Holonics.Zeta.CriticalChart
open Holonics.Zeta.HeatFlowEntire
open Holonics.Zeta.RiemannXi
open Holonics.Zeta.FoldedSourceTail
open Holonics.Zeta.FoldedSourceDerivative
open Holonics.Zeta.RectangleCauchy
open Holonics.Zeta.RectangleWindingContinuity
open Holonics.Zeta.RectangleCountStable
...
theorem differentiable_Hstd (t : ℝ) : Differentiable ℂ (Hstd t) := by
  ...
  have hheat := Holonics.Zeta.XiGrowth.differentiable_heatE_riemannXi (-t / 4)
...
theorem tendstoUniformlyOn_deriv_partialSource (t : ℝ) {K : Set ℂ} (hK : IsCompact K) :
  ...
  (Holonics.Zeta.HurwitzLine.tendstoUniformlyOn_deriv
...
theorem eventually_rectangle_count_eq (t : ℝ) {z w : ℂ}
  ...
  obtain ⟨hu, _⟩ := Holonics.Zeta.HurwitzLine.tendstoUniformlyOn_deriv hF hlim hK
  obtain ⟨m, hm, hbd⟩ := Holonics.Zeta.HurwitzLine.eventually_norm_ge
    hu hK hG.continuous.continuousOn hne
  have hwi := Holonics.Zeta.HurwitzLine.tendsto_rectIntegral hF hG hlim hne
  ...
  have heq : k = k₀ := by
    apply Holonics.Zeta.HurwitzPolynomial.nat_eq_of_norm_sub_lt
...
end Holonics.Zeta.FoldedSourceZeros

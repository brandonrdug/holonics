...
namespace Holonics.Zeta.ThresholdRefinement

open Set Filter Topology Complex
open Polynomial
open Holonics.Zeta.RealZeroTimes
open Holonics.Zeta.DeBruijnLimit
open Holonics.Zeta.DeBruijnSeal
open Holonics.Zeta.LinePreservation
...
theorem lambda_eq_zero_of_halfClosed (hhalf : HalfClosed) : Λ_DN = 0 := by
  have hle : Λ_DN ≤ 0 := by
    ...
    exact this
  exact le_antisymm hle Holonics.Zeta.DescentZeros.Λ_DN_nonneg
...
theorem squareClosed_iff_RH : SquareClosed ↔ RiemannHypothesis := by
  ...
  · intro hs
    ...
    · have hpos : 0 < Λ_DN := lt_of_le_of_ne
        Holonics.Zeta.DescentZeros.Λ_DN_nonneg (Ne.symm hzero)
...
theorem heat_quadratic_zero (a t : ℝ) :
    Holonics.Zeta.HeatFlowOfPolynomials.heat t (quadratic a 0) = quadratic a t := by
  classical
  have hdeg : (quadratic a 0).natDegree = 2 := by
    unfold quadratic
    convert (Polynomial.natDegree_X_pow_add_C (R := ℂ) (n := 2)
      (r := ((a ^ 2 : ℝ) : ℂ))) using 1 <;> ring
  unfold Holonics.Zeta.HeatFlowOfPolynomials.heat
  rw [hdeg]
...
end Holonics.Zeta.ThresholdRefinement

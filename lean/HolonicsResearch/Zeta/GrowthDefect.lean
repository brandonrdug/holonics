...
`Zeta.Growth` discharged order-one growth of `Λ₀`.  Feeding it into `Jensen.TheOrderOneGrowthOfXi`
...
namespace Holonics.Zeta.GrowthDefect

open Holonics.Zeta.Jensen Metric Set Real Complex MeromorphicOn
...
Found by attempting to discharge it from `Zeta.Growth`; the shapes did not meet, and this is why.
...
correct hypothesis must let the bound depend on the centre as well as the radius.  `Zeta.Growth`
...
end Holonics.Zeta.GrowthDefect

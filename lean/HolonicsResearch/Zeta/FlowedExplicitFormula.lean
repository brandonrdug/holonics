...
namespace Holonics.Zeta.FlowedExplicitFormula
...
open Holonics.Zeta.HeatFlowBinding
...
theorem foster_form_heatE (t : ℝ) {z : ℂ} (hz : heatE t riemannXi z ≠ 0) :
    logDeriv (heatE t riemannXi) z =
      ∑' u : Holonics.Zeta.FosterClassCount.Zero (heatE t riemannXi),
        ((Holonics.Zeta.FosterClassCount.mult (heatE t riemannXi) (u : ℂ)).toNat : ℂ) *
          ((z - 1 / 2) / ((z - 1 / 2) ^ 2 - ((u : ℂ) - 1 / 2) ^ 2)) :=
  Holonics.Zeta.FosterClassHadamard.foster_form (f := heatE t riemannXi) hz
...
end Holonics.Zeta.FlowedExplicitFormula

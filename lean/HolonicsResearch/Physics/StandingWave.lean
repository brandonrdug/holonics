...
a *discrete* set inside a continuum.  That is the same fusion as `Zeta.Balance`: on the critical line
...
namespace Holonics.Physics.StandingWave
...
theorem theEscapingSideHasNoNodes {k t : ℝ} (hk : 0 < k) (ht : 0 < t) :
    Real.sinh (k * t) ≠ 0 :=
  ne_of_gt (Holonics.Geometry.Deviation.theNegativeCaseHasNoFocus hk ht)
...
theorem theNodeExistsOnlyOnTheClosingSide {k : ℝ} (hk : 0 < k) :
  ...
  ⟨Holonics.Geometry.Deviation.thePositiveCaseHasAFocus (ne_of_gt hk),
...
end Holonics.Physics.StandingWave

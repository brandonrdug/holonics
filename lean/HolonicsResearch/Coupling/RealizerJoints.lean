...
* **Birch–Swinnerton-Dyer ⇐ Hodge.**  The rank-12 Mestre height form of
  `RealizedForms` is realized: `realizer v = (√dᵢ · Lᵢ v)ᵢ` with the twelve pivots `dᵢ`
...
* **P vs NP ⇐ the Gram.**  `Aperture.theFamilyTooSmallCollapses` says `|Y|^{|F|} < |X|` forces two
  ...
  `RealizedForms` the same statement is a rank bound: `n` readings in a carrier of
...
namespace Holonics.Coupling.RealizerJoints
...
end Holonics.Coupling.RealizerJoints

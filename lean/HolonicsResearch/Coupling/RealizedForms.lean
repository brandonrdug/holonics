...
`ReceiverForm` proves the shared theorems for a `ReceiverForm` — positive, definite,
...
* **Riemann — the Gram of the spectral readings.**  `gramForm w` is the Gram form of any finite
  ...
  *definite ⟺ separating* on the nose (`gramForm_definite_iff_linearIndependent`).  With
  ...
  `Zeta.WeilPositivity` on the pair, which under RH is the nonnegative real that theorem returns.
...
namespace Holonics.Coupling.RealizedForms
...
end Holonics.Coupling.RealizedForms

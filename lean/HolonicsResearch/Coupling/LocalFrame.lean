...
* `theCollapseIsTheCommonKernel` — so the null cone is the *common kernel* of the local readings,
  which by `ReceiverForm.theNullConeIsTheCollapsedPopulation` is the collapsed population of
  the induced family.
...
namespace Holonics.Coupling.LocalFrame

open Holonics.Coupling.ReceiverForm
open Holonics.Coupling.ReceiverForm.ReceiverForm
open Holonics.Coupling.Pivots Finset
...
theorem theCollapseIsTheCommonKernel (r : ι → V) (v : V) :
    Holonics.Foundation.Separation.collapseOf (frameForm r).readings v 0 ↔ ∀ i, (inner ℝ (r i) v : ℝ) = 0 := by
...
end Holonics.Coupling.LocalFrame

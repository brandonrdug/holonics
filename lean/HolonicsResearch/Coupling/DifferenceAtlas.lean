...
namespace Holonics.Coupling.DifferenceAtlas
...
theorem criticalSeamDifference_eq_zero_iff_mem (s : ℂ) :
    criticalSeamDifference s = 0 ↔
      s ∈ Holonics.Zeta.Seam.theSelfConjugateSeam := by
  rw [criticalSeamDifference, sub_eq_zero]
  exact (Holonics.Zeta.Seam.mem_seam_iff s).symm
...
end Holonics.Coupling.DifferenceAtlas

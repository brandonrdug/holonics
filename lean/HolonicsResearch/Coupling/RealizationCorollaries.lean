...
namespace Holonics.Coupling.RealizationCorollaries

open Holonics.Coupling.ReceiverForm
open Holonics.Coupling.RealizedForms
open Holonics.Coupling.PositivityIsRealization
open Holonics.Coupling.RealizerJoints
open Holonics.Coupling.Pivots
open Holonics.Hodge.HodgeIndex
open Holonics.Coupling.LocalFrame
open Holonics.Fluid.HelicityAsLinking
open Holonics.Fluid.Crossings
open Finset
...
theorem collapse_iff_realizer_kernel (G : ReceiverForm V) (f : V →L[ℝ] W)
    (hf : ∀ x y, inner ℝ (f x) (f y) = G.B x y) (v : V) :
    Holonics.Foundation.Separation.collapseOf G.readings v 0 ↔ f v = 0 := by
...
theorem vacuum_is_realizer_kernel (G : ReceiverForm V) (f : V →L[ℝ] W)
  ...
  ⟨(nullCone_iff_realizer_kernel G f hf e).mp he,
    fun v c => Holonics.Gauge.MassGap.theVacuumIsInvisible
...
theorem not_coercive_of_realizer_kernel (G : ReceiverForm V) (f : V →L[ℝ] W)
    ...
    ¬ G.IsCoercive :=
  Holonics.Gauge.MassGap.theVacuumRefusesAGlobalGap hne
...
section Hilbert
open Holonics.Coupling.Coupling.HilbertTransportChain
universe u
...
variable (T : Holonics.Coupling.Coupling.HilbertTransportChain A B C)
...
end Holonics.Coupling.RealizationCorollaries

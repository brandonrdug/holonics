...
namespace Holonics.Foundation.HolonicTerminalCurrent
...
theorem finitePartitionCurrent_eq_exterior
  ...
  simpa [intervalCurrent] using
    (Holonics.finite_telescoping (fun piece ↦ state (time piece)) pieces)
...
def intervalBoundaryHolon {Occurrence : Type*} (state : ℝ → G)
    ...
    Holonics.BoundaryHolon G G where
...
end Holonics.Foundation.HolonicTerminalCurrent

...
namespace Holonics.RH.FlowedGamma
...
theorem integral_flowedTerm_eq_rebase (t : ℝ) (z : ℂ) {n : ℤ} (hn : n ≠ 0) :
    ∫ u : ℝ, flowedTerm t z n u =
      Holonics.Transport.GaussianRebase.act (t : ℂ)
        (Real.log |(n : ℝ)| : ℂ) (Γt t) z := by
  rw [integral_flowedTerm_eq_shift t z hn]
  simp only [Holonics.Transport.GaussianRebase.act,
    Holonics.Transport.GaussianRebase.amplitude,
    Holonics.Transport.GaussianRebase.shift, sub_eq_add_neg, Complex.exp_add]
  ring

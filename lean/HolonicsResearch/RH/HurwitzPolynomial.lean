...
open Holonics.RH.RectangleArgumentPrinciple
open Holonics.Millennium.BirchSwinnertonDyerWinding
open Holonics.RH.ExplicitFormulaLimit

namespace Holonics.RH.HurwitzPolynomial
...
theorem nonreal_le_of_tendsto {q : ℕ → ℝ[X]} {p : ℝ[X]} {n : ℕ}
    ...
    (hcoeff : ∀ k, Tendsto (fun N => (q N).coeff k) atTop (𝓝 (p.coeff k))) (hp0 : p ≠ 0) :
    ∀ᶠ N in atTop,
      Holonics.RH.PolyaStep.nonreal p ≤ Holonics.RH.PolyaStep.nonreal (q N) := by
  classical
  ...
  have hleft : Holonics.RH.PolyaStep.nonreal p = ∑ z ∈ S, rootMultiplicity z P := by
    rw [Holonics.RH.PolyaStep.nonreal_eq_card_filter, card_filter_im_eq]
    unfold rootCount
    ...
    intro x _
    split_ifs <;> rfl
  have hright : Holonics.RH.PolyaStep.nonreal (q N) =
...
end Holonics.RH.HurwitzPolynomial

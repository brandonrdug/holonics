import HolonicsResearch.RH.FoldedSourceTail
import HolonicsResearch.RH.FoldedSourceDerivative
...
namespace Holonics.RH.FoldedSourceZeros

open Complex Metric Filter Topology Set MeasureTheory
open Holonics.RH.CriticalChart
open Holonics.RH.HeatFlowEntire
open Holonics.RH.RiemannXi
open Holonics.RH.FoldedSourceTail
open Holonics.RH.FoldedSourceDerivative
open Holonics.RH.RectangleCauchy
open Holonics.RH.RectangleWindingContinuity
open Holonics.RH.RectangleCountStable
...
theorem differentiable_Hstd (t : ℝ) : Differentiable ℂ (Hstd t) := by
  ...
  have hheat := Holonics.RH.XiGrowth.differentiable_heatE_riemannXi (-t / 4)
...
theorem tendstoUniformlyOn_deriv_partialSource (t : ℝ) {K : Set ℂ} (hK : IsCompact K) :
  ...
  (Holonics.RH.HurwitzLine.tendstoUniformlyOn_deriv
...
theorem eventually_rectangle_count_eq (t : ℝ) {z w : ℂ}
  ...
  obtain ⟨hu, _⟩ := Holonics.RH.HurwitzLine.tendstoUniformlyOn_deriv hF hlim hK
  obtain ⟨m, hm, hbd⟩ := Holonics.RH.HurwitzLine.eventually_norm_ge
    hu hK hG.continuous.continuousOn hne
  have hwi := Holonics.RH.HurwitzLine.tendsto_rectIntegral hF hG hlim hne
  ...
  have heq : k = k₀ := by
    apply Holonics.RH.HurwitzPolynomial.nat_eq_of_norm_sub_lt
...
end Holonics.RH.FoldedSourceZeros

...
namespace Holonics.RH.FlowedExplicitFormula
...
open Holonics.RH.HeatFlowBinding
...
theorem foster_form_heatE (t : ℝ) {z : ℂ} (hz : heatE t riemannXi z ≠ 0) :
    logDeriv (heatE t riemannXi) z =
      ∑' u : Holonics.RH.FosterClassCount.Zero (heatE t riemannXi),
        ((Holonics.RH.FosterClassCount.mult (heatE t riemannXi) (u : ℂ)).toNat : ℂ) *
          ((z - 1 / 2) / ((z - 1 / 2) ^ 2 - ((u : ℂ) - 1 / 2) ^ 2)) :=
  Holonics.RH.FosterClassHadamard.foster_form (f := heatE t riemannXi) hz
...
end Holonics.RH.FlowedExplicitFormula

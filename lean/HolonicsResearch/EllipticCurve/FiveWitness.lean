...
namespace Holonics.EllipticCurve.FiveWitness

open Real Complex
open Holonics.EllipticCurve.FiveTheta
open Holonics.EllipticCurve.FiveDerivative
open Holonics.EllipticCurve.FivePositivity
...
open Holonics.EllipticCurve.HeckeWitness
...
lemma fiveCoeff_mul {a b : ℕ} (hab : Nat.Coprime a b) :
  ...
  unfold fiveCoeff
  rw [chi5_natCast_mul, Holonics.EllipticCurve.HeckeEuler.heckeCoeff_mul hab]
  ring
...
lemma fiveCoeff_prime_pow {p : ℕ} [Fact p.Prime] (hp2 : p ≠ 2) (hp5 : p ≠ 5) (k : ℕ) :
  ...
  have hrec := Holonics.EllipticCurve.HeckeEuler.heckeCoeff_prime_pow_recursion
    (p := p) hp2 k
...
theorem fiveCoeff_prime (p : ℕ) [Fact p.Prime] (hp2 : p ≠ 2) (hp5 : p ≠ 5) :
  ...
  have htwist := Holonics.EllipticCurve.FiveTwist.theTraceTwistLaw (p := p) hp2 hp5
...
private lemma fiber_hasSum5 (y : ℝ) (m : ℕ) :
  ...
  have hvanish : ∀ p : ℤ × ℤ,
    ...
    · exfalso
      apply hp
      have hb := HeckeTheta.norm_shell_bound (latticeNorm5_eq_iff.mp hmem)
...
private lemma theta5_hasSum_exp {t : ℝ} (ht : t ∈ Set.Ioi (0 : ℝ)) :
    HasSum (fun m : ℕ => ((fiveCoeff m : ℤ) : ℂ) *
      rexp (-(Holonics.EllipticCurve.FivePositivity.al * m) * t))
      ((theta5 t : ℝ) : ℂ) := by
  ...
  have hfun : (fun m : ℕ => ((fiveCoeff m : ℤ) : ℂ) *
      ...
      = fun m : ℕ => ((fiveCoeff m : ℤ) : ℂ) *
          rexp (-(Holonics.EllipticCurve.FivePositivity.al * m) * t) := by
    funext m
    congr 2
    unfold Holonics.EllipticCurve.FivePositivity.al
    push_cast
    ring
  rwa [hfun] at h
...
private lemma dirichlet_summable5 {s : ℂ} (hs : 3 < s.re) :
    Summable fun m : ℕ => ‖((fiveCoeff m : ℤ) : ℂ)‖ /
      (Holonics.EllipticCurve.FivePositivity.al * m) ^ s.re := by
  have hC := Holonics.EllipticCurve.FivePositivity.al_pos
  set σ : ℝ := s.re with hσ
  set C : ℝ := Holonics.EllipticCurve.FivePositivity.al with hC_def
...
end Holonics.EllipticCurve.FiveWitness

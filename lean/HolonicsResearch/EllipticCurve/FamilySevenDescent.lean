...
namespace Holonics.EllipticCurve.FamilySevenDescent

open Holonics.EllipticCurve.FamilyGenocchi
open Holonics.EllipticCurve.FamilyFiveDescent
...
lemma classOf_mem_cells7 (hp2' : p ≠ 2) (hp8 : p % 8 = 7)
    ...
    FamilyMordell.classOf (Fact.out : p.Prime).pos P ∈ cells7 p := by
  have hp : p.Prime := Fact.out
  have hn : 0 < p := hp.pos
  have hpq0 : (((p : ℕ) : ℚ)) ≠ 0 := by exact_mod_cast hp.pos.ne'
  have hpqpos : (0 : ℚ) < ((p : ℕ) : ℚ) := by exact_mod_cast hp.pos
...
theorem theClassesCollideOnTheSevenModEightBranch (hp8 : p % 8 = 7) {α : Type}
  ...
  have hp2' : p ≠ 2 := by intro h; omega
  have hn : 0 < p := hp.pos
  have hmaps : ∀ a : α, FamilyMordell.classOf hn (f a) ∈ cells7 p := fun a =>
...
end Holonics.EllipticCurve.FamilySevenDescent

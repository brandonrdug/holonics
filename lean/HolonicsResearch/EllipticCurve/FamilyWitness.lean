...
namespace Holonics.EllipticCurve.FamilyWitness

open Real Complex
open Holonics.EllipticCurve.BirchSwinnertonDyer
open Holonics.EllipticCurve.FamilyDuplication
open Holonics.EllipticCurve.FamilyThetaFE
open Holonics.EllipticCurve.HeckeTheta
open Holonics.EllipticCurve.HeckeEuler
...
theorem theTraceTwistLawAtEveryModulus (n : ℕ) {q : ℕ} [Fact q.Prime]
  ...
  have hn0 : ((n : ℕ) : ZMod q) ≠ 0 := by
    ...
    exact hqn
  rw [Holonics.EllipticCurve.FiveTwist.trace_eq_neg_charSum n hq2,
    Holonics.EllipticCurve.FiveTwist.trace_eq_neg_charSum 1 hq2]
...
theorem pCoeff_prime_odd (p : ℕ) [Fact p.Prime] (hp2 : p ≠ 2)
  ...
  · -- the blind frame: both sides vanish
    have hzero : heckeCoeff q = 0 :=
      Holonics.EllipticCurve.HeckeTheta.heckeCoeff_three_mod_four hq4
...
end Holonics.EllipticCurve.FamilyWitness
